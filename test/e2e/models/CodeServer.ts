...
export class CodeServerPage {
  ...
  async executeCommandViaMenus(command: string) {
    ...
    // The current workbench keeps focus inside the command palette until the
    // selected command has actually started.  The old generic menu helper
    // raced that focus state and could loop forever even though the command
    // was present.  Drive the command palette through its input instead.
    const input = widget.locator("input").first()
    await input.fill(`>${command}`)
    await widget.getByText(command, { exact: true }).waitFor({ state: "visible" })
  ...
  async navigateMenus(menus: string[]): Promise<void> {
    await this.navigateItems(menus, '.menubar-menu-button[aria-haspopup="true"]', async (selector) => {
