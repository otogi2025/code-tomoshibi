...
describe("createApp", () => {
  ...
  it("should reject errors that happen before the server can listen", async () => {
    // Occupying an ephemeral port is deterministic for privileged and
    // unprivileged builders alike; low-numbered ports are writable as root.
    ...
    try {
      ...
      if (!address || typeof address === "string") {
        throw new Error("failed to allocate an occupied TCP port")
      }

      const defaultArgs = await setDefaults({ port: address.port })
      await expect(createApp(defaultArgs)).rejects.toThrow(/listen EADDRINUSE/)
    } finally {
