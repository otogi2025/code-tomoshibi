...
const options = {
	...
	out: getArgValue('--out'),
	target: getArgValue('--target'), // 'server' | 'server-web' | 'web' -- required, see assertBuildTarget
	sourceMapBaseUrl: getArgValue('--source-map-base-url'),
};

// Build targets
type BuildTarget = 'server' | 'server-web' | 'web';
const BUILD_TARGETS: readonly BuildTarget[] = ['server', 'server-web', 'web'];

function assertBuildTarget(value: string | undefined): BuildTarget {
	if (!value || !BUILD_TARGETS.includes(value as BuildTarget)) {
		throw new Error(`--target is required and must be one of: ${BUILD_TARGETS.join(', ')} (got: ${value ?? 'nothing'})`);
	}
...
Options for 'bundle':
	...
	--target <target>  Build target (required): server, server-web, web
...
async function main(): Promise<void> {
	...
	try {
		switch (command) {
			...
			case 'bundle':
				await bundle(options.out ?? OUT_VSCODE_DIR, options.minify, options.nls, options.manglePrivates, assertBuildTarget(options.target), options.sourceMapBaseUrl);
				break;
