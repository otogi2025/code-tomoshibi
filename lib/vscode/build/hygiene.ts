...
export function hygiene(some: NodeJS.ReadWriteStream | string[] | undefined, runEslint = true): NodeJS.ReadWriteStream {
	...
	const unicode = es.through(function (file: VinylFileWithLines) {
		...
		// tomoshibi: UI strings are written in Chinese on purpose. A file-level
		// `allow-any-unicode-file` marker skips the unicode check for that file.
		if (lines.some(line => /allow-any-unicode-file/.test(line))) {
			this.emit('data', file);
			return;
		}
		let skipNext = false;
