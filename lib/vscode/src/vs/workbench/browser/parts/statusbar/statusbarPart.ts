...
class StatusbarPart extends Part implements IStatusbarEntryContainer {
	...
	addEntry(entry: IStatusbarEntry, id: string, alignment: StatusbarAlignment, priorityOrLocation: number | IStatusbarEntryLocation | IStatusbarEntryPriority = 0): IStatusbarEntryAccessor {
		if (!this.isTomoshibiStatusbarEntry(id)) {
			// This is a product-level omission, not a hidden DOM item. Callers get
			// a harmless accessor while no element, view model entry or hover is made.
	...
	private getContextMenuActions(event: StandardMouseEvent): IAction[] {
		...
		actions.push(toAction({ id: ToggleStatusbarVisibilityAction.ID, label: localize('hideStatusBar', "隐藏状态栏"), run: () => this.instantiationService.invokeFunction(accessor => new ToggleStatusbarVisibilityAction().run(accessor)) }));
		...
		if (statusEntryUnderMouse) {
			actions.push(new Separator());
			// Code-Tomoshibi: the marketplace UI is gone, so the "管理扩展" entry was dropped.
