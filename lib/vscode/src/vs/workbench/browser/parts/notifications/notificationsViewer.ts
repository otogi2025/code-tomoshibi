		...
		for (const node of message.linkedText.nodes) {
			if (typeof node === 'string') {
				...
				if (!title && node.href.startsWith('command:')) {
					title = localize('executeCommand', "单击以执行命令 \"{0}\"", node.href.substr('command:'.length));
				} else if (!title) {
					title = node.href;
				}
...
export class NotificationRenderer implements IListRenderer<INotificationViewItem, INotificationTemplateData> {
	...
	renderTemplate(container: HTMLElement): INotificationTemplateData {
		...
		data.toolbar = new ActionBar(
			toolbarContainer,
			{
				ariaLabel: localize('notificationActions', "通知操作"),
							...
							getActions() {
								...
								if (isNotificationSource(source)) {
									...
									actions.push(toAction({
										id: source.id,
										label: isSourceFiltered ? localize('turnOnNotifications', "启用来自“{0}”的所有通知", source.label) : localize('turnOffNotifications', "禁用来自“{0}”的信息和警告通知", source.label),
...
export class NotificationTemplateRenderer extends Disposable {
	...
	private renderSource(notification: INotificationViewItem, sourceCustomHover: IManagedHover): void {
		if (notification.expanded && notification.source) {
			this.template.source.textContent = localize('notificationSource', "来源: {0}", notification.source);
