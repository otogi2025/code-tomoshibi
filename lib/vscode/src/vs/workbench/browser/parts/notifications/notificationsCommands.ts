...
export function registerNotificationCommands(center: INotificationsCenterController, toasts: INotificationsToastController, model: NotificationsModel): void {
	...
	CommandsRegistry.registerCommand(TOGGLE_DO_NOT_DISTURB_MODE_BY_SOURCE, accessor => {
		...
		picker.placeholder = localize('selectSources', "选择要从中启用所有通知的源");
	...
	const category = localize2('notifications', '通知');
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, { command: { id: SHOW_NOTIFICATIONS_CENTER, title: localize2('showNotifications', '显示通知'), category } });
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, { command: { id: HIDE_NOTIFICATIONS_CENTER, title: localize2('hideNotifications', '隐藏通知'), category }, when: NotificationsCenterVisibleContext });
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, { command: { id: CLEAR_ALL_NOTIFICATIONS, title: localize2('clearAllNotifications', '清除所有通知'), category } });
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, { command: { id: ACCEPT_PRIMARY_ACTION_NOTIFICATION, title: localize2('acceptNotificationPrimaryAction', '接受通知主要操作'), category } });
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, { command: { id: TOGGLE_DO_NOT_DISTURB_MODE, title: localize2('toggleDoNotDisturbMode', '切换“请勿打扰”模式'), category } });
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, { command: { id: TOGGLE_DO_NOT_DISTURB_MODE_BY_SOURCE, title: localize2('toggleDoNotDisturbModeBySource', '按源切换“请勿打扰”模式...'), category } });
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, { command: { id: FOCUS_NOTIFICATION_TOAST, title: localize2('focusNotificationToasts', '将焦点放在通知横幅上'), category }, when: NotificationsToastsVisibleContext });

	// Bell icon in the title bar (when notifications are positioned at top-right)
	MenuRegistry.appendMenuItem(MenuId.TitleBar, {
		command: {
			id: TOGGLE_NOTIFICATIONS_CENTER,
			title: localize('toggleNotifications', "切换通知"),
			icon: Codicon.bell,
		},
		group: 'navigation',
		order: 10000,
...
registerAction2(class SetNotificationsPositionBottomRight extends Action2 {
	constructor() {
		super({
			...
			title: localize2('positionBottomRight', '底部右对齐'),
...
registerAction2(class SetNotificationsPositionBottomLeft extends Action2 {
	constructor() {
		super({
			...
			title: localize2('positionBottomLeft', '底部左对齐'),
...
registerAction2(class SetNotificationsPositionTopRight extends Action2 {
	constructor() {
		super({
			...
			title: localize2('positionTopRight', '顶部右对齐'),
