	...
	getProvider(accessor: ServicesAccessor) {
		...
		function getProvider() {
			...
			function getContentForNotification(): string | undefined {
				...
				return withSeverityPrefix(notification.source ? localize('notification.accessibleViewSrc', '{0} 源: {1}', message, notification.source) : message, notification.severity);
			}
...
function getActionsFromNotification(notification: INotificationViewItem, accessibilitySignalService: IAccessibilitySignalService): IAction[] | undefined {
	...
	if (actions) {
		actions.push({
			id: 'clearNotification', label: localize('clearNotification', "清除通知"), tooltip: localize('clearNotification', "清除通知"), run: () => {
				notification.close();
