...
export class NotificationsAlerts extends Disposable {
	...
	private doTriggerAriaAlert(notification: INotificationViewItem): void {
		...
		if (notification.severity === Severity.Error) {
			alertText = localize('alertErrorMessage', "错误: {0}", notification.message.linkedText.toString());
		} else if (notification.severity === Severity.Warning) {
			alertText = localize('alertWarningMessage', "警告: {0}", notification.message.linkedText.toString());
		} else {
			alertText = localize('alertInfoMessage', "信息: {0}", notification.message.linkedText.toString());
		}
