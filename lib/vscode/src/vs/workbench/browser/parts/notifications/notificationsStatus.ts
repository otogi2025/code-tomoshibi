...
export class NotificationsStatus extends Disposable {
	...
	private updateNotificationsCenterStatusItem(): void {
		...
		let statusProperties: IStatusbarEntry = {
			name: localize('status.notifications', "通知"),
			text: `${notificationsInProgress > 0 || this.newNotificationsCount > 0 ? '$(bell-dot)' : '$(bell)'}`,
			ariaLabel: localize('status.notifications', "通知"),
		...
		if (this.notificationService.getFilter() === NotificationsFilter.ERROR) {
			statusProperties = {
				...
				ariaLabel: localize('status.doNotDisturb', "请勿打扰"),
				tooltip: localize('status.doNotDisturbTooltip', "已启用“请勿打扰”模式")
			};
		}
	...
	private getTooltip(notificationsInProgress: number): string {
		if (this.isNotificationsCenterVisible) {
			return localize('hideNotifications', "隐藏通知");
		}

		if (this.model.notifications.length === 0) {
			return localize('zeroNotifications', "没有通知");
		}

		if (notificationsInProgress === 0) {
			if (this.newNotificationsCount === 0) {
				return localize('noNotifications', "无新通知");
			}

			if (this.newNotificationsCount === 1) {
				return localize('oneNotification', "1 条新通知");
			}

			return localize({ key: 'notifications', comment: ['{0} will be replaced by a number'] }, "{0} 条新通知", this.newNotificationsCount);
		}

		if (this.newNotificationsCount === 0) {
			return localize({ key: 'noNotificationsWithProgress', comment: ['{0} will be replaced by a number'] }, "无新通知({0} 正在进行中)", notificationsInProgress);
		}

		if (this.newNotificationsCount === 1) {
			return localize({ key: 'oneNotificationWithProgress', comment: ['{0} will be replaced by a number'] }, "1 条新通知({0} 条正在进行中)", notificationsInProgress);
		}

		return localize({ key: 'notificationsWithProgress', comment: ['{0} and {1} will be replaced by a number'] }, "{0} 条新通知({1} 个正在进行中)", this.newNotificationsCount, notificationsInProgress);
	}
	...
	private doSetStatusMessage(item: IStatusMessageViewItem): void {
		...
		let showHandle: Timeout | undefined = setTimeout(() => {
			statusMessageEntry = this.statusbarService.addEntry(
				{
					name: localize('status.message', "状态消息"),
					text: message,
					ariaLabel: message
				},
				'status.message',
