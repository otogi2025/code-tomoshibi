...
export class NotificationsToasts extends Themable implements INotificationsToastController {
	...
	private static readonly SHOW_TOASTS = false;
	...
	private addToast(item: INotificationViewItem): void {
		// Code-Tomoshibi is terminal-first and intentionally never interrupts the
		// active session with toast popups. Notifications remain in the shared
		// model and are still available from the notification center.
		if (!NotificationsToasts.SHOW_TOASTS) {
			return;
		}
	...
	private doAddToast(item: INotificationViewItem, itemDisposables: DisposableStore): void {
		...
		const notificationList = this.instantiationService.createInstance(NotificationsList, notificationToast, {
			...
			widgetAriaLabel: (() => {
				if (!item.source) {
					return withSeverityPrefix(localize('notificationAriaLabel', "{0}，通知", item.message.raw), item.severity);
				}

				return withSeverityPrefix(localize('notificationWithSourceAriaLabel', "{0}，源: {1}，通知", item.message.raw, item.source), item.severity);
			})()
		});
