...
export class NotificationAccessibilityProvider implements IListAccessibilityProvider<INotificationViewItem> {
	...
	getAriaLabel(element: INotificationViewItem): string {
		...
		if (this._configurationService.getValue('accessibility.verbosity.notification')) {
			accessibleViewHint = keybinding ? localize('notificationAccessibleViewHint', "使用 {0} 检查辅助视图中的响应", keybinding) : localize('notificationAccessibleViewHintNoKb', "通过命令“打开辅助视图”在辅助视图中检查响应，该命令当前无法通过键绑定触发");
		}

		if (!element.source) {
			return withSeverityPrefix(accessibleViewHint ? localize('notificationAriaLabelHint', "{0}，通知 {1}", element.message.raw, accessibleViewHint) : localize('notificationAriaLabel', "{0}，通知", element.message.raw), element.severity);
		}

		return withSeverityPrefix(accessibleViewHint ? localize('notificationWithSourceAriaLabelHint', "{0}，源: {1}，通知 {2}", element.message.raw, element.source, accessibleViewHint) : localize('notificationWithSourceAriaLabel', "{0}，源: {1}，通知", element.message.raw, element.source), element.severity);
	}

	getWidgetAriaLabel(): string {
		return this._options.widgetAriaLabel ?? localize('notificationsList', "通知列表");
	}
