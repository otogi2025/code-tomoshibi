...
export abstract class MenubarControl extends Disposable {
	...
	private notifyUserOfCustomMenubarAccessibility(): void {
		...
		const message = localize('menubar.customTitlebarAccessibilityNotification', "为你启用了辅助功能支持。对于最易于访问的体验，我们建议使用自定义菜单样式。");
...
export class CustomMenubarControl extends MenubarControl {
	...
	private getUpdateAction(): IAction | null {
		...
		switch (state.type) {
			...
			case StateType.Cancelling:
				return toAction({ id: 'update.cancelling', label: localize('cancellingUpdate', "正在取消更新..."), enabled: false, run: () => { } });

			case StateType.Ready:
				return toAction({
					id: 'update.restart', label: localize({ key: 'restartToUpdate', comment: ['&& denotes a mnemonic'] }, "重新启动以更新(&&U)"), enabled: true, run: () =>
	...
	private getMenuBarOptions(): IMenuBarOptions {
		return {
			...
			// The Code-Tomoshibi compact menu is complete by itself. Browser navigation actions
			// stay in Safari and must not inflate the hamburger menu.
