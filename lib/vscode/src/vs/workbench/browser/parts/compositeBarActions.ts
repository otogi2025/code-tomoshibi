...
export class CompositeOverflowActivityAction extends CompositeBarAction {
	...
	constructor(
		...
		super({
			...
			name: localize('additionalViews', "更多视图"),
...
export class CompositeActionViewItem extends CompositeBarActionViewItem {
	...
	private showContextMenu(container: HTMLElement): void {
		...
		if (isPinned) {
			this.toggleCompositePinnedAction.label = localize('hide', "隐藏「{0}」", this.compositeBarActionItem.name);
			this.toggleCompositePinnedAction.checked = false;
			this.toggleCompositePinnedAction.enabled = this.compositeBar.getPinnedCompositeIds().length > 1;
		} else {
			this.toggleCompositePinnedAction.label = localize('keep', "保留「{0}」", this.compositeBarActionItem.name);
		...
		if (isBadgeEnabled) {
			this.toggleCompositeBadgeAction.label = localize('hideBadge', "隐藏角标");
		} else {
			this.toggleCompositeBadgeAction.label = localize('showBadge', "显示角标");
		}
...
export class ToggleCompositePinnedAction extends Action {
	...
	constructor(
		...
		super('show.toggleCompositePinned', activity ? activity.name : localize('toggle', "切换视图固定"));
...
export class ToggleCompositeBadgeAction extends Action {
	constructor(
		...
		super('show.toggleCompositeBadge', compositeBarActionItem ? compositeBarActionItem.name : localize('toggleBadge', "切换视图角标"));
