...
const separatorIcon = registerIcon('breadcrumb-separator', Codicon.chevronRight, localize('separatorIcon', '痕迹导航中分隔符的图标。'));
...
export class BreadcrumbsControl {
	...
	static readonly CK_BreadcrumbsPossible = new RawContextKey('breadcrumbsPossible', false, localize('breadcrumbsPossible', "编辑器是否可以显示面包屑"));
	static readonly CK_BreadcrumbsVisible = new RawContextKey('breadcrumbsVisible', false, localize('breadcrumbsVisible', "面包屑当前是否可见"));
	static readonly CK_BreadcrumbsActive = new RawContextKey('breadcrumbsActive', false, localize('breadcrumbsActive', "面包屑是否具有焦点"));
	static readonly CK_BreadcrumbsHasSymbols = new RawContextKey('breadcrumbsHasSymbols', false, localize('breadcrumbsHasSymbols', "面包屑是否包含符号项"));
		...
		const updateBreadcrumbs = () => {
			...
			if (items.length === 0) {
				...
				this._widget.setItems([new class extends BreadcrumbsItem {
					render(container: HTMLElement): void {
						container.textContent = localize('empty', "无元素");
					}
					equals(other: BreadcrumbsItem): boolean {
						return other === this;
					}
	...
	private _updateEditorTypeControl(): void {
		...
		if (!available || !hasDefaultEditorAssociation(available, configuredDefaultEditor)) {
			...
			editorTypeHover.update(localize('editorType.hover', "编辑器：{0}", label));
		}
...
registerAction2(class ToggleBreadcrumb extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('cmd.toggle', "切换面包屑"),
			shortTitle: localize2('cmd.toggle.short', "面包屑"),
			category: Categories.View,
			toggled: {
				condition: ContextKeyExpr.equals('config.breadcrumbs.enabled', true),
				title: localize('cmd.toggle2', "面包屑"),
				mnemonicTitle: localize({ key: 'miBreadcrumbs2', comment: ['&& denotes a mnemonic'] }, "面包屑")
			},
			menu: [
...
registerAction2(class FocusAndSelectBreadcrumbs extends Action2 {
	constructor() {
		super({
			...
			title: localize2('cmd.focusAndSelect', "聚焦并选择面包屑"),
...
registerAction2(class FocusBreadcrumbs extends Action2 {
	constructor() {
		super({
			...
			title: localize2('cmd.focus', "聚焦面包屑"),
			precondition: BreadcrumbsControl.CK_BreadcrumbsVisible,
			keybinding: {
				weight: KeybindingWeight.WorkbenchContrib,
...
registerAction2(class CopyBreadcrumbPath extends Action2 {
	constructor() {
		super({
			...
			title: localize2('cmd.copyPath', "复制面包屑路径"),
			category: Categories.View,
