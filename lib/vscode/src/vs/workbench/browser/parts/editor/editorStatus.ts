...
const nlsSingleSelectionRange = localize('singleSelectionRange', "第 {0} 行，第 {1} 列（已选择 {2} 个）");
const nlsSingleSelection = localize('singleSelection', "第 {0} 行，第 {1} 列");
const nlsMultiSelectionRange = localize('multiSelectionRange', "{0} 个选定内容（已选择 {1} 个字符）");
const nlsMultiSelection = localize('multiSelection', "{0} 个选定内容");
...
class EditorStatus extends Disposable {
	...
	private async showIndentationPicker(): Promise<unknown> {
		...
		if (this.editorService.activeEditor?.isReadonly()) {
			return this.quickInputService.pick([{ label: localize('noWritableCodeEditor', "活动代码编辑器为只读。") }]);
		}
				...
				update.indentation = (
					modelOpts.insertSpaces
						? modelOpts.tabSize === modelOpts.indentSize
							...
							: localize('spacesAndTabsSize', "空格：{0}（Tab 大小：{1}）", modelOpts.indentSize, modelOpts.tabSize)
						: localize({ key: 'tabSize', comment: ['Tab corresponds to the tab key'] }, "Tab 大小：{0}", modelOpts.tabSize)
				);
			}
		}
