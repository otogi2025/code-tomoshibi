		...
		super(prefix,
			{
				...
				noResultsPick: {
					label: localize('noViewResults', "没有匹配的编辑器"),
					groupId: -1
				}
			}
		);
			...
			return {
				...
				ariaLabel: (() => {
					if (mapGroupIdToGroupAriaLabel.size > 1) {
						return isDirty ?
							localize('entryAriaLabelWithGroupDirty', "{0}, 个未保存的更改，{1}", nameAndDescription, mapGroupIdToGroupAriaLabel.get(groupId)) :
							localize('entryAriaLabelWithGroup', "{0}, {1}", nameAndDescription, mapGroupIdToGroupAriaLabel.get(groupId));
					}

					return isDirty ? localize('entryAriaLabelDirty', "{0}，个未保存的更改", nameAndDescription) : nameAndDescription;
				})(),
				description,
