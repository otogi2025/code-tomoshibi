...
export class WorkspaceTrustRequiredPlaceholderEditor extends EditorPlaceholder {
	...
	private static readonly LABEL = localize('trustRequiredEditor', "需要工作区信任");
	...
	protected async getContents(): Promise<IEditorPlaceholderContents> {
		return {
			...
			label: isSingleFolderWorkspaceIdentifier(toWorkspaceIdentifier(this.workspaceService.getWorkspace())) ?
				localize('requiresFolderTrustText', "该文件未在编辑器中显示，因为尚未向该文件夹授予信任。") :
				localize('requiresWorkspaceTrustText', "该文件未在编辑器中显示，因为尚未向该工作区夹授予信任。"),
			actions: [
				{
					label: localize('manageTrust', "管理工作区信任"),
...
export class ErrorPlaceholderEditor extends EditorPlaceholder {
	...
	private static readonly LABEL = localize('errorEditor', "编辑器错误");
	...
	protected async getContents(input: EditorInput, options: IErrorEditorPlaceholderOptions, disposables: DisposableStore): Promise<IEditorPlaceholderContents> {
		...
		let label: string;
		if (isFileNotFound) {
			label = localize('unavailableResourceErrorEditorText', "由于找不到该文件，因此无法打开编辑器。");
		} else if (isEditorOpenError(error) && error.forceMessage) {
			label = error.message;
		} else if (error) {
			label = localize('unknownErrorEditorTextWithError', "由于意外错误，无法打开编辑器。有关详细信息，请参阅日志。");
		} else {
			label = localize('unknownErrorEditorTextWithoutError', "由于意外错误，无法打开编辑器。");
		}
