...
export class EditorGroupView extends Themable implements IEditorGroupView {
	...
	private createContainerToolbar(): void {
		...
		const containerToolbar = this._register(new ActionBar(toolbarContainer, {
			ariaLabel: localize('ariaLabelGroupActions', "空编辑器组操作"),
	...
	private updateContainer(): void {
		...
		if (this.isEmpty) {
			...
			this.element.setAttribute('aria-label', localize('emptyEditorGroup', "{0}（空）", this.ariaLabel));
		}
	...
	get label(): string {
		if (this.groupsLabel) {
			return localize('groupLabelLong', "{0}：编辑器组 {1}", this.groupsLabel, this._index + 1);
		}

		return localize('groupLabel', "编辑器组 {0}", this._index + 1);
	}

	get ariaLabel(): string {
		if (this.groupsLabel) {
			return localize('groupAriaLabelLong', "{0}：编辑器组 {1}", this.groupsLabel, this._index + 1);
		}

		return localize('groupAriaLabel', "编辑器组 {0}", this._index + 1);
	}
	...
	private doMoveOrCopyEditorAcrossGroups(editor: EditorInput, target: EditorGroupView, openOptions?: IEditorOpenOptions, internalOptions?: IInternalMoveCopyOptions): boolean {
		...
		if (!keepCopy || editor.hasCapability(EditorInputCapabilities.Singleton) /* singleton editors will always move */) {
			...
			if (typeof canMoveVeto === 'string') {
				this.dialogService.error(canMoveVeto, localize('moveErrorDetails', "请先保存或还原该编辑器，然后重试。"));
		...
		if (activeEditorPane instanceof EditorPane) {
			...
			if (menuId === MenuId.EditorTitle) {
				...
				if (available) {
					...
					const reopenWithSubmenu = new SubmenuAction('editor.reopenWith', localize('reopenWith', "用其他编辑器打开"), editorTypeActions);
