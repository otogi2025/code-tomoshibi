...
export class ModalEditorPart {
	...
	async create(options?: IModalEditorPartOptions): Promise<ICreateModalEditorPartResult> {
		...
		const sidebarToggleAction = disposables.add(new Action(TOGGLE_MODAL_EDITOR_SIDEBAR_COMMAND_ID, localize('toggleSidebar', "切换边栏"), ThemeIcon.asClassName(sidebarToggleIcon), true));
		...
		const previousButton = disposables.add(new Button(navigationContainer, { title: localize('previousItem', "上一步") }));
		...
		const nextButton = disposables.add(new Button(navigationContainer, { title: localize('nextItem', "下一步") }));
		...
		disposables.add(Event.runAndSubscribe(editorPart.onDidChangeNavigation, ((navigation: IModalEditorNavigation | undefined) => {
			if (navigation && navigation.total > 1) {
				...
				navigationLabel.textContent = localize('navigationCounter', "{0}/{1}", navigation.current + 1, navigation.total);
...
class ModalEditorPartImpl extends EditorPart implements IModalEditorPart {
	...
	constructor(
		...
		super(editorPartsView, `workbench.parts.modalEditor.${id}`, localize('modalEditorPart', "模式编辑器区域"), windowId, instantiationService, themeService, configurationService, storageService, layoutService, hostService, modalContextKeyService);
