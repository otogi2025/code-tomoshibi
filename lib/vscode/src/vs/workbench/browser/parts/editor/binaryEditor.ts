...
export abstract class BaseBinaryResourceEditor extends EditorPlaceholder {
	...
	override getTitle(): string {
		return this.input ? this.input.getName() : localize('binaryEditor', "二进制查看器");
	}
	...
	protected async getContents(input: EditorInput, options: IEditorOptions): Promise<IEditorPlaceholderContents> {
		...
		return {
			icon: '$(warning)',
			label: localize('binaryError', "此文件是二进制文件或使用了不受支持的文本编码，所以无法在文本编辑器中显示。"),
			actions: [
				{
					label: localize('openAnyway', "仍然打开"),
					run: async () => {
