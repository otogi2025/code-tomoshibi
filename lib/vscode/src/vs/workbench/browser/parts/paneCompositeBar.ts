...
export interface IPaneCompositeBarOptions {
	...
	/**
	 * Product-level allowlist for view containers represented by this bar.
	 * The underlying containers and commands remain registered; disallowed
	 * containers never create actions, listeners or DOM in this bar.
	 */
	readonly viewContainerFilter?: (id: string) => boolean;
}
...
export class PaneCompositeBar extends Disposable {
	...
	constructor(
		...
		const cachedItems = this.cachedViewContainers
			.filter(container => this.acceptsViewContainer(container.id))
			.map(container => ({
				id: container.id,
				name: container.name,
	...
	private getContextMenuActionsForComposite(compositeId: string): IAction[] {
		...
		actions.push(new SubmenuAction('moveToMenu', localize('moveToMenu', "移动到"), moveActions));

		// Reset Location
		if (defaultLocation !== currentLocation) {
			actions.push(toAction({
				id: 'resetLocationAction', label: localize('resetLocation', "重置位置"), run: () => {
	...
	private createMoveAction(viewContainer: ViewContainer, newLocation: ViewContainerLocation, defaultLocation: ViewContainerLocation): IAction {
		return toAction({
			...
			label: newLocation === ViewContainerLocation.Panel ? localize('panel', "面板") : newLocation === ViewContainerLocation.Sidebar ? localize('sidebar', "主侧边栏") : localize('auxiliarybar', "辅助侧边栏"),
			run: () => {
	...
	private onDidChangeViewContainers(added: readonly { container: ViewContainer; location: ViewContainerLocation }[], removed: readonly { container: ViewContainer; location: ViewContainerLocation }[]) {
		...
		this.onDidRegisterViewContainers(added.filter(({ location, container }) => location === this.location && this.acceptsViewContainer(container.id)).map(({ container }) => container));
	}
	...
	private onDidChangeViewContainerLocation(container: ViewContainer, from: ViewContainerLocation, to: ViewContainerLocation) {
		...
		if (to === this.location) {
			if (this.acceptsViewContainer(container.id)) {
	...
	private onDidViewContainerVisible(id: string): void {
		if (!this.acceptsViewContainer(id)) {
			return;
		}
		const viewContainer = this.getViewContainer(id);
		if (viewContainer) {
	...
	private onDidRegisterViewContainers(viewContainers: readonly ViewContainer[]): void {
		for (const viewContainer of viewContainers) {
			if (!this.acceptsViewContainer(viewContainer.id)) {
				continue;
			}
			this.addComposite(viewContainer);
	...
	private getViewContainers(): readonly ViewContainer[] {
		return this.viewDescriptorService.getViewContainersByLocation(this.location).filter(container => this.acceptsViewContainer(container.id));
	}

	private acceptsViewContainer(id: string): boolean {
		return this.options.viewContainerFilter?.(id) ?? true;
	}
