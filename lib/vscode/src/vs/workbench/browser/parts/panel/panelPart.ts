...
export class PanelPart extends AbstractPaneCompositePart {
	...
	private fillExtraContextMenuActions(actions: IAction[]): void {
		...
		const toggleShowLabelsAction = toAction({
			...
			label: panelShowLabels ? localize('showIcons', "显示图标") : localize('showLabels', "显示标签"),
			run: () => this.configurationService.updateValue('workbench.panel.showLabels', !panelShowLabels)
		});

		actions.push(...[
			new Separator(),
			new SubmenuAction('workbench.action.panel.position', localize('panel position', "面板位置"), positionActions),
			new SubmenuAction('workbench.action.panel.align', localize('align panel', "对齐面板"), alignActions),
			toggleShowLabelsAction,
			toAction({ id: TogglePanelAction.ID, label: localize('hidePanel', "\"隐藏\" 面板"), run: () => this.commandService.executeCommand(TogglePanelAction.ID) }),
		]);
	}
