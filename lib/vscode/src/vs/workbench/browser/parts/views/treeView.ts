...
const noDataProviderMessage = localize('no-dataprovider', "没有可提供视图数据的已注册数据提供程序。");
...
abstract class AbstractTreeView extends Disposable implements ITreeView {
	...
	private initializeShowCollapseAllAction(startingValue: boolean = false) {
		if (!this.collapseAllContext) {
			this.collapseAllContextKey = new RawContextKey<boolean>(`treeView.${this.id}.enableCollapseAll`, startingValue, localize('treeView.enableCollapseAll', "ID 为 {0} 的树状视图是否启用全部折叠。", this.id));
	...
	private initializeShowRefreshAction(startingValue: boolean = false) {
		if (!this.refreshContext) {
			this.refreshContextKey = new RawContextKey<boolean>(`treeView.${this.id}.enableRefresh`, startingValue, localize('treeView.enableRefresh', "ID 为 {0} 的树状视图是否启用刷新。", this.id));
		...
		this._register(registerAction2(class extends Action2 {
			constructor() {
				super({
					...
					title: localize('collapseAll', "全部折叠"),
					menu: {
						id: MenuId.ViewTitle,
			...
			accessibilityProvider: {
				getAriaLabel(element: ITreeItem): string | null {
					...
					if (isString(element.tooltip)) {
						return treeMenus.getResourceActions([element]).length > 0 ? localize('treeAriaLabelHasActionsTooltip', "{0}，有操作", element.tooltip) : element.tooltip;
					} else {
						...
						if (treeMenus.getResourceActions([element]).length > 0) {
							buildAriaLabel = buildAriaLabel ? localize('treeAriaLabelHasActionsSuffix', "{0}，有操作", buildAriaLabel.trim()) : localize('treeAriaLabelHasActions', "有操作");
						}
	...
	private initializeCollapseAllToggle() {
		if (!this.collapseAllToggleContext) {
			this.collapseAllToggleContextKey = new RawContextKey<boolean>(`treeView.${this.id}.toggleCollapseAll`, false, localize('treeView.toggleCollapseAll', "ID 为 {0} 的树状视图是否切换为全部折叠。", this.id));
...
class TreeRenderer extends Disposable implements ITreeRenderer<ITreeItem, FuzzyScore, ITreeExplorerTemplateData> {
	...
	renderElement(element: ITreeNode<ITreeItem, FuzzyScore>, index: number, templateData: ITreeExplorerTemplateData): void {
		...
		if (menuActions.length > 0) {
			...
			templateData.actionBar.setAriaLabel(itemName ? localize('treeActionBarAriaLabel', "针对 {0} 的操作", itemName) : localize('treeActionBarAriaLabelNoName', "操作"));
		} else {
	...
	constructor(notificationService: INotificationService, private getSelectedResources: (() => ITreeItem[])) {
		...
		this._register(this.onDidRun(e => {
			if (e.error && !isCancellationError(e.error)) {
				notificationService.error(localize('command-error', '运行命令 {1} 错误: {0}。这可能是由提交 {1} 的扩展引起的。', e.error.message, e.action.id));
			}
		}));
	}
