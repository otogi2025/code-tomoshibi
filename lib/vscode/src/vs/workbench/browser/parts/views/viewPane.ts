...
const viewPaneContainerExpandedIcon = registerIcon('view-pane-container-expanded', Codicon.chevronDown, nls.localize('viewPaneContainerExpandedIcon', '已展开的视图窗格容器的图标。'));
const viewPaneContainerCollapsedIcon = registerIcon('view-pane-container-collapsed', Codicon.chevronRight, nls.localize('viewPaneContainerCollapsedIcon', '已折叠的视图窗格容器的图标。'));
...
class ViewWelcomeController {
	...
	private onDidChangeViewWelcomeState(): void {
		...
		this.element = $('.welcome-view-content', { tabIndex: 0, role: 'region', 'aria-label': nls.localize('welcomeViewAriaLabel', "欢迎") });
		if (this._wide) {
...
export abstract class ViewPane extends Pane implements IView {
	...
	protected renderHeader(container: HTMLElement): void {
		...
		this.toolbar = this.instantiationService.createInstance(WorkbenchToolBar, actions, {
			...
			ariaLabel: nls.localize('viewToolbarAriaLabel', "{0}操作", this.title),
	...
	private _getAriaLabel(title: string, description: string | undefined): string {
		...
		return nls.localize('viewAccessibilityHelp', '使用 Alt+F1 获取辅助功能帮助 {0}', title);
	}
