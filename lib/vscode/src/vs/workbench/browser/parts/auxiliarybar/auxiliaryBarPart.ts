...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	private fillExtraContextMenuActions(actions: IAction[]): void {
		...
		const toggleShowLabelsAction = toAction({
			...
			label: this.configuration.showLabels ? localize('showIcons', "显示图标") : localize('showLabels', "显示标签"),
		...
		actions.push(...[
			new Separator(),
			new SubmenuAction('workbench.action.panel.position', localize('activity bar position', "活动栏位置"), positionActions),
			toAction({ id: ToggleSidebarPositionAction.ID, label: currentPositionRight ? localize('move second side bar left', "向左移动辅助边栏") : localize('move second side bar right', "向右移动辅助边栏"), run: () => this.commandService.executeCommand(ToggleSidebarPositionAction.ID) }),
			toggleShowLabelsAction,
			toAction({ id: ToggleAuxiliaryBarAction.ID, label: localize('hide second side bar', "隐藏辅助侧栏"), run: () => this.commandService.executeCommand(ToggleAuxiliaryBarAction.ID) })
		]);
	}
