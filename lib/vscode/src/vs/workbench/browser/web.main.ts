...
export class BrowserMain extends Disposable {
	...
	private async initServices(): Promise<{ serviceCollection: ServiceCollection; configurationService: IWorkbenchConfigurationService; logService: ILogService }> {
		...
		// Code-Tomoshibi has no product account system. Keep only the inert policy and
		// account contracts that configuration / editor services inject; no account or
		// sign-up path is created, and every account query answers "signed out".
		const policyService = new NullPolicyService();
		serviceCollection.set(IPolicyService, policyService);
		serviceCollection.set(IDefaultAccountService, new NullDefaultAccountService());
