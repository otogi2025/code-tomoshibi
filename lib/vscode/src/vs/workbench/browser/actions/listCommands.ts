...
registerAction2(class ToggleStickyScroll extends Action2 {
	constructor() {
		super({
			...
			title: {
				...localize2('toggleTreeStickyScroll', "切换树粘滞滚动"),
				mnemonicTitle: localize({ key: 'mitoggleTreeStickyScroll', comment: ['&& denotes a mnemonic'] }, "切换树粘滞滚动(&&T)"),
			},
			category: 'View',
			metadata: { description: localize('toggleTreeStickyScrollDescription', "切换树结构顶部的粘滞滚动小组件，例如文件资源管理器和“调试变量”视图。") },
			f1: true
		});
	}
