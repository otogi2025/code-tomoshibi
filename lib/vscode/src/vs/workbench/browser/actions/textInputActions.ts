...
export function createTextInputActions(clipboardService: IClipboardService, logService: ILogService): IAction[] {
	return [
		...
		toAction({ id: 'undo', label: localize('undo', "撤消"), run: () => getActiveDocument().execCommand('undo') }),
		toAction({ id: 'redo', label: localize('redo', "恢复"), run: () => getActiveDocument().execCommand('redo') }),
		new Separator(),
		toAction({
		...
		new Separator(),
		toAction({ id: 'editor.action.selectAll', label: localize('selectAll', "选择全部"), run: () => getActiveDocument().execCommand('selectAll') })
	];
}
