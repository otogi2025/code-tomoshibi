			...
			if (folders !== undefined && commandService !== undefined) {
				if (folders.length === 1) {
					...
					const options: IPickOptions<IQuickPickItem> = {
						placeHolder: localize('workbench.action.terminal.newWorkspacePlaceholder', "选择当前工作目录新建终端")
					};
...
export class TerminalLaunchHelpAction extends Action {
	...
	constructor(
		...
		super('workbench.action.terminal.launchHelp', localize('terminalLaunchHelp', "打开帮助"));
	}
...
/** The panel title bar's Session tree button. Its flyout lives in terminalView.ts. */
export const TOMOSHIBI_SESSION_TREE_ACTION_ID = 'tomoshibi.session.tree';

/**
 * Session management shared by the strip's context menu, the overflow (...) menu and the Session
 * manager quick pick, so the three entry points can never drift apart.
 */
export async function renameTomoshibiSession(instance: ITerminalInstance, sessionService: ITomoshibiSessionService, anchor?: HTMLElement): Promise<void> {
	const value = await promptTomoshibiInput(anchor, localize('tomoshibi.session.renamePrompt', "输入 Session 名称"), {
		value: sessionService.getTitle(instance) || instance.title,
	});
	if (value === undefined) {
		return;
	}
	const title = value.trim();
	sessionService.setTitle(instance, title || undefined);
	await instance.rename(title || undefined);
}

export async function chooseTomoshibiSessionGroup(instance: ITerminalInstance, sessionService: ITomoshibiSessionService, anchor?: HTMLElement): Promise<void> {
	const current = sessionService.getGroupOf(instance);
	const choices: ITomoshibiPopoverChoice[] = [
		{ id: '__new__', icon: 'add', label: localize('tomoshibi.session.newGroup', "新建分组") },
		// Leaving a group is only offered once the Session is actually in one.
		...(current ? [{ id: '', icon: 'circle-slash', label: localize('tomoshibi.session.leaveGroup', "移出分组") }] : []),
		...sessionService.groups.map(candidate => ({ id: candidate.id, icon: 'folder', label: candidate.name, current: candidate.id === current?.id })),
	];
	const picked = await pickTomoshibiChoice(anchor, localize('tomoshibi.session.groupPrompt', "把这个 Session 放进哪个分组？"), choices);
	if (picked === undefined) {
		return;
	}
	let groupId = picked;
	if (groupId === '__new__') {
		const name = (await promptTomoshibiInput(anchor, localize('tomoshibi.session.newGroupPrompt', "输入新分组名称")))?.trim() ?? '';
		if (!name) {
			return;
		}
		groupId = sessionService.createGroup(name).id;
	}
	sessionService.setGroupOf(instance, groupId || undefined);
	if (groupId) {
		// Dropping a Session into a collapsed group would otherwise make it vanish from the strip
		// with no visible feedback, which on iPad reads as the command having failed.
		expandOnlyTomoshibiGroup(sessionService, groupId);
	}
}

/** `pinned` is the state to move to, not the state the Session is in. */
export function setTomoshibiSessionPinned(instance: ITerminalInstance, pinned: boolean, sessionService: ITomoshibiSessionService, groupService: ITerminalGroupService): void {
	sessionService.setPinned(instance, pinned);
	if (!pinned) {
		groupService.moveGroupToEnd(instance);
		return;
	}
	const first = groupService.groups[0]?.activeInstance;
	if (first && first !== instance) {
		groupService.moveGroup(instance, first);
	}
}
...
export function registerTerminalActions() {
	registerTerminalAction({
		id: 'tomoshibi.newTerminal',
		title: localize2('tomoshibi.newTerminal', '新建终端'),
		f1: false,
		run: async c => {
			const instance = await c.service.createTerminal({ location: TerminalLocation.Panel });
	...
	// `c.service.activeInstance` rather than the group service's, so terminals opened in the
	// editor area are covered too.
	registerTerminalAction({
		id: 'tomoshibi.escape',
		title: localize2('tomoshibi.escape', '发送 Esc'),
		f1: false,
		run: c => c.service.activeInstance?.sendText('\u001b', false),
	});

	registerTerminalAction({
		id: 'tomoshibi.enter',
		title: localize2('tomoshibi.enter', '发送回车'),
		f1: false,
		run: c => c.service.activeInstance?.sendText('\r', false),
	});

	// Shift+Enter 发「反斜杠 + CR」，跟 terminalInstance.ts 里 iPad 硬件键盘那条路同一串。
	// 行尾反斜杠是 bash 的续行符（回车后进续行提示符，命令不执行），Claude Code 也把
	// 「反斜杠 + Enter」读成换行 —— 它官方 /terminal-setup 写进 keybindings 的就是这串，两边都对。
	// ⛔ 不用 ESC + CR：那在 bash 的 readline 里是 M-CR、默认没绑定，裸 shell 下按了毫无反应。
	// ⛔ 末尾不许补 LF —— bash 会在续行状态下把它当成又一次回车，直接把命令提交掉。
	// `when` is terminalFocus so it also covers editor terminals.
	registerTerminalAction({
		id: 'tomoshibi.newline',
		title: localize2('tomoshibi.newline', '插入换行'),
		f1: false,
		keybinding: {
	...
	registerTerminalAction({
		id: 'tomoshibi.scrollBottom',
		title: localize2('tomoshibi.scrollBottom', '回到最新'),
		f1: false,
		run: c => c.groupService.activeInstance?.xterm?.scrollToBottom(),
	});

	registerAction2(class extends Action2 {
		constructor() {
			super({ id: 'tomoshibi.logout', title: localize2('tomoshibi.logout', '回到密码页'), f1: false });
		}

		override run(accessor: ServicesAccessor): Promise<unknown> {
			// Use code-server's registered logout command so the cookie is cleared
			// with the exact base path on root and sub-path deployments.
			return accessor.get(ICommandService).executeCommand('code-server.logout');
		}
	});

	// The settings page owns this switch; default on so an unset profile keeps the old behaviour.
	const shouldInsertUploadPath = (accessor: ServicesAccessor): boolean =>
		accessor.get(IConfigurationService).getValue<boolean>('tomoshibi.upload.insertPath') !== false;

	registerAction2(class extends Action2 {
		constructor() {
			super({ id: 'tomoshibi.upload', title: localize2('tomoshibi.upload', '上传文件'), f1: false });
		}

		override run(accessor: ServicesAccessor): Promise<unknown> {
			return accessor.get(ICommandService).executeCommand('explorer.upload', {
				tomoshibiTarget: 'activeTerminal',
				insertIntoTerminal: shouldInsertUploadPath(accessor),
			});
		}
	});

	registerTerminalAction({
		id: 'workbench.action.terminal.tomoshibiSessionManager',
		title: localize2('workbench.action.terminal.tomoshibiSessionManager', 'Session 管理中心'),
		menu: {
			id: MenuId.ViewTitle,
			group: '2_tomoshibi',
			order: 1,
	...
	// The five entries of the terminal panel's overflow (...) menu. They act on the active Session
	// and share their implementation with the strip's own context menu.
	const tomoshibiSessionMenu = (order: number, when?: ContextKeyExpression) => ({
		id: MenuId.ViewTitle,
		group: '1_tomoshibi_session',
		order,
		when: when ? ContextKeyExpr.and(ContextKeyExpr.equals('view', TERMINAL_VIEW_ID), when) : ContextKeyExpr.equals('view', TERMINAL_VIEW_ID),
	});

	registerTerminalAction({
		id: 'tomoshibi.session.rename',
		title: localize2('tomoshibi.session.rename', '重命名'),
		menu: tomoshibiSessionMenu(1),
		precondition: sharedWhenClause.terminalAvailable,
		run: async (c, accessor) => {
			const instance = c.groupService.activeInstance;
			if (instance) {
				await renameTomoshibiSession(instance, accessor.get(ITomoshibiSessionService));
			}
		},
	});

	registerTerminalAction({
		id: 'tomoshibi.session.joinGroup',
		title: localize2('tomoshibi.session.joinGroup', '加入分组…'),
		menu: tomoshibiSessionMenu(2),
		precondition: sharedWhenClause.terminalAvailable,
		run: async (c, accessor) => {
			const instance = c.groupService.activeInstance;
			if (instance) {
				await chooseTomoshibiSessionGroup(instance, accessor.get(ITomoshibiSessionService));
			}
		},
	});

	registerTerminalAction({
		id: 'tomoshibi.session.pin',
		title: localize2('tomoshibi.session.pin', '固定'),
		menu: tomoshibiSessionMenu(3, TerminalContextKeys.tomoshibiActiveSessionPinned.negate()),
		precondition: sharedWhenClause.terminalAvailable,
		run: (c, accessor) => {
			const instance = c.groupService.activeInstance;
			if (instance) {
				setTomoshibiSessionPinned(instance, true, accessor.get(ITomoshibiSessionService), c.groupService);
			}
		},
	});

	registerTerminalAction({
		id: 'tomoshibi.session.unpin',
		title: localize2('tomoshibi.session.unpin', '取消固定'),
		menu: tomoshibiSessionMenu(3, TerminalContextKeys.tomoshibiActiveSessionPinned),
		precondition: sharedWhenClause.terminalAvailable,
		run: (c, accessor) => {
			const instance = c.groupService.activeInstance;
			if (instance) {
				setTomoshibiSessionPinned(instance, false, accessor.get(ITomoshibiSessionService), c.groupService);
			}
		},
	});

	registerTerminalAction({
		id: 'tomoshibi.session.close',
		title: localize2('tomoshibi.session.close', '关闭 Session'),
		menu: tomoshibiSessionMenu(4),
		precondition: sharedWhenClause.terminalAvailable,
		run: async (c, accessor) => {
			const instance = c.groupService.activeInstance;
			if (instance) {
				await closeTomoshibiSession(instance, accessor.get(ITomoshibiSessionService), c.service, c.groupService);
			}
		},
	});
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.newInActiveWorkspace', '创建新终端(在活动工作区中)'),
		run: async (c) => {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.createTerminalEditor', '在编辑器区域内创建新终端'),
		run: async (c, _, args) => {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.createTerminalEditorSide', '在一侧的编辑器区域内创建新终端'),
		run: async (c) => {
			const instance = await c.service.createTerminal({
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.focusPreviousPane', '在终端组中聚焦上一终端'),
		keybinding: {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.focusNextPane', '在终端组中聚焦下一终端'),
		keybinding: {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.resizePaneLeft', '向左重设终端大小'),
		keybinding: {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.resizePaneRight', '向右重设终端大小'),
		keybinding: {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.resizePaneUp', '向上重设终端大小'),
		keybinding: {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.resizePaneDown', '向下重设终端大小'),
		keybinding: {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.focus.tabsView', '焦点终端选项卡视图'),
		keybinding: {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.focusNext', '聚焦下一终端组'),
		precondition: sharedWhenClause.terminalAvailable,
		keybinding: {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.focusPrevious', '聚焦上一终端组'),
		precondition: sharedWhenClause.terminalAvailable,
		keybinding: {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.runSelectedText', '在活动终端运行所选文本'),
		run: async (c, accessor) => {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.runActiveFile', '在活动终端中运行活动文件'),
		precondition: sharedWhenClause.terminalAvailable,
		run: async (c, accessor) => {
			const codeEditorService = accessor.get(ICodeEditorService);
			...
			if ((!isRemote && uri.scheme !== Schemas.file && uri.scheme !== Schemas.vscodeUserData) || (isRemote && uri.scheme !== Schemas.vscodeRemote)) {
				notificationService.warn(localize('workbench.action.terminal.runActiveFile.noFile', '只有磁盘上的文件可在终端上运行'));
				return;
			}
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.scrollDown', '向下滚动(行)'),
		keybinding: {
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.scrollDownPage', '向下滚动(页)'),
		keybinding: {
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.scrollToBottom', '滚动到底部'),
		keybinding: {
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.scrollUp', '向上滚动(行)'),
		keybinding: {
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.scrollUpPage', '向上滚动(页)'),
		f1: true,
		keybinding: {
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.scrollToTop', '滚动到顶部'),
		keybinding: {
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.clearSelection', '取消选择'),
		keybinding: {
	...
	registerActiveInstanceAction({
		...
		title: localize2('workbench.action.terminal.detachSession', '拆离会话'),
		run: (activeInstance) => activeInstance.detachProcessAndDispose(TerminalExitReason.User)
	});

	registerTerminalAction({
		id: TerminalCommandId.AttachToSession,
		title: localize2('workbench.action.terminal.attachToSession', '附加到会话'),
		run: async (c, accessor) => {
			...
			if (items.length === 0) {
				notificationService.info(localize('noUnattachedTerminals', '没有未附加但要附加到的终端'));
				return;
			}
	...
	registerActiveInstanceAction({
		...
		menu: [
			// Code-Tomoshibi's compact panel overflow menu is reserved for Session management; the
			// command and its Cmd+Up / Cmd+Down keybinding stay.
	...
	registerActiveInstanceAction({
		...
		title: localize2('workbench.action.terminal.selectToPreviousCommand', '选择到上一条命令'),
		keybinding: {
	...
	registerActiveInstanceAction({
		...
		title: localize2('workbench.action.terminal.selectToNextCommand', '选择到下一条命令'),
		keybinding: {
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.selectToPreviousLine', '选择到上一行'),
		precondition: sharedWhenClause.terminalAvailable,
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.selectToNextLine', '选择到下一行'),
		precondition: sharedWhenClause.terminalAvailable,
		run: async (xterm, _, instance) => {
		...
		metadata: {
			...
			args: [{
				...
				schema: {
					...
					properties: {
						cwd: {
							description: localize('workbench.action.terminal.newWithCwd.cwd', "启动终端的目录"),
							type: 'string'
						}
					},
				}
	...
	registerActiveInstanceAction({
		...
		metadata: {
			...
			args: [{
				...
				schema: {
					...
					properties: {
						name: {
							description: localize('workbench.action.terminal.renameWithArg.name', "终端的新名称"),
							type: 'string',
							minLength: 1
						}
					}
		...
		run: async (activeInstance, c, accessor, args) => {
			...
			if (!name) {
				notificationService.warn(localize('workbench.action.terminal.renameWithArg.noName', "未提供名称参数"));
				return;
			}
	...
	registerActiveInstanceAction({
		...
		title: localize2('workbench.action.terminal.relaunch', '重新启动活动终端'),
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.joinInstance', '联接终端'),
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.join', '联接终端...'),
		precondition: sharedWhenClause.terminalAvailable,
		run: async (c, accessor) => {
			...
			if (c.groupService.instances.length <= 1) {
				notificationService.warn(localize('workbench.action.terminal.join.insufficientTerminals', '终端不足，无法执行联接操作'));
				return;
			}
			...
			if (picks.length === 0) {
				notificationService.warn(localize('workbench.action.terminal.join.onlySplits', '所有终端已联接'));
				return;
			}
	...
	registerActiveInstanceAction({
		...
		title: localize2('workbench.action.terminal.splitInActiveWorkspace', '拆分终端 (活动工作区)'),
		run: async (instance, c) => {
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.selectAll', '选择全部'),
		precondition: sharedWhenClause.terminalAvailable,
		keybinding: [{
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.new', '创建新的终端'),
		precondition: ContextKeyExpr.or(TerminalContextKeys.processSupported, TerminalContextKeys.webExtensionContributedProfile),
	...
	// The panel title bar's close button. It goes through closeTomoshibiSession so that it forgets
	// the Session metadata exactly like the overflow menu's entry does; killing the instance alone
	// used to leave a stale Session entry behind.
	registerTerminalAction({
		id: TerminalCommandId.Kill,
		title: localize2('workbench.action.terminal.kill', '关闭 Session'),
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.killAll', '终止所有终端'),
		precondition: ContextKeyExpr.or(sharedWhenClause.terminalAvailable, TerminalContextKeys.isOpen),
		icon: Codicon.trash,
		run: async (c) => {
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.killEditor', '终止编辑器区域中的活动终端'),
		precondition: sharedWhenClause.terminalAvailable,
		keybinding: {
	...
	registerActiveInstanceAction({
		...
		title: localize2('workbench.action.terminal.clear', '清除'),
		precondition: sharedWhenClause.terminalAvailable,
		keybinding: [{
			primary: 0,
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.selectDefaultShell', '选择默认配置文件'),
		run: (c) => c.service.showProfileQuickPick('setDefault')
	});

	registerTerminalAction({
		id: TerminalCommandId.ConfigureTerminalSettings,
		title: localize2('workbench.action.terminal.openSettings', '配置终端设置'),
		precondition: sharedWhenClause.terminalAvailable,
	...
	registerActiveInstanceAction({
		...
		title: localize2('workbench.action.terminal.setFixedDimensions', '设置固定维度'),
		precondition: sharedWhenClause.terminalAvailable_and_opened,
	...
	registerTerminalAction({
		...
		title: localize2('workbench.action.terminal.switchTerminal', '切换 Session'),
		precondition: sharedWhenClause.terminalAvailable,
		run: c => c.groupService.showPanel(true),
	});
}
...
function getSelectedViewInstances(accessor: ServicesAccessor, args?: unknown, args2?: unknown): ITerminalInstance[] | undefined {
	const contextualInstances = getSelectedViewInstances2(accessor, args) ?? getSelectedViewInstances2(accessor, args2);
	if (contextualInstances?.length) {
		return contextualInstances;
	}
	const terminalGroupService = accessor.get(ITerminalGroupService);
	return terminalGroupService.activeInstance ? [terminalGroupService.activeInstance] : undefined;
}

export function validateTerminalName(name: string): { content: string; severity: Severity } | null {
	if (!name || name.trim().length === 0) {
		return {
			content: localize('emptyTerminalNameInfo', "不提供名称会将其重置为默认值"),
	...
	newWithProfileAction = registerAction2(class extends Action2 {
		constructor() {
			super({
				...
				title: localize2('workbench.action.terminal.newWithProfile', '创建新终端(具有个人资料)'),
				f1: true,
				precondition: ContextKeyExpr.or(TerminalContextKeys.processSupported, TerminalContextKeys.webExtensionContributedProfile),
				metadata: {
					...
					args: [{
						...
						schema: {
							...
							properties: {
								profileName: {
									description: localize('workbench.action.terminal.newWithProfile.profileName', "要创建的配置文件的名称"),
									type: 'string',
									enum: profileEnum.values,
									markdownEnumDescriptions: profileEnum.markdownDescriptions
								},
								location: {
									description: localize('newWithProfile.location', "创建终端的位置"),
									type: 'string',
									enum: ['view', 'editor'],
									enumDescriptions: [
										localize('newWithProfile.location.view', '在终端视图中创建终端'),
										localize('newWithProfile.location.editor', '在编辑器中创建终端'),
									]
								}
							}
						}
...
async function pickTerminalCwd(accessor: ServicesAccessor, cancel?: CancellationToken): Promise<WorkspaceFolderCwdPair | undefined> {
	...
	const folderPicks: Item[] = shrinkedPairs.map(pair => {
		...
		const description = pair.isOverridden
			? localize('workbench.action.terminal.overriddenCwdDescription', "(已替代) {0}", labelService.getUriLabel(pair.cwd, { relative: !pair.isAbsolute }))
...
async function renameWithQuickPick(c: ITerminalServicesCollection, accessor: ServicesAccessor, resource?: unknown) {
	...
	if (instance) {
		const title = await accessor.get(IQuickInputService).input({
			...
			prompt: localize('workbench.action.terminal.rename.prompt', "输入终端名称"),
		});
		if (title) {
