 ...
 *
 * `shiftEnterNewline` mirrors the `tomoshibi.terminal.shiftEnterNewline` setting: when it is
 * off, Shift+Return sends a plain carriage return like an unmodified Return.
 *
 * 换行序列是「反斜杠 + CR」（`'\\\r'`），不是 ESC + CR：这条 iPad 硬件路径 preventDefault 之后
 * 到不了键绑定系统，所以它发什么就是什么。ESC + CR 在 bash 的 readline 里是 M-CR、默认没有绑定，
 * 裸 shell 下按 Shift+Return 会毫无反应（iPad 上实测）。行尾反斜杠是 bash 的续行符，
 * Claude Code 也把「反斜杠 + Enter」读成换行（它官方 /terminal-setup 写进 keybindings 的就是这串），
 * 两边都对。⛔ 末尾不许再补 LF —— bash 会在续行状态下把它当成又一次回车，直接把命令提交掉。
 */
export function getIPadHardwareEnterData(
	event: KeyboardEvent,
	isiOS: boolean = isIOS,
	compositionActive: boolean = event.isComposing || event.keyCode === 229,
	shiftEnterNewline: boolean = true
): string | null | undefined {
	const isEnter = event.key === 'Enter' || event.key === 'Return' || event.code === 'Enter' || event.code === 'NumpadEnter' || event.keyCode === 13;
	if (!isiOS || !isEnter || compositionActive || event.ctrlKey || event.altKey || event.metaKey) {
		return undefined;
	}
	return event.type === 'keydown' ? (event.shiftKey && shiftEnterNewline ? '\\\r' : '\r') : null;
}

/**
 * 返回 iPad 中文输入法下 `beforeinput` 需要我们自己写进 PTY 的数据，`undefined` 表示交回 xterm 处理。
 *
 * 为什么需要这段（xterm 6.1.0-beta.291，node_modules/@xterm/xterm/src/browser，⛔ 别当无用代码删）：
 * 1. `input/CompositionHelper.ts` 的 `keydown()`（约 :113-137）对 keyCode 229 一律 `return false`：
 *    合成中直接返回；未合成时只排一个 `setTimeout(0)` 去 diff textarea。这条路一个字节都不会发。
 * 2. `CoreBrowserTerminal.ts` 的 `_inputEvent()`（约 :1032）兜底条件是
 *    `ev.data && ev.inputType === 'insertText' && (!ev.composed || !this._keyDownSeen) && !screenReaderMode`。
 *    受信 InputEvent 的 `composed` 恒为 true，所以只有「当前没有未配对的 keydown」时才会补发。
 *    `_keyDownSeen` 在 `_keyDown` 开头置 true（约 :849）、只在 `_keyUp` 复位（约 :951）。
 * iOS 中文键盘打「，」「。」时 Safari 先发 keyCode 229 的 keydown（1 把它吃掉、不发数据），
 * 紧接着的 input 又因为 `_keyDownSeen` 仍为 true 被 2 丢弃 —— 于是标点整个消失。
...
export function getIPadImeBeforeInputData(
	...
	// 自动更正（insertReplacementText）等一律放行，接管过来反而会改坏现有行为。
	if (inputType === 'insertText') {
		return data ? data : undefined;
	}
	// 中文键盘下的 Return 也可能只报 keyCode 229、key/code 都认不出，于是既进不了
	// `getIPadHardwareEnterData`，又被上面第 1 条吃掉。确认候选的那次 Return 会被输入法自己消化、
	// 不产生 insertLineBreak，所以这里只会收到「真的要换一行」的那种。
...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	public static getXtermConstructor(keybindingService: IKeybindingService, contextKeyService: IContextKeyService) {
		...
		xtermConstructor = Promises.withAsyncBody<typeof XTermTerminal>(async (resolve) => {
			...
			// Localize strings
			Terminal.strings.promptLabel = nls.localize('terminal.integrated.a11yPromptLabel', '终端输入');
			Terminal.strings.tooMuchOutput = keybinding ? nls.localize('terminal.integrated.useAccessibleBuffer', '使用可访问缓冲区 {0} 手动查看输出', keybinding.getLabel()) : nls.localize('terminal.integrated.useAccessibleBufferNoKb', '使用终端: 聚焦可访问缓冲区命令以手动查看输出');
			resolve(Terminal);
		});
		return xtermConstructor;
	}
	...
	protected async _createXterm(): Promise<XtermTerminal | undefined> {
			...
			this._register(xterm.raw.onBell(() => {
				if (this._configurationService.getValue(TerminalSettingId.EnableBell) || this._configurationService.getValue(TerminalSettingId.EnableVisualBell)) {
					this.statusList.add({
						...
						icon: Codicon.bell,
						tooltip: nls.localize('bellStatus', "铃")
		...
		this._register(VSCodeEvent.runAndSubscribe(xterm.shellIntegration.onDidChangeSeenSequences, () => {
	...
	private _refreshShellIntegrationInfoStatus(instance: ITerminalInstance) {
		...
		const cmdDetectionType = (
			instance.capabilities.get(TerminalCapability.CommandDetection)?.hasRichCommandDetection
				? nls.localize('shellIntegration.rich', '丰富')
				: instance.capabilities.has(TerminalCapability.CommandDetection)
					? nls.localize('shellIntegration.basic', '基本')
					: instance.usedShellIntegrationInjection
						? nls.localize('shellIntegration.injectionFailed', "注入未能激活")
						: nls.localize('shellIntegration.no', '否')
		);
		...
		instance.statusList.add({
			...
			tooltip: `${nls.localize('shellIntegration', "Shell 集成")}: ${cmdDetectionType}`,
			detailedTooltip: `${nls.localize('shellIntegration', "Shell 集成")}: ${cmdDetectionType}${detailedAdditionsString}`
		});
	}
	...
	private _open(): void {
		...
		// Safari sometimes leaves KeyboardEvent.isComposing/keyCode 229 stuck after the
		// iPad switches away from the tab. Track the real composition lifecycle on the
		// xterm textarea instead, and explicitly cancel an unfinished composition when
		// the page is backgrounded/restored. Otherwise hardware Return is discarded
		// until the software keyboard happens to emit another compositionend event.
		let iPadCompositionActive = false;
		...
		if (isIOS) {
			...
			const suppressIPadImeCopyText = (event: Event) => {
				...
				if (event.type === 'input' && xterm.raw.textarea!.value.toLowerCase() === 'c') {
					xterm.raw.textarea!.value = '';
				}
			};
			this._register(dom.addDisposableListener(xterm.raw.textarea, 'beforeinput', suppressIPadImeCopyText, true));
			this._register(dom.addDisposableListener(xterm.raw.textarea, 'input', suppressIPadImeCopyText, true));
			this._register(dom.addDisposableListener(xterm.raw.textarea, 'compositionend', suppressIPadImeCopyText, true));

			// 中文输入法下的「，」「。」和部分 Return 会被 xterm 整个丢掉（原因见
			// getIPadImeBeforeInputData 的注释）。这里镜像 xterm 的 `_keyDownSeen`，在它一定会丢弃的
			...
			let restoreFocusOnResume = false;
			let pendingFocusRestore: IDisposable | undefined;
			const suspendIPadTerminal = () => {
				restoreFocusOnResume ||= xterm.raw.textarea!.ownerDocument.activeElement === xterm.raw.textarea || this.hasFocus;
				pendingFocusRestore?.dispose();
				pendingFocusRestore = undefined;
				resetIPadComposition();
			};
			const resumeIPadTerminal = () => {
				resetIPadComposition();
				if (!restoreFocusOnResume || !this._isVisible) {
					return;
				}
				restoreFocusOnResume = false;
				pendingFocusRestore?.dispose();
				pendingFocusRestore = dom.scheduleAtNextAnimationFrame(targetWindow, () => {
					pendingFocusRestore = undefined;
					if (this._isVisible) {
						xterm.raw.focus();
					}
			...
			this._register(dom.addDisposableListener(targetWindow.document, 'visibilitychange', () => {
			...
			this._register(dom.addDisposableListener(targetWindow, 'pagehide', suspendIPadTerminal));
			this._register(dom.addDisposableListener(targetWindow, 'pageshow', resumeIPadTerminal));
			this._register(dom.addDisposableListener(targetWindow, 'blur', suspendIPadTerminal));
			this._register(dom.addDisposableListener(targetWindow, 'focus', resumeIPadTerminal));
			this._register(toDisposable(() => pendingFocusRestore?.dispose()));
		}
		...
		xterm.raw.attachCustomKeyEventHandler((event: KeyboardEvent): boolean => {
			...
			// A Chinese IME masks the physical key as `Process`, but `code` still
			// identifies Cmd+C/Cmd+V. Copy explicitly once and let Cmd+V continue
	...
	protected _createProcessManager(): TerminalProcessManager {
		...
		this._register(processManager.onPtyDisconnect(() => {
			...
			this.statusList.add({
				...
				tooltip: nls.localize('disconnectStatus', "与进程的连接中断")
			});
		}));
	...
	private async _createProcess(): Promise<void> {
		...
		if (!trusted && !(isRemoteTerminal && this._workbenchEnvironmentService.remoteAuthority)) {
			this._onProcessExit({ message: nls.localize('workspaceNotTrustedCreateTerminal', "无法在不受信任的工作区中启动终端流程") });
			...
			this._onProcessExit({
				message: nls.localize('workspaceEmptyCreateTerminalCwd', "无法在 cwd {0} 与 userHome {1} 不同的空工作区中启动终端进程", this._cwd, this._userHome)
			});
		}
	...
	private _relaunchWithShellIntegrationDisabled(exitMessage: string | undefined): void {
		...
		this.statusList.add({
			...
			tooltip: `${exitMessage} ` + nls.localize('launchFailed.exitCodeOnlyShellIntegration', '在用户设置中禁用 shell 集成可能会有所帮助。'),
			hoverActions: [{
				commandId: TerminalCommandId.ShellIntegrationLearnMore,
				label: nls.localize('shellIntegration.learnMore', "了解有关 shell 集成的详细信息"),
				run: () => {
					this._openerService.open('https://code.visualstudio.com/docs/terminal/shell-integration?referrer=in-product');
				}
			}, {
				commandId: 'workbench.action.openSettings',
				label: nls.localize('shellIntegration.openSettings', "打开用户设置"),
				run: () => {
	...
	private async _trust(): Promise<boolean> {
		...
		const trustRequest = await this._workspaceTrustRequestService.requestWorkspaceTrust({
			message: nls.localize('terminal.requestTrust', "创建终端流程需要执行代码")
		});
	...
	private _setAriaLabel(xterm: XTermTerminal | undefined, terminalId: number, title: string | undefined): void {
		...
		if (xterm && xterm.textarea) {
			if (title && title.length > 0) {
				labelParts.push(nls.localize('terminalTextBoxAriaLabelNumberAndTitle', "终端 {0}，{1}", terminalId, title));
			} else {
				labelParts.push(nls.localize('terminalTextBoxAriaLabel', "终端 {0}", terminalId));
			}
			const screenReaderOptimized = this._accessibilityService.isScreenReaderOptimized();
			if (!screenReaderOptimized) {
				labelParts.push(nls.localize('terminalScreenReaderMode', "运行命令: 切换屏幕阅读器辅助功能模式以获得优化的屏幕阅读器体验"));
			}
			...
			if (this._configurationService.getValue(AccessibilityVerbositySettingId.Terminal) && accessibilityHelpKeybinding) {
				labelParts.push(nls.localize('terminalHelpAriaLabel', "使用 {0} 获取终端辅助功能帮助", accessibilityHelpKeybinding));
			}
	...
	async setFixedDimensions(): Promise<void> {
		const cols = await this._quickInputService.input({
			title: nls.localize('setTerminalDimensionsColumn', "设置固定维度: 列"),
		...
		const rows = await this._quickInputService.input({
			title: nls.localize('setTerminalDimensionsRow', "设置固定维度: 行"),
	...
	private _onEnvironmentVariableInfoChanged(info: IEnvironmentVariableInfo): void {
		if (info.requiresAction) {
			this.xterm?.raw.textarea?.setAttribute('aria-label', nls.localize('terminalStaleTextBoxAriaLabel', "终端 {0} 环境已过时，请运行“显示环境信息”命令以获取详细信息", this._instanceId));
		}
	...
	async changeColor(color?: string, skipQuickPick?: boolean): Promise<string | undefined> {
		...
		quickPick.placeholder = nls.localize('changeColor', '选择终端的颜色');
		quickPick.show();
...
class TerminalInstanceDragAndDropController extends Disposable implements dom.IDragAndDropObserverCallbacks {
	...
	get onDropFile(): VSCodeEvent<string | URI> { return this._onDropFile.event; }
	private readonly _onDropTerminal = this._register(new Emitter<IRequestAddInstanceToGroupEvent>());
	get onDropTerminal(): VSCodeEvent<IRequestAddInstanceToGroupEvent> { return this._onDropTerminal.event; }
	...
	/**
	 * 拖拽落点的判定方向恒为 HORIZONTAL（左右并排）：分屏窗格一律左右排，落点只能按 X 坐标分前后，
	 * 再按面板位置算会在面板不在底部时改用 Y 坐标，把「插在前面 / 插在后面」判反。
	 * ⛔ 这跟 terminalGroup._getOrientation() 是一对，改一个必须同时改另一个。
	 */
	...
	switch (typeof exitCodeOrError) {
		case 'number': {
			...
			if (processState === ProcessState.KilledDuringLaunch) {
				if (commandLine) {
					message = nls.localize('launchFailed.exitCodeAndCommandLine', "终端进程“{0}”启动失败(退出代码: {1})。", commandLine, code);
				} else {
					message = nls.localize('launchFailed.exitCodeOnly', "终端进程启动失败(退出代码: {0})。", code);
				}
			} else {
				if (commandLine) {
					message = nls.localize('terminated.exitCodeAndCommandLine', "终端进程“{0}”已终止，退出代码: {1}。", commandLine, code);
				} else {
					message = nls.localize('terminated.exitCodeOnly', "终端进程已终止，退出代码: {0}。", code);
				}
			}
			break;
		}
		case 'object': {
			...
			message = nls.localize('launchFailed.errorMessage', "终端进程启动失败: {0}。", innerMessage);
			break;
		}
	}
