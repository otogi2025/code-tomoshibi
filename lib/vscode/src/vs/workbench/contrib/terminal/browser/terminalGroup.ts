...
export class TerminalGroup extends Disposable implements ITerminalGroup {
	...
	attachToElement(element: HTMLElement): void {
		...
		if (!this._splitPaneContainer) {
			// 位置只留给 resizePane 判方向用；分屏方向恒为左右并排，见 _getOrientation。
			this._panelPosition = this._layoutService.getPanelPosition();
			this._terminalLocation = this._viewDescriptorService.getViewLocationById(TERMINAL_VIEW_ID)!;
			this._splitPaneContainer = this._instantiationService.createInstance(SplitPaneContainer, this._groupElement, this._getOrientation());
	...
	/**
	 * 分屏方向恒为 HORIZONTAL（左右并排），不再跟着面板/视图位置转。
	 * ⛔ resizePane 也读这里，两边必须同一个值，否则左右方向键会去拉整块面板而不是拉窗格分隔线。
	 */
