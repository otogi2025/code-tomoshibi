...
export function getTerminalDecorationHoverContent(command: ITerminalCommand | undefined, hoverMessage?: string, showCommandActions?: boolean): string {
	let hoverContent = showCommandActions ? `${localize('terminalPromptContextMenu', "显示命令操作")}\n\n---\n\n` : '';
	if (!command) {
		if (hoverMessage) {
		...
		if (isNumber(command.duration)) {
			...
			if (command.exitCode) {
				if (command.exitCode === -1) {
					hoverContent += localize('terminalPromptCommandFailed.duration', '命令已执行 {0}，使用了 {1}，但失败', fromNow(command.timestamp, true), durationText);
				} else {
					hoverContent += localize('terminalPromptCommandFailedWithExitCode.duration', '命令已执行 {0}，使用了 {1}，但失败(退出代码 {2})', fromNow(command.timestamp, true), durationText, command.exitCode);
				}
			} else {
				hoverContent += localize('terminalPromptCommandSuccess.duration', '命令已执行 {0} 并使用了 {1}', fromNow(command.timestamp, true), durationText);
			}
		} else {
			if (command.exitCode) {
				if (command.exitCode === -1) {
					hoverContent += localize('terminalPromptCommandFailed', '命令已执行 {0} 并失败', fromNow(command.timestamp, true));
				} else {
					hoverContent += localize('terminalPromptCommandFailedWithExitCode', '命令已执行 {0} 并失败(退出代码 {1})', fromNow(command.timestamp, true), command.exitCode);
				}
			} else {
				hoverContent += localize('terminalPromptCommandSuccess', '命令现在已执行 {0}');
			}
		}
	}
	return hoverContent;
...
const unknownText = localize('terminalCommandDecoration.unknown', '未知');
const runningText = localize('terminalCommandDecoration.running', '正在运行');
...
export function getTerminalCommandDecorationTooltip(command?: ITerminalCommand, storedState?: ITerminalCommandDecorationPersistedState): string {
	...
	if (typeof duration === 'number') {
		...
		if (exitCode) {
			if (exitCode === -1) {
				hoverContent += localize('terminalPromptCommandFailed.duration', '命令已执行 {0}，使用了 {1}，但失败', fromNowText, durationText);
			} else {
				hoverContent += localize('terminalPromptCommandFailedWithExitCode.duration', '命令已执行 {0}，使用了 {1}，但失败(退出代码 {2})', fromNowText, durationText, exitCode);
			}
		} else {
			hoverContent += localize('terminalPromptCommandSuccess.duration', '命令已执行 {0} 并使用了 {1}', fromNowText, durationText);
		}
	} else {
		if (exitCode) {
			if (exitCode === -1) {
				hoverContent += localize('terminalPromptCommandFailed', '命令已执行 {0} 并失败', fromNowText);
			} else {
				hoverContent += localize('terminalPromptCommandFailedWithExitCode', '命令已执行 {0} 并失败(退出代码 {1})', fromNowText, exitCode);
			}
		} else {
			hoverContent += localize('terminalPromptCommandSuccess.', '命令已执行 {0}', fromNowText);
		}
	}
	return hoverContent;
}
