...
export class TerminalProfileQuickpick {
	...
	private async _createAndShow(type: 'setDefault' | 'createInstance'): Promise<IProfileQuickPickItem | undefined> {
		...
		const options: IPickOptions<IProfileQuickPickItem> = {
			placeHolder: type === 'createInstance' ? nls.localize('terminal.integrated.selectProfileToCreate', "选择要创建的终端配置文件") : nls.localize('terminal.integrated.chooseDefaultProfile', "选择默认的终端配置文件"),
			onDidTriggerItemButton: async (context) => {
				...
				const name = await this._quickInputService.input({
					prompt: nls.localize('enterTerminalProfileName', "输入终端配置文件名称"),
					value: context.item.profile.profileName,
					validateInput: async input => {
						if (existingProfiles.includes(input)) {
							return nls.localize('terminalProfileAlreadyExists', "有终端配置文件已具有此名称");
						}
						return undefined;
					}
				});
		...
		if (configProfiles.length > 0) {
			quickPickItems.push({ type: 'separator', label: nls.localize('terminalProfiles', "配置文件") });
		...
		quickPickItems.push({ type: 'separator', label: nls.localize('ICreateContributedTerminalProfileOptions', "已贡献") });
		...
		if (autoDetectedProfiles.length > 0) {
			quickPickItems.push({ type: 'separator', label: nls.localize('terminalProfiles.detected', "已检测") });
	...
	private async _isProfileSafe(profile: ITerminalProfile | IExtensionTerminalProfile): Promise<boolean> {
		...
		return await new Promise<boolean>(r => {
			...
			const handle = this._notificationService.prompt(
				Severity.Warning,
				nls.localize('unsafePathWarning', '此终端配置使用了可由其他用户({0})修改的潜在不安全路径。是否确定要使用它?', `"${unsafePaths.join(',')}"`),
				[{
					label: nls.localize('yes', '是'),
					run: () => r(true)
				}, {
	...
	private _createProfileQuickPickItem(profile: ITerminalProfile): IProfileQuickPickItem {
		const buttons: IQuickInputButton[] = [{
			...
			tooltip: nls.localize('createQuickLaunchProfile', "配置终端配置文件")
		}];
