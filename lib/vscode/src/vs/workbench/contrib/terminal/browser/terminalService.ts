...
export class TerminalService extends Disposable implements ITerminalService {
	...
	private async _reconnectToRemoteTerminals(): Promise<void> {
		...
		await this._firstReconnectedTerminalGroup;
		mark('code/terminal/didRecreateTerminalGroups');
		void this._reconnectedTerminalGroups.then(() => {
			// Do not persist the deliberately partial staged layout. Only start saving once every
			// background Session has reattached.
			this._attachProcessLayoutListeners();
			this._logService.trace('Reconnected to remote terminals');
		}, error => this._logService.error('Failed to restore remote terminal layout', error));
	}
	...
	private async _reconnectToLocalTerminals(): Promise<void> {
		...
		mark('code/terminal/didGetTerminalLayoutInfo');
		// Hidden/background PTYs are intentionally not part of Code-Tomoshibi's
		// session model. Shorten the orphan grace period after claiming the saved
		// visible groups so the backend closes anything that has no Session tab.
		localBackend.reduceConnectionGraceTime();
		...
		const restored = this._reconnectedTerminalGroups ?? Promise.resolve([]);
		void restored.then(() => {
			// Starting these listeners before staged restoration completes would save a partial
			// layout and make untouched Sessions disappear on the next refresh.
			this._attachProcessLayoutListeners();
			this._logService.trace('Reconnected to local terminals');
		}, error => this._logService.error('Failed to restore local terminal layout', error));
	}

	private async _recreateTerminalGroups(layoutInfo?: ITerminalsLayoutInfo): Promise<ITerminalGroup[]> {
		if (!layoutInfo) {
			this._firstReconnectedTerminalGroup = Promise.resolve();
			return [];
		}

		const pending = layoutInfo.tabs
			.map((tabLayout, index) => ({
				tabLayout,
				index,
				terminalLayouts: tabLayout.terminals.filter(t => t.terminal && t.terminal.isOrphan)
			}))
			.filter(entry => entry.terminalLayouts.length > 0)
			.sort((a, b) => Number(b.tabLayout.isActive) - Number(a.tabLayout.isActive) || a.index - b.index);
		if (!pending.length) {
			this._firstReconnectedTerminalGroup = Promise.resolve();
			return [];
		}
		let resolveFirstRestored!: () => void;
		this._firstReconnectedTerminalGroup = new Promise<void>(resolve => resolveFirstRestored = resolve);
		let firstRestored = false;
		const markFirstRestored = () => {
			if (!firstRestored) {
				firstRestored = true;
				resolveFirstRestored();
			}
		};

		// allow-any-unicode-next-line
		/* 弱性能 iPad 先只挂载上次正在看的 Session。原实现会并发创建全部
		 * xterm，多个历史缓冲、字体测量和布局互相争抢，导致当前页反而最后才出现。
		 * 当前组完成后再每隔一小段空档逐组恢复；PTY 一直在服务端运行，不会丢进程。 */
		const restoredBySavedIndex = new Map<number, ITerminalGroup>();
		let selectedGroup: ITerminalGroup | undefined;
		let selectedInstance: ITerminalInstance | undefined;
		try {
			for (let i = 0; i < pending.length; i++) {
				...
				this._restoredGroupCount += entry.terminalLayouts.length;
				const group = await this._recreateTerminalGroup(entry.tabLayout, entry.terminalLayouts);
				if (group) {
					restoredBySavedIndex.set(entry.index, group);
					if (entry.tabLayout.isActive) {
						selectedGroup = group;
						selectedInstance = this.instances.find(instance => instance.shellLaunchConfig.attachPersistentProcess?.id === entry.tabLayout.activePersistentProcessId);
						this._terminalGroupService.activeGroup = selectedGroup;
						if (selectedInstance) {
							this.setActiveInstance(selectedInstance);
							this._restoreActiveViewport(selectedInstance);
						}
					}
				}
				if (i === 0) {
					// The connection and boot mask may proceed as soon as the foreground Session is real.
					markFirstRestored();
				}
			}
		} finally {
		...
		/* 后台恢复完成后还原保存时的标签顺序；首屏期间当前 Session 始终保持选中。 */
		const restored = [...restoredBySavedIndex.entries()]
			.sort((a, b) => a[0] - b[0])
			.map(entry => entry[1]);
		for (let i = 0; i < restored.length; i++) {
			const current = this._terminalGroupService.groups[i];
			const desired = restored[i];
			if (current !== desired && current?.activeInstance && desired.activeInstance) {
				this._terminalGroupService.moveGroup(desired.activeInstance, current.activeInstance);
			}
		}
		// moveGroup preserves the active group by identity. Do not select `selectedGroup` again here:
		// the user may already have switched while the remaining Sessions restored in background.
		if (selectedInstance) {
			this._drainXtermAndScrollToBottom(selectedInstance);
		}
		return restored;
	}

	private _restoreActiveViewport(instance: ITerminalInstance): void {
		/* A persistent terminal is attached before its buffered output has finished replaying.
		 * Put only the saved active Session at live output. raw.write's callback runs after all
		 * xterm writes already queued ahead of it, unlike onWriteParsed which can fire too early. */
		this._drainXtermAndScrollToBottom(instance);
		const replayCompleteListener = instance.onProcessReplayComplete(() => {
			replayCompleteListener.dispose();
			this._drainXtermAndScrollToBottom(instance);
		});
		// Some backends can finish replay before the listener is attached. This bounded fallback
		// drains the queue once more without installing a recurring timer.
		void timeout(1500).then(() => {
			replayCompleteListener.dispose();
			this._drainXtermAndScrollToBottom(instance);
		});
	}

	private _drainXtermAndScrollToBottom(instance: ITerminalInstance): void {
	...
	private _onWillShutdown(e: WillShutdownEvent): void {
		// Code-Tomoshibi runs in a browser: closing the iPad tab is a normal disconnect, not a
		// request to terminate the server-side shells. Detach persistent terminals for every
		// workbench shutdown reason so the next page can reattach to the same processes.
	...
	protected async _showTerminalCloseConfirmation(singleTerminal?: boolean): Promise<boolean> {
		...
		if (foregroundInstances.length === 1 || singleTerminal) {
			message = nls.localize('terminalService.terminalCloseConfirmationSingular', "是否要终止活动终端会话?");
		} else {
			message = nls.localize('terminalService.terminalCloseConfirmationPlural', "是否要终止{0}活动终端会话?", foregroundInstances.length);
		}
		const { confirmed } = await this._dialogService.confirm({
			type: 'warning',
			message,
			primaryButton: nls.localize({ key: 'terminate', comment: ['&& denotes a mnemonic'] }, "终止(&&T)")
		});
		return !confirmed;
	}
	...
	async createTerminal(options?: ICreateTerminalOptions): Promise<ITerminalInstance> {
		...
		if (shellLaunchConfig.hideFromUser) {
			throw new Error('Code-Tomoshibi does not create terminals without a visible Session');
		}
	...
	private _evaluateLocalCwd(shellLaunchConfig: IShellLaunchConfig) {
		...
		if (!isString(shellLaunchConfig.cwd) && shellLaunchConfig.cwd?.scheme === Schemas.file) {
			if (VirtualWorkspaceContext.getValue(this._contextKeyService)) {
				shellLaunchConfig.initialText = formatMessageForTerminal(nls.localize('localTerminalVirtualWorkspace', "此 shell 对{0}本地{1}文件夹开放，而不是虚拟文件夹", '\x1b[3m', '\x1b[23m'), { excludeLeadingNewLine: true, loudFormatting: true });
				shellLaunchConfig.type = 'Local';
			} else if (this._remoteAgentService.getConnection()) {
				shellLaunchConfig.initialText = formatMessageForTerminal(nls.localize('localTerminalRemote', "此 shell 正在{0}本地{1}计算机上运行，而不是在连接的远程计算机上运行", '\x1b[3m', '\x1b[23m'), { excludeLeadingNewLine: true, loudFormatting: true });
				shellLaunchConfig.type = 'Local';
			}
		}
	}
	...
	moveToBackground(instance: ITerminalInstance): void {
		// A terminal that leaves the visible Session model must not keep consuming
		// VPS resources. Code-Tomoshibi closes it instead of maintaining VS Code's
		// separate hidden-terminal pool.
		instance.dispose(TerminalExitReason.User);
	}
