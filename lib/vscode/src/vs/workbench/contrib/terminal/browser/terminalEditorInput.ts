...
export class TerminalEditorInput extends EditorInput implements IEditorCloseHandler {
	...
	async confirm(terminals: ReadonlyArray<IEditorIdentifier>): Promise<ConfirmResult> {
		const { confirmed } = await this._dialogService.confirm({
			...
			message: localize('confirmDirtyTerminal.message', "是否要终止正在运行的进程?"),
			primaryButton: localize({ key: 'confirmDirtyTerminal.button', comment: ['&& denotes a mnemonic'] }, "&&终止"),
			detail: terminals.length > 1 ?
				terminals.map(terminal => terminal.editor.getName()).join('\n') + '\n\n' + localize('confirmDirtyTerminals.detail', "关闭将终止此终端中正在运行的进程。") :
				localize('confirmDirtyTerminal.detail', "关闭将终止此终端中正在运行的进程。")
		});
	...
	private _setupInstanceListeners(): void {
		...
		this._register(this._lifecycleService.onWillShutdown((e: WillShutdownEvent) => {
			...
			// A browser window close must detach, not terminate, the server-side shell.
			const shouldPersistTerminals = this._configurationService.getValue<boolean>(TerminalSettingId.EnablePersistentSessions);
