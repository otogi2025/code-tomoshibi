...
export class TerminalGroupService extends Disposable implements ITerminalGroupService {
	...
	moveGroup(source: SingleOrMany<ITerminalInstance>, target: ITerminalInstance) {
		...
		this._preserveActiveGroupAfterMove(activeGroup);
		this._onDidChangeInstances.fire();
	}
	...
	/**
	 * Moving groups changes their array indexes. Keep the active group attached to the same
	 * terminal instead of silently making whichever group landed at the old index active.
	 */
	private _preserveActiveGroupAfterMove(activeGroup: ITerminalGroup | undefined): void {
		if (!activeGroup) {
			return;
		}
		const index = this.groups.indexOf(activeGroup);
		if (index === -1 || index === this.activeGroupIndex) {
			return;
		}
		this.activeGroupIndex = index;
		this._onDidChangeActiveGroup.fire(activeGroup);
		this._onDidChangeActiveInstance.fire(activeGroup.activeInstance);
	}
