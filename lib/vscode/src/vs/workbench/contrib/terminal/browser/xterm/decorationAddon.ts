...
export class DecorationAddon extends Disposable implements ITerminalAddon, IDecorationAddon {
	...
	private _getContextMenuActions(): IAction[] {
		const label = localize('workbench.action.terminal.toggleVisibility', "切换可见性");
		return [
			{
	...
	private async _getCommandActions(command: ITerminalCommand): Promise<IAction[]> {
		...
		if (command.command !== '') {
			const labelRun = localize("terminal.rerunCommand", '重新运行命令');
			actions.push({
				...
				run: async () => {
					...
					if (!command.isTrusted) {
						const shouldRun = await new Promise<boolean>(r => {
							this._notificationService.prompt(Severity.Info, localize('rerun', '是否要运行命令: {0}', command.command), [{
								label: localize('yes', '是'),
								run: () => r(true)
							}, {
								label: localize('no', '否'),
								run: () => r(false)
			...
			const labelCopy = localize("terminal.copyCommand", '复制命令');
			actions.push({
		...
		if (command.hasOutput()) {
			const labelCopyCommandAndOutput = localize("terminal.copyCommandAndOutput", '复制命令和输出');
			actions.push({
			...
			const labelText = localize("terminal.copyOutput", '复制输出');
			actions.push({
			...
			const labelHtml = localize("terminal.copyOutputAsHtml", '将输出复制为 HTML');
			actions.push({
	...
	private _showToggleVisibilityQuickPick() {
		...
		quickPick.title = localize('toggleVisibility', '切换可见性');
