...
const EXTENSION_STORE_REMOVED_MESSAGE = nls.localize('tomoshibi.extensionStoreRemoved', "Code-Tomoshibi 已移除扩展商店，扩展只能在服务器上用命令行安装");
		...
		if (this.gallery) {
			...
			if (!cache) {
				if (this.gallery.assets.manifest) {
					...
					this.logService.error(nls.localize('Manifest is not found', "未找到清单文件"), this.identifier.id);
				}
			}
			return cache;
		}
...
export class ExtensionsWorkbenchService extends Disposable implements IExtensionsWorkbenchService, IURLHandler {
	...
	async updateAutoUpdateForAllExtensions(isAutoUpdateEnabled: boolean): Promise<void> {
		...
		const result = await this.dialogService.confirm({
			title: nls.localize('confirmEnableDisableAutoUpdate', "自动更新扩展"),
			message: isAutoUpdateEnabled
				? nls.localize('confirmEnableAutoUpdate', "是否要为扩展启用自动更新?")
				: nls.localize('confirmDisableAutoUpdate', "是否要为扩展禁用自动更新?"),
			detail: nls.localize('confirmEnableDisableAutoUpdateDetail', "这将重置你为单个扩展设置的任何自动更新设置。"),
		});
		if (!result.confirmed) {
			return;
		}
	...
	private updateExtensionsPinnedState(pinned: boolean): Promise<void> {
		return this.progressService.withProgress({
			...
			title: nls.localize('updatingExtensions', "正在更新扩展自动更新状态"),
	...
	private computeExtensionsNotifications(): Array<Omit<IExtensionsNotification, 'dismiss'> & { key: string }> {
		...
		if (invalidExtensions.length) {
			if (invalidExtensions.some(e => e.local && e.local.manifest.engines?.vscode &&
				...
				computedNotificiations.push({
					message: nls.localize('incompatibleExtensions', "由于版本不兼容，某些扩展被禁用。查看并更新它们。"),
					severity: Severity.Warning,
				...
				computedNotificiations.push({
					message: nls.localize('invalidExtensions', "检测到无效的扩展。查看它们。"),
					severity: Severity.Warning,
					extensions: invalidExtensions,
		...
		if (!this.configurationService.getValue(AutoRestartConfigurationKey)) {
			...
			if (restartRequiredExtensions.length) {
				...
				computedNotificiations.push({
					message: needsReload
						? nls.localize('extensions need reload', "扩展需要重新加载窗口后才能应用更新。")
						: nls.localize('extensions need restart', "所有扩展都需要重启后才能应用更新。"),
					severity: Severity.Info,
					extensions: restartRequiredExtensions,
					query: '@restartrequired',
					action: {
						label: needsReload
							? nls.localize('reload window', "重新加载窗口")
							: nls.localize('restart extensions action', "重启扩展"),
						run: () => {
							if (needsReload) {
		...
		if (deprecatedExtensions.length) {
			computedNotificiations.push({
				message: nls.localize('deprecated extensions', "检测到已弃用的扩展。查看它们并迁移到替代项。"),
				severity: Severity.Warning,
		...
		if (privateMarketplaceUrl) {
			...
			message.appendMarkdown(nls.localize('privateMarketplace', "此窗口连接到由组织管理的 [专用扩展市场]({0})。", linkUri));
			computedNotificiations.push({
				message,
				severity: Severity.Info,
				extensions: [],
	...
	async open(extension: IExtension | string, options?: IExtensionEditorOptions): Promise<void> {
		// Code-Tomoshibi: the extension marketplace UI (extension editor / extensions view) has been removed.
		// The method is kept because it is part of IExtensionsWorkbenchService and is called from outside this
		// folder, but there is nothing left to open - tell the user instead of silently doing nothing.
		this.notificationService.info(EXTENSION_STORE_REMOVED_MESSAGE);
	}

	async openSearch(searchValue: string, preserveFocus?: boolean): Promise<void> {
		// Code-Tomoshibi: see open() above - the extensions view container no longer exists.
		this.notificationService.info(EXTENSION_STORE_REMOVED_MESSAGE);
	}
	...
	async updateRunningExtensions(message = nls.localize('restart', "正在更改扩展启用"), auto: boolean = false): Promise<void> {
			...
			if (await this.extensionService.stopExtensionHosts(message, auto)) {
				...
				if (auto) {
					this.notificationService.notify({
						severity: Severity.Info,
						message: nls.localize('extensionsAutoRestart', "已自动重启扩展来启用更新。"),
	...
	private getRuntimeState(extension: IExtension): ExtensionRuntimeState | undefined {
		...
		const reloadActionLabel = reloadAction === ExtensionRuntimeActionType.ReloadWindow ? nls.localize('reload', "重新加载窗口") : nls.localize('restart extensions', "重新启动扩展");
		...
		if (isUninstalled) {
			...
			if (!canRemoveRunningExtension && isSameExtensionRunning && !runningExtension.isUnderDevelopment) {
				return { action: reloadAction, reason: nls.localize('postUninstallTooltip', "请使用 {0} 以完成此扩展的卸载。", reloadActionLabel) };
			}
			return undefined;
		}
		if (extension.local) {
			...
			if (runningExtension) {
				if (isEnabled) {
					...
					if (isSameExtensionRunning) {
						...
						if (!runningExtension.isUnderDevelopment && (extension.version !== runningExtension.version || extension.local.targetPlatform !== runningExtension.targetPlatform)) {
							...
							if (productUpdateVersion
								...
								if (state.type === StateType.AvailableForDownload) {
									return { action: ExtensionRuntimeActionType.DownloadUpdate, reason: nls.localize('postUpdateDownloadTooltip', "请更新 {0} 以启用更新后的扩展。", this.productService.nameLong) };
								}
								if (state.type === StateType.Downloaded) {
									return { action: ExtensionRuntimeActionType.ApplyUpdate, reason: nls.localize('postUpdateUpdateTooltip', "请更新 {0} 以启用更新后的扩展。", this.productService.nameLong) };
								}
								if (state.type === StateType.Ready) {
									return { action: ExtensionRuntimeActionType.QuitAndInstall, reason: nls.localize('postUpdateRestartTooltip', "请重启 {0} 以启用更新后的扩展。", this.productService.nameLong) };
								}
								return undefined;
							}
							return { action: reloadAction, reason: nls.localize('postUpdateTooltip', "请使用 {0} 以启用更新后的扩展。", reloadActionLabel) };
						}
						...
						if (this.extensionsServers.length > 1) {
							...
							if (extensionInOtherServer) {
								...
								if (runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.localExtensionManagementServer) {
									return { action: reloadAction, reason: nls.localize('enable locally', "请使用 {0} 以在本地启用此扩展。", reloadActionLabel) };
								}
								...
								if (runningExtensionServer === this.extensionManagementServerService.localExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.remoteExtensionManagementServer) {
									return { action: reloadAction, reason: nls.localize('enable remote', "请 {0} 以在 {1} 启用此扩展。", reloadActionLabel, this.extensionManagementServerService.remoteExtensionManagementServer?.label) };
								}
							}
						}
						...
						if (extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
							...
							if (this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest)) {
								return { action: reloadAction, reason: nls.localize('postEnableTooltip', "请使用 {0} 以启用此扩展。", reloadActionLabel) };
							}
						}
					...
					if (isSameExtensionRunning && !runningExtension.isUnderDevelopment) {
						return { action: reloadAction, reason: nls.localize('postDisableTooltip', "请使用 {0} 以禁用此扩展。", reloadActionLabel) };
					}
				}
				return undefined;
			}
	...
	async downloadVSIX(extensionId: string, versionKind: 'prerelease' | 'release' | 'any'): Promise<void> {
		...
		if (!galleryExtension) {
			throw new Error(nls.localize('extension not found', "找不到扩展“{0}”。", extensionId));
		}
		...
		for (const targetPlatform of version?.targetPlatforms ?? galleryExtension.allTargetPlatforms) {
			if (targetPlatform !== TargetPlatform.UNKNOWN && targetPlatform !== TargetPlatform.UNIVERSAL) {
				options.push({
					label: targetPlatform === TargetPlatform.UNDEFINED ? nls.localize('allplatforms', "所有平台") : TargetPlatformToString(targetPlatform),
					id: targetPlatform
				});
			}
		}
		if (options.length > 1) {
			const message = nls.localize('platform placeholder', "请选择要为其下载 VSIX 的平台");
		...
		const result = await this.fileDialogService.showOpenDialog({
			title: nls.localize('download title', "选择文件夹以下载 VSIX"),
			canSelectFiles: false,
			canSelectFolders: true,
			canSelectMany: false,
			openLabel: nls.localize('download', "下载"),
		});
		...
		this.progressService.withProgress({ location: ProgressLocation.Notification }, async progress => {
			try {
				progress.report({ message: nls.localize('downloading...', "正在下载 VSIX...") });
				...
				this.notificationService.info(nls.localize('download.completed', "已成功下载 VSIX"));
			} catch (error) {
				this.notificationService.error(nls.localize('download.failed', "下载 VSIX 时出错: {0}", getErrorMessage(error)));
			}
		});
	}
	...
	private async pickVersionToDownload(extensionId: string): Promise<IGalleryExtensionVersion | undefined> {
		...
		if (!allVersions.length) {
			await this.dialogService.info(nls.localize('no versions', "此扩展没有其他版本。"));
			return;
		}
		...
		const picks = allVersions.map((v, i) => {
			return {
				id: v.version,
				label: v.version,
				description: `${fromNow(new Date(Date.parse(v.date)), true)}${v.isPreReleaseVersion ? ` (${nls.localize('pre-release', "预发布")})` : ''}`,
		...
		const pick = await this.quickInputService.pick(picks,
			{
				placeHolder: nls.localize('selectVersion', "选择要下载的版本"),
				matchOnDetail: true
			});
		return pick?.data;
	}
	...
	async shouldRequireConsentToUpdate(extension: IExtension): Promise<string | undefined> {
		...
		if (extension.local.identifier.uuid && extension.local.identifier.uuid !== extension.gallery.identifier.uuid) {
			return nls.localize('consentRequiredToUpdateRepublishedExtension', "此扩展的市场元数据已更改，这可能是重新发布所致。");
		}
		...
		return nls.localize('consentRequiredToUpdate', "{0} 扩展的更新引入了当前安装的版本中不存在的可执行代码。", extension.displayName);
	}
	...
	async canInstall(extension: IExtension): Promise<true | IMarkdownString> {
		if (!(extension instanceof Extension)) {
			return new MarkdownString().appendText(nls.localize('not an extension', "提供的对象不是扩展。"));
		}

		if (extension.isMalicious) {
			return new MarkdownString().appendText(nls.localize('malicious', "报告称该扩展存在问题。"));
		}

		if (extension.deprecationInfo?.disallowInstall) {
			return new MarkdownString().appendText(nls.localize('disallowed', "不允许安装此扩展。"));
		}
		...
		if (extension.gallery) {
			if (!extension.gallery.isSigned && shouldRequireRepositorySignatureFor(extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
				return new MarkdownString().appendText(nls.localize('not signed', "此扩展未签名。"));
			}
			...
			return localResult ?? remoteResult ?? webResult ?? new MarkdownString().appendText(nls.localize('cannot be installed', "无法安装“{0}”扩展，因为它在此安装程序中不可用。", extension.displayName ?? extension.identifier.id));
		}
		...
		return new MarkdownString().appendText(nls.localize('cannot be installed', "无法安装“{0}”扩展，因为它在此安装程序中不可用。", extension.displayName ?? extension.identifier.id));
	}
	...
	async install(arg: string | URI | IExtension, installOptions: InstallExtensionOptions = {}, progressLocation?: ProgressLocation | string): Promise<IExtension> {
		...
		if (!extension) {
			throw new Error(nls.localize('unknown', "无法安装扩展"));
		}
		...
		if (installOptions.enable) {
			if (extension.enablementState === EnablementState.DisabledWorkspace || extension.enablementState === EnablementState.DisabledGlobally) {
				if (installOptions.justification) {
					const result = await this.dialogService.confirm({
						title: nls.localize('enableExtensionTitle', "启用扩展"),
						message: nls.localize('enableExtensionMessage', "是否要启用“{0}”扩展?", extension.displayName),
						detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
						primaryButton: isString(installOptions.justification) ? nls.localize({ key: 'enableButtonLabel', comment: ['&& denotes a mnemonic'] }, "启用扩展(&&E)") : nls.localize({ key: 'enableButtonLabelWithAction', comment: ['&& denotes a mnemonic'] }, "启用扩展和 {0}(&&E)", installOptions.justification.action),
					});
					if (!result.confirmed) {
	...
	private async _install(arg: string | URI | IExtension, installOptions: InstallExtensionOptions = {}, progressLocation?: ProgressLocation | string): Promise<IExtension | undefined> {
		...
		if (arg instanceof URI) {
			...
			if (extension?.isMalicious) {
				throw new Error(nls.localize('malicious', "报告称该扩展存在问题。"));
			}
			...
			if (!servers || servers.length) {
				if (!installable) {
					if (!gallery) {
						...
						const reportIssueMessage = reportIssueUri ? nls.localize('report issue', "如果此问题仍然存在，请通过 {0} 报告问题", reportIssueUri.toString()) : '';
						if (installOptions.version) {
							const message = nls.localize('not found version', "无法安装扩展“{0}”，因为找不到请求的版本“{1}”。", id, installOptions.version);
							...
							const message = nls.localize('not found', "无法安装扩展“{0}”，因为找不到它。", id);
							throw new ExtensionManagementError(reportIssueMessage ? `${message} ${reportIssueMessage}` : message, ExtensionManagementErrorCode.NotFound);
						}
					}
					installable = gallery;
			...
			if (installOptions.justification) {
				...
				buttons.push({
					label: isString(installOptions.justification) || !installOptions.justification.action
						? nls.localize({ key: 'installButtonLabel', comment: ['&& denotes a mnemonic'] }, "安装扩展(&&I)")
						: nls.localize({ key: 'installButtonLabelWithAction', comment: ['&& denotes a mnemonic'] }, "安装扩展和 {0}(&&I)", installOptions.justification.action), run: () => true
				});
				if (!extension) {
					buttons.push({ label: nls.localize('open', "打开扩展"), run: () => { this.open(extension!); return false; } });
				}
				const result = await this.dialogService.prompt<boolean>({
					title: nls.localize('installExtensionTitle', "安装扩展"),
					message: extension ? nls.localize('installExtensionMessage', "是否要从“{1}”安装“{0}”扩展?", extension.displayName, extension.publisherDisplayName) : nls.localize('installVSIXMessage', "是否要安装扩展?"),
					detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
					cancelButton: true,
					buttons,
				});
	...
	async installInServer(extension: IExtension, server: IExtensionManagementServer, installOptions?: InstallOptions): Promise<void> {
		await this.doInstall(extension, async () => {
			...
			if (!isTargetPlatformCompatible(local.targetPlatform, [local.targetPlatform], targetPlatform)) {
				throw new Error(nls.localize('incompatible', "无法安装“{0}”扩展，因为它不兼容。", extension.identifier.id));
			}
	...
	async uninstall(e: IExtension): Promise<void> {
		...
		if (extension.local.isApplicationScoped && this.userDataProfilesService.profiles.length > 1) {
			const { confirmed } = await this.dialogService.confirm({
				title: nls.localize('uninstallApplicationScoped', "卸载扩展"),
				type: Severity.Info,
				message: nls.localize('uninstallApplicationScopedMessage', "是否要从所有配置文件中卸载 {0} ?", extension.displayName),
				primaryButton: nls.localize('uninstallAllProfiles', "卸载(所有配置文件)")
			});
			if (!confirmed) {
				throw new CancellationError();
			}
		...
		return this.withProgress({
			location: ProgressLocation.Extensions,
			title: nls.localize('uninstallingExtension', '正在卸载扩展...'),
	...
	private getErrorMessageForUninstallingAnExtensionWithDependents(extension: IExtension, dependents: ILocalExtension[]): string {
		if (dependents.length === 1) {
			return nls.localize('singleDependentUninstallError', "无法单独卸载 \"{0}\" 扩展。\"{1}\" 扩展依赖于它。是否要卸载所有这些扩展?", extension.displayName, dependents[0].manifest.displayName);
		}
		if (dependents.length === 2) {
			return nls.localize('twoDependentsUninstallError', "无法单独卸载 \"{0}\" 扩展。\"{1}\" 和 \"{2}\" 扩展依赖于它。是否要卸载所有这些扩展?",
				extension.displayName, dependents[0].manifest.displayName, dependents[1].manifest.displayName);
		}
		return nls.localize('multipleDependentsUninstallError', "无法单独卸载 \"{0}\" 扩展。\"{1}\"、\"{2}\" 和其他扩展依赖于它。是否要卸载所有这些扩展?",
			extension.displayName, dependents[0].manifest.displayName, dependents[1].manifest.displayName);
	}
	...
	private doInstall(extension: IExtension | undefined, installTask: () => Promise<ILocalExtension>, progressLocation?: ProgressLocation | string): Promise<IExtension> {
		const title = extension ? nls.localize('installing named extension', "正在安装“{0}”扩展...", extension.displayName) : nls.localize('installing extension', '正在安装扩展...');
		return this.withProgress({
	...
	private getErrorMessageForDisablingAnExtensionWithDependents(extension: IExtension, dependents: IExtension[]): string {
		if (dependents.length === 1) {
			return nls.localize('singleDependentError', "无法单独禁用 \"{0}\" 扩展。\"{1}\" 扩展依赖于此扩展。要禁用所有这些扩展吗?", extension.displayName, dependents[0].displayName);
		}
		if (dependents.length === 2) {
			return nls.localize('twoDependentsError', "无法单独禁用 \"{0}\" 扩展。\"{1}\" 和 \"{2}\" 扩展依赖于此扩展。要禁用所有这些扩展吗?",
				extension.displayName, dependents[0].displayName, dependents[1].displayName);
		}
		return nls.localize('multipleDependentsError', "无法单独禁用 \"{0}\" 扩展。\"{1}\"、\"{2}\" 和其他扩展依赖于此扩展。要禁用所有这些扩展吗?",
			extension.displayName, dependents[0].displayName, dependents[1].displayName);
	}
