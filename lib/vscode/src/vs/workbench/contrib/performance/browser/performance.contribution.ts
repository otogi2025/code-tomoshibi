...
registerAction2(class extends Action2 {
	...
	constructor() {
		super({
			id: 'perfview.show',
			title: localize2('show.label', '启动性能'),
...
registerAction2(class PrintServiceTraces extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('insta.trace', '打印服务跟踪'),
			category: Categories.Developer,
			f1: true
		});
	}
...
registerAction2(class PrintEventProfiling extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('emitter', '打印发射器配置文件'),
			category: Categories.Developer,
			f1: true
		});
	}
...
Registry.as<IConfigurationRegistry>(ConfigExt.Configuration).registerConfiguration({
	id: 'performance',
	order: 101,
	title: localize('performanceConfigurationTitle', "性能"),
	type: 'object',
	properties: {
		'telemetry.performance.inputLatencySamplingProbability': {
			...
			maximum: 1,
			tags: ['experimental'],
			markdownDescription: localize('telemetry.performance.inputLatencySamplingProbability', "此会话报告输入延迟遥测数据的概率(0 到 1)。设置为 0 表示禁用，设置为 1 表示始终报告。"),
			experiment: {
				mode: 'auto'
			}
		}
