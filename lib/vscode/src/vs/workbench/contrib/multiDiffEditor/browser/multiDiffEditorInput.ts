...
export class MultiDiffEditorInput extends EditorInput implements ILanguageSupport {
	...
	constructor(
		...
		this._register(autorun((reader) => {
			...
			const label = this.label ?? localize('name', "多差异编辑器");
			if (resources && resources.length === 1) {
				this._name = localize({ key: 'nameWithOneFile', comment: ['{0} is the name of the editor'] }, "{0} (1 个文件)", label);
			} else if (resources) {
				this._name = localize({ key: 'nameWithFiles', comment: ['{0} is the name of the editor', '{1} is the number of files being shown'] }, "{0} ({1} 文件)", label, resources.length);
			} else {
				this._name = label;
			}
