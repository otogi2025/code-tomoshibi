...
export const manageExtensionIcon = registerIcon('theme-selection-manage-extension', Codicon.gear, localize('manageExtensionIcon', '主题选择快速选取中“管理”操作的图标。'));
...
class MarketplaceThemesPicker implements IDisposable {
	...
	public openQuickPick(value: string, currentTheme: IWorkbenchTheme | undefined, selectTheme: (theme: IWorkbenchTheme | undefined, applyTheme: boolean) => void): Promise<PickerResult> {
		...
		return new Promise<PickerResult>((s, _) => {
			...
			quickpick.placeholder = localize('themes.selectMarketplaceTheme', "键入以搜索更多内容。选择以安装。按向上/向下键进行预览");
			...
			disposables.add(this.onDidChange(() => {
				...
				if (this._searchOngoing) {
					...
					items = [{ label: `$(error) ${localize('search.error', '搜索主题时出错: {0}', this._searchError)}`, id: undefined, alwaysShow: true }];
				}
	...
	private async installExtension(galleryExtension: IGalleryExtension) {
		...
		const result = await this.dialogService.confirm({
			message: localize('installExtension.confirm', "这将安装“{1}”发布的扩展“{0}”。是否要继续?", galleryExtension.displayName, galleryExtension.publisherDisplayName),
			primaryButton: localize('installExtension.button.ok', "确定")
		});
		if (!result.confirmed) {
			return false;
		}
		try {
			await this.progressService.withProgress({
				...
				title: localize('installing extensions', "正在安装扩展 {0}...", galleryExtension.displayName)
			}, async () => {
...
registerAction2(class extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('selectTheme.label', '颜色主题'),
	...
	private getTitle(colorScheme: ColorScheme | undefined): string {
		switch (colorScheme) {
			case ColorScheme.DARK: return localize('themes.selectTheme.darkScheme', "选择系统深色模式的颜色主题");
			case ColorScheme.LIGHT: return localize('themes.selectTheme.lightScheme', "选择系统浅色模式的颜色主题");
			case ColorScheme.HIGH_CONTRAST_DARK: return localize('themes.selectTheme.darkHC', "选择高对比度深色模式的颜色主题");
			case ColorScheme.HIGH_CONTRAST_LIGHT: return localize('themes.selectTheme.lightHC', "选择高对比度浅色模式的颜色主题");
			default:
				return localize('themes.selectTheme.default', "选择颜色主题(检测禁用的系统颜色模式)");
		}
	}
	...
	override async run(accessor: ServicesAccessor) {
		...
		const modeConfigureButton: IQuickInputButton = {
			tooltip: preferredColorScheme
				? localize('themes.configure.switchingEnabled', '检测启用的系统颜色模式。单击可开始配置。')
				: localize('themes.configure.switchingDisabled', '检测禁用的系统颜色模式。单击可开始配置。'),
			iconClass: ThemeIcon.asClassName(Codicon.colorMode),
			location: QuickInputButtonLocation.Inline
		};

		const options = {
			installMessage: localize('installColorThemes', "安装其他颜色主题..."),
			browseMessage: '$(plus) ' + localize('browseColorThemes', "浏览其他颜色主题..."),
		...
		const lightEntries = toEntries(themes.filter(t => t.type === ColorScheme.LIGHT), localize('themes.category.light', "浅色主题"));
		const darkEntries = toEntries(themes.filter(t => t.type === ColorScheme.DARK), localize('themes.category.dark', "深色主题"));
		const hcEntries = toEntries(themes.filter(t => isHighContrast(t.type)), localize('themes.category.hc', "高对比度主题"));
...
registerAction2(class extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('selectIconTheme.label', '文件图标主题'),
			category: Categories.Preferences,
			f1: true
		});
	}
	...
	override async run(accessor: ServicesAccessor) {
		...
		const options = {
			installMessage: localize('installIconThemes', "安装其他文件图标主题..."),
			placeholderMessage: localize('themes.selectIconTheme', "选择文件图标主题(向上/向下键以预览)"),
		...
		const picks: QuickPickInput<ThemeItem>[] = [
			{ type: 'separator', label: localize('fileIconThemeCategory', '图标主题') },
			{ id: '', theme: FileIconThemeData.noIconTheme, label: localize('noIconThemeLabel', '无'), description: localize('noIconThemeDesc', '禁用文件图标') },
...
registerAction2(class extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('selectProductIconTheme.label', '产品图标主题'),
			category: Categories.Preferences,
			f1: true
		});
	}
	...
	override async run(accessor: ServicesAccessor) {
		...
		const options = {
			installMessage: localize('installProductIconThemes', "安装其他产品图标主题..."),
			browseMessage: '$(plus) ' + localize('browseProductIconThemes', "浏览其他产品图标主题..."),
			placeholderMessage: localize('themes.selectProductIconTheme', "选择产品图标主题(向上/向下键以预览)"),
		...
		const picks: QuickPickInput<ThemeItem>[] = [
			{ type: 'separator', label: localize('productIconThemeCategory', '产品图标主题') },
			{ id: DEFAULT_PRODUCT_ICON_THEME_ID, theme: ProductIconThemeData.defaultTheme, label: localize('defaultProductIconThemeLabel', '默认值') },
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('tryNewDefaultThemes', "尝试新的默认主题"),
			category: Categories.Preferences,
			f1: true,
		});
	}
	override async run(accessor: ServicesAccessor) {
		...
		picker.items = items;
		picker.placeholder = localize('pickNewTheme', "选择新的默认主题");
...
const defaultThemeDescriptions: Record<string, string> = {
	[ThemeSettingDefaults.COLOR_THEME_LIGHT]: localize('defaultLight', "默认浅色主题"),
	[ThemeSettingDefaults.COLOR_THEME_DARK]: localize('defaultDark', "默认深色主题"),
};
...
const configureButton: IQuickInputButton = {
	...
	tooltip: localize('manage extension', "管理扩展"),
};
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('generateColorTheme.label', '使用当前设置生成颜色主题'),
...
registerAction2(class extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('toggleLightDarkThemes.label', '在浅色/深色主题之间切换'),
			category: Categories.Preferences,
			f1: true,
		});
	}
	...
	override async run(accessor: ServicesAccessor) {
		...
		if (configurationService.getValue(ThemeSettings.DETECT_COLOR_SCHEME)) {
			const message = localize({ key: 'cannotToggle', comment: ['{0} is a setting name'] }, "在设置中启用“{0}”时，无法在浅色和深色主题之间切换。", ThemeSettings.DETECT_COLOR_SCHEME);
...
registerAction2(class extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('browseColorThemeInMarketPlace.label', '在市场中浏览颜色主题'),
			category: Categories.Preferences,
			f1: true,
		});
	}
...
MenuRegistry.appendMenuItem(ThemesSubMenu, {
	command: {
		id: SelectColorThemeCommandId,
		title: localize('selectTheme.label', '颜色主题')
	},
	order: 1
});
...
MenuRegistry.appendMenuItem(ThemesSubMenu, {
	command: {
		id: SelectFileIconThemeCommandId,
		title: localize('themes.selectIconTheme.label', "文件图标主题")
	},
	order: 2
});
...
MenuRegistry.appendMenuItem(ThemesSubMenu, {
	command: {
		id: SelectProductIconThemeCommandId,
		title: localize('themes.selectProductIconTheme.label', "产品图标主题")
	},
	order: 3
});
