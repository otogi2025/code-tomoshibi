...
export class TypeHierarchyTreePeekWidget extends peekView.PeekViewWidget {
	...
	private async _updatePreview() {
		...
		const title = this._direction === TypeHierarchyDirection.Supertypes
			? localize('supertypes', "“{0}”的父类型", element.model.root.name)
			: localize('subtypes', "“{0}”的子类型", element.model.root.name);
		this.setTitle(title);
	}

	showLoading(): void {
		this._parent.dataset['state'] = State.Loading;
		this.setTitle(localize('title.loading', "正在加载..."));
		this._show();
	}
	...
	async showModel(model: TypeHierarchyModel): Promise<void> {
		...
		if (root.children.length === 0) {
			this.showMessage(this._direction === TypeHierarchyDirection.Supertypes
				? localize('empt.supertypes', "没有“{0}”的父类型", model.root.name)
				: localize('empt.subtypes', "没有“{0}”的子类型", model.root.name));
