...
const _ctxHasTypeHierarchyProvider = new RawContextKey<boolean>('editorHasTypeHierarchyProvider', false, localize('editorHasTypeHierarchyProvider', '类型层次结构提供程序是否可用'));
const _ctxTypeHierarchyVisible = new RawContextKey<boolean>('typeHierarchyVisible', false, localize('typeHierarchyVisible', '当前是否正在显示类型层次结构预览'));
const _ctxTypeHierarchyDirection = new RawContextKey<string>('typeHierarchyDirection', undefined, { type: 'string', description: localize('typeHierarchyDirection', '类型层次结构是否显示父类型或子类型') });
...
class TypeHierarchyController implements IEditorContribution {
	...
	private _showTypeHierarchyWidget(position: Position, direction: TypeHierarchyDirection, model: Promise<TypeHierarchyModel | undefined>, cts: CancellationTokenSource) {
		...
		model.then(model => {
			...
			this._widget!.showMessage(localize('error', "未能显示类型层次结构"));
		});
	}
...
registerAction2(class PeekTypeHierarchyAction extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('title', '速览类型层次结构'),
			menu: {
...
registerAction2(class extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('title.supertypes', '显示父类型'),
...
registerAction2(class extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('title.subtypes', '显示子类型'),
...
registerAction2(class extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('title.refocusTypeHierarchy', '重新专注类型层次结构'),
