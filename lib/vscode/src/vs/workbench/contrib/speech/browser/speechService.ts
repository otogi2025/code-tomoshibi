...
const speechProvidersExtensionPoint = ExtensionsRegistry.registerExtensionPoint<ISpeechProviderDescriptor[]>({
	...
	jsonSchema: {
		description: localize('vscode.extension.contributes.speechProvider', '提供语音提供程序'),
		type: 'array',
		items: {
			...
			properties: {
				name: {
					description: localize('speechProviderName', "此语音提供程序的唯一名称。"),
					type: 'string'
				},
				description: {
					description: localize('speechProviderDescription', "此语音提供程序的说明，显示在 UI 中。"),
					type: 'string'
				}
			}
		}
