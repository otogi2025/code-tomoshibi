...
const _ctxHasCallHierarchyProvider = new RawContextKey<boolean>('editorHasCallHierarchyProvider', false, localize('editorHasCallHierarchyProvider', '调用层次结构提供程序是否可用'));
const _ctxCallHierarchyVisible = new RawContextKey<boolean>('callHierarchyVisible', false, localize('callHierarchyVisible', '当前是否正在显示调用层次结构预览'));
const _ctxCallHierarchyDirection = new RawContextKey<string>('callHierarchyDirection', undefined, { type: 'string', description: localize('callHierarchyDirection', '通话层次结构是否显示来电或去电') });
	...
	private _showCallHierarchyWidget(position: IPosition, direction: CallHierarchyDirection, model: Promise<CallHierarchyModel | undefined>, cts: CancellationTokenSource) {
		...
		model.then(model => {
			...
			if (model) {
				...
				this._widget!.showMessage(localize('no.item', "无结果"));
			}
		}).catch(err => {
			...
			this._widget!.showMessage(localize('error', "未能显示调用层次结构"));
		});
	}
...
registerAction2(class PeekCallHierarchyAction extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('title', '速览调用层次结构'),
			menu: {
...
registerAction2(class extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('title.incoming', '显示来电'),
			icon: registerIcon('callhierarchy-incoming', Codicon.callIncoming, localize('showIncomingCallsIcons', '通话层次结构视图中来电的图标。')),
...
registerAction2(class extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('title.outgoing', '显示去电'),
			icon: registerIcon('callhierarchy-outgoing', Codicon.callOutgoing, localize('showOutgoingCallsIcon', '通话层次结构视图中去电的图标。')),
...
registerAction2(class extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('title.refocus', '重新聚焦调用层次结构'),
