...
export class SimpleSettingRenderer {
	...
	private viewInSettingsMessage(settingId: string, alreadyDisplayed: boolean) {
		if (alreadyDisplayed) {
			return nls.localize('viewInSettings', "在“设置”中查看");
		} else {
			const displayName = settingKeyToDisplayFormat(settingId);
			return nls.localize('viewInSettingsDetailed', "在“设置”中查看“{0}: {1}”", displayName.category, displayName.label);
		}
	}

	private restorePreviousSettingMessage(settingId: string): string {
		const displayName = settingKeyToDisplayFormat(settingId);
		return nls.localize('restorePreviousValue', "还原“{0}: {1}”的值", displayName.category, displayName.label);
	}
	...
	private booleanSettingMessage(setting: ISetting, booleanValue: boolean): string | undefined {
		...
		if (this.isAlreadySet(setting, booleanValue)) {
			if (booleanValue) {
				return nls.localize('alreadysetBoolTrue', "“{0}: {1}”已启用", displayName.category, displayName.label);
			} else {
				return nls.localize('alreadysetBoolFalse', "“{0}: {1}”已禁用", displayName.category, displayName.label);
			}
		}

		if (booleanValue) {
			return nls.localize('trueMessage', "启用“{0}: {1}”", displayName.category, displayName.label);
		} else {
			return nls.localize('falseMessage', "禁用“{0}: {1}”", displayName.category, displayName.label);
		}
	}
	...
	private stringSettingMessage(setting: ISetting, stringValue: string): string | undefined {
		...
		if (this.isAlreadySet(setting, stringValue)) {
			return nls.localize('alreadysetString', "“{0}: {1}”已设置为“{2}”", displayName.category, displayName.label, stringValue);
		}

		return nls.localize('stringValue', "将“{0}: {1}”设置为“{2}”", displayName.category, displayName.label, stringValue);
	}
	...
	private numberSettingMessage(setting: ISetting, numberValue: number): string | undefined {
		...
		if (this.isAlreadySet(setting, numberValue)) {
			return nls.localize('alreadysetNum', "“{0}: {1}”已设置为 {2}", displayName.category, displayName.label, numberValue);
		}

		return nls.localize('numberValue', "将“{0}: {1}”设置为 {2}", displayName.category, displayName.label, numberValue);
	...
	private renderSetting(setting: ISetting, newValue: string | undefined): string | undefined {
		...
		const title = nls.localize('changeSettingTitle', "查看或更改设置");
	...
	getActions(uri: URI) {
		...
		actions.push({
			...
			enabled: true,
			id: 'copySettingId',
			tooltip: nls.localize('copySettingId', "复制设置 ID"),
			label: nls.localize('copySettingId', "复制设置 ID"),
			run: () => {
