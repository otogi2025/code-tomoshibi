...
export function provideDecorations(fileStat: ExplorerItem): IDecorationData | undefined {
	if (fileStat.isRoot && fileStat.error) {
		return {
			tooltip: localize('canNotResolve', "无法解析工作区文件夹 ({0})", toErrorMessage(fileStat.error)),
			letter: '!',
	...
	if (fileStat.isSymbolicLink) {
		return {
			tooltip: localize('symbolicLlink', "符号链接"),
			letter: '\u2937'
		};
	}
	if (fileStat.isUnknown) {
		return {
			tooltip: localize('unknown', "未知文件类型"),
			letter: '?'
		};
	}
...
export class ExplorerDecorationsProvider implements IDecorationsProvider {
	...
	readonly label: string = localize('label', "资源管理器");
