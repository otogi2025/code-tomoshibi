...
export class OpenEditorsView extends ViewPane {
	...
	static readonly NAME: ILocalizedString = nls.localize2({ key: 'openEditors', comment: ['Open is an adjective'] }, "打开的文件");
	...
	private updateDirtyIndicator(workingCopy?: IWorkingCopy): void {
		...
		if (dirty === 0) {
			...
			this.dirtyCountElement.textContent = nls.localize('dirtyCounter', "{0} 个未保存", dirty);
...
class OpenEditorsAccessibilityProvider implements IListAccessibilityProvider<OpenEditor | IEditorGroup> {
	...
	getWidgetAriaLabel(): string {
		return nls.localize('openEditors', "打开的文件");
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: nls.localize2('flipLayout', "切换垂直/水平编辑器布局"),
			f1: true,
			keybinding: {
...
MenuRegistry.appendMenuItem(MenuId.MenubarLayoutMenu, {
	...
	command: {
		...
		title: {
			...nls.localize2('miToggleEditorLayoutWithoutMnemonic', "翻转布局"),
			mnemonicTitle: nls.localize({ key: 'miToggleEditorLayout', comment: ['&& denotes a mnemonic'] }, "翻转布局(&&L)")
		}
	},
	order: 1
});
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: nls.localize2('newUntitledFile', "新的无标题文本文件"),
			f1: false,
