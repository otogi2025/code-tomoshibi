...
KeybindingsRegistry.registerCommandAndKeybindingRule({
	...
	handler: async (accessor, resource: URI | object) => {
		...
		if (uri && fileService.hasProvider(uri)) {
			...
			const editorLabel = nls.localize('modifiedLabel', "{0} (在文件中) ↔ {1}", name, name);
...
async function doSaveEditors(accessor: ServicesAccessor, editors: IEditorIdentifier[], options?: ISaveEditorsOptions): Promise<void> {
	...
	try {
		...
		if (!isCancellationError(error)) {
			const actions: IAction[] = [toAction({ id: 'workbench.action.files.saveEditors', label: nls.localize('retry', "重试"), run: () => instantiationService.invokeFunction(accessor => doSaveEditors(accessor, editors, options)) })];
			...
			if (editorsToRevert.length > 0) {
				actions.push(toAction({ id: 'workbench.action.files.revertEditors', label: editorsToRevert.length > 1 ? nls.localize('revertAll', "全部还原") : nls.localize('revert', "还原"), run: () => editorService.revert(editorsToRevert) }));
			}

			notificationService.notify({
				id: editors.map(({ editor }) => hash(editor.resource?.toString())).join(), // ensure unique notification ID per set of editor
				severity: Severity.Error,
				message: nls.localize({ key: 'genericSaveError', comment: ['{0} is the resource that failed to save and {1} the error message'] }, "未能保存“{0}”: {1}", editors.map(({ editor }) => editor.getName()).join(', '), toErrorMessage(error, false)),
...
CommandsRegistry.registerCommand({
	...
	handler: async accessor => {
		...
		try {
			...
			const notificationService = accessor.get(INotificationService);
			notificationService.error(nls.localize('genericRevertError', "未能还原“{0}”: {1}", editors.map(({ editor }) => editor.getName()).join(', '), toErrorMessage(error, false)));
		}
	}
});
...
CommandsRegistry.registerCommand({
	...
	handler: async (accessor, args?: { languageId?: string; viewType?: string; fileName?: string }) => {
		...
		const createFileLocalized = nls.localize('newFileCommand.saveLabel', "创建文件");
