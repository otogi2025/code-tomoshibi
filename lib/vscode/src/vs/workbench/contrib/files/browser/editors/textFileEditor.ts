...
export class TextFileEditor extends AbstractTextCodeEditor<ICodeEditorViewState> {
	...
	override getTitle(): string {
		...
		return localize('textFileEditor', "文本文件编辑器");
	}
	...
	protected async handleSetInputError(error: Error, input: FileEditorInput, options: ITextEditorOptions | undefined): Promise<void> {
		...
		if ((<FileOperationError>error).fileOperationResult === FileOperationResult.FILE_IS_DIRECTORY) {
			...
			actions.push(toAction({
				id: 'workbench.files.action.openFolder', label: localize('openFolder', "打开文件夹"), run: async () => {
			...
			if (this.contextService.isInsideWorkspace(input.preferredResource)) {
				actions.push(toAction({
					id: 'workbench.files.action.reveal', label: localize('reveal', "显示文件夹"), run: async () => {
			...
			throw createEditorOpenError(localize('fileIsDirectory', "此文件未显示在文本编辑器中，因为它是一个目录。"), actions, { forceMessage: true });
		}
		...
		if ((<FileOperationError>error).fileOperationResult === FileOperationResult.FILE_TOO_LARGE) {
			...
			if (error instanceof TooLargeFileOperationError) {
				message = localize('fileTooLargeForHeapErrorWithSize', "此文件未显示在文本编辑器中，因为它非常大({0})。", ByteSize.formatSize(error.size));
			} else {
				message = localize('fileTooLargeForHeapErrorWithoutSize', "此文件未显示在文本编辑器中，因为它非常大。");
			}
		...
		if (
			...
			const fileNotFoundError = createEditorOpenError(new FileOperationError(localize('unavailableResourceErrorEditorText', "由于找不到该文件，因此无法打开编辑器。"), FileOperationResult.FILE_NOT_FOUND), [
				toAction({
					id: 'workbench.files.action.createMissingFile', label: localize('createFile', "创建文件"), run: async () => {
