...
const explorerViewIcon = registerIcon('explorer-view-icon', Codicon.files, localize('explorerViewIcon', '查看资源管理器视图的图标。'));
const openEditorsViewIcon = registerIcon('open-editors-view-icon', Codicon.book, localize('openEditorsIcon', '查看打开编辑器视图的图标。'));
...
export class ExplorerViewletViewsContribution extends Disposable implements IWorkbenchContribution {
	...
	private createOpenEditorsViewDescriptor(): IViewDescriptor {
		return {
			...
			canMoveView: true,
			collapsed: false,
			hideByDefault: false, // touch devices need a permanent way to close editors, so show this view by default
			focusCommand: {
...
viewsRegistry.registerViewWelcomeContent(EmptyView.ID, {
	content: localize({ key: 'noFolderButEditorsHelp', comment: ['Please do not translate the word "command", it is part of our internal syntax which must not change'] },
		"尚未打开文件夹。\r\n{0}\r\n打开文件夹将关闭所有当前打开的编辑器。要使其保持打开状态，请改为 {1}。", openFolderButton, addAFolderButton),
	when: ContextKeyExpr.and(
		// editors are opened
