	...
	private onDidWatchError(error: Error): void {
		...
		if (msg.indexOf('ENOSPC') >= 0) {
			...
			this.notificationService.prompt(
				Severity.Warning,
				localize('enospcError', "无法监视文件更改。请按照说明链接来解决此问题。"),
				[{
					label: localize('learnMore', "说明"),
		...
		// Detect when the watcher throws an error unexpectedly
			...
			this.notificationService.prompt(
				Severity.Warning,
				localize('eshutdownError', "文件更改观察程序意外停止。重新加载窗口可能再次启用观察程序，除非无法监视工作区的文件更改。"),
				[{
					label: localize('reload', "重新加载"),
