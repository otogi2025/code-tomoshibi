...
export class BrowserFileUpload {
	...
	upload(target: ExplorerItem, source: DragEvent | FileList | readonly File[], options?: IBrowserFileUploadOptions): Promise<readonly URI[]> {
		...
		const uploadPromise = this.progressService.withProgress(
			{
				...
				delay: 800,
				cancellable: true,
				title: localize('uploadingFiles', "正在上传")
			},
	...
	private toTransfer(source: DragEvent | FileList | readonly File[]): IWebkitDataTransfer {
	...
	private async doUpload(target: ExplorerItem, source: IWebkitDataTransfer, progress: IProgress<IProgressStep>, token: CancellationToken, options?: IBrowserFileUploadOptions): Promise<readonly URI[]> {
		...
		const uploadLimiter = new Limiter<IBrowserFileUploadResult | undefined>(BrowserFileUpload.MAX_PARALLEL_UPLOADS);
			...
			return uploadLimiter.queue(async () => {
				...
				if (target && entry.name && target.getChild(entry.name)) {
					...
					await this.explorerService.applyBulkEdit([new ResourceFileEdit(joinPath(target.resource, entry.name), undefined, { recursive: true, folder: target.getChild(entry.name)?.isDirectory })], {
						undoLabel: localize('overwrite', "覆盖 {0}", entry.name),
						progressLabel: localize('overwriting', "正在覆盖 {0}", entry.name),
					});
	...
	private async doUploadEntry(entry: IWebkitDataTransferItemEntry, parentResource: URI, target: ExplorerItem | undefined, progress: IProgress<IProgressStep>, operation: IBrowserUploadOperation, token: CancellationToken): Promise<{ isFile: boolean; resource: URI } | undefined> {
		...
		const reportProgress = (fileSize: number, bytesUploaded: number): void => {
			...
			if (fileSize < ByteSize.MB) {
				if (operation.filesTotal === 1) {
					...
					message = localize('uploadProgressSmallMany', "{0} 个文件，共 {1} 个({2}/秒)", operation.filesUploaded, operation.filesTotal, ByteSize.formatSize(bytesUploadedPerSecond));
				}
			}

			// Large file
			else {
				message = localize('uploadProgressLarge', "{0} ({1}/{2}，{3}/秒)", entry.name, ByteSize.formatSize(fileBytesUploaded), ByteSize.formatSize(fileSize), ByteSize.formatSize(bytesUploadedPerSecond));
			}
		...
		// Handle folder upload
			...
			const fileUploadQueue = new Limiter<IBrowserFileUploadResult | undefined>(BrowserFileUpload.MAX_PARALLEL_UPLOADS);
		...
		const importPromise = this.progressService.withProgress(
			{
				...
				delay: 800,
				cancellable: true,
				title: localize('copyingFiles', "正在复制...")
			},
	...
	private async doImport(target: ExplorerItem, source: DragEvent, targetWindow: Window, token: CancellationToken): Promise<void> {
		...
		if (folders.length > 0 && target.isRoot) {
			...
			const buttons: IPromptButton<ImportChoice | undefined>[] = [
				{
					label: folders.length > 1 ?
						localize('copyFolders', "复制文件夹(&&C)") :
						localize('copyFolder', "复制文件夹(&&C)"),
			...
			if (folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0)) {
				buttons.unshift({
					label: folders.length > 1 ?
						localize('addFolders', "将文件夹添加到工作区(&&A)") :
						localize('addFolder', "将文件夹添加到工作区(&&A)"),
					run: () => ImportChoice.Add
				});
				message = folders.length > 1 ?
					localize('dropFolders', "是否要复制文件夹或将其添加到工作区?") :
					localize('dropFolder', "是否要复制“{0}”或将“{0}”作为文件夹添加工作区?", basename(folders[0].uri));
			} else {
				message = folders.length > 1 ?
					localize('copyfolders', "确定要复制文件夹吗?") :
					localize('copyfolder', "确定要复制“{0}”吗?", basename(folders[0].uri));
			}
	...
	private async importResources(target: ExplorerItem, resources: URI[], token: CancellationToken): Promise<void> {
		if (resources && resources.length > 0) {
			...
			if (inaccessibleFileCount > 0) {
				this.notificationService.error(inaccessibleFileCount > 1 ? localize('filesInaccessible', "无法访问部分或所有已删除文件，以进行导入。") : localize('fileInaccessible', "无法访问已删除文件以进行导入。"));
			}
			...
			await this.explorerService.applyBulkEdit(resourceFileEdits, {
				undoLabel: resourcesFiltered.length === 1 ?
					localize({ comment: ['substitution will be the name of the file that was imported'], key: 'importFile' }, "导入{0}", basename(resourcesFiltered[0])) :
					localize({ comment: ['substitution will be the number of files that were imported'], key: 'importnFile' }, "导入 {0} 个资源", resourcesFiltered.length),
				progressLabel: resourcesFiltered.length === 1 ?
					localize({ comment: ['substitution will be the name of the file that was copied'], key: 'copyingFile' }, "正在复制 {0}", basename(resourcesFiltered[0])) :
					localize({ comment: ['substitution will be the number of files that were copied'], key: 'copyingnFile' }, "正在复制 {0} 资源", resourcesFiltered.length),
...
export class FileDownload {
		...
		const downloadPromise = this.progressService.withProgress(
			{
				...
				delay: 800,
				cancellable: isWeb,
				title: localize('downloadingFiles', "正在下载")
			},
	...
	private reportProgress(name: string, fileSize: number, bytesDownloaded: number, operation: IDownloadOperation): void {
		...
		if (fileSize < ByteSize.MB) {
			if (operation.filesTotal === 1) {
				...
				message = localize('downloadProgressSmallMany', "{0} 个文件，共 {1} 个({2}/秒)", operation.filesDownloaded, operation.filesTotal, ByteSize.formatSize(bytesDownloadedPerSecond));
			}
		}

		// Large file
		else {
			message = localize('downloadProgressLarge', "{0} ({1}/{2}，{3}/秒)", name, ByteSize.formatSize(operation.fileBytesDownloaded), ByteSize.formatSize(fileSize), ByteSize.formatSize(bytesDownloadedPerSecond));
		}
	...
	private async doDownloadNative(explorerItem: ExplorerItem, progress: IProgress<IProgressStep>, cts: CancellationTokenSource): Promise<void> {
		...
		const destination = await this.fileDialogService.showSaveDialog({
			...
			saveLabel: localize('downloadButton', "下载"),
			title: localize('chooseWhereToDownload', "选择下载位置"),
			defaultUri
		});
		...
		if (destination) {
			...
			await this.explorerService.applyBulkEdit([new ResourceFileEdit(explorerItem.resource, destination, { overwrite: true, copy: true })], {
				undoLabel: localize('downloadBulkEdit', "下载 {0}", explorerItem.name),
				progressLabel: localize('downloadingBulkEdit', "正在下载 {0}", explorerItem.name),
				progressLocation: ProgressLocation.Window
			});
		} else {
...
export function getFileOverwriteConfirm(name: string): IConfirmation {
	return {
		message: localize('confirmOverwrite', "目标文件夹中已存在名称为\"{0}\"的文件或文件夹。是否要替换它？", name),
		detail: localize('irreversible', "此操作不可逆!"),
		primaryButton: localize({ key: 'replaceButtonLabel', comment: ['&& denotes a mnemonic'] }, "替换(&&R)"),
		type: 'warning'
	};
}
...
export function getMultipleFilesOverwriteConfirm(files: URI[]): IConfirmation {
	if (files.length > 1) {
		return {
			message: localize('confirmManyOverwrites', "目标文件夹中已存在以下 {0} 个文件和/或文件夹。是否要替换它们?", files.length),
			detail: getFileNamesMessage(files) + '\n' + localize('irreversible', "此操作不可逆!"),
			primaryButton: localize({ key: 'replaceButtonLabel', comment: ['&& denotes a mnemonic'] }, "替换(&&R)"),
			type: 'warning'
		};
	}
