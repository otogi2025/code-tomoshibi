...
export class TunnelViewModel implements ITunnelViewModel {
	...
	readonly input = {
		...
		privacy: {
			...
			label: nls.localize('tunnelPrivacy.private', "专用")
		},
		strip: () => undefined
	};
...
class LocalAddressColumn implements ITableColumn<ITunnelItem, ActionBarCell> {
	...
	readonly tooltip: string = nls.localize('tunnel.addressColumn.tooltip', "转发端口所在的地址。");
	readonly weight: number = 1;
	readonly templateId: string = 'actionbar';
	project(row: ITunnelItem): ActionBarCell {
		if (row.tunnelType === TunnelType.Add) {
...
class RunningProcessColumn implements ITableColumn<ITunnelItem, ActionBarCell> {
	readonly label: string = nls.localize('tunnel.processColumn.label', "正在运行的进程");
...
class OriginColumn implements ITableColumn<ITunnelItem, ActionBarCell> {
	readonly label: string = nls.localize('tunnel.originColumn.label', "源");
...
class ActionBarRenderer implements ITableRenderer<ActionBarCell, IActionBarTemplateData> {
	...
	private renderInputBox(templateData: IActionBarTemplateData, editableData: IEditableData): void {
		...
		const inputBox = new InputBox(container, this.contextViewService, {
			ariaLabel: nls.localize('remote.tunnelsView.input', "按 Enter 键确认，或按 Esc 键取消。"),
			validationOptions: {
				validation: (value) => {
...
class TunnelItem implements ITunnelItem {
	...
	get processDescription(): string | undefined {
		...
		if (this.runningProcess) {
			...
			description = nls.localize('tunnelView.runningProcess.inacessable', "流程信息不可用");
		}
	...
	get iconTooltip(): string {
		...
		if (!isAdd) {
			return `${this.processDescription ? nls.localize('tunnel.iconColumn.running', "端口有正在运行的进程。") :
				nls.localize('tunnel.iconColumn.notRunning', "没有正在运行的进程。")}`;
		} else {
			return this.label;
		}
	}
	...
	get portTooltip(): string {
		const isAdd = this.tunnelType === TunnelType.Add;
		if (!isAdd) {
			return `${this.name ? nls.localize('remote.tunnel.tooltipName', "标记为 {0} 的端口。", this.name) : ''}`;
		} else {
			return '';
		}
	}
...
const TunnelViewFocusContextKey = new RawContextKey<boolean>('tunnelViewFocus', false, nls.localize('tunnel.focusContext', "“端口”视图是否具有焦点。"));
...
const invalidPortString: string = nls.localize('remote.tunnelsView.portNumberValid', "转发的端口应为数字或主机:端口。");
const maxPortNumber: number = 65536;
const invalidPortNumberString: string = nls.localize('remote.tunnelsView.portNumberToHigh', "端口号必须大于等于 0 且小于 {0}。", maxPortNumber);
const requiresSudoString: string = nls.localize('remote.tunnelView.inlineElevationMessage', "可能需要 sudo");
...
export namespace ForwardPortAction {
	...
	function error(notificationService: INotificationService, tunnelOrError: RemoteTunnel | string | void, host: string, port: number) {
		if (!tunnelOrError) {
			notificationService.warn(nls.localize('remote.tunnel.forwardError', "无法转发{0}: {1}。主机可能不可用，或者远程端口可能已被转发", host, port));
...
function makeTunnelPicks(tunnels: Tunnel[], remoteExplorerService: IRemoteExplorerService, tunnelService: ITunnelService): QuickPickInput<QuickPickTunnel>[] {
	...
	if (picks.length === 0) {
		picks.push({
			label: nls.localize('remote.tunnel.closeNoPorts', "当前未转发端口。尝试运行{0}命令", ForwardPortAction.LABEL.value)
		});
	}
	return picks;
}
...
namespace ClosePortAction {
	...
	export function commandPaletteHandler(): ICommandHandler {
		return async (accessor) => {
			...
			const result = await quickInputService.pick(picks, { placeHolder: nls.localize('remote.tunnel.closePlaceholder', "选择停止转发的端口") });
...
namespace OpenPortInBrowserCommandPaletteAction {
	...
	export function handler(): ICommandHandler {
		return async (accessor, arg) => {
			...
			if (options.length === 0) {
				options.push({
					label: nls.localize('remote.tunnel.openCommandPaletteNone', "当前没有转发端口。若要开始，请打开端口视图。")
				});
			} else {
				options.push({
...
namespace CopyAddressAction {
	...
	export const COMMANDPALETTE_LABEL = nls.localize('remote.tunnel.copyAddressCommandPalette', "复制转发的端口地址");
	...
	export function commandPaletteHandler(): ICommandHandler {
		return async (accessor, arg) => {
			...
			const result = await quickInputService.pick(makeTunnelPicks(tunnels, remoteExplorerService, tunnelService), { placeHolder: nls.localize('remote.tunnel.copyAddressPlaceholdter', "选择转发的端口") });
			if (result && result.tunnel) {
...
registerColor('ports.iconRunningProcessForeground', STATUS_BAR_REMOTE_ITEM_BACKGROUND, nls.localize('portWithRunningProcess.foreground', "具有关联的正在运行的进程的端口图标颜色。"));
