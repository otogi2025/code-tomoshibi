...
export class InitialRemoteConnectionHealthContribution implements IWorkbenchContribution {
	...
	private async _confirmConnection(): Promise<boolean> {
		...
		const { result, checkboxChecked } = await this.dialogService.prompt<ConnectionChoice>({
			...
			message: localize('unsupportedGlibcWarning', "你即将连接到不受 {0} 支持的 OS 版本。", this.productService.nameLong),
			buttons: [
				{
					label: localize({ key: 'allow', comment: ['&& denotes a mnemonic'] }, "允许(&&A)"),
		...
		try {
			...
			if (environment && environment.isUnsupportedGlibc) {
				...
				if (allowed) {
					this._showVersionedBanner(
						...
						localize('unsupportedGlibcWarning.banner', "你已连接到不受 {0} 支持的 OS 版本。", this.productService.nameLong),
						[
							{
								label: localize('unsupportedGlibcBannerLearnMore', "了解详细信息"),
			...
			if (isArm32Server) {
				this._showVersionedBanner(
					...
					localize('arm32ServerDeprecation.banner', "已弃用对 32 位 ARM 远程服务器的支持，并将在 {0} 的未来版本中移除。", this.productService.nameLong),
					[
						{
							label: localize('arm32ServerDeprecationBannerLearnMore', "了解详细信息"),
	...
	private _showVersionedBanner(id: string, storageKey: string, message: string, actions?: ReadonlyArray<ILinkDescriptor>): void {
		...
		this.bannerService.show({
			...
			closeLabel: localize('remoteBannerDoNotShowAgainThisVersion', "在 v{0} 中不再显示", this.productService.version),
			onClose: () => {
