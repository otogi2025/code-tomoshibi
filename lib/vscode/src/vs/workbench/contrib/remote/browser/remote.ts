	...
	private updateItems() {
		...
		if (getStarted.length) {
			...
			const getStartedHelpItem = this.items?.find(item => item.icon === icons.getStartedIcon) ?? new GetStartedHelpItem(
				...
				nls.localize('remote.help.getStarted', "入门"),
				helpItemValues,
		...
		if (documentation.length) {
			...
			const documentationHelpItem = this.items?.find(item => item.icon === icons.documentationIcon) ?? new HelpItem(
				...
				nls.localize('remote.help.documentation', "阅读文档"),
				helpItemValues,
				this.quickInputService,
				this.environmentService,
				this.openerService,
		...
		if (issues.length) {
			...
			const reviewIssuesHelpItem = this.items?.find(item => item.icon === icons.reviewIssuesIcon) ?? new HelpItem(
				...
				nls.localize('remote.help.issues', "审查问题"),
				helpItemValues,
				this.quickInputService,
				this.environmentService,
				this.openerService,
...
abstract class HelpItemBase implements IHelpItem {
	...
	async handleClick() {
		...
		if (this.values.length > 1) {
			...
			if (actions.length) {
				const action = await this.quickInputService.pick(actions, { placeHolder: nls.localize('pickRemoteExtension', "选择要打开的 URL") });
				if (action) {
...
class HelpPanel extends ViewPane {
	...
	static readonly TITLE = nls.localize2('remote.help', "帮助和反馈");
		...
		this.tree = this.instantiationService.createInstance(WorkbenchAsyncDataTree<HelpModel, IHelpItem, IHelpItem>,
			...
			new HelpDataSource(),
			{
				accessibilityProvider: {
					...
					getWidgetAriaLabel: () => nls.localize('remotehelp', "远程帮助")
				}
			}
		);
...
class RemoteViewPaneContainer extends FilterViewPaneContainer implements IViewModel {
	...
	getTitle(): string {
		const title = nls.localize('remote.explorer', "远程资源管理器");
		return title;
	}
}
...
Registry.as<IViewContainersRegistry>(Extensions.ViewContainersRegistry).registerViewContainer(
	{
		id: VIEWLET_ID,
		title: nls.localize2('remote.explorer', "远程资源管理器"),
...
class ReconnectionTimer implements IDisposable {
	...
	private _render() {
		...
		if (remainingTime === 1) {
			this._parent.report(nls.localize('reconnectionWaitOne', "正在尝试在 {0} 秒内重新连接...", remainingTime));
		} else {
			this._parent.report(nls.localize('reconnectionWaitMany', "正在尝试在 {0} 秒内重新连接...", remainingTime));
		}
	}
}
	...
	constructor(
		...
		if (connection) {
			...
			const reconnectButton = {
				label: nls.localize('reconnectNow', "立即重新连接"),
				callback: () => {
			...
			const reloadButton = {
				label: nls.localize('reloadWindow', "重新加载窗口"),
				callback: () => {
				...
				switch (e.type) {
					case PersistentConnectionEventType.ConnectionLost:
						...
						if (visibleProgress || e.millisSinceLastIncomingData > DISCONNECT_PROMPT_TIME) {
							...
							visibleProgress.report(nls.localize('connectionLost', "连接中断"));
						}
						break;
					...
					case PersistentConnectionEventType.ReconnectionRunning:
						...
						if (visibleProgress || e.millisSinceLastIncomingData > DISCONNECT_PROMPT_TIME) {
							...
							visibleProgress.report(nls.localize('reconnectionRunning', "已断开连接。正在尝试重新连接…"));
					...
					case PersistentConnectionEventType.ReconnectionPermanentFailure:
						...
						if (e.handled) {
							...
							dialogService.confirm({
								type: Severity.Error,
								message: nls.localize('reconnectionPermanentFailure', "无法重新连接。请重新加载窗口。"),
								primaryButton: nls.localize({ key: 'reloadWindow.dialog', comment: ['&& denotes a mnemonic'] }, "重新加载窗口(&&R)")
							}).then(result => {
								if (result.confirmed) {
									commandService.executeCommand(ReloadWindowAction.ID);
								}
