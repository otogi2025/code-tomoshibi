		...
		if (disableLightbulb || !editorUri) {
			...
			if (lang && lang !== existing && skip[existing] !== lang) {
				...
				const tooltip = this._keybindingService.appendKeybinding(
					localize('status.autoDetectLanguage', "接受检测到的语言: {0}", detectedName),
					detectLanguageCommandId
				);

				const props: IStatusbarEntry = {
					name: localize('langDetection.name', "语言检测"),
					ariaLabel: localize('langDetection.aria', "更改为检测到的语言: {0}", lang),
					tooltip,
...
registerAction2(class extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('detectlang', "检测内容中的语言"),
			f1: true,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (editorUri) {
			...
			if (lang) {
				...
				notificationService.warn(localize('noDetection', "无法检测编辑器语言"));
			}
		}
	}
});
