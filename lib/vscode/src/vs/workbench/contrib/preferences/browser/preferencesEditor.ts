...
export class PreferencesEditor extends EditorPane {
	...
	constructor(
		...
		this.preferencesTabActionBar = this._register(new ActionBar(preferencesTabsContainer, {
			...
			ariaLabel: localize('preferencesTabSwitcherBarAriaLabel', "偏好设置标签切换器"),
			ariaRole: 'tablist',
		}));
	...
	private onDidSelectPreferencesEditorPane(id: string): void {
		...
		if (selectedAction) {
			this.searchWidget.inputBox.setPlaceHolder(localize('FullTextSearchPlaceholder', "搜索 {0}", selectedAction.descriptor.title));
			this.searchWidget.inputBox.setAriaLabel(localize('FullTextSearchPlaceholder', "搜索 {0}", selectedAction.descriptor.title));
		}
