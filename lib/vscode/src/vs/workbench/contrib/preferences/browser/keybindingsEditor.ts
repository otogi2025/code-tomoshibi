...
export class KeybindingsEditor extends EditorPane<IKeybindingsEditorMemento> implements IKeybindingsEditorPane {
	...
	constructor(
		...
		this.recordKeysAction = this._register(new Action(KEYBINDINGS_EDITOR_COMMAND_RECORD_SEARCH_KEYS, localize('recordKeysLabel', "录制按键"), ThemeIcon.asClassName(keybindingsRecordKeysIcon)));
		this.recordKeysAction.checked = false;

		this.sortByPrecedenceAction = this._register(new Action(KEYBINDINGS_EDITOR_COMMAND_SORTBY_PRECEDENCE, localize('sortByPrecedeneLabel', "按优先级排序(最高优先)"), ThemeIcon.asClassName(keybindingsSortIcon)));
	...
	private createHeader(parent: HTMLElement): void {
		...
		const fullTextSearchPlaceholder = localize('SearchKeybindings.FullTextSearchPlaceholder', "在此键入搜索按键绑定");
		const keybindingsSearchPlaceholder = localize('SearchKeybindings.KeybindingsSearchPlaceholder', "正在录制按键。按 Esc 键退出");

		const clearInputAction = this._register(new Action(KEYBINDINGS_EDITOR_COMMAND_CLEAR_SEARCH_RESULTS, localize('clearInput', "清除键绑定搜索输入"), ThemeIcon.asClassName(preferencesClearInputIcon), false, async () => this.clearSearchResults()));
	...
	private createRecordingBadge(container: HTMLElement): HTMLElement {
		...
		recordingBadge.textContent = localize('recording', "正在录制按键");
	...
	private getAriaLabel(keybindingsEntries: IKeybindingItemEntry[]): string {
		...
		if (this.sortByPrecedenceAction.checked) {
			label = localize('show sorted keybindings', "按优先级顺序显示 {0} 个按键绑定", keybindingsEntries.length);
		} else {
			label = localize('show keybindings', "按字母顺序显示 {0} 个按键绑定", keybindingsEntries.length);
		}
		if (this.configurationService.getValue(AccessibilityVerbositySettingId.KeybindingsEditor)) {
			...
			if (kb) {
				label += '. ' + localize('navigateToResults', "使用 {0} 导航到结果表。", kb);
			}
		}
		return label;
	}
	...
	private createDefineKeybindingAction(keybindingItemEntry: IKeybindingItemEntry): IAction {
		return <IAction>{
			label: keybindingItemEntry.keybindingItem.keybinding ? localize('changeLabel', "更改键绑定…") : localize('addLabel', "添加键绑定…"),
			enabled: true,
	...
	private createDefineWhenExpressionAction(keybindingItemEntry: IKeybindingItemEntry): IAction {
		return <IAction>{
			label: localize('editWhen', "更改 When 表达式"),
	...
	private createRemoveAction(keybindingItem: IKeybindingItemEntry): IAction {
		return <IAction>{
			label: localize('removeLabel', "删除键绑定"),
	...
	private createResetAction(keybindingItem: IKeybindingItemEntry): IAction {
		return <IAction>{
			label: localize('resetLabel', "重置按键绑定"),
	...
	private createShowConflictsAction(keybindingItem: IKeybindingItemEntry): IAction {
		return <IAction>{
			label: localize('showSameKeybindings', "显示相同的按键绑定"),
			enabled: !!keybindingItem.keybindingItem.keybinding,
	...
	private createCopyCommandAction(keybinding: IKeybindingItemEntry): IAction {
		return <IAction>{
			label: localize('copyCommandLabel', "复制命令 ID"),
			enabled: true,
	...
	private createCopyCommandTitleAction(keybinding: IKeybindingItemEntry): IAction {
		return <IAction>{
			label: localize('copyCommandTitleLabel', "复制命令标题"),
	...
	private onKeybindingEditingError(error: unknown): void {
		this.notificationService.error(typeof error === 'string' ? error : localize('error', "编辑按键绑定时发生错误“{0}”。请打开 \"keybindings.json\" 文件并检查错误。", `${error}`));
	}
}
...
class ActionsColumnRenderer implements ITableRenderer<IKeybindingItemEntry, IActionsColumnTemplateData> {
	...
	private createEditAction(keybindingItemEntry: IKeybindingItemEntry): IAction {
		return <IAction>{
			...
			enabled: true,
			id: 'editKeybinding',
			tooltip: this.keybindingsService.appendKeybinding(localize('editKeybindingLabel', "更改键绑定"), KEYBINDINGS_EDITOR_COMMAND_DEFINE),
	...
	private createAddAction(keybindingItemEntry: IKeybindingItemEntry): IAction {
		return <IAction>{
			...
			enabled: true,
			id: 'addKeybinding',
			tooltip: this.keybindingsService.appendKeybinding(localize('addKeybindingLabel', "添加键绑定"), KEYBINDINGS_EDITOR_COMMAND_DEFINE),
			run: () => this.keybindingsEditor.defineKeybinding(keybindingItemEntry, false)
		};
	}
...
class SourceColumnRenderer implements ITableRenderer<IKeybindingItemEntry, ISourceColumnTemplateData> {
	...
	renderTemplate(container: HTMLElement): ISourceColumnTemplateData {
		...
		const extensionLabel = DOM.append<HTMLAnchorElement>(extensionContainer, $('a.extension-label'));
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: ISourceColumnTemplateData): void {
		...
		if (isString(keybindingItemEntry.keybindingItem.source)) {
			...
			templateData.sourceColumnHover.update(localize('extension label', "扩展({0})", extensionLabel));
			// Code-Tomoshibi: the marketplace UI is gone, so this used to be a link to the extension page
			// and is now plain text.
...
class AccessibilityProvider implements IListAccessibilityProvider<IKeybindingItemEntry> {
	...
	getWidgetAriaLabel(): string {
		return localize('keybindingsLabel', "键绑定");
	}
	...
	getAriaLabel({ keybindingItem }: IKeybindingItemEntry): string {
		const ariaLabel = [
			...
			keybindingItem.keybinding?.getAriaLabel() || localize('noKeybinding', "未分配键绑定"),
			keybindingItem.when ? keybindingItem.when : localize('noWhen', "没有时间上下文"),
		...
		if (this.configurationService.getValue(AccessibilityVerbositySettingId.KeybindingsEditor)) {
			const kbEditorAriaLabel = localize('keyboard shortcuts aria label', "使用空格或 Enter 来更改键绑定。");
