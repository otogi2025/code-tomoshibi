...
export class ListSettingWidget<TListDataItem extends IListDataItem> extends AbstractListSettingWidget<TListDataItem> {
	...
	protected renderEdit(item: TListDataItem, idx: number): HTMLElement {
		...
		okButton.label = localize('okButton', "确定");
		...
		cancelButton.label = localize('cancelButton', "取消");
	...
	protected addTooltipsToRow(rowElementGroup: RowElementGroup, { value, sibling }: TListDataItem) {
		const title = isUndefinedOrNull(sibling)
			? localize('listValueHintLabel', "列出项目\"{0}\"", value.data)
			: localize('listSiblingHintLabel', "列出与\"${1}\"同级的项目\"{0}\"", value.data, sibling);
	...
	protected getLocalizedStrings() {
		return {
			deleteActionTooltip: localize('removeItem', "删除项"),
			editActionTooltip: localize('editItem', "编辑项"),
			addButtonLabel: localize('addItem', "添加项"),
			inputPlaceholder: localize('itemInputPlaceholder', "项..."),
			siblingInputPlaceholder: localize('listSiblingInputPlaceholder', "同级..."),
		};
	}
...
export class ExcludeSettingWidget extends ListSettingWidget<IIncludeExcludeDataItem> {
	...
	protected override addTooltipsToRow(rowElementGroup: RowElementGroup, item: IIncludeExcludeDataItem): void {
		let title = isUndefinedOrNull(item.sibling)
			? localize('excludePatternHintLabel', "排除与“{0}”匹配的文件", item.value.data)
			: localize('excludeSiblingHintLabel', "仅当存在匹配“{1}”的文件时，才排除匹配“{0}”的文件", item.value.data, item.sibling);

		if (item.source) {
			title += localize('excludeIncludeSource', ".“{0}”提供的默认值", item.source);
		}
	...
	protected override getLocalizedStrings() {
		return {
			deleteActionTooltip: localize('removeExcludeItem', "删除排除项"),
			editActionTooltip: localize('editExcludeItem', "编辑排除项目"),
			addButtonLabel: localize('addPattern', "添加模式"),
			inputPlaceholder: localize('excludePatternInputPlaceholder', "排除项的模式..."),
			siblingInputPlaceholder: localize('excludeSiblingInputPlaceholder', "当符合此模式的项目存在时..."),
		};
	}
}
...
export class IncludeSettingWidget extends ListSettingWidget<IIncludeExcludeDataItem> {
	...
	protected override addTooltipsToRow(rowElementGroup: RowElementGroup, item: IIncludeExcludeDataItem): void {
		let title = isUndefinedOrNull(item.sibling)
			? localize('includePatternHintLabel', "包括与 `{0}` 匹配的文件", item.value.data)
			: localize('includeSiblingHintLabel', "仅当存在匹配 `{0}` 的文件时，才包含匹配 `{1}` 的文件", item.value.data, item.sibling);
	...
	protected override getLocalizedStrings() {
		return {
			deleteActionTooltip: localize('removeIncludeItem', "删除包含项"),
			editActionTooltip: localize('editIncludeItem', "编辑包含项"),
			addButtonLabel: localize('addPattern', "添加模式"),
			inputPlaceholder: localize('includePatternInputPlaceholder', "包括模式..."),
			siblingInputPlaceholder: localize('includeSiblingInputPlaceholder', "当符合此模式的项目存在时..."),
		};
	}
}
...
export class ObjectSettingDropdownWidget extends AbstractListSettingWidget<IObjectDataItem> {
	...
	private renderStringEditWidget(
		...
		const inputBox = new InputBox(wrapper, this.contextViewService, {
			placeholder: isKey
				? localize('objectKeyInputPlaceholder', "键")
				: localize('objectValueInputPlaceholder', "值"),
			inputBoxStyles: getInputBoxStyle({
				inputBackground: settingsTextInputBackground,
				inputForeground: settingsTextInputForeground,
				inputBorder: settingsTextInputBorder
	...
	protected addTooltipsToRow(rowElementGroup: RowElementGroup, item: IObjectDataItem): void {
		...
		if (item.source) {
			accessibleDescription = localize('objectPairHintLabelWithSource', "“{2}”已将属性“{0}”设置为“{1}”。", item.key.data, item.value.data, item.source);
		} else {
			accessibleDescription = localize('objectPairHintLabel', "属性“{0}”设置为“{1}”。", item.key.data, item.value.data);
		}
	...
	protected getLocalizedStrings() {
		return {
			deleteActionTooltip: localize('removeItem', "删除项"),
			resetActionTooltip: localize('resetItem', "重置项"),
			editActionTooltip: localize('editItem', "编辑项"),
			addButtonLabel: localize('addItem', "添加项"),
			keyHeaderText: localize('objectKeyHeader', "项"),
			valueHeaderText: localize('objectValueHeader', "值"),
		};
	}
}
...
export class ObjectSettingCheckboxWidget extends AbstractListSettingWidget<IBoolObjectDataItem> {
	...
	protected addTooltipsToRow(rowElementGroup: RowElementGroup, item: IBoolObjectDataItem): void {
		const accessibleDescription = localize('objectPairHintLabel', "属性“{0}”设置为“{1}”。", item.key.data, item.value.data);
