...
export class SettingsTargetsWidget extends Widget {
	...
	private resetLabels() {
		...
		this.userLocalSettings.label = localize('userSettings', "用户");
		this.userRemoteSettings.label = localize('userSettingsRemote', "远程") + (hostLabel ? ` [${hostLabel}]` : '');
		this.workspaceSettings.label = this.contextService.getWorkspace().name || localize('workspaceSettings', "工作区");
		this.folderSettingsAction.label = localize('folderSettings', "文件夹");
	}
	...
	private create(parent: HTMLElement): void {
		...
		this.settingsSwitcherBar = this._register(new ActionBar(settingsTabsWidget, {
			...
			ariaLabel: localize('settingsSwitcherBarAriaLabel', "设置转换器"),
			ariaRole: 'tablist',
		...
		this.userLocalSettings.tooltip = localize('userSettings', "用户");
		...
		const remoteAuthority = this.environmentService.remoteAuthority;
		const hostLabel = remoteAuthority && this.labelService.getHostLabel(Schemas.vscodeRemote, remoteAuthority);
		this.userRemoteSettings.tooltip = localize('userSettingsRemote', "远程") + (hostLabel ? ` [${hostLabel}]` : '');
	...
	setResultCount(settingsTarget: SettingsTarget, count: number): void {
		if (settingsTarget === ConfigurationTarget.WORKSPACE) {
			let label = this.contextService.getWorkspace().name ?? localize('workspaceSettings', "工作区");
			if (count) {
				label += ` (${count})`;
			}
			...
			let label = localize('userSettings', "用户");
			if (count) {
				label += ` (${count})`;
			}
	...
	private async update(): Promise<void> {
		...
		this.workspaceSettings.tooltip = localize('workspaceSettings', "工作区");
	}
}
