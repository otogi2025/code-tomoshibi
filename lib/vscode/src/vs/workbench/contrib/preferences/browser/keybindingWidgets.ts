...
export class DefineKeybindingWidget extends Widget {
	...
	constructor(
		...
		const message = nls.localize('defineKeybinding.initial', "先按所需的组合键，再按 Enter 键。");
	...
	printExisting(numberOfExisting: number): void {
		if (numberOfExisting > 0) {
			...
			const text = numberOfExisting === 1 ? nls.localize('defineKeybinding.oneExists', "已有 1 条命令的按键绑定与此相同", numberOfExisting) : nls.localize('defineKeybinding.existing', "已有 {0} 条命令的按键绑定与此相同", numberOfExisting);
	...
	private onKeybinding(keybinding: ResolvedKeybinding[] | null): void {
		...
		if (this._chords) {
			for (let i = 1; i < this._chords.length; i++) {
				this._outputNode.appendChild(document.createTextNode(nls.localize('defineKeybinding.chordsTo', "加上")));
