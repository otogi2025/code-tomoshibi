...
export class ConfigureLanguageBasedSettingsAction extends Action {
	...
	static readonly LABEL = nls.localize2('configureLanguageBasedSettings', "配置语言特定的设置...");
	...
	override async run(): Promise<void> {
		...
		await this.quickInputService.pick(picks, { placeHolder: nls.localize('pickLanguage', "选择语言") })
			.then(pick => {
				if (pick) {
