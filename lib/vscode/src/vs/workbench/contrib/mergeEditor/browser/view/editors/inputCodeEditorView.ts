	...
	constructor(
		...
		this._register(autorunOpts({ debugName: `input${this.inputNumber}: update labels & text model` }, reader => {
			...
			const title = this.inputNumber === 1
				? vm.model.input1.title || localize('input1', '输入 1')
				: vm.model.input2.title || localize('input2', '输入 2');
		...
		return [
			this.baseRange.input1Diffs.length > 0
				? action(
					...
					localize('mergeEditor.accept', '接受 {0}', this.model.input1.title),
					state.toggle(1),
			...
			this.baseRange.input2Diffs.length > 0
				? action(
					...
					localize('mergeEditor.accept', '接受 {0}', this.model.input2.title),
					state.toggle(2),
			...
			setFields(
				new Action(
					...
					localize('mergeEditor.markAsHandled', '标记为已处理'),
					undefined,
					true,
					() => {
						transaction((tx) => {
		...
		this._register(
			autorun(reader => {
				...
				const iconMap: Record<InputState, { icon: ThemeIcon | undefined; checked: boolean; title: string }> = {
					[InputState.excluded]: { icon: undefined, checked: false, title: localize('accept.excluded', "接受") },
					[InputState.unrecognized]: { icon: Codicon.circleFilled, checked: false, title: localize('accept.conflicting', "接受(结果为脏)") },
					[InputState.first]: { icon: Codicon.check, checked: true, title: localize('accept.first', "撤消接受") },
					[InputState.second]: { icon: Codicon.checkAll, checked: true, title: localize('accept.second', "撤消接受(当前为第二个)") },
				};
