	...
	constructor(
		...
		this._register(
			autorun(reader => {
				...
				reset(this.htmlElements.title, ...renderLabelWithIcons(localize('base', '基础')));

				const baseShowDiffAgainst = vm.baseShowDiffAgainst.read(reader);

				let node: Node | undefined = undefined;
				if (baseShowDiffAgainst) {
					const label = localize('compareWith', '正在与 {0} 比较', baseShowDiffAgainst === 1 ? vm.model.input1.title : vm.model.input2.title);
					const tooltip = localize('compareWithTooltip', '差异以背景色突出显示。');
