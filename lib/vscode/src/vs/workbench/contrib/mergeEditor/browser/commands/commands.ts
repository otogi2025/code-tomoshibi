...
export class SetMixedLayout extends Action2 {
	constructor() {
		super({
			...
			title: localize2('layout.mixed', "混合布局"),
...
export class SetColumnLayout extends Action2 {
	constructor() {
		super({
			...
			title: localize2('layout.column', '列布局'),
...
export class ShowNonConflictingChanges extends Action2 {
	constructor() {
		super({
			...
			title: localize2('showNonConflictingChanges', "显示不冲突的更改"),
...
export class ShowHideBase extends Action2 {
	constructor() {
		super({
			id: 'merge.showBase',
			title: localize2('layout.showBase', "显示基础"),
...
export class ShowHideTopBase extends Action2 {
	constructor() {
		super({
			...
			title: localize2('layout.showBaseTop', "显示基顶部"),
...
export class ShowHideCenterBase extends Action2 {
	constructor() {
		super({
			...
			title: localize2('layout.showBaseCenter', "显示基中心"),
...
const mergeEditorCategory: ILocalizedString = localize2('mergeEditor', "合并编辑器");
...
export class GoToNextUnhandledConflict extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('merge.goToNextUnhandledConflict', "转到下一个未处理的冲突"),
...
export class GoToPreviousUnhandledConflict extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('merge.goToPreviousUnhandledConflict', "转到上一个未处理的冲突"),
...
export class ToggleActiveConflictInput1 extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('merge.toggleCurrentConflictFromLeft', "从左侧切换当前冲突"),
			f1: true,
			precondition: ctxIsMergeEditor,
		});
	}
...
export class ToggleActiveConflictInput2 extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('merge.toggleCurrentConflictFromRight', "从右侧切换当前冲突"),
			f1: true,
			precondition: ctxIsMergeEditor,
		});
	}
...
export class CompareInput1WithBaseCommand extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('mergeEditor.compareInput1WithBase', "将输入 1 与基本值进行比较"),
			shortTitle: localize('mergeEditor.compareWithBase', '与基线进行比较'),
			f1: true,
			precondition: ctxIsMergeEditor,
...
export class CompareInput2WithBaseCommand extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('mergeEditor.compareInput2WithBase', "将输入 2 与基本值进行比较"),
			shortTitle: localize('mergeEditor.compareWithBase', '与基线进行比较'),
			f1: true,
			precondition: ctxIsMergeEditor,
...
export class OpenBaseFile extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('merge.openBaseEditor', "打开基本文件"),
			f1: true,
			precondition: ctxIsMergeEditor,
		});
	}
...
export class AcceptAllInput1 extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('merge.acceptAllInput1', "接受来自左侧的所有传入更改"),
			f1: true,
			precondition: ctxIsMergeEditor,
			menu: { id: MenuId.MergeInput1Toolbar, group: 'primary' },
...
export class AcceptAllInput2 extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('merge.acceptAllInput2', "接受来自右侧的所有当前更改"),
			f1: true,
			precondition: ctxIsMergeEditor,
			menu: { id: MenuId.MergeInput2Toolbar, group: 'primary' },
			icon: Codicon.checkAll,
...
export class ResetToBaseAndAutoMergeCommand extends MergeEditorAction {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('mergeEditor.resetResultToBaseAndAutoMerge', "重置结果"),
			shortTitle: localize('mergeEditor.resetResultToBaseAndAutoMerge.short', '重置'),
			f1: true,
			precondition: ctxIsMergeEditor,
...
export class ResetCloseWithConflictsChoice extends Action2 {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('mergeEditor.resetChoice', "重置“关闭并存在冲突”选项"),
			f1: true,
		});
	}
	run(accessor: ServicesAccessor): void {
...
export class AcceptAllCombination extends MergeEditorAction2 {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('mergeEditor.acceptAllCombination', "接受所有组合"),
			f1: true,
		});
	}
...
export class AcceptMerge extends MergeEditorAction2 {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('mergeEditor.acceptMerge', "完成合并"),
			f1: true,
			precondition: ctxIsMergeEditor,
			keybinding: [
				{
	...
	override async runWithMergeEditor({ inputModel, editorIdentifier, viewModel }: MergeEditorAction2Args, accessor: ServicesAccessor) {
		...
		if (viewModel.model.unhandledConflictsCount.get() > 0) {
			const { confirmed } = await dialogService.confirm({
				message: localize('mergeEditor.acceptMerge.unhandledConflicts.message', "要完成 {0} 的合并吗?", basename(inputModel.resultUri)),
				detail: localize('mergeEditor.acceptMerge.unhandledConflicts.detail', "文件包含未经处理的冲突。"),
				primaryButton: localize({ key: 'mergeEditor.acceptMerge.unhandledConflicts.accept', comment: ['&& denotes a mnemonic'] }, "完成但出现冲突(&&C)")
			});
...
export class ToggleBetweenInputs extends MergeEditorAction2 {
	constructor() {
		super({
			...
			category: mergeEditorCategory,
			title: localize2('mergeEditor.toggleBetweenInputs', "在合并编辑器输入之间切换"),
			f1: true,
			precondition: ctxIsMergeEditor,
			keybinding: [
				{
