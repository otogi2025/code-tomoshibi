...
export class MergeEditorInput extends AbstractTextResourceEditorInput implements ILanguageSupport {
	...
	override getName(): string {
		return localize('name', "正在合并: {0}", super.getName());
	}
...
function alertFocusedEditor(editor: MergeEditorType) {
	switch (editor) {
		case 'input1':
			alert(localize('mergeEditor.input1', "传入，左侧输入"));
			break;
		case 'input2':
			alert(localize('mergeEditor.input2', "当前，右侧输入"));
			break;
		case 'result':
			alert(localize('mergeEditor.result', "合并结果"));
			break;
	}
}
