...
const MERGE_EDITOR_CATEGORY: ILocalizedString = localize2('mergeEditor', '合并编辑器(开发)');
...
export class MergeEditorCopyContentsToJSON extends Action2 {
	constructor() {
		super({
			...
			title: localize2('merge.dev.copyState', "将合并编辑器状态复制为 JSON"),
	...
	run(accessor: ServicesAccessor): void {
		...
		if (!(activeEditorPane instanceof MergeEditor)) {
			notificationService.info({
				name: localize('mergeEditor.name', '合并编辑器'),
				message: localize('mergeEditor.noActiveMergeEditor', "无活动合并编辑器")
			});
			return;
		}
		...
		notificationService.info({
			name: localize('mergeEditor.name', '合并编辑器'),
			message: localize('mergeEditor.successfullyCopiedMergeEditorContents', "已成功复制合并编辑器状态"),
		});
	}
}
...
export class MergeEditorSaveContentsToFolder extends Action2 {
	constructor() {
		super({
			...
			category: MERGE_EDITOR_CATEGORY,
			title: localize2('merge.dev.saveContentsToFolder', "将合并编辑器状态保存到文件夹"),
			icon: Codicon.layoutCentered,
			f1: true,
			precondition: ctxIsMergeEditor,
		});
	...
	async run(accessor: ServicesAccessor) {
		...
		const result = await dialogService.showOpenDialog({
			...
			canSelectMany: false,
			title: localize('mergeEditor.selectFolderToSaveTo', '选择要保存到的文件夹')
		});
		if (!result) {
			return;
		}
		...
		notificationService.info({
			name: localize('mergeEditor.name', '合并编辑器'),
			message: localize('mergeEditor.successfullySavedMergeEditorContentsToFolder', "已成功将合并编辑器状态保存到文件夹"),
		});
	}
}
...
export class MergeEditorLoadContentsFromFolder extends Action2 {
	constructor() {
		super({
			...
			category: MERGE_EDITOR_CATEGORY,
			title: localize2('merge.dev.loadContentsFromFolder', "从文件夹加载合并编辑器状态"),
			icon: Codicon.layoutCentered,
			f1: true
		});
	}
