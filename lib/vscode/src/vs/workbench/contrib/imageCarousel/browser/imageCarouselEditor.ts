...
export class ImageCarouselEditor extends EditorPane {
	...
	private buildSlideshow(): void {
		...
		if (this._flatImages.length === 0) {
			...
			empty.root.textContent = localize('imageCarousel.noImages', "没有可显示的映像");
		...
		const elements = h('div.slideshow-container', [
			h('div.image-area@imageArea', [
				...
				h('button.nav-arrow.prev-arrow@prevBtn', { ariaLabel: localize('imageCarousel.previousImage', "上一张图像") }, [
					h('span.codicon.codicon-chevron-left', { ariaHidden: 'true' }),
				]),
				h('button.nav-arrow.next-arrow@nextBtn', { ariaLabel: localize('imageCarousel.nextImage', "下一张图像") }, [
		...
		elements.root.setAttribute('aria-label', localize('imageCarousel.ariaLabel', "图像预览"));
		...
		elements.sectionsContainer.setAttribute('aria-label', localize('imageCarousel.thumbnails', "图像缩略图"));
		...
		for (let s = 0; s < this._sections.length; s++) {
			...
			for (let i = 0; i < section.images.length; i++) {
				...
				btn.ariaLabel = isItemVideo
					? localize('imageCarousel.thumbnailLabelVideo', "视频 {0}/{1}", currentFlatIndex + 1, this._flatImages.length)
					: localize('imageCarousel.thumbnailLabelImage', "第 {0} 张图像(共 {1} 张)", currentFlatIndex + 1, this._flatImages.length);
...
window.addEventListener("message",function(e){var m=e.data;if(m.type==="loadVideo"){var b=new Blob([m.data],{type:m.mimeType});document.getElementById("v").src=URL.createObjectURL(b);}});
		...
		const itemKind = isVideo
			? localize('imageCarousel.kindVideo', "视频")
			: localize('imageCarousel.kindImage', "图像");
		this._elements.ariaStatus.textContent = currentImage.caption
			? localize('imageCarousel.statusWithCaption', "{0} {1}/{2}: {3}", itemKind, navigationIndex + 1, this._flatImages.length, currentImage.caption)
			: localize('imageCarousel.statusWithName', "{0} {1}/{2}: {3}", itemKind, navigationIndex + 1, this._flatImages.length, currentImage.name);
