...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	id: 'imageCarousel',
	title: localize('imageCarouselConfigurationTitle', "图像预览"),
	type: 'object',
	properties: {
		'imageCarousel.explorerContextMenu.enabled': {
			type: 'boolean',
			default: true,
			markdownDescription: localize('imageCarousel.explorerContextMenu.enabled', "控制“在图像预览中打开”选项是否显示在“资源管理器”上下文菜单中。"),
			tags: ['experimental'],
		},
		'imageCarousel.chat.enabled': {
			type: 'boolean',
			default: true,
			description: localize('imageCarousel.chat.enabled', "控制点击聊天中的图像附件是否打开图像预览查看器。"),
		},
	}
});
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(
		...
		localize('imageCarouselEditor', "图像预览")
	),
	[
...
class OpenImageInCarouselAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('openImageInCarousel', "在图像预览中打开"),
			f1: false
		});
	}
	...
	async run(accessor: ServicesAccessor, args?: unknown): Promise<void> {
		...
		if (isCollectionArgs(args)) {
			...
			collection = {
				id: generateUuid(),
				title: args.title ?? localize('imageCarousel.title', "图像预览"),
				sections: [{
					title: '',
					images: [{
						id: generateUuid(),
...
class OpenImagesInCarouselFromExplorerAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('openImagesInCarousel', "在图像预览中打开"),
			f1: false,
			menu: [{
	...
	async run(accessor: ServicesAccessor, resource?: URI): Promise<void> {
		...
		try {
			...
			notificationService.error(localize('folderReadError', "无法读取文件夹内容。"));
			return;
		}

		if (imageUris.length === 0) {
			notificationService.info(localize('noImagesFound', "未在此文件夹中找到图像。"));
			return;
		}
		...
		const collection: IImageCarouselCollection = {
			id: generateUuid(),
			title: localize('imageCarousel.explorerTitle', "图像预览"),
			sections: [{
				title: '',
				images,
			}],
