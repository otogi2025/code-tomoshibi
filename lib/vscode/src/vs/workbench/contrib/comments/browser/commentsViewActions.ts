...
registerAction2(class extends ViewAction<ICommentsView> {
	constructor() {
		super({
			...
			title: localize('focusCommentsList', "焦点评论视图"),
			keybinding: {
...
registerAction2(class extends ViewAction<ICommentsView> {
	constructor() {
		super({
			...
			title: localize('commentsClearFilterText', "清除筛选器文本"),
			keybinding: {
				when: CommentsViewFilterFocusContextKey,
				weight: KeybindingWeight.WorkbenchContrib,
...
registerAction2(class extends ViewAction<ICommentsView> {
	constructor() {
		super({
			...
			title: localize('focusCommentsFilter', "焦点评论筛选器"),
			keybinding: {
...
registerAction2(class extends ViewAction<ICommentsView> {
	constructor() {
		super({
			...
			title: localize('toggle unresolved', "显示未解决评论"),
			category: localize('comments', "注释"),
			toggled: {
				condition: CONTEXT_KEY_SHOW_UNRESOLVED,
				title: localize('unresolved', "显示未解决"),
			},
			menu: {
...
registerAction2(class extends ViewAction<ICommentsView> {
	constructor() {
		super({
			...
			title: localize('toggle resolved', "显示已解决评论"),
			category: localize('comments', "注释"),
			toggled: {
				condition: CONTEXT_KEY_SHOW_RESOLVED,
				title: localize('resolved', "显示已解决"),
			},
			menu: {
				id: viewFilterSubmenu,
				group: '1_filter',
...
MenuRegistry.appendMenuItem(viewFilterSubmenu, {
	...
	title: localize('comment sorts', "排序依据"),
	group: '2_sort',
...
registerAction2(class extends ViewAction<ICommentsView> {
	constructor() {
		super({
			...
			title: localize('toggle sorting by resource', "在文件中的位置"),
			category: localize('comments', "注释"),
			icon: Codicon.history,
			viewId: COMMENTS_VIEW_ID,
			toggled: {
				condition: ContextKeyExpr.equals(CONTEXT_KEY_SORT_BY.key, CommentsSortOrder.ResourceAscending),
				title: localize('sorting by position in file', "在文件中的位置"),
			},
			menu: {
				id: commentSortSubmenu,
				group: 'navigation',
