...
export class CommentReply<T extends IRange> extends Disposable {
	...
	setCommentEditorDecorations() {
		...
		const placeholder = hasExistingComments
			? (this._commentOptions?.placeHolder || nls.localize('reply', "回复..."))
			: (this._commentOptions?.placeHolder || nls.localize('newComment', "键入新注释"));
	...
	private createReplyButton(commentEditor: ICodeEditor, commentForm: HTMLElement) {
		...
		this._register(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), this._reviewThreadReplyButton, this._commentOptions?.prompt || nls.localize('reply', "回复...")));

		this._reviewThreadReplyButton.textContent = this._commentOptions?.prompt || nls.localize('reply', "回复...");
