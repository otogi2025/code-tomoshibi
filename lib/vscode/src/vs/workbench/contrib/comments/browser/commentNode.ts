...
export class CommentNode<T extends IRange> extends Disposable {
	...
	private createReactionPicker(reactionGroup: languages.CommentReaction[]): ToggleReactionsAction {
		...
		}, nls.localize('commentToggleReaction', "切换反应")));
				...
				return this._reactionActions.add(new Action(`reaction.command.${reaction.label}`, `${reaction.label}`, '', true, async () => {
					try {
						...
						const error = e.message
							? nls.localize('commentToggleReactionError', "切换评论反应失败: {0}。", e.message)
							: nls.localize('commentToggleReactionDefaultError', "切换评论反应失败");
			...
			const action = this._reactionActions.add(new ReactionAction(`reaction.${reaction.label}`, `${reaction.label}`, reaction.hasReacted && (reaction.canEdit || hasReactionHandler) ? 'active' : '', (reaction.canEdit || hasReactionHandler), async () => {
				try {
					...
					if (reaction.hasReacted) {
						error = e.message
							? nls.localize('commentDeleteReactionError', "未能删除评论回应: {0}。", e.message)
							: nls.localize('commentDeleteReactionDefaultError', "未能删除评论回应");
					} else {
						error = e.message
							? nls.localize('commentAddReactionError', "未能删除评论回应: {0}。", e.message)
							: nls.localize('commentAddReactionDefaultError', "未能删除评论回应");
					}
					this.notificationService.error(error);
				}
