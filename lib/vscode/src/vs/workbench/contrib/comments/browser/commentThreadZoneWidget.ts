	...
	private async confirmCollapse(): Promise<boolean> {
		...
		if (confirmSetting === 'whenHasUnsubmittedComments' && this._commentThreadWidget.hasUnsubmittedComments) {
			const result = await this.dialogService.confirm({
				message: nls.localize('confirmCollapse', "折叠此评论线程将舍弃未提交的评论。确定要舍弃这些评论吗?"),
				primaryButton: nls.localize('discard', "舍弃"),
				type: Severity.Warning,
				checkbox: { label: nls.localize('neverAskAgain', "不再询问我"), checked: false }
			});
