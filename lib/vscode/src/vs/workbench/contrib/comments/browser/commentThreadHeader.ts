...
const collapseIcon = registerIcon('review-comment-collapse', Codicon.chevronUp, nls.localize('collapseIcon', '用于折叠审阅注释的图标。'));
...
export class CommentThreadHeader<T = IRange> extends Disposable {
	...
	protected _fillHead(): void {
		...
		this._collapseAction = new Action(CommentCommandId.Hide, nls.localize('label.collapse', "折叠"), collapseClass, true, () => this._delegate.collapse());
	...
	createThreadLabel() {
		...
		if (label === undefined) {
			if (!(this._commentThread.comments && this._commentThread.comments.length)) {
				label = nls.localize('startThread', "开始讨论");
			}
		}
