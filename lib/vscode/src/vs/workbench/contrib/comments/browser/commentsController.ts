	...
	private _findNearestCommentingRange(reverse?: boolean): void {
		...
		if (this.accessibilityService.isScreenReaderOptimized()) {
			...
			if (commentRangeStart && commentRangeEnd) {
				...
				oneLine ? status(nls.localize('commentRange', "行 {0}", commentRangeStart)) : status(nls.localize('commentRangeStart', "第 {0} 到 {1} 行", commentRangeStart, commentRangeEnd));
			}
		}
	}
			...
			if (!this._hasRespondedToEditorChange) {
				if (this._commentInfos.some(commentInfo => commentInfo.commentingRanges.ranges.length > 0 || commentInfo.commentingRanges.fileComments)) {
					...
					if (verbose) {
						...
						if (keybinding) {
							status(nls.localize('hasCommentRangesKb', "编辑器有注释范围，有关详细信息，请运行“打开辅助功能帮助”({0}) 命令。", keybinding));
						} else {
							status(nls.localize('hasCommentRangesNoKb', "编辑器具有注释范围，有关详细信息，请运行当前无法通过键绑定触发的“打开辅助功能帮助”命令。"));
						}
					} else {
						status(nls.localize('hasCommentRanges', "编辑器具有注释范围。"));
					}
				}
			}
		});
	...
	public addCommentAtLine(range: Range | undefined, e: IEditorMouseEvent | undefined): Promise<void> {
		...
		if (!newCommentInfos.length || !this.editor?.hasModel()) {
			...
			if (!newCommentInfos.length) {
				if (range) {
					this.notificationService.error(nls.localize('comments.addCommand.error', "光标必须在评论范围内才能添加评论。"));
				} else {
					this.notificationService.error(nls.localize('comments.addFileCommentCommand.error', "此文件上不允许有文件注释。"));
				}
			}
			return Promise.resolve();
		}
		...
		if (newCommentInfos.length > 1) {
			if (e && range) {
				...
				return this.quickInputService.pick(picks, { placeHolder: nls.localize('pickCommentService', "选择评论提供程序"), matchOnDescription: true }).then(pick => {
					if (!pick) {
						return;
					}
