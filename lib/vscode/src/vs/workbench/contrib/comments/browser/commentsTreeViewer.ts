...
export const COMMENTS_VIEW_TITLE: ILocalizedString = nls.localize2('comments.view.title', "评论");
...
export class CommentNodeRenderer implements IListRenderer<ITreeNode<CommentNode>, ICommentThreadTemplateData> {
	...
	private getCountString(commentCount: number): string {
		if (commentCount > 2) {
			return nls.localize('commentsCountReplies', "{0} 条回复", commentCount - 1);
		} else if (commentCount === 2) {
			return nls.localize('commentsCountReply', "1 条回复");
		} else {
			return nls.localize('commentCount', "1 条注释");
		}
	}
	...
	private getRenderedComment(commentBody: IMarkdownString) {
		...
		for (let i = 0; i < images.length; i++) {
			...
			textDescription.textContent = image.alt ? nls.localize('imageWithLabel', "图片: {0}", image.alt) : nls.localize('image', "图片");
	...
	renderElement(node: ITreeNode<CommentNode>, index: number, templateData: ICommentThreadTemplateData): void {
		...
		if (node.element.threadRelevance === CommentThreadApplicability.Outdated) {
			...
			templateData.threadMetadata.relevance.innerText = nls.localize('outdated', "已过时");
		...
		templateData.repliesMetadata.lastReplyDetail.textContent = nls.localize('lastReplyFrom', "来自 {0} 的最新回复", lastComment.userName);
