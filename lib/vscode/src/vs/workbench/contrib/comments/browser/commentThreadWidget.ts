...
export class CommentThreadWidget<T extends IRange = IRange> extends Disposable implements ICommentThreadWidget {
	...
	private _setAriaLabel(): void {
		let ariaLabel = localize('commentLabel', "注释");
		...
		if (keybinding) {
			ariaLabel = localize('commentLabelWithKeybinding', "{0}，请使用 ({1}) 获取辅助功能帮助", ariaLabel, keybinding);
		} else if (verbose) {
			ariaLabel = localize('commentLabelWithKeybindingNoKeybinding', "{0}，运行当前无法通过键绑定触发的“打开辅助功能帮助”命令。", ariaLabel);
		}
