...
export class CommentsPanel extends FilterViewPane implements ICommentsView {
	...
	constructor(
		...
		super({
			...options,
			filterOptions: {
				placeholder: nls.localize('comments.filter.placeholder', "筛选器(例如文本、作者)"),
				ariaLabel: nls.localize('comments.filter.ariaLabel', "筛选评论"),
	...
	private updateFilter() {
		...
		this.filterWidget.updateBadge(total === filtered || total === 0 ? undefined : nls.localize('showing filtered results', "正在显示第 {0} 页(共 {1} 页)", filtered, total));
	...
	private makeCommentLocationLabel(file: URI, range?: IRange) {
		...
		if (!range) {
			return nls.localize('fileCommentLabel', "{0} 中", fileLabel);
		}
		if (range.startLineNumber === range.endLineNumber) {
			return nls.localize('oneLineCommentLabel', "行 {0} 列 {1} {2}", range.startLineNumber, range.startColumn, fileLabel);
		} else {
			return nls.localize('multiLineCommentLabel', "从行 {0} 到 {2} 中的行 {1}", range.startLineNumber, range.endLineNumber, fileLabel);
		}
	}
	...
	private getReplyCountAsString(node: CommentNode, forAriaLabel?: boolean): string {
		return node.replies.length && !forAriaLabel ? nls.localize('replyCount', " {0} 条回复，", node.replies.length) : '';
	}
			...
			accessibilityProvider: {
				getAriaLabel: (element: any): string => {
					if (element instanceof CommentsModel) {
						return nls.localize('rootCommentsLabel', "当前工作区的评论");
					}
					if (element instanceof ResourceWithCommentThreads) {
						return nls.localize('resourceWithCommentThreadsLabel', "{0} 中的评论，完整路径: {1}", basename(element.resource), element.resource.fsPath);
					}
