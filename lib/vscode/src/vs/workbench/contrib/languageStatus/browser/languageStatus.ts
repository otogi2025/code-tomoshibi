...
class LanguageStatus {
	...
	private _update(): void {
		...
		if (model.combined.length === 0) {
			...
			const props: IStatusbarEntry = {
				name: localize('langStatus.name', "编辑器语言状态"),
				ariaLabel: localize('langStatus.aria', "编辑器语言状态: {0}", ariaLabels.join(', next: ')),
	...
	private _renderStatus(container: HTMLElement, status: ILanguageStatus, showSeverity: boolean, isPinned: boolean, store: DisposableStore): HTMLElement {
		...
		const actionLabel: string = isPinned ? localize('unpin', "从状态栏中删除") : localize('pin', "添加到状态栏");
	...
	private static _asStatusbarEntry(item: ILanguageStatus): IStatusbarEntry {
		...
		return {
			name: localize('name.pattern', '{0} (语言状态)', item.name),
...
export class ResetAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('reset', "重置语言状态交互计数器"),
