...
export class InlayHintsAccessibility implements IEditorContribution {
	...
	static readonly IsReading = new RawContextKey<boolean>('isReadingLineWithInlayHints', false, { type: 'boolean', description: localize('isReadingLineWithInlayHints', "当前行及其内嵌提示是否是当前焦点") });
	...
	constructor(
		...
		this._ariaElement.setAttribute('aria-description', localize('description', "具有内嵌提示信息的代码"));
...
registerAction2(class StartReadHints extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('read.title', "使用内嵌提示读取行"),
...
registerAction2(class StopReadHints extends EditorAction2 {
	...
	constructor() {
		super({
			...
			title: localize2('stop.title', "停止内嵌提示读取"),
