...
export class SetLogLevelAction extends Action {
	...
	static readonly TITLE = nls.localize2('setLogLevel', "设置日志级别...");
	...
	private async setLogLevelForChannel(logChannel: LogChannelQuickPickItem): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			...
			quickPick.placeholder = logChannel ? nls.localize('selectLogLevelFor', " {0}: 选择日志级别", logChannel?.label) : nls.localize('selectLogLevel', "选择日志级别");
			quickPick.items = entries;
	...
	private getLogLevelEntries(defaultLogLevel: LogLevel, currentLogLevel: LogLevel, canSetDefaultLogLevel: boolean): LogLevelQuickPickItem[] {
		const button: IQuickInputButton | undefined = canSetDefaultLogLevel ? { iconClass: ThemeIcon.asClassName(Codicon.checkAll), tooltip: nls.localize('resetLogLevel', "设置为默认日志级别") } : undefined;
		return [
	...
	private getDescription(level: LogLevel, defaultLogLevel: LogLevel): string | undefined {
		return defaultLogLevel === level ? nls.localize('default', "默认值") : undefined;
	}
...
export class OpenWindowSessionLogFileAction extends Action {
	...
	static readonly TITLE = nls.localize2('openSessionLogFile', "打开窗口日志文(会话)...");
	...
	override async run(): Promise<void> {
		const sessionResult = await this.quickInputService.pick(
			this.getSessions().then(sessions => sessions.map((s, index): IQuickPickItem => ({
				id: s.toString(),
				label: basename(s),
				description: index === 0 ? nls.localize('current', "当前") : undefined
			}))),
			{
				canPickMany: false,
				placeHolder: nls.localize('sessions placeholder', "选择会话")
			});
		if (sessionResult) {
			const logFileResult = await this.quickInputService.pick(
				...
				{
					canPickMany: false,
					placeHolder: nls.localize('log placeholder', "选择日志文件")
				});
			if (logFileResult) {
