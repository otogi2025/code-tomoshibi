...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: nls.localize2('setDefaultLogLevel', "设置默认日志级别"),
			category: Categories.Developer,
		});
	}
...
class LogOutputChannels extends Disposable implements IWorkbenchContribution {
	...
	private registerLogChannel(logger: ILoggerResource): void {
		...
		const label = hasToAppendRemote ? nls.localize('remote name', "{0} (远程)", logger.name ?? logger.id) : logger.name ?? logger.id;
		...
		this._register(registerAction2(class ShowWindowLogAction extends Action2 {
			constructor() {
				super({
					...
					title: nls.localize2('show window log', "显示窗口日志"),
					category: Categories.Developer,
					f1: true
				});
			}
