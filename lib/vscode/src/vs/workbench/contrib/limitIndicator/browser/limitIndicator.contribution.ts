...
const configureSettingsLabel = nls.localize('status.button.configure', "配置");
...
class ColorDecorationAccessor implements LanguageFeatureAccessor {
	...
	readonly name = nls.localize('colorDecoratorsStatusItem.name', '颜色装饰器状态');
	readonly label = nls.localize('status.limitedColorDecorators.short', '颜色修饰器');
	readonly source = nls.localize('colorDecoratorsStatusItem.source', '颜色修饰器');
...
class FoldingRangeAccessor implements LanguageFeatureAccessor {
	...
	readonly name = nls.localize('foldingRangesStatusItem.name', '正在折叠状态');
	readonly label = nls.localize('status.limitedFoldingRanges.short', '折叠范围');
	readonly source = nls.localize('foldingRangesStatusItem.source', '正在折叠');
	...
	private updateStatusItem(info: LimitInfo | undefined) {
		...
		if (info && info.limited !== false) {
			const status: ILanguageStatus = {
				...
				detail: nls.localize('status.limited.details', '出于性能原因，仅显示了 {0}', info.limited),
