...
const SearchEditorIcon = registerIcon('search-editor-label-icon', Codicon.search, localize('searchEditorLabelIcon', '搜索编辑器标签的图标。'));
...
export class SearchEditorInput extends EditorInput {
	...
	override getName(maxLength = 12): string {
		...
		if (this.backingUri) {
			...
			return localize('searchTitle.withQuery', "搜索: {0}", basename((originalURI ?? this.backingUri).path, SEARCH_EDITOR_EXT));
		}

		const query = this._cachedConfigurationModel?.config?.query?.trim();
		if (query) {
			return localize('searchTitle.withQuery', "搜索: {0}", trimToMax(query));
		}
		return localize('searchTitle', "搜索");
	}
