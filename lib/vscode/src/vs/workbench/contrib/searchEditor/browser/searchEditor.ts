...
export class SearchEditor extends AbstractTextCodeEditor<SearchEditorViewState> {
	...
	private createQueryEditor(container: HTMLElement, scopedInstantiationService: IInstantiationService, inputBoxFocusedContextKey: IContextKey<boolean>) {
		...
		const toggleQueryDetailsLabel = localize('moreSearch', "切换搜索详细信息");
		...
		const filesToIncludeTitle = localize('searchScope.includes', "包含的文件");
		...
		this.inputPatternIncludes = this._register(scopedInstantiationService.createInstance(IncludePatternInputWidget, folderIncludesList, this.contextViewService, {
			ariaLabel: localize('label.includes', '搜索包含模式'),
		...
		const excludesTitle = localize('searchScope.excludes', "排除的文件");
		...
		this.inputPatternExcludes = this._register(scopedInstantiationService.createInstance(ExcludePatternInputWidget, excludesList, this.contextViewService, {
			ariaLabel: localize('label.excludes', '搜索排除模式'),
			inputBoxStyles: searchEditorInputboxStyles
		}));
	...
	private toggleRunAgainMessage(show: boolean) {
		...
		if (show) {
			const runAgainLink = DOM.append(this.messageBox, DOM.$('a.pointer.prominent.message', {}, localize('runSearch', "运行搜索")));
	...
	private iterateThroughMatches(reverse: boolean) {
		...
		alert(localize('searchResultItem', "在文件 {2} 的 {1} 中匹配到 {0}", matchText, matchLineText, file.slice(0, file.length - 1)));
	}
	...
	getAriaLabel() {
		return this.getInput()?.getName() ?? localize('searchEditor', "搜索");
	}
}

const searchEditorTextInputBorder = registerColor('searchEditor.textInputBorder', inputBorder, localize('textInputBoxBorder', "搜索编辑器文本输入框的边框。"));
