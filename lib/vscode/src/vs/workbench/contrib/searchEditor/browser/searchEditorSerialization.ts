		...
		for (let i = 0; i < str.length; i++) {
			if (str[i] === '\\') {
				...
				if (escaped === 'n') {
					...
					throw Error(localize('invalidQueryStringError', "查询字符串中的所有反斜杠都必须转义(\\\\)"));
				}
			} else {
				out += str[i];
			}
...
export const serializeSearchResultForEditor =
	(searchResult: ISearchResult, rawIncludePattern: string, rawExcludePattern: string, contextLines: number, labelFormatter: (x: URI) => string, sortOrder: SearchSortOrder, limitHit?: boolean): { matchRanges: Range[]; text: string; config: Partial<SearchConfiguration> } => {
		...
		const filecount = searchResult.fileCount() > 1 ? localize('numFiles', "{0} 文件", searchResult.fileCount()) : localize('oneFile', "1 个文件");
		const resultcount = searchResult.count() > 1 ? localize('numResults', "{0} 个结果", searchResult.count()) : localize('oneResult', "1 个结果");

		const info = [
			searchResult.count()
				? `${resultcount} - ${filecount}`
				: localize('noResults', "无结果"),
		];
		if (limitHit) {
			info.push(localize('searchMaxResultsWarning', "结果集仅包含所有匹配项的子集。请使你的搜索更加具体以减少结果。"));
		}
		info.push('');
