...
export class BaseIssueReporterService extends Disposable {
	...
	constructor(
		...
		this.createAction = this._register(new Action('issueReporter.create', localize('create', "在 GitHub 上创建"), undefined, true, async () => {
		...
		this.previewAction = this._register(new Action('issueReporter.preview', localize('preview', "在 GitHub 中预览"), undefined, true, async () => {
			this.delayedSubmit.trigger(async () => {
				this.setSubmittingState(true);
				try {
		...
		this.privateAction = this._register(new Action('issueReporter.privateCreate', localize('privateCreate', "在内部创建"), undefined, true, async () => {
			this.delayedSubmit.trigger(async () => {
				this.setSubmittingState(true);
				try {
	...
	private updateInternalFilingNote(container: HTMLElement) {
		...
		filingNote.textContent = escape(localize('internalPreviewMessage', '如果 copilot 调试日志包含私人信息:'));
	}
	...
	private updatePublicGithubButton(container: HTMLElement): void {
		...
		if (!this.acknowledged && this.needsUpdate) { // * old version and hasn't ack'd
			...
			this.publicGithubButton.label = localize('acknowledge', "确认版本收讫");
			...
			this.publicGithubButton.label = localize('createOnGitHub', "在 GitHub 上创建");
			...
			this.publicGithubButton.label = localize('previewOnGitHub', "在 GitHub 中预览");
			this.publicGithubButton.enabled = true;
		}
	...
	private updateInternalGithubButton(container: HTMLElement): void {
		...
		if (this.data.githubAccessToken && this.data.privateUri) {
			...
			this.internalGithubButton.label = localize('createInternally', "在内部创建");
	...
	private setSubmittingState(submitting: boolean): void {
		...
		if (submitting) {
			...
			this.publicGithubButton.label = localize('submittingIssue', "正在提交...");
	...
	private updateExtensionSelector(extensions: IssueReporterExtensionData[]): void {
		...
		if (extensionsSelector) {
			...
			reset(extensionsSelector, this.makeOption('', localize('selectExtension', "选择扩展"), true), ...extensionOptions.map(extension => makeOption(extension, selectedExtension)));
	...
	public setEventHandlers(): void {
			...
			(showInfo as HTMLAnchorElement).addEventListener('click', (e: MouseEvent) => {
				...
				if (label) {
					...
					if (info && info.classList.contains('hidden')) {
						show(info);
						label.textContent = localize('hide', "隐藏");
					} else {
						hide(info);
						label.textContent = localize('show', "显示");
					}
				}
			});
		}
		...
		this.addEventListener('issue-source', 'change', (e: Event) => {
			...
			if (value === IssueSource.VSCode) {
				descriptionTextArea.placeholder = localize('vscodePlaceholder', "例如 Workbench 缺少问题面板");
			} else if (value === IssueSource.Extension) {
				descriptionTextArea.placeholder = localize('extensionPlaceholder', "例如，扩展自述文件图像上缺少可选文字");
			} else if (value === IssueSource.Marketplace) {
				descriptionTextArea.placeholder = localize('marketplacePlaceholder', "例如，无法禁用已安装的扩展");
			} else {
				descriptionTextArea.placeholder = localize('undefinedPlaceholder', "请输入标题。");
			}
	...
	private displaySearchResults(results: SearchResult[]) {
		...
		if (results.length) {
			...
			issuesText.textContent = localize('similarIssues', "类似问题");
			...
			for (let i = 0; i < this.numberOfSearchResultsDisplayed; i++) {
				...
				if (issue.state) {
					...
					issueStateLabel.textContent = issue.state === 'open' ? localize('open', "打开") : localize('closed', "已关闭");

					issueState.title = issue.state === 'open' ? localize('open', "打开") : localize('closed', "已关闭");
	...
	private setUpTypes(): void {
		...
		reset(typeSelect,
			makeOption(IssueType.Bug, localize('bugReporter', "Bug 报告")),
			makeOption(IssueType.FeatureRequest, localize('featureRequest', "功能请求")),
			makeOption(IssueType.PerformanceIssue, localize('performanceIssue', "性能问题(冻结、缓慢、崩溃)"))
		);
	...
	public setSourceOptions(): void {
		...
		sourceSelect.append(this.makeOption('', localize('selectSource', "选择源"), true));
		sourceSelect.append(this.makeOption(IssueSource.VSCode, localize('vscode', "Visual Studio Code"), false));
		sourceSelect.append(this.makeOption(IssueSource.Extension, localize('extension', "VS Code 扩展"), false));
		if (this.product.reportMarketplaceIssueUrl) {
			sourceSelect.append(this.makeOption(IssueSource.Marketplace, localize('marketplace', "扩展市场"), false));
		}

		if (issueType !== IssueType.FeatureRequest) {
			sourceSelect.append(this.makeOption(IssueSource.Unknown, localize('unknown', "不知道"), false));
		}
	...
	public async renderBlocks(): Promise<void> {
		...
		if (extensionData && extensionData.length > MAX_EXTENSION_DATA_LENGTH) {
			...
			const handleLinkClick = async () => {
				const downloadPath = await this.fileDialogService.showSaveDialog({
					title: localize('saveExtensionData', "保存扩展数据"),
		...
		if (selectedExtension && this.nonGitHubIssueUrl) {
			...
			reset(descriptionTitle, localize('handlesIssuesElsewhere', "此扩展处理 VS Code 之外的问题"));
			reset(descriptionSubtitle, localize('elsewhereDescription', "\"{0}\" 扩展更倾向于使用外部问题报告器。要转到该问题报告体验，请点击下面的按钮。", selectedExtension.displayName));
			this.publicGithubButton.label = localize('openIssueReporter', "打开外部问题报告器");
			return;
		}
		...
		if (issueType === IssueType.Bug) {
			...
			reset(descriptionTitle, localize('stepsToReproduce', "重现步骤") + ' ', $('span.required-input', undefined, '*'));
			reset(descriptionSubtitle, localize('bugDescription', "请分享能稳定重现此问题的必要步骤，并包含实际和预期的结果。我们支持 GitHub 版的 Markdown。您将能在 GitHub 上预览时编辑这个问题并添加截图。"));
			...
			reset(descriptionTitle, localize('stepsToReproduce', "重现步骤") + ' ', $('span.required-input', undefined, '*'));
			reset(descriptionSubtitle, localize('performanceIssueDesciption', "这个性能问题是在什么时候发生的? 是在启动时，还是在一系列特定的操作之后? 我们支持 GitHub 版的 Markdown。您将能在 GitHub 上预览时编辑这个问题并添加截图。"));
		} else if (issueType === IssueType.FeatureRequest) {
			reset(descriptionTitle, localize('description', "描述") + ' ', $('span.required-input', undefined, '*'));
			reset(descriptionSubtitle, localize('featureRequestDescription', "请描述您希望能够使用的功能。我们支持 GitHub 版的 Markdown。您将能在 GitHub 上预览时编辑问题并添加截图。"));
		}
	}
	...
	public async writeToClipboard(baseUrl: string, issueBody: string): Promise<string> {
		...
		return baseUrl + `&body=${encodeURIComponent(localize('pasteData', "所需的数据太大，无法直接发送。我们已经将其写入剪贴板，请粘贴。"))}`;
	}
	...
	public updateExtensionTable(extensions: IssueReporterExtensionData[], numThemeExtensions: number): void {
		...
		if (target) {
			if (this.disableExtensions) {
				reset(target, localize('disabledExtensions', "已禁用扩展"));
				return;
			}
