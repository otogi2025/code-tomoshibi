...
export class ScreenshotAnnotationEditor {
	...
	private createUI(): void {
		...
		const drawingTools: { tool: AnnotationTool; label: string; icon: HTMLSpanElement }[] = [
			{ tool: AnnotationTool.Select, label: localize('select', "选择/移动"), icon: renderIcon(Codicon.inspect) },
			{ tool: AnnotationTool.Pan, label: localize('pan', "平移"), icon: renderIcon(Codicon.move) },
		];
		...
		cropBtn.title = localize('crop', "裁剪");
		cropBtn.setAttribute('aria-label', localize('crop', "裁剪"));
		...
		const moreDrawingTools: { tool: AnnotationTool; label: string; icon: HTMLSpanElement }[] = [
			{ tool: AnnotationTool.Freehand, label: localize('freehand', "绘制"), icon: renderIcon(Codicon.edit) },
			{ tool: AnnotationTool.Rectangle, label: localize('rectangle', "矩形"), icon: renderIcon(Codicon.primitiveSquare) },
			{ tool: AnnotationTool.Ellipse, label: localize('ellipse', "椭圆形"), icon: renderIcon(Codicon.circle) },
			{ tool: AnnotationTool.Arrow, label: localize('arrow', "箭头"), icon: renderIcon(Codicon.arrowRight) },
			{ tool: AnnotationTool.Eraser, label: localize('eraser', "橡皮擦"), icon: renderIcon(Codicon.eraser) },
		];
		...
		// 4. Text tool
		this.addToolButton(toolbar, AnnotationTool.Text, localize('text', "文本"), renderIcon(Codicon.symbolString));
		...
		undoBtn.title = localize('undo', "撤消");
		undoBtn.setAttribute('aria-label', localize('undo', "撤消"));
		...
		redoBtn.title = localize('redo', "恢复");
		redoBtn.setAttribute('aria-label', localize('redo', "恢复"));
		...
		discardBtn.label = localize('discard', "放弃");
		...
		saveBtn.label = localize('save', "保存");
		...
		cropCancelBtn.label = localize('cancel', "取消");
		...
		cropApplyBtn.label = localize('apply', "应用");
		...
		hint.textContent = localize('annotationHint', "编辑屏幕截图以突出显示问题");
	...
	private renderToolOptions(): void {
		...
		this.toolOptionsPopover.setAttribute('aria-label', localize('toolOptions', "工具选项"));

		this.appendColorOptions(
			this.toolOptionsPopover,
			this.activeTool === AnnotationTool.Text ? localize('textColor', "文本颜色") : localize('strokeColor', "笔划颜色"),
			COLORS,
			this.activeStrokeColor,
			localize('setStrokeColor', "设置笔划颜色"),
			color => {
		...
		if (this.activeTool !== AnnotationTool.Freehand && this.activeTool !== AnnotationTool.Arrow) {
			this.appendColorOptions(
				this.toolOptionsPopover,
				this.activeTool === AnnotationTool.Text ? localize('textBackgroundColor', "背景色") : localize('fillColor', "填充颜色"),
				FILL_COLORS,
				this.activeFillColor,
				localize('setFillColor', "设置填充颜色"),
				color => {
	...
	private appendColorOptions(container: HTMLElement, label: string, colors: string[], selectedColor: string, ariaLabelPrefix: string, onSelect: (color: string) => void): void {
		...
		for (const color of colors) {
			...
			swatch.setAttribute('aria-label', isTransparent ? localize('transparentColor', "{0}: 透明", ariaLabelPrefix) : localize('colorValue', "{0}: {1}", ariaLabelPrefix, color));
	...
	private appendSizeOptions(container: HTMLElement): void {
		...
		const group = append(container, $('div.annotation-tool-options-group'));
		append(group, $('span.annotation-tool-options-label')).textContent = isText ? localize('textSize', "文本大小") : localize('strokeWidth', "笔划宽度");
		...
		for (const value of values) {
			...
			button.setAttribute('aria-label', isText ? localize('setTextSize', "将文本大小设置为 {0} px", value) : localize('setStrokeWidth', "将笔划宽度设置为 {0} px", value));
	...
	private appendOpacityOptions(container: HTMLElement): void {
		...
		label.textContent = localize('opacity', "不透明度");
		...
		input.setAttribute('aria-label', localize('setOpacity', "设置不透明度"));
	...
	private startTextEdit(pos: { x: number; y: number }, width: number, showBoxOutline: boolean): void {
		...
		editor.setAttribute('aria-label', localize('typeText', "输入文本"));
