...
export function extractIssueData(issueBody: string): ExtractedIssueData | undefined {
	...
	return {
		body: bodyParts.join('\n\n').replace(/\n{3,}/g, '\n\n').trimEnd(),
		fileContent: `# ${localize('issueData', "问题数据")}\n\n${detailsBlocks.join('\n\n')}\n`,
	};
}
...
export class IssueFormService extends Disposable implements IIssueFormService {
	...
	async submitIssue(host: IIssueSubmissionHost, data: IssueReporterData, title: string, body: string): Promise<boolean> {
		...
		if (hasAttachments && data.githubAccessToken && gitHubDetails) {
			...
			try {
				...
				if (filesToProcess.length > 0) {
					...
					mediaMarkdown = `\n\n### ${localize('issueReporter.attachmentsHeading', "附件")}\n\n`;
				...
				mediaMarkdown = `\n\n### ${localize('issueReporter.attachmentsHeading', "附件")}\n\n> ${localize('issueReporter.attachmentsUploadFailed', "上传失败。请手动拖放附件。")}\n\n`;
			} finally {
		...
		if (url.length > MAX_URL_LENGTH) {
			...
			url = this.createIssuePreviewUrl(baseUrl, localize('pasteData', "所需的数据太大，无法直接发送。我们已经将其写入剪贴板，请粘贴。"), gitHubDetails, data.issueSource);
		}
	...
	async showConfirmCloseDialog(): Promise<void> {
		await this.dialogService.prompt({
			...
			message: localize('confirmCloseIssueReporter', "您的输入将不会保存。确实要关闭此窗口吗？"),
			buttons: [
				{
					label: localize({ key: 'yes', comment: ['&& denotes a mnemonic'] }, "是(&&Y)"),
					run: () => {
						this.closeReporter();
						this.issueReporterWindow = null;
					}
	...
	async showClipboardDialog(): Promise<boolean> {
		...
		await this.dialogService.prompt({
			type: Severity.Warning,
			message: localize('issueReporterWriteToClipboard', "数据太多，无法直接发送到 GitHub。数据将被复制到剪贴板，请将其粘贴到打开的 GitHub 问题页。"),
			buttons: [
				{
