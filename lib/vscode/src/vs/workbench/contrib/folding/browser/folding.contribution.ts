...
class DefaultFoldingRangeProvider extends Disposable implements IWorkbenchContribution {
	...
	private async _updateConfigValues(): Promise<void> {
		...
		DefaultFoldingRangeProvider.extensionItemLabels.push(nls.localize('null', '全部'));
		DefaultFoldingRangeProvider.extensionDescriptions.push(nls.localize('nullFormatterDescription', "所有活动折叠范围提供程序"));
...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	...
	properties: {
		[DefaultFoldingRangeProvider.configName]: {
			description: nls.localize('formatter.default', "定义优先于所有其他折叠范围提供程序的默认折叠范围提供程序。必须是提供折叠范围提供程序的扩展的标识符。"),
			type: ['string', 'null'],
			default: null,
