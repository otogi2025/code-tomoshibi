...
export class SpotlightOverlay extends Disposable {
	...
	constructor(
		...
		this._skipButton.label = localize('spotlight.endTour', "结束导览");
		this._skipButton.setTitle(localize('spotlight.endTour.tooltip', "结束导览(Esc)"));
		...
		this._backButton.label = localize('spotlight.back', "返回");
		...
		this._nextButton.label = localize('spotlight.next', "下一步");
	...
	private _renderContent(content: ISpotlightContent): void {
		...
		this._counter.textContent = localize('spotlight.counter', "{0}/{1}", content.stepIndex + 1, content.stepCount);

		this._backButton.element.style.display = content.canGoBack ? '' : 'none';
		this._nextButton.label = content.isLastStep
			? localize('spotlight.done', "完成")
			: localize('spotlight.next', "下一步");
	}
