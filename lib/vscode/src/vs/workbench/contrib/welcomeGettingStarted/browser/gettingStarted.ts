...
export class GettingStartedPage extends EditorPane {
		...
		this.container = $('.gettingStartedContainer',
			{
				role: 'document',
				tabindex: 0,
				'aria-label': localize('welcomeAriaLabel', "关于编辑器快速入门的概述。")
			});
			...
			if (category.id === this.currentWalkthrough?.id) {
				...
				badgeelements.forEach(badgeelement => {
					if (step.done) {
						...
						badgeelement.setAttribute('aria-label', localize('stepDone', "{0}: 已完成", step.title));
					}
					else {
						...
						badgeelement.setAttribute('aria-label', localize('stepNotDone', "{0}: 未完成", step.title));
					}
				});
				if (step.done) {
					status(localize('stepAutoCompleted', "步骤 {0} 已完成", step.title));
				}
			}
	...
	private async openWalkthroughSelector() {
		const selection = await this.quickInputService.pick(this.gettingStartedCategories
			...
			})), { canPickMany: false, matchOnDescription: true, matchOnDetail: true, title: localize('pickWalkthroughs', "打开演练...") });
		if (selection) {
	...
	private provideScreenReaderUpdate(): string {
		if (this.configurationService.getValue(AccessibilityVerbositySettingId.Walkthrough)) {
			...
			return kbLabel ? localize('acessibleViewHint', "在无障碍视图中检查此项({0})。\r\n", kbLabel) : localize('acessibleViewHintNoKbOpen', "通过命令“打开无障碍视图”在无障碍视图中检查此项，该命令当前无法通过键绑定触发。\r\n");
		}
		return '';
	}
	...
	private buildRecentlyOpenedList(): GettingStartedIndexList<RecentEntry> {
		const renderRecent = (recent: RecentEntry) => {
			...
			link.innerText = name;
			link.title = fullPath;
			link.setAttribute('aria-label', localize('welcomePage.openFolderWithPath', "打开路径为 {1} 的文件夹 {0}", name, parentPath));
			...
			const deleteButton = $('a.codicon.codicon-close.hide-category-button.recently-opened-delete-button', {
				'tabindex': 0,
				'role': 'button',
				'title': localize('welcomePage.removeRecent', "从最近打开中删除"),
				'aria-label': localize('welcomePage.removeRecentAriaLabel', "从“最近打开”中移除 {0}", name),
			});
		...
		const recentlyOpenedList = this.recentlyOpenedList.value = new GettingStartedIndexList(
			{
				...
				limit: 5,
				empty: $('.empty-recent', {},
					localize('noRecents', "你没有最近使用的文件夹，"),
					...
					$('button.button-link',
						{
							...
							title: localize('show more recents', "显示所有最近使用的文件夹 {0}", this.getKeybindingLabel(OpenRecentAction.ID))
						}, localize('showAll', "更多..."))),
	...
	private buildGettingStartedWalkthroughsList(): GettingStartedIndexList<IResolvedWalkthrough> {
		...
		const renderGetttingStaredWalkthrough = (category: IResolvedWalkthrough): HTMLElement => {
			...
			if (category.newEntry) {
				reset(newBadge, $('.new-category', {}, localize('new', "新建")));
	...
	private updateCategoryProgress() {
		...
		this.window.document.querySelectorAll('.category-progress').forEach(element => {
			...
			if (stats.stepsTotal === stats.stepsComplete) {
				bar.title = localize('gettingStarted.allStepsComplete', "所有 {0} 个步骤均已完成!", stats.stepsComplete);
			}
			else {
				bar.title = localize('gettingStarted.someStepsComplete', "已完成 {0} 个步骤，共 {1} 个步骤", stats.stepsComplete, stats.stepsTotal);
			}
		});
	}
		...
		for (const linkedText of text) {
			if (linkedText.nodes.length === 1 && typeof linkedText.nodes[0] !== 'string') {
				...
				if (isCommand) {
					...
					if (keybinding) {
						const shortcutMessage = $('span.shortcut-message', {}, localize('gettingStarted.keyboardTip', '提示: 使用键盘快捷方式 '));
	...
	private buildCategorySlide(categoryID: string, selectedStep?: string, preserveFocus?: boolean) {
					...
					const codicon = $('.codicon' + (step.done ? '.complete' + ThemeIcon.asCSSSelector(gettingStartedCheckedCodicon) : ThemeIcon.asCSSSelector(gettingStartedUncheckedCodicon)),
						{
							...
							'aria-label': step.done
								? localize('stepDone', "{0}: 已完成", step.title)
								: localize('stepNotDone', "{0}: 未完成", step.title),
						});
		...
		const stepsContainer = $(
			...
			stepListContainer,
			$('.done-next-container', {},
				$('button.button-link.all-done', { 'x-dispatch': 'allDone' }, $('span.codicon.codicon-check-all'), localize('allDone', "标记为完成")),
				...(showNextCategory
	...
	private buildTelemetryFooter(parent: HTMLElement) {
		...
		const text = localize({ key: 'footer', comment: ['fist substitution is "vs code", second is "privacy statement", third is "opt out".'] },
			"{0} 收集使用情况数据。阅读我们的 {1} 并了解如何 {2}。", this.productService.nameShort, privacyStatementButton, optOutButton);
