		...
		if (shared) {
				...
				try {
					...
					if (!this.agentNetworkFilterService.isUriAllowed(uri)) {
						await this.dialogService.info(
							localize('browserView.shareBlocked.title', "无法与智能体共享"),
			...
			if (!storedChoice) {
				...
				const result = await this.dialogService.confirm({
					type: 'question',
					title: localize('browserView.shareWithAgent.title', '与代理共享?'),
					message: localize('browserView.shareWithAgent.message', '是否与代理共享此浏览器页面?'),
					detail: localize(
						'browserView.shareWithAgent.detail',
						'代理将能够读取和修改浏览器内容及保存的数据(包括 Cookie)。'
					),
					primaryButton: localize('browserView.shareWithAgent.allow', '允许(&&A)'),
					cancelButton: localize('browserView.shareWithAgent.deny', '拒绝'),
					checkbox: { label: localize('browserView.shareWithAgent.dontAskAgain', "不再询问"), checked: false },
				});
