...
class EmptyTextEditorHintContentWidget extends Disposable implements IContentWidget {
		...
		this.contextMenuService.showContextMenu({
			...
			getActions: () => {
				return [{
					...
					label: localize('disableEditorEmptyHint', "禁用空编辑器提示"),
					tooltip: localize('disableEditorEmptyHint', "禁用空编辑器提示"),
					enabled: true,
					class: undefined,
					run: () => {
						disableHint();
	...
	private getHint() {
		...
		}, '[[选择语言]]({0})以开始使用。开始输入以关闭或[[不再显示]]此信息。', keybindingLabels.at(0) ?? '').replaceAll(' ()', '');
		...
		const ariaLabel = localize('defaultHintAriaLabelWithoutInlineChat', '执行 {0} 以选择语言并开始使用。开始输入以关闭。', ...keybindingLabels);
	...
	getDomNode(): HTMLElement {
		if (!this.domNode) {
			...
			this.ariaLabel = ariaLabel.concat(localize('disableHint', ' 在设置中切换 {0} 以禁用此提示。', AccessibilityVerbositySettingId.EmptyEditorHint));
