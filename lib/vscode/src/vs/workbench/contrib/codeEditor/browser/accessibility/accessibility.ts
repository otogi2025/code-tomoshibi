...
class ToggleScreenReaderMode extends Action2 {
	...
	constructor() {
		super({
			...
			title: nls.localize2('toggleScreenReaderMode', "切换屏幕阅读器辅助功能模式"),
			metadata: {
				description: nls.localize2('toggleScreenReaderModeDescription', "切换优化模式以使用屏幕阅读器、盲文设备和其他辅助技术。"),
			},
			f1: true,
			keybinding: [{
...
class AnnounceCursorPosition extends Action2 {
	constructor() {
		super({
			...
			title: nls.localize2('announceCursorPosition', "播报光标位置"),
			f1: true,
			metadata: {
				description: nls.localize2('announceCursorPosition.description', "通过屏幕阅读器播报当前光标位置（行和列）。")
			},
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		alert(nls.localize('screenReader.lineColPosition', "行 {0}, 列 {1}", position.lineNumber, visibleColumn));
	}
}
