	...
	private installNavigablePicks(picker: IQuickPick<IGotoSymbolQuickPickItem, { useSeparators: true }>, disposables: DisposableStore, entries: readonly INavigablePickEntry[]): void {
		...
		const updatePickerItems = () => {
			...
			if (filteredItems.length === 0) {
				const label = localize('empty', '无匹配项');
...
class GotoSymbolAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: {
				...localize2('gotoSymbol', "转到编辑器中的符号..."),
				mnemonicTitle: localize({ key: 'miGotoSymbolInEditor', comment: ['&& denotes a mnemonic'] }, "转到编辑器中的符号(&&S)…"),
			},
			f1: true,
			keybinding: {
...
Registry.as<IQuickAccessRegistry>(QuickaccessExtensions.Quickaccess).registerQuickAccessProvider({
	...
	placeholder: localize('gotoSymbolQuickAccessPlaceholder', "键入要转到的符号的名称。"),
	helpEntries: [
		{
			description: localize('gotoSymbolQuickAccess', "转到编辑器中的符号"),
			prefix: AbstractGotoSymbolQuickAccessProvider.PREFIX,
			commandId: GotoSymbolAction.ID,
			commandCenterOrder: 40
		},
		{
			description: localize('gotoSymbolByCategoryQuickAccess', "按类别转到编辑器中的符号"),
