...
export abstract class SimpleFindWidget extends Widget implements IVerticalSashLayoutProvider {
	...
	private _updateFindInputAriaLabel(): void {
		...
		if (!this._accessibilityHelpHintAnnounced && this._configurationService.getValue('accessibility.verbosity.find') && this._accessibilityService.isScreenReaderOptimized()) {
			...
			if (keybinding) {
				findLabel += ', ' + nls.localize('accessibilityHelpHintInLabel', "按 {0} 可获取辅助功能帮助", keybinding);
	...
	private _announceSearchResults(label: string, searchString?: string): string {
		if (!searchString) {
			return nls.localize('ariaSearchNoInput', "输入搜索输入");
		}
		if (label === NLS_NO_RESULTS) {
			return searchString === ''
				? nls.localize('ariaSearchNoResultEmpty', "已找到 {0}", label)
				: nls.localize('ariaSearchNoResult', "为“{1}”找到 {0}", label, searchString);
		}

		return nls.localize('ariaSearchNoResultWithLineNumNoCurrentMatch', "为“{1}”找到 {0}", label, searchString);
	}
}

export const simpleFindWidgetSashBorder = registerColor('simpleFindWidget.sashBorder', { dark: '#454545', light: '#C8C8C8', hcDark: '#6FC3DF', hcLight: '#0F4A85' }, nls.localize('simpleFindWidget.sashBorder', '窗体边框颜色。'));
