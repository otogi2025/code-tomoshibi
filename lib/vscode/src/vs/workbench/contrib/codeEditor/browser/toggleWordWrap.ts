...
const EDITOR_WORD_WRAP = new RawContextKey<boolean>('editorWordWrap', false, nls.localize('editorWordWrap', '编辑器当前是否正在使用自动换行。'));
...
class ToggleWordWrapAction extends EditorAction {
	...
	constructor() {
		super({
			...
			label: nls.localize2('toggle.wordwrap', "查看: 切换自动换行"),
...
MenuRegistry.appendMenuItem(MenuId.EditorTitle, {
	command: {
		id: TOGGLE_WORD_WRAP_ID,
		title: nls.localize('unwrapMinified', "在此文件禁用折行"),
...
MenuRegistry.appendMenuItem(MenuId.EditorTitle, {
	command: {
		id: TOGGLE_WORD_WRAP_ID,
		title: nls.localize('wrapMinified', "在此文件启用折行"),
		icon: Codicon.wordWrap
	},
	group: 'navigation',
	order: 1,
