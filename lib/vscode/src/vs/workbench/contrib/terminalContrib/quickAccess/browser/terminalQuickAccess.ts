...
export class TerminalQuickAccessProvider extends PickerQuickAccessProvider<IPickerQuickAccessItem> {
	...
	protected _getPicks(filter: string): Array<IPickerQuickAccessItem | IQuickPickSeparator> {
		...
		const createTerminalLabel = localize("workbench.action.terminal.newplus", "创建新的终端");
		terminalPicks.push({
			label: `$(plus) ${createTerminalLabel}`,
			ariaLabel: createTerminalLabel,
			accept: () => this._commandService.executeCommand(TerminalCommandId.New)
		});
		const createWithProfileLabel = localize("workbench.action.terminal.newWithProfilePlus", "创建具有配置文件的新终端...");
		terminalPicks.push({
				...
				buttons: [
					{
						...
						tooltip: localize('renameTerminal', "重命名终端")
					},
					{
