	...
	if (instance) {
		...
		if (!text) {
			text = await quickInputService.input({
				...
				prompt: localize('workbench.action.terminal.sendSequence.prompt', "输入要发送到终端的序列"),
			});
			if (!text) {
				return;
			}
...
const sendSequenceString = localize2('sendSequence', "发送序列");
	...
	metadata: {
		...
		args: [{
			...
			schema: {
				...
				properties: {
					text: {
						description: localize('sendSequence.text.desc', "要发送到终端的文本序列"),
						type: 'string'
					}
				},
			}
