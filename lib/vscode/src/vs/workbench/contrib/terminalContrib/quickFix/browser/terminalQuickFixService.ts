...
const quickFixExtensionPoint = ExtensionsRegistry.registerExtensionPoint<ITerminalCommandSelector[]>({
	...
	jsonSchema: {
		description: localize('vscode.extension.contributes.terminalQuickFixes', '提供终端快速修复。'),
		type: 'array',
		items: {
			type: 'object',
			...
			properties: {
				id: {
					description: localize('vscode.extension.contributes.terminalQuickFixes.id', "快速修复提供程序的 ID"),
					type: 'string',
				},
				commandLineMatcher: {
					description: localize('vscode.extension.contributes.terminalQuickFixes.commandLineMatcher', "用于测试命令行的正则表达式或字符串"),
					type: 'string',
				},
				outputMatcher: {
					markdownDescription: localize('vscode.extension.contributes.terminalQuickFixes.outputMatcher', "与输出的单行匹配的正则表达式或字符串，它提供要在 terminalCommand 和 URI 中引用的组。\r\n\r\n例如:\r\n\r\n `lineMatcher: /git push --set-upstream origin (?<branchName>[^s]+)/;`\r\n\r\n`terminalCommand: 'git push --set-upstream origin ${group:branchName}';`\r\n"),
					type: 'object',
				...
				commandExitResult: {
					description: localize('vscode.extension.contributes.terminalQuickFixes.commandExitResult', "要匹配的命令退出结果"),
					enum: ['success', 'error'],
					enumDescriptions: [
				...
				kind: {
					description: localize('vscode.extension.contributes.terminalQuickFixes.kind', "生成的快速修复的类型。这将更改快速修复的显示方式。默认为 {0}。", '`"fix"`'),
