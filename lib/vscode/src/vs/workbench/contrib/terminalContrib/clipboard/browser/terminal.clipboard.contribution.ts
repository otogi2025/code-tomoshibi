...
export class TerminalClipboardContribution extends Disposable implements ITerminalContribution {
	...
	async copySelection(asHtml?: boolean, command?: ITerminalCommand): Promise<void> {
		await this._xterm?.copySelection(asHtml, command);
	}
...
registerActiveInstanceAction({
	...
	title: localize2('workbench.action.terminal.copyLastCommand', "复制最后一个命令"),
...
registerActiveInstanceAction({
	...
	title: localize2('workbench.action.terminal.copyLastCommandOutput', "复制最后一个命令输出"),
	precondition: terminalAvailableWhenClause,
	run: async (instance, c, accessor) => {
		const clipboardService = accessor.get(IClipboardService);
		const commands = instance.capabilities.get(TerminalCapability.CommandDetection)?.commands;
...
registerActiveInstanceAction({
	...
	title: localize2('workbench.action.terminal.copyLastCommandAndOutput', "复制最后一个命令和输出"),
	precondition: terminalAvailableWhenClause,
	run: async (instance, c, accessor) => {
		const clipboardService = accessor.get(IClipboardService);
		const commands = instance.capabilities.get(TerminalCapability.CommandDetection)?.commands;
...
if (BrowserFeatures.clipboard.writeText) {
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.copySelection', '复制所选内容'),
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.copyAndClearSelection', '复制和清除选择'),
		precondition: ContextKeyExpr.or(TerminalContextKeys.textSelectedInFocused, ContextKeyExpr.and(terminalAvailableWhenClause, TerminalContextKeys.textSelected)),
		keybinding: [{
	...
	registerActiveXtermAction({
		...
		title: localize2('workbench.action.terminal.copySelectionAsHtml', '将所选内容复制为 HTML'),
		f1: true,
...
if (BrowserFeatures.clipboard.readText) {
	registerActiveInstanceAction({
		...
		title: localize2('workbench.action.terminal.paste', '粘贴到活动终端中'),
		precondition: terminalAvailableWhenClause,
		keybinding: [{
...
if (BrowserFeatures.clipboard.readText && isLinux) {
	registerActiveInstanceAction({
		...
		title: localize2('workbench.action.terminal.pasteSelection', '将所选内容粘贴到活动终端'),
		precondition: terminalAvailableWhenClause,
		keybinding: [{
