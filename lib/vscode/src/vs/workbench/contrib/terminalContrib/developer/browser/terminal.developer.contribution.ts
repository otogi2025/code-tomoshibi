...
registerTerminalAction({
	...
	title: localize2('workbench.action.terminal.showTextureAtlas', '显示终端纹理图集'),
...
registerTerminalAction({
	...
	title: localize2('workbench.action.terminal.writeDataToTerminal', '将数据写入终端'),
	category: Categories.Developer,
	run: async (c, accessor) => {
		...
		const data = await quickInputService.input({
			...
			prompt: localize('workbench.action.terminal.writeDataToTerminal.prompt', "输入数据以直接写入终端，从而绕过 pty"),
		});
		if (!data) {
			return;
		}
...
registerTerminalAction({
	...
	title: localize2('workbench.action.terminal.recordSession', '记录终端会话'),
	category: Categories.Developer,
	run: async (c, accessor) => {
		...
		const text = localize('workbench.action.terminal.recordSession.recording', "正在记录终端会话...");
...
registerTerminalAction({
	...
	title: localize2('workbench.action.terminal.restartPtyHost', '重启 pty 主机'),
	category: Categories.Developer,
	run: async (c, accessor) => {
