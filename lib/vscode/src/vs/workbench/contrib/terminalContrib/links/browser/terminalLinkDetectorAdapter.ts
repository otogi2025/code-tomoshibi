...
export class TerminalLinkDetectorAdapter extends Disposable implements ILinkProvider {
	...
	private _getLabel(type: TerminalLinkType): string {
		switch (type) {
			case TerminalBuiltinLinkType.Search: return localize('searchWorkspace', '搜索工作区');
			case TerminalBuiltinLinkType.LocalFile: return localize('openFile', '在编辑器中打开文件');
			case TerminalBuiltinLinkType.LocalFolderInWorkspace: return localize('focusFolder', '聚焦资源管理器中的文件夹');
			case TerminalBuiltinLinkType.LocalFolderOutsideWorkspace: return localize('openFolder', '在新窗口中打开文件夹');
			case TerminalBuiltinLinkType.Url:
			default:
				return localize('followLink', '打开链接');
		}
	}
}
