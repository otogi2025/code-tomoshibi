...
export class TerminalLinkManager extends DisposableStore {
			...
			activate: async (event, text) => {
				...
				if (terminalConfigurationService.config.allowedLinkSchemes.indexOf(scheme) === -1) {
					const userAllowed = await new Promise<boolean>((resolve) => {
						notificationService.prompt(Severity.Warning, nls.localize('scheme', '打开 URI 可能不安全，是否要允许使用方案 {0} 打开链接?', scheme), [
							{
								label: nls.localize('allow', '允许 {0}', scheme),
								run: () => {
	...
	private _getLinkHoverString(uri: string, label: string | undefined): IMarkdownString {
		...
		if (editorConf.multiCursorModifier === 'ctrlCmd') {
			if (isMacintosh) {
				clickLabel = nls.localize('terminalLinkHandler.followLinkAlt.mac', "Option + 单击");
			} else {
				clickLabel = nls.localize('terminalLinkHandler.followLinkAlt', "Alt + 单击");
			}
		} else {
			if (isMacintosh) {
				clickLabel = nls.localize('terminalLinkHandler.followLinkCmd', "Cmd + 单击");
			} else {
				clickLabel = nls.localize('terminalLinkHandler.followLinkCtrl', "Ctrl + 单击");
			}
		}

		let fallbackLabel = nls.localize('followLink', "跟踪链接");
		try {
			if (this._tunnelService.canTunnel(URI.parse(uri))) {
				fallbackLabel = nls.localize('followForwardedLink', "执行使用转发端口的链接");
			}
		} catch {
		...
		if (/(\s|&nbsp;)/.test(uri)) {
			uri = nls.localize('followLinkUrl', '链接');
		}
