...
export class ExternalUriOpenerService extends Disposable implements IExternalUriOpenerService, IExternalOpener {
	...
	private async showOpenerPrompt(
		...
		items.push(
			{
				label: isWeb
					? nls.localize('selectOpenerDefaultLabel.web', '在新浏览器窗口中打开')
					: nls.localize('selectOpenerDefaultLabel', '在默认浏览器中打开'),
				opener: undefined
			},
			{ type: 'separator' },
			{
				label: nls.localize('selectOpenerConfigureTitle', "配置默认开启程序…"),
				opener: 'configureDefault'
			});

		const picked = await this.quickInputService.pick(items, {
			placeHolder: nls.localize('selectOpenerPlaceHolder', "你希望以何种方式打开: {0}", targetUri.toString())
		});
