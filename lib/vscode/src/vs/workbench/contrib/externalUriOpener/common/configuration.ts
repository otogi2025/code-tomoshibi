...
export const externalUriOpenersConfigurationNode: IConfigurationNode = {
	...
	properties: {
		[externalUriOpenersSettingId]: {
			type: 'object',
			markdownDescription: nls.localize('externalUriOpeners', "配置开启程序以用于外部 URI (即 http、https)。"),
			defaultSnippets: [{
				body: {
					'example.com': '$1'
				}
			...
			additionalProperties: {
				anyOf: [
					{
						type: 'string',
						markdownDescription: nls.localize('externalUriOpeners.uri', "将 URI 模式映射到开启程序 ID。\r\n示例模式: \r\n{0}", exampleUriPatterns),
					},
					{
						type: 'string',
						markdownDescription: nls.localize('externalUriOpeners.uri', "将 URI 模式映射到开启程序 ID。\r\n示例模式: \r\n{0}", exampleUriPatterns),
						enum: [defaultExternalUriOpenerId],
						enumDescriptions: [nls.localize('externalUriOpeners.defaultId', "使用 VS Code 的标准打开器打开。")],
					},
