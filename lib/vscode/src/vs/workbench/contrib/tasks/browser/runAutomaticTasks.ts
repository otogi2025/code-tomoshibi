	...
	private _showPrompt(notificationService: INotificationService, storageService: IStorageService, openerService: IOpenerService, configurationService: IConfigurationService, taskNames: string[], locations: Map<string, URI>): Promise<boolean> {
		return new Promise<boolean>(resolve => {
			notificationService.prompt(Severity.Info, nls.localize('tasks.run.allowAutomatic',
				"此工作区定义了任务({0}) ({1})，这些任务可以在你打开此工作区时自动启动进程。是否允许自动任务在所有受信任的工作区中运行?",
				taskNames.join(', '),
				Array.from(locations.keys()).join(', ')
			),
				[{
					label: nls.localize('allow', "允许"),
					run: () => {
						resolve(true);
				...
				{
					label: nls.localize('disallow', "禁止"),
					run: () => {
						resolve(false);
				...
				{
					label: locations.size === 1 ? nls.localize('openTask', "打开文件") : nls.localize('openTasks', "打开文件"),
					run: async () => {
...
export class ManageAutomaticTaskRunning extends Action2 {
	...
	public static readonly LABEL = nls.localize('workbench.action.tasks.manageAutomaticRunning', "管理自动任务");
	...
	public async run(accessor: ServicesAccessor): Promise<void> {
		...
		const allowItem: IQuickPickItem = { label: nls.localize('workbench.action.tasks.allowAutomaticTasks', "允许自动任务") };
		const disallowItem: IQuickPickItem = { label: nls.localize('workbench.action.tasks.disallowAutomaticTasks', "禁止自动任务") };
