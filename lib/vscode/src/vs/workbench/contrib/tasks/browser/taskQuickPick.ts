...
const SHOW_ALL: string = nls.localize('taskQuickPick.showAll', "显示所有任务...");

export const configureTaskIcon = registerIcon('tasks-list-configure', Codicon.gear, nls.localize('configureTaskIcon', '任务选择列表中的“配置”图标。'));
const removeTaskIcon = registerIcon('tasks-remove', Codicon.close, nls.localize('removeTaskIcon', '任务选择列表中的“删除”图标。'));
...
export class TaskQuickPick extends Disposable {
	...
	private _createTaskEntry(task: Task | ConfiguringTask, extraButtons: IQuickInputButton[] = []): ITaskTwoLevelQuickPickEntry {
		const buttons: IQuickInputButton[] = [
			{ iconClass: ThemeIcon.asClassName(configureTaskIcon), tooltip: nls.localize('configureTask', "配置任务") },
			...extraButtons
		];
	...
	private _createTypeEntries(entries: QuickPickInput<ITaskTwoLevelQuickPickEntry>[], types: string[]) {
		entries.push({ type: 'separator', label: nls.localize('contributedTasks', "已提供") });
		types.forEach(type => {
			entries.push({ label: `$(folder) ${type}`, task: type, ariaLabel: nls.localize('taskType', "全部 {0} 个任务", type) });
		});
	...
	public async getTopLevelEntries(defaultEntry?: ITaskQuickPickEntry): Promise<{ entries: QuickPickInput<ITaskTwoLevelQuickPickEntry>[]; isSingleConfigured?: Task | ConfiguringTask }> {
		...
		if (recentTasks.length > 0) {
			const removeRecentButton: IQuickInputButton = {
				...
				tooltip: nls.localize('removeRecent', '删除最近使用的任务')
			};
			this._createEntriesForGroup(this._topLevelEntries, recentTasks, nls.localize('recentlyUsed', '最近使用过'), [removeRecentButton]);
		}
		if (configuredTasks.length > 0) {
			if (dedupedConfiguredTasks.length > 0) {
				this._createEntriesForGroup(this._topLevelEntries, dedupedConfiguredTasks, nls.localize('configured', '已配置'));
			}
		}

		if (defaultEntry && (configuredTasks.length === 0)) {
			this._topLevelEntries.push({ type: 'separator', label: nls.localize('configured', '已配置') });
	...
	public async handleSettingOption(selectedType: string) {
		const { confirmed } = await this._dialogService.confirm({
			...
			message: nls.localize('TaskQuickPick.changeSettingDetails',
				"{0} 任务的任务检测会导致打开的任何工作区中的文件作为代码运行。启用 {0} 任务检测是用户设置，并将应用于打开的任何工作区。\r\n\r\n是否要为所有工作区启用 {0} 任务检测?", selectedType),
			cancelButton: nls.localize('TaskQuickPick.changeSettingNo', "否")
		});
		if (confirmed) {
			await this._configurationService.updateValue(`${selectedType}.autoDetect`, 'on');
			await new Promise<void>(resolve => setTimeout(() => resolve(), 100));
			return this.show(nls.localize('TaskService.pickRunTask', '选择要运行的任务'), undefined, selectedType);
		}
		return undefined;
	}
	...
	public static getSettingEntry(configurationService: IConfigurationService, type: string): (ITaskTwoLevelQuickPickEntry & { settingType: string }) | undefined {
		if (configurationService.getValue(`${type}.autoDetect`) === 'off') {
			return {
				label: '$(gear) ' + nls.localize('TaskQuickPick.changeSettingsOptions', "{0} 任务检测已关闭。启用 {1} 任务检测...",
	...
	private async _getEntriesForProvider(type: string): Promise<QuickPickInput<ITaskTwoLevelQuickPickEntry>[]> {
		...
		if (tasks.length > 0) {
			...
			taskQuickPickEntries.push({
				...
				label: nls.localize('TaskQuickPick.goBack', '返回'),
				task: null,
				alwaysShow: true
			});
		} else {
			taskQuickPickEntries = [{
				label: nls.localize('TaskQuickPick.noTasksForType', '未找到任务 {0}。返回↩', type),
				task: null,
				alwaysShow: true
			}];
		}
	...
	private async _toTask(task: Task | ConfiguringTask): Promise<Task | undefined> {
		...
		if (!resolvedTask) {
			this._notificationService.error(nls.localize('noProviderForTask', "没有为“{0}”类型的任务注册任务提供程序。", task.type));
		}
		return resolvedTask;
	}
}
