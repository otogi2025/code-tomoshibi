...
export namespace ConfigureTaskAction {
	...
	export const TEXT = nls.localize2('ConfigureTaskRunnerAction.label', "配置任务");
}
...
export abstract class AbstractTaskService extends Disposable implements ITaskService {
	...
	public static OutputChannelLabel: string = nls.localize('tasks', "任务");
	...
	constructor(
		...
		this._configurationResolverService.contributeVariable('defaultBuildTask', async (): Promise<string | undefined> => {
			...
			if (tasks && tasks.length > 0) {
				entry = await this._showQuickPick(tasks, nls.localize('TaskService.pickBuildTaskForLabel', '选择生成任务(未定义默认生成任务)'));
			}
		...
		this._register(this.onDidStateChange(async e => {
			this._log(nls.localize('taskEvent', '任务事件类型: {0}', e.kind), true);
			switch (e.kind) {
	...
	private _attemptTaskReconnection(): void {
		if (this._lifecycleService.startupKind !== StartupKind.ReloadedWindow) {
			this._log(nls.localize('TaskService.skippingReconnection', '启动类型而不是窗口重载，设置连接的任务和删除持久性任务'), true);
			this._tasksReconnected = true;
			this._storageService.remove(AbstractTaskService.PersistentTasks_Key, StorageScope.WORKSPACE);
		}
		if (!this._configurationService.getValue(TaskSettingId.Reconnection) || this._tasksReconnected) {
			this._log(nls.localize('TaskService.notConnecting', '通过设置已连接任务的配置值 {0}，任务已重新连接 {1}', this._configurationService.getValue(TaskSettingId.Reconnection), this._tasksReconnected), true);
			this._tasksReconnected = true;
			return;
		}
		this._log(nls.localize('TaskService.reconnecting', '正在重新连接到正在运行的任务...'), true);
		this.getWorkspaceTasks(TaskRunSource.Reconnect).then(async () => {
			this._tasksReconnected = await this._reconnectTasks();
			this._log(nls.localize('TaskService.reconnected', '已重新连接到正在运行的任务。'), true);
			this._onDidReconnectToTasks.fire();
		});
	}
	...
	private async _handleLongRunningTaskCompletion(event: ITaskProcessEndedEvent | ITaskInactiveEvent, durationMs: number): Promise<void> {
		...
		const message = taskLabel
			? nls.localize('task.longRunningTaskCompletedWithLabel', '任务 "{0}" 已在 {1} 中完成。', taskLabel, durationText)
			: nls.localize('task.longRunningTaskCompleted', '任务已在 {0} 中完成。', durationText);
	...
	private _formatTaskDuration(durationMs: number): string {
		...
		if (minutes > 0) {
			return seconds > 0
				? nls.localize('task.longRunningTaskDurationMinutesSeconds', '{0} 分钟 {1} 秒', minutes, seconds)
				: nls.localize('task.longRunningTaskDurationMinutes', '{0} 分钟', minutes);
		}
		return nls.localize('task.longRunningTaskDurationSeconds', '{0} 秒', seconds);
	}

	private async _reconnectTasks(): Promise<boolean> {
		const tasks = await this.getSavedTasks('persistent');
		if (!tasks.length) {
			this._log(nls.localize('TaskService.noTasks', '没有要重新连接的持久性任务。'), true);
			return true;
		}
		const taskLabels = tasks.map(task => task._label).join(', ');
		this._log(nls.localize('TaskService.reconnectingTasks', '正在重新连接到 {0} 任务...', taskLabels), true);
		...
		CommandsRegistry.registerCommand({
			...
			metadata: {
				...
				args: [{
					name: 'args',
					isOptional: true,
					description: nls.localize('runTask.arg', "筛选快速入门中显示的任务"),
					schema: {
						anyOf: [
							{
								type: 'string',
								description: nls.localize('runTask.label', "任务的标签或要作为筛选依据的术语")
							},
							{
								type: 'object',
								properties: {
									type: {
										type: 'string',
										description: nls.localize('runTask.type', "参与的任务类型")
									},
									task: {
										type: 'string',
										description: nls.localize('runTask.task', "任务的标签或要作为筛选依据的术语")
									}
								}
							}
						]
	...
	protected _showOutput(runSource: TaskRunSource = TaskRunSource.User, userRequested?: boolean, _errorMessage?: string): void {
		if (!VirtualWorkspaceContext.getValue(this._contextKeyService) && ((runSource === TaskRunSource.User) || (runSource === TaskRunSource.ConfigurationChange))) {
			if (userRequested) {
				...
				const actions = [{
					label: nls.localize('showOutput', "显示输出"),
					run: () => {
						this._outputService.showChannel(this._outputChannel.id, true);
					}
				}];
				this._notificationService.prompt(Severity.Warning, nls.localize('taskServiceOutputPrompt', '任务出现错误。请查看输出结果，了解更多详细信息'), actions);
			}
		}
	}
	...
	public async getTask(folder: IWorkspace | IWorkspaceFolder | string, identifier: string | ITaskIdentifier, compareId: boolean = false, type: string | undefined = undefined): Promise<Task | undefined> {
		...
		if (this.ignoredWorkspaceFolders.some(ignored => ignored.name === name)) {
			return Promise.reject(new Error(nls.localize('TaskServer.folderIgnored', '由于使用任务版本 0.1.0，文件夹 {0} 将被忽略', name)));
		}
	...
	private _getPersistentTasks(): LRUCache<string, string> {
		if (this._persistentTasks) {
			this._log(nls.localize('taskService.gettingCachedTasks', '正在返回缓存的任务 {0}', this._persistentTasks.size), true);
	...
	public async getSavedTasks(type: 'persistent' | 'historical'): Promise<(Task | ConfiguringTask)[]> {
		...
		this._log(nls.localize('taskService.getSavedTasks', '正在从任务存储中提取任务。'), true);
		...
		for (const entry of storedTasks.entries()) {
			try {
				...
				this._log(nls.localize('taskService.getSavedTasks.reading', '正在从任务存储中读取任务，{0}，{1}，{2}', key, task, folderInfo.folder), true);
				addTaskToMap(folderInfo.isWorkspaceFile ? workspaceToTaskMap : folderToTasksMap, folderInfo.folder, task);
			} catch (error) {
				this._log(nls.localize('taskService.getSavedTasks.error', '从任务存储提取任务失败：{0}。', error), true);
			}
		}
		...
		for (const key of storedTasks.keys()) {
			if (readTasksMap.has(key)) {
				...
				this._log(nls.localize('taskService.getSavedTasks.resolved', '已解决任务 {0}', key), true);
			} else {
				this._log(nls.localize('taskService.getSavedTasks.unresolved', '无法解析任务 {0} ', key), true);
			}
		}
		return tasks;
	}
	...
	public removePersistentTask(key: string) {
		this._log(nls.localize('taskService.removePersistentTask', '正在删除持久性任务 {0}', key), true);
	...
	private async _setPersistentTask(task: Task): Promise<void> {
		...
		if (!InMemoryTask.is(task) && key) {
			...
			this._log(nls.localize('taskService.setPersistentTask', '正在设置持久性任务 {0}', key), true);
	...
	private _savePersistentTasks(): void {
		...
		this._log(nls.localize('savePersistentTask', '正在保存持久性任务: {0}', keys.join(', ')), true);
		...
		if (!runnable || !runnable.task) {
			if (test) {
				if (this.schemaVersion === JsonSchemaVersion.V0_1_0) {
					throw new TaskError(Severity.Info, nls.localize('TaskService.noTestTask1', '未定义任何测试任务。使用 "isTestCommand" 在 tasks.json 文件中标记任务。'), TaskErrors.NoTestTask);
				} else {
					throw new TaskError(Severity.Info, nls.localize('TaskService.noTestTask2', '未定义任何测试任务。在 tasks.json 文件中将任务标记为 "test" 组。'), TaskErrors.NoTestTask);
				}
			} else {
				if (this.schemaVersion === JsonSchemaVersion.V0_1_0) {
					throw new TaskError(Severity.Info, nls.localize('TaskService.noBuildTask1', '未定义任何生成任务。使用 "isBuildCommand" 在 tasks.json 文件中标记任务。'), TaskErrors.NoBuildTask);
				} else {
					throw new TaskError(Severity.Info, nls.localize('TaskService.noBuildTask2', '未定义任何生成任务。在 tasks.json 文件中将任务标记为 "build" 组。'), TaskErrors.NoBuildTask);
				}
			}
		}
	...
	public async run(task: Task | undefined, options?: IProblemMatcherRunOptions, runSource: TaskRunSource = TaskRunSource.System): Promise<ITaskSummary | undefined> {
		...
		if (!task) {
			throw new TaskError(Severity.Info, nls.localize('TaskServer.noTask', '未定义要执行的任务'), TaskErrors.TaskNotFound);
		}
	...
	private async _attachProblemMatcher(task: ContributedTask | CustomTask): Promise<Task | undefined> {
		...
		entries.unshift({ type: 'separator', label: nls.localize('TaskService.associate', '关联') });
		let taskType: string;
		if (CustomTask.is(task)) {
			const configProperties = task._source.config.element as TaskConfig.ICustomTask;
		...
		entries.unshift(
			{ label: nls.localize('TaskService.attachProblemMatcher.continueWithout', '继续而不扫描任务输出'), matcher: undefined },
			{ label: nls.localize('TaskService.attachProblemMatcher.never', '从不扫描此任务的任务输出'), matcher: undefined, never: true },
			{ label: nls.localize('TaskService.attachProblemMatcher.neverType', '从不扫描 {0} 个任务的任务输出', taskType), matcher: undefined, setting: taskType },
			{ label: nls.localize('TaskService.attachProblemMatcher.learnMoreAbout', '了解有关扫描任务输出的详细信息'), matcher: undefined, learnMore: true }
		);
		const problemMatcher = await this._quickInputService.pick(entries, { placeHolder: nls.localize('selectProblemMatcher', '选择针对何种错误和警告扫描任务输出') });
		if (!problemMatcher) {
			return task;
		}
	...
	public async customize(task: ContributedTask | CustomTask | ConfiguringTask, properties?: ICustomizationProperties, openConfig?: boolean): Promise<void> {
		...
		if (configuration.hasParseErrors) {
			this._notificationService.warn(nls.localize('customizeParseErrors', '当前任务配置存在错误。请先更正错误，再自定义任务。'));
		...
		if (!fileConfig) {
			...
			let content = [
				'{',
				nls.localize('tasksJsonComment', '\t// 请参阅 https://go.microsoft.com/fwlink/?LinkId=733558 \r\n\t//查看有关 tasks.json 格式的文档'),
	...
	private _createRunnableTask(tasks: TaskMap, group: TaskGroup): { task: Task; resolver: ITaskResolver } | undefined {
		...
		if (workspaceTasks.length > 0) {
			if (workspaceTasks.length > 1) {
				this._log(nls.localize('moreThanOneBuildTask', 'tasks.json 中定义了很多生成任务。正在执行第一个任务。'));
			}
	...
	private async _saveBeforeRun(): Promise<boolean> {
		...
		if (saveBeforeRunTaskConfig === SaveBeforeRunConfigOptions.Never) {
			...
			const { confirmed } = await this._dialogService.confirm({
				message: nls.localize('TaskSystem.saveBeforeRun.prompt.title', "是否保存所有编辑器?"),
				detail: nls.localize('detail', "是否要在运行任务前保存所有编辑器?"),
				primaryButton: nls.localize({ key: 'saveBeforeRun.save', comment: ['&& denotes a mnemonic'] }, '保存(&&S)'),
				cancelButton: nls.localize({ key: 'saveBeforeRun.dontSave', comment: ['&& denotes a mnemonic'] }, "不保存(&&N)"),
			});
	...
	private async _handleExecuteResult(executeResult: ITaskExecuteResult, runSource?: TaskRunSource): Promise<ITaskSummary> {
		...
		if (executeResult.kind === TaskExecuteKind.Active) {
			...
			if (active && active.same) {
				...
				throw new TaskError(Severity.Warning, nls.localize('TaskSystem.active', '当前已有任务正在运行。请先终止它，然后再执行另一项任务。'), TaskErrors.RunningTask);
			}
		}
	...
	private _handleInstancePolicy(task: Task, policy?: InstancePolicy): void {
		...
		switch (policy) {
			...
			case InstancePolicy.warn:
				this._notificationService.warn(nls.localize('TaskSystem.InstancePolicy.warn', '已达到此任务的实例限制。'));
				break;
			...
			default: {
				...
				this._showQuickPick(this._taskSystem!.getActiveTasks().filter(t => task._id === t._id),
					nls.localize('TaskService.instanceToTerminate', '选择要终止的实例'),
					{
						label: nls.localize('TaskService.noInstanceRunning', '当前没有运行任何实例'),
						task: undefined
					},
					false, true,
					undefined
	...
	private async _restart(task: Task): Promise<void> {
		...
		if (isTaskRunning) {
			...
			if (!response.success) {
				this._notificationService.warn(nls.localize('TaskSystem.restartFailed', '未能终止并重启任务 {0}', Types.isString(task) ? task : task.configurationProperties.name));
				return;
			}
		}
		...
		try {
			...
			if (updatedTask) {
				...
				if (!success || (typeof success.exitCode === 'number' && success.exitCode !== 0)) {
					...
					this._notificationService.warn(nls.localize('TaskSystem.taskNoLongerExists', '任务 {0} 不再存在或已被修改。无法重启。', task.configurationProperties.name));
				}
			}
		} catch {
						...
						raceTimeout(provider.provideTasks(validTypes).then((taskSet: ITaskSet) => {
							...
							for (const task of taskSet.tasks) {
								if (task.type !== this._providerTypes.get(handle)) {
									this._log(nls.localize('unexpectedTaskType', "“{0}”任务的任务提供程序意外提供了“{1}”类型的任务。\r\n", this._providerTypes.get(handle), task.type));
	...
	private async _computeWorkspaceFolderTasks(workspaceFolder: IWorkspaceFolder, runSource: TaskRunSource = TaskRunSource.User): Promise<IWorkspaceFolderTaskResult> {
		...
		if (problemReporter.status.isFatal()) {
			problemReporter.fatal(nls.localize('TaskSystem.configurationErrors', '错误: 提供的任务配置具有验证错误，无法使用。请首先改正错误。'));
	...
	private _testParseExternalConfig(config: TaskConfig.IExternalTaskRunnerConfiguration | undefined, location: string): { config: TaskConfig.IExternalTaskRunnerConfiguration | undefined; hasParseErrors: boolean } {
		...
		if (parseErrors) {
			...
			if (isAffected) {
				this._log(nls.localize({ key: 'TaskSystem.invalidTaskJsonOther', comment: ['Message notifies of an error in one of several places there is tasks related json, not necessarily in a file named tasks.json'] }, '错误: {0} 中 tasks json 的内容存在语法错误。请先纠正它们，然后再执行任务。', location));
				this._showOutput(undefined, undefined, nls.localize({ key: 'TaskSystem.invalidTaskJsonOther', comment: ['Message notifies of an error in one of several places there is tasks related json, not necessarily in a file named tasks.json'] }, '错误: {0} 中 tasks json 的内容存在语法错误。请先纠正它们，然后再执行任务。', location));
	...
	private async _computeWorkspaceFileTasks(workspaceFolder: IWorkspaceFolder, runSource: TaskRunSource = TaskRunSource.User): Promise<IWorkspaceFolderTaskResult> {
		...
		const configuration = this._testParseExternalConfig(workspaceFileConfig.config, nls.localize('TasksSystem.locationWorkspaceConfig', '工作区文件'));
		...
		if (engine === ExecutionEngine.Process) {
			this._notificationService.warn(nls.localize('TaskSystem.versionWorkspaceFile', '工作区配置文件中只允许 2.0.0 版本的任务。'));
			return this._emptyWorkspaceTaskResults(workspaceFolder);
		}
	...
	private async _computeUserTasks(workspaceFolder: IWorkspaceFolder, runSource: TaskRunSource = TaskRunSource.User): Promise<IWorkspaceFolderTaskResult> {
		...
		const configuration = this._testParseExternalConfig(userTasksConfig.config, nls.localize('TasksSystem.locationUserConfig', '用户设置'));
		const customizedTasks: { byIdentifier: IStringDictionary<ConfiguringTask> } = {
			byIdentifier: Object.create(null)
		};
		...
		const engine = configuration.config ? TaskConfig.ExecutionEngine.from(configuration.config) : ExecutionEngine.Terminal;
		if (engine === ExecutionEngine.Process) {
			this._notificationService.warn(nls.localize('TaskSystem.versionSettings', '用户设置中只允许版本为 2.0.0 的任务。'));
			return this._emptyWorkspaceTaskResults(workspaceFolder);
		}
		return { workspaceFolder, set: { tasks: custom }, configurations: customizedTasks, hasErrors: configuration.hasParseErrors };
	}
	...
	protected _getConfiguration(workspaceFolder: IWorkspaceFolder, source?: string): { config: TaskConfig.IExternalTaskRunnerConfiguration | undefined; hasParseErrors: boolean } {
		...
		if (parseErrors) {
			...
			if (isAffected) {
				this._log(nls.localize('TaskSystem.invalidTaskJson', '错误: tasks.json 文件的内容存在语法错误。请先纠正它们，然后再执行任务。'));
				this._showOutput(undefined, undefined, nls.localize('TaskSystem.invalidTaskJson', '错误: tasks.json 文件的内容存在语法错误。请先纠正它们，然后再执行任务。'));
		...
		if (err instanceof TaskError) {
			...
			if (needsConfig || needsTerminate) {
				this._notificationService.prompt(buildError.severity, buildError.message, [{
					label: needsConfig ? ConfigureTaskAction.TEXT.value : nls.localize('TerminateAction.label', "终止任务"),
					run: () => {
						if (needsConfig) {
							this._runConfigureTasks();
						} else {
			...
			this._notificationService.error(nls.localize('TaskSystem.unknownError', '运行任务时发生了错误。请参见任务日志了解详细信息。'));
		}
		if (showOutput) {
	...
	private async _createTaskQuickPickEntries(tasks: Task[], group: boolean = false, sort: boolean = false, selectedEntry?: ITaskQuickPickEntry, includeRecents: boolean = true): Promise<ITaskQuickPickEntry[]> {
		...
		function fillEntries(entries: QuickPickInput<ITaskQuickPickEntry>[], tasks: Task[], groupLabel: string): void {
			...
			for (const task of tasks) {
				...
				entry.buttons = [{ iconClass: ThemeIcon.asClassName(configureTaskIcon), tooltip: nls.localize('configureTask', "配置任务") }];
		...
		if (group) {
			...
			if (tasks.length === 1) {
				...
				if (includeRecents) {
					fillEntries(entries, recent, nls.localize('recentlyUsed', '最近使用的任务'));
				}
				configured = configured.sort((a, b) => sorter.compare(a, b));
				fillEntries(entries, configured, nls.localize('configured', '配置的任务'));
				detected = detected.sort((a, b) => sorter.compare(a, b));
				fillEntries(entries, detected, nls.localize('detected', '检测到的任务'));
			}
		} else {
			if (sort) {
				const sorter = this.createSorter();
	...
	private _showIgnoredFoldersMessage(): Promise<void> {
		...
		this._notificationService.prompt(
			Severity.Info,
			nls.localize('TaskService.ignoredFolder', '由于使用任务版本 0.1.0，以下工作区文件夹将被忽略: {0}', this.ignoredWorkspaceFolders.map(f => f.name).join(', ')),
			[{
				label: nls.localize('TaskService.notAgain', "不再显示"),
				isSecondary: true,
				run: () => {
	...
	private async _trust(): Promise<boolean> {
		...
		if (!this._workspaceTrustManagementService.isWorkspaceTrusted()) {
			return (await this._workspaceTrustRequestService.requestWorkspaceTrust(
				{
					message: nls.localize('TaskService.requestTrust', "列出和运行任务要求此工作区中的某些文件作为代码执行。")
				})) === true;
		}
		return true;
	}
	...
	private _doRunTaskCommand(tasks?: Task[], type?: string, name?: string): void {
		...
		const placeholder = nls.localize('TaskService.pickRunTask', '选择要运行的任务');
		...
		this._showIgnoredFoldersMessage().then(() => {
			if (this._configurationService.getValue(USE_SLOW_PICKER)) {
				...
				this._showQuickPick(tasks ? tasks : taskResult!.tasks, placeholder,
					{
						label: '$(plus) ' + nls.localize('TaskService.noEntryToRun', '配置任务'),
						task: null
					},
	...
	private _runBuildCommand(): void {
		...
		return this._runTaskGroupCommand(TaskGroup.Build, {
			fetching: nls.localize('TaskService.fetchingBuildTasks', '正在获取生成任务...'),
			select: nls.localize('TaskService.pickBuildTask', '选择要运行的生成任务'),
			notFoundConfigure: nls.localize('TaskService.noBuildTask', '没有找到要运行的生成任务。配置生成任务...')
	...
	private _runTestCommand(): void {
		return this._runTaskGroupCommand(TaskGroup.Test, {
			fetching: nls.localize('TaskService.fetchingTestTasks', '正在获取测试任务...'),
			select: nls.localize('TaskService.pickTestTask', '选择要运行的测试任务'),
			notFoundConfigure: nls.localize('TaskService.noTestTaskTerminal', '没有找到要运行的测试任务。配置任务...')
	...
	private _runTerminateCommand(arg?: string | ITaskIdentifier): void {
		...
		const runQuickPick = (promise?: Promise<Task[]>) => {
			this._showQuickPick(promise || this.getActiveTasks(),
				nls.localize('TaskService.taskToTerminate', '选择要终止的任务'),
				{
					label: nls.localize('TaskService.noTaskRunning', '当前没有运行中的任务'),
					task: undefined
				},
				false, true,
				undefined,
				[{
					label: nls.localize('TaskService.terminateAllRunningTasks', '所有正在运行的任务'),
					id: 'terminateAll',
					task: undefined
				}]
			).then(entry => {
				...
				if (active) {
					this._terminateAll().then((responses) => {
						...
						if (response.code && response.code === TerminateResponseCode.ProcessNotFound) {
							this._notificationService.error(nls.localize('TerminateAction.noProcess', '启动的进程不再存在。如果任务生成的后台任务退出 VS Code，则可能会导致出现孤立的进程。'));
						} else {
							this._notificationService.error(nls.localize('TerminateAction.failed', '未能终止运行中的任务'));
						}
					});
				}
			});
	...
	private async _runRestartTaskCommand(arg?: string | ITaskIdentifier): Promise<void> {
		...
		if (this.inTerminal()) {
			...
			const entry = await this._showQuickPick(
				activeTasks,
				nls.localize('TaskService.taskToRestart', '选择要重启的任务'),
				{
					label: nls.localize('TaskService.noTaskToRestart', '没有要重启的任务'),
					task: null
				},
				false,
				true
	...
	private async _runRerunAllRunningTasksCommand(): Promise<void> {
		...
		if (activeTasks.length === 0) {
			this._notificationService.info(nls.localize('TaskService.noRunningTasks', '没有要重启的正在运行的任务'));
			return;
		}
	...
	private _openTaskFile(resource: URI, taskSource: string) {
		...
		this._fileService.stat(resource).then((stat) => stat, () => undefined).then(async (stat) => {
			...
			let content;
			if (!tasksExistInFile) {
				const pickTemplateResult = await this._quickInputService.pick(getTaskTemplates(), { placeHolder: nls.localize('TaskService.template', '选择任务模板') });
	...
	public getTaskDescription(task: Task | ConfiguringTask): string | undefined {
		...
		if (task._source.kind === TaskSourceKind.User) {
			description = nls.localize('taskQuickPick.userSettings', '用户');
	...
	private async _runConfigureTasks(): Promise<void> {
		...
		const createLabel = nls.localize('TaskService.createJsonFile', '使用模板创建 tasks.json 文件');
		const openLabel = nls.localize('TaskService.openJsonFile', '打开 tasks.json 文件');
		...
		this._quickInputService.pick(entriesWithSettings,
			{ placeHolder: nls.localize('TaskService.pickTask', '选择要配置的任务') }, cancellationToken).
				...
				this._showIgnoredFoldersMessage().then(async () => {
					...
					for (const task of tasks) {
						if (task === defaultBuildTask) {
							const label = nls.localize('TaskService.defaultBuildTaskExists', '{0} 已被标记为默认生成任务', TaskQuickPick.getTaskLabelWithIcon(task, task.getQualifiedLabel()));
							selectedTask = task;
					...
					this._quickInputService.pick(entries, {
						placeHolder: nls.localize('TaskService.pickDefaultBuildTask', '选择要用作默认生成任务的任务')
					}).
						then((entry) => {
							const task: Task | undefined | null = entry && Object.hasOwn(entry, 'task') ? (entry as IQuickPickItem & { task: Task }).task : undefined;
							if ((task === undefined) || (task === null)) {
	...
	private _runConfigureDefaultTestTask(): void {
		if (this.schemaVersion === JsonSchemaVersion.V2_0_0) {
			this.tasks().then((tasks => {
				...
				if (selectedTask) {
					selectedEntry = {
						label: nls.localize('TaskService.defaultTestTaskExists', '{0} 已被标记为默认测试任务。', selectedTask.getQualifiedLabel()),
						task: selectedTask,
				...
				this._showIgnoredFoldersMessage().then(() => {
					this._showQuickPick(tasks,
						nls.localize('TaskService.pickDefaultTestTask', '选择要用作默认测试任务的任务'), undefined, true, false, selectedEntry).then((entry) => {
	...
	public async runShowTasks(): Promise<void> {
		...
		if (activeTasks.length === 1) {
			...
			this._showQuickPick(activeTasksPromise,
				nls.localize('TaskService.pickShowTask', '选择要显示输出的任务'),
				{
					label: nls.localize('TaskService.noTaskIsRunning', '没有运行中的任务'),
					task: null
				},
				false, true
			).then((entry) => {
	...
	private async _upgrade(): Promise<void> {
		...
		this._notificationService.prompt(Severity.Warning,
			fileDiffs.length === 1 ?
				nls.localize('taskService.upgradeVersion', "已删除弃用的任务版本 0.1.0。你的任务已升级到 2.0.0 版本。打开差异以查看升级内容。")
				: nls.localize('taskService.upgradeVersionPlural', "已删除弃用的任务版本 0.1.0。你的任务已升级到 2.0.0 版本。打开差异以查看升级内容。"),
			[{
				label: fileDiffs.length === 1 ? nls.localize('taskService.openDiff', "打开差异") : nls.localize('taskService.openDiffs', "打开差异"),
				run: async () => {
