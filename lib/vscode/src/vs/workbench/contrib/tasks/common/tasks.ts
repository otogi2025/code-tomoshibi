...
export const TASK_RUNNING_STATE = new RawContextKey<boolean>('taskRunning', false, nls.localize('tasks.taskRunningContext', "任务当前是否正在运行。"));
/** Whether the active terminal is a task terminal. */
export const TASK_TERMINAL_ACTIVE = new RawContextKey<boolean>('taskTerminalActive', false, nls.localize('taskTerminalActive', "活动终端是否为任务终端。"));
export const TASKS_CATEGORY = nls.localize2('tasksCategory', "任务");
					...
					switch (schema.type) {
						...
						default:
							reporter.error(nls.localize(
								...
								'错误: 任务标识符“{0}”缺失必要属性“{1}”。将忽略该标识符。', JSON.stringify(external, undefined, 0), property
							));
							return undefined;
					}
				}
...
export const rerunTaskIcon = registerIcon('rerun-task', Codicon.refresh, nls.localize('rerunTaskIcon', '查看重新运行任务的图标。'));
