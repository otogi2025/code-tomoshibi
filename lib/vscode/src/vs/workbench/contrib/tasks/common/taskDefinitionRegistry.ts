...
const taskDefinitionSchema: IJSONSchema = {
	...
	properties: {
		type: {
			type: 'string',
			description: nls.localize('TaskDefinition.description', '实际任务类型。请注意，以 "$" 开头的类型仅保留内部使用。')
		},
		required: {
			type: 'array',
			markdownDescription: nls.localize('TaskDefinition.required', '要使此类型任务被视为匹配，必须提供 `properties` 对象中的属性名称。VS Code 用于将 `tasks.json` 条目与已注册任务提供程序关联。'),
			items: {
				type: 'string'
			}
		},
		properties: {
			type: 'object',
			description: nls.localize('TaskDefinition.properties', '任务类型的其他属性'),
		...
		when: {
			type: 'string',
			markdownDescription: nls.localize('TaskDefinition.when', '启用此类型任务是必需为 true 的条件。请考虑根据此任务定义使用 `shellExecutionSupported`、`processExecutionSupported` 和 `customExecutionSupported`。有关详细信息，请参阅 [API 文档](https://code.visualstudio.com/api/extension-guides/task-provider#when-clause)。'),
			default: ''
		}
	}
};
...
namespace Configuration {
	...
	export function from(value: ITaskDefinition, extensionId: ExtensionIdentifier, messageCollector: ExtensionMessageCollector): Tasks.ITaskDefinition | undefined {
		...
		if (!taskType || taskType.length === 0) {
			messageCollector.error(nls.localize('TaskTypeConfiguration.noType', '任务类型配置缺少必需的 "taskType" 属性'));
			return undefined;
		}
...
const taskDefinitionsExtPoint = ExtensionsRegistry.registerExtensionPoint<Configuration.ITaskDefinition[]>({
	...
	jsonSchema: {
		description: nls.localize('TaskDefinitionExtPoint', '配置任务种类'),
		type: 'array',
