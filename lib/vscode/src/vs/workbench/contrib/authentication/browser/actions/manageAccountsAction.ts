...
class ManageAccountsActionImpl {
	...
	public async run() {
		...
		if (!accounts.length) {
			await this.quickInputService.pick([{ label: localize('noActiveAccounts', "没有活动帐户。") }], { placeHolder });
			return;
		}
	...
	private async showAccountActions(account: AccountQuickPickItem): Promise<void> {
		...
		const items: AccountActionQuickPickItem[] = [{
			label: localize('manageTrustedExtensions', "管理受信任扩展"),
