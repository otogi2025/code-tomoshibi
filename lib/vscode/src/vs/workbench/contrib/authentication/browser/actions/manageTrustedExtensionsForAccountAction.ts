...
class ManageTrustedExtensionsForAccountActionImpl {
	...
	private async _getItems(accountQuery: IAccountQuery) {
		...
		if (!filteredExtensions.length) {
			this._dialogService.info(localize('noTrustedExtensions', "尚无任何扩展使用此帐户。"));
			return [];
		}
	...
	private _toQuickPickItem(extension: AllowedExtension): TrustedExtensionsQuickPickItem {
		...
		const description = lastUsed
			? localize({ key: 'accountLastUsedDate', comment: ['The placeholder {0} is a string with time information, such as "3 days ago"'] }, "上次使用此帐户的时间: {0}", fromNow(lastUsed, true))
			: localize('notUsed', "未使用此帐户");
		let tooltip: string | undefined;
		let disabled: boolean | undefined;
		if (extension.trusted) {
			tooltip = localize('trustedExtensionTooltip', "此扩展受 Microsoft 信任并且\r\n始终有权访问此帐户");
			disabled = true;
		}
		return {
	...
	private _createQuickPick(accountQuery: IAccountQuery) {
		...
		quickPick.title = localize('manageTrustedExtensions', "管理受信任扩展");
		quickPick.placeholder = localize('manageExtensions', "选择可以访问此帐户的扩展");
