...
export class BulkEditPane extends ViewPane {
	...
	protected override renderBody(parent: HTMLElement): void {
		...
		btnConfirm.label = localize('ok', '应用');
		btnConfirm.onDidClick(() => this.accept(), this, this._disposables);

		const btnCancel = buttonBar.addButton({ ...defaultButtonStyles, secondary: true });
		btnCancel.label = localize('cancel', '放弃');
		btnCancel.onDidClick(() => this.discard(), this, this._disposables);

		// message
		this._message = document.createElement('span');
		this._message.className = 'message';
		this._message.innerText = localize('empty.msg', "调用代码操作(如重命名操作)，在此处查看其更改的预览。");
	...
	accept(): void {
		...
		if (conflicts.length === 1) {
			message = localize('conflict.1', "无法应用重构，因为“{0}”在此期间进行了修改。", this._labelService.getUriLabel(conflicts[0], { relative: true }));
		} else {
			message = localize('conflict.N', "无法应用重构，因为其他 {0} 个文件在此期间进行了修改。", conflicts.length);
		}
