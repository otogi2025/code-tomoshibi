...
class BulkEdit {
	...
	ariaMessage(): string {
		...
		if (this._edits.length === 0) {
			return localize('summary.0', "未做编辑");
		} else if (otherResources.size === 0) {
			if (textEditCount > 1 && textEditResources.size > 1) {
				return localize('summary.nm', "在 {1} 个文件中进行了 {0} 次编辑", textEditCount, textEditResources.size);
			} else {
				return localize('summary.n0', "在 1 个文件中进行了 {0} 次编辑", textEditCount);
			}
		} else {
			return localize('summary.textFiles', "已在 {1} 文件中执行 {0} 文本编辑，且已创建或删除{2} 文件", textEditCount, textEditResources.size, otherResources.size);
		}
	}
	...
	private async _performFileEdits(edits: ResourceFileEdit[], undoRedoGroup: UndoRedoGroup, undoRedoSource: UndoRedoSource | undefined, confirmBeforeUndo: boolean, progress: IProgress<void>): Promise<readonly URI[]> {
		...
		const model = this._instaService.createInstance(BulkFileEdits, this._label || localize('workspaceEdit', "工作区编辑"), this._code || 'undoredo.workspaceEdit', undoRedoGroup, undoRedoSource, confirmBeforeUndo, progress, this._token, edits);
	...
	private async _performTextEdits(edits: ResourceTextEdit[], undoRedoGroup: UndoRedoGroup, undoRedoSource: UndoRedoSource | undefined, progress: IProgress<void>, reason: TextModelEditSource | undefined): Promise<readonly URI[]> {
		...
		const model = this._instaService.createInstance(BulkTextEdits, this._label || localize('workspaceEdit', "工作区编辑"), this._code || 'undoredo.workspaceEdit', this._editor, undoRedoGroup, undoRedoSource, progress, this._token, edits);
...
export class BulkEditService implements IBulkEditService {
	...
	async apply(editsIn: ResourceEdit[] | WorkspaceEdit, options?: IBulkEditOptions): Promise<IBulkEditResult> {
		...
		if (edits.length === 0) {
			return { ariaSummary: localize('nothing', "未做编辑"), isApplied: false };
		}
	...
	private async _shouldVeto(label: string | undefined, reason: ShutdownReason): Promise<boolean> {
		...
		switch (reason) {
			case ShutdownReason.CLOSE:
				message = localize('closeTheWindow.message', "是否确实要关闭窗口?");
				break;
			case ShutdownReason.LOAD:
				message = localize('changeWorkspace.message', "是否确实要更改工作区?");
				break;
			case ShutdownReason.RELOAD:
				message = localize('reloadTheWindow.message', "是否确实要重新加载窗口?");
				break;
			default:
				message = isMacintosh ? localize('quitMessageMac', "是否确实要退出?") : localize('quitMessage', "是否确实要退出?");
				break;
		}

		const result = await this._dialogService.confirm({
			message,
			detail: localize('areYouSureQuiteBulkEdit.detail', "{0} 正在进行。", label || localize('fileOperation', "文件操作")),
		});
...
Registry.as<IConfigurationRegistry>(Extensions.Configuration).registerConfiguration({
	...
	properties: {
		[autoSaveSetting]: {
			description: localize('refactoring.autoSave', "控制是否自动保存作为重构一部分的文件"),
			default: true,
			type: 'boolean'
		}
	}
