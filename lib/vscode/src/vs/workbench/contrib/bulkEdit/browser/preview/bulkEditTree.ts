	...
	getAriaLabel(element: BulkEditElement): string | null {
		...
		if (element instanceof TextEditElement) {
			if (element.selecting.length > 0 && element.inserting.length > 0) {
				// edit: replace
				return localize('aria.replace', "行{0}，用{2}替换{1}", element.edit.textEdit.textEdit.range.startLineNumber, element.selecting, element.inserting);
			} else if (element.selecting.length > 0 && element.inserting.length === 0) {
				// edit: delete
				return localize('aria.del', "行 {0}，正在删除 {1}", element.edit.textEdit.textEdit.range.startLineNumber, element.selecting);
			} else if (element.selecting.length === 0 && element.inserting.length > 0) {
				// edit: insert
				return localize('aria.insert', "行{0}，插入{1}", element.edit.textEdit.textEdit.range.startLineNumber, element.selecting);
			}
		}
...
class FileElementTemplate {
	...
	set(element: FileElement, score: FuzzyScore | undefined) {
		...
		if (element.edit.type & BulkFileOperationType.Rename && element.edit.newUri) {
			...
			this._details.innerText = localize('detail.rename', "(重命名)");
			...
			if (element.edit.type & BulkFileOperationType.Create) {
				this._details.innerText = localize('detail.create', "(正在创建)");
			} else if (element.edit.type & BulkFileOperationType.Delete) {
				this._details.innerText = localize('detail.del', "(删除)");
