	...
	private async _previewEdit(edits: ResourceEdit[]): Promise<ResourceEdit[]> {
		...
		if (view.hasInput()) {
			const { confirmed } = await this._dialogService.confirm({
				type: Severity.Info,
				message: localize('overlap', "正在预览另一个重构。"),
				detail: localize('detail', "按\"继续\"放弃以前的重构，继续当前重构。"),
				primaryButton: localize({ key: 'continue', comment: ['&& denotes a mnemonic'] }, "继续(&&C)")
			});
...
registerAction2(class ApplyAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('apply', "应用重构"),
			category: localize2('cat', "重构预览"),
			icon: Codicon.check,
...
registerAction2(class ToggleAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('toogleSelection', "切换更改"),
			category: localize2('cat', "重构预览"),
			precondition: BulkEditPreviewContribution.ctxEnabled,
			keybinding: {
...
registerAction2(class GroupByFile extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('groupByFile', "按文件分组更改"),
			category: localize2('cat', "重构预览"),
...
registerAction2(class GroupByType extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('groupByType', "按类型分组更改"),
			category: localize2('cat', "重构预览"),
...
const refactorPreviewViewIcon = registerIcon('refactor-preview-view-icon', Codicon.lightbulb, localize('refactorPreviewViewIcon', '查看重构预览视图的图标。'));

const container = Registry.as<IViewContainersRegistry>(ViewContainerExtensions.ViewContainersRegistry).registerViewContainer({
	id: BulkEditPane.ID,
	title: localize2('panel', "重构预览"),
	hideIfEmpty: true,
