	...
	constructor(
		...
		editorResolverService.registerEditor(
			`${ProcessExplorerEditorInput.RESOURCE.scheme}:**/**`,
			{
				...
				label: localize('promptOpenWith.processExplorer.displayName', "进程资源管理器"),
...
class OpenProcessExplorer extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('openProcessExplorer', '打开进程资源管理器'),
...
MenuRegistry.appendMenuItem(MenuId.MenubarHelpMenu, {
	...
	command: {
		id: OpenProcessExplorer.ID,
		title: localize({ key: 'miOpenProcessExplorerer', comment: ['&& denotes a mnemonic'] }, "打开进程资源管理器(&&P)")
	},
	when: supported,
	order: 2
});
