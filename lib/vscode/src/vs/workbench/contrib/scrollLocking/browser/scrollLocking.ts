	...
	private toggleStatusbarItem(active: boolean): void {
		if (active) {
			if (!this.statusBarEntry.value) {
				const text = localize('mouseScrolllingLocked', '已锁定滚动');
				const tooltip = localize('mouseLockScrollingEnabled', '已启用锁定滚动');
		...
		this._register(registerAction2(class extends Action2 {
			constructor() {
				super({
					id: 'workbench.action.toggleLockedScrolling',
					title: {
						...localize2('toggleLockedScrolling', "切换编辑器之间的锁定滚动"),
						mnemonicTitle: localize({ key: 'miToggleLockedScrolling', comment: ['&& denotes a mnemonic'] }, "锁定滚动"),
					},
					category: Categories.View,
					f1: true,
					metadata: {
						description: localize('synchronizeScrolling', "同步滚动编辑器"),
					}
				});
			}
		...
		this._register(registerAction2(class extends Action2 {
			constructor() {
				super({
					...
					title: {
						...localize2('holdLockedScrolling', "保持编辑器之间的锁定滚动"),
						mnemonicTitle: localize({ key: 'miHoldLockedScrolling', comment: ['&& denotes a mnemonic'] }, "锁定滚动"),
					},
					category: Categories.View,
				});
			}
