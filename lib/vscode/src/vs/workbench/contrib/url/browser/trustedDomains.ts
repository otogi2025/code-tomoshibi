...
export const manageTrustedDomainSettingsCommand = {
	...
	description: {
		description: localize2('trustedDomain.manageTrustedDomain', '管理受信任的域'),
		args: []
	},
...
export async function configureOpenerTrustedDomainsHandler(
	...
	options.push({
		type: 'item',
		label: localize('trustedDomain.trustDomain', '信任 {0}', domainToConfigure),
		id: 'trust',
	...
	if (isIP) {
		if (parsedDomainToConfigure.authority.includes(':')) {
			...
			options.push({
				type: 'item',
				label: localize('trustedDomain.trustAllPorts', '信任所有端口上的 {0}', base),
				toTrust: base + ':*',
				id: 'trust'
			});
		}
		...
		options.push({
			type: 'item',
			label: localize('trustedDomain.trustSubDomain', '信任 {0} 及其所有子域', domainEnd),
	...
	options.push({
		type: 'item',
		label: localize('trustedDomain.trustAllDomains', '信任所有域(禁用链接保护)'),
		toTrust: '*',
		id: 'trust'
	});
	options.push({
		type: 'item',
		label: localize('trustedDomain.manageTrustedDomains', '管理受信任的域'),
		id: 'manage'
	});
