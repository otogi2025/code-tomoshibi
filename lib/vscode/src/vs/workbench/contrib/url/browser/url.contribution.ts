...
class OpenUrlAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('openUrl', '打开 URL'),
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		return quickInputService.input({ prompt: localize('urlToOpen', "要打开的 URL"), value }).then(input => {
			if (input) {
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		'workbench.trustedDomains.promptInTrustedWorkspace': {
			...
			type: 'boolean',
			default: false,
			description: localize('workbench.trustedDomains.promptInTrustedWorkspace', "启用后，在受信任的工作区中打开链接时，将显示受信任的域提示。")
		}
	}
});
