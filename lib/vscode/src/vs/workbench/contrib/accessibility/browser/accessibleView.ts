...
export class AccessibleView extends Disposable {
	...
	configureKeybindings(unassigned: boolean): void {
		...
		quickPick.title = localize('keybindings', '配置键绑定');
		quickPick.placeholder = localize('selectKeybinding', '选择要为其配置键绑定的命令 ID');
		quickPick.show();
	...
	disableHint(): void {
		...
		alert(localize('disableAccessibilityHelp', '{0} 辅助功能详细程度现已禁用', this._currentProvider.verbositySettingKey));
	}
	...
	private _render(provider: AccesibleViewContentProvider, container: HTMLElement, showAccessibleViewHelp?: boolean, updatedContent?: string): IDisposable {
		...
		this._getTextModel(stableUri).then((model) => {
			...
			if (verbose && !showAccessibleViewHelp && hasActions) {
				actionsHint = provider.options.position ? localize('ariaAccessibleViewActionsBottom', '浏览禁用此提示 (Shift+Tab) 等操作，使用 Esc 退出此对话框。') : localize('ariaAccessibleViewActions', '浏览禁用此提示 (Shift+Tab )等操作。');
			}
			let ariaLabel = provider.options.type === AccessibleViewType.Help ? localize('accessibility-help', "辅助功能帮助") : localize('accessible-view', "辅助视图");
			this._title.textContent = ariaLabel;
			if (actionsHint && provider.options.type === AccessibleViewType.View) {
				ariaLabel = localize('accessible-view-hint', "辅助视图。{0}", actionsHint);
			} else if (actionsHint) {
	...
	private _updateToolbar(providedActions?: IAction[], type?: AccessibleViewType): void {
		this._toolbar.setAriaLabel(type === AccessibleViewType.Help ? localize('accessibleHelpToolbar', '辅助功能帮助') : localize('accessibleViewToolbar', "辅助视图"));
	...
	private _accessibleViewHelpDialogContent(providerHasSymbols?: boolean): string {
		...
		let hint = localize('intro', "在辅助视图中，可以:\r\n");
		if (navigationHint) {
			hint += ' - ' + navigationHint + '\n';
		}
		if (goToSymbolHint) {
	...
	private _disableVerbosityHint(provider: AccesibleViewContentProvider): string {
		if (provider.options.type === AccessibleViewType.Help && this._verbosityEnabled()) {
			return localize('acessibleViewDisableHint', "\r\n禁用此功能的辅助功能详细程度 ({0})。", `<keybinding:${AccessibilityCommandId.DisableVerbosityHint}>`);
		}
		return '';
	}
	...
	private _configureUnassignedKbHint(): string {
		...
		return localize('configureKb', '\r\n为缺少键绑定的命令配置键绑定 {0}。', keybindingToConfigureQuickPick);
	}
	...
	private _configureAssignedKbHint(): string {
		...
		return localize('configureKbAssigned', '\r\n为已分配的命令配置键绑定 {0}。', keybindingToConfigureQuickPick);
	}
	...
	private _readMoreHint(provider: AccesibleViewContentProvider): string {
		return provider.options.readMoreUrl ? localize("openDoc", "\r\n打开浏览器窗口，其中包含与辅助功能 ({0}) 相关的详细信息。", `<keybinding:${AccessibilityCommandId.AccessibilityHelpOpenHelpLink}>`) : '';
	}
}
...
export class AccessibleViewService extends Disposable implements IAccessibleViewService {
	...
	getOpenAriaHint(verbositySettingKey: AccessibilityVerbositySettingId): string | null {
		...
		let hint = null;
		if (keybinding) {
			hint = localize('acessibleViewHint', "在辅助视图中用 {0} 检查此项", keybinding);
		} else {
			hint = localize('acessibleViewHintNoKbEither', "通过命令“打开辅助视图”在辅助视图中检查此项，该命令当前无法通过键绑定触发。");
		}
		return hint;
	}
	disableHint(): void {
...
class AccessibleViewSymbolQuickPick {
	...
	show(provider: AccesibleViewContentProvider): void {
		...
		quickPick.placeholder = localize('accessibleViewSymbolQuickPickPlaceholder', "键入以搜索符号");
		quickPick.title = localize('accessibleViewSymbolQuickPickTitle', "转到符号辅助视图");
		const picks = [];
