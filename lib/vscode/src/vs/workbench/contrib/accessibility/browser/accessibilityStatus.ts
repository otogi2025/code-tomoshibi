...
export class AccessibilityStatus extends Disposable implements IWorkbenchContribution {
	...
	private showScreenReaderNotification(): void {
		this.screenReaderNotification = this.notificationService.prompt(
			Severity.Info,
			localize('screenReaderDetectedExplanation.question', "检测到屏幕阅读器使用情况。是否要启用 {0} 以针对屏幕阅读器使用情况优化编辑器？", 'editor.accessibilitySupport'),
			[{
			...
			{
				label: localize('screenReaderDetectedExplanation.answerLearnMore', "了解详细信息"),
				run: () => {
	...
	private updateScreenReaderModeElement(visible: boolean): void {
		if (visible) {
			if (!this.screenReaderModeElement.value) {
				const text = localize('screenReaderDetected', "已为屏幕阅读器优化");
