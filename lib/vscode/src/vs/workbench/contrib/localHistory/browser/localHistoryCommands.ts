...
const LOCAL_HISTORY_CATEGORY = localize2('localHistory.category', '本地历史记录');
...
export const COMPARE_WITH_FILE_LABEL = localize2('localHistory.compareWithFile', '与文件进行比较');
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('localHistory.compareWithPrevious', '与上一个版本比较'),
			menu: {
				id: MenuId.TimelineItemContext,
				group: '1_compare',
				order: 2,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('localHistory.selectForCompare', '选择以进行比较'),
			menu: {
				id: MenuId.TimelineItemContext,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('localHistory.compareWithSelected', '与已选项目进行比较'),
			menu: {
				id: MenuId.TimelineItemContext,
				group: '2_compare_with',
				order: 1,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('localHistory.open', '显示内容'),
			menu: {
				id: MenuId.TimelineItemContext,
				group: '3_contents',
				order: 1,
...
const RESTORE_CONTENTS_LABEL = localize2('localHistory.restore', '还原内容');
...
const restoreSaveSource = SaveSourceRegistry.registerSource('localHistoryRestore.source', localize('localHistoryRestore.source', "已还原文件"));
...
async function restore(accessor: ServicesAccessor, item: ITimelineCommandArgument): Promise<void> {
	...
	if (entry) {
		...
		const { confirmed } = await dialogService.confirm({
			type: 'warning',
			message: localize('confirmRestoreMessage', "是否要还原“{0}”的内容?", basename(entry.workingCopy.resource)),
			detail: localize('confirmRestoreDetail', "还原将放弃任何未保存的更改。"),
			primaryButton: localize({ key: 'restoreButtonLabel', comment: ['&& denotes a mnemonic'] }, "还原(&&R)")
		});
		...
		try {
			...
			await dialogService.error(localize('unableToRestore', "无法还原“{0}”。", basename(entry.workingCopy.resource)), toErrorMessage(error));
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('localHistory.restoreViaPicker', '查找要还原的条目'),
			f1: true,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		resourcePicker.placeholder = localize('restoreViaPicker.filePlaceholder', "选择要显示其本地历史记录的文件");
		...
		entryPicker.placeholder = localize('restoreViaPicker.entryPlaceholder', "选择要打开的本地历史记录条目");
...
MenuRegistry.appendMenuItem(MenuId.TimelineTitle, { command: { id: 'workbench.action.localHistory.restoreViaPicker', title: localize2('localHistory.restoreViaPickerMenu', '本地历史记录: 查找要还原的条目...') }, group: 'submenu', order: 1, when: CTX_LOCAL_HISTORY_ENABLED });
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('localHistory.rename', '重命名'),
			menu: {
				id: MenuId.TimelineItemContext,
				group: '5_edit',
				order: 1,
	...
	async run(accessor: ServicesAccessor, item: ITimelineCommandArgument): Promise<void> {
		...
		if (entry) {
			...
			inputBox.title = localize('renameLocalHistoryEntryTitle', "重命名本地历史记录条目");
			inputBox.ignoreFocusOut = true;
			inputBox.placeholder = localize('renameLocalHistoryPlaceholder', "输入本地历史记录条目的新名称");
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('localHistory.delete', '删除'),
			menu: {
				id: MenuId.TimelineItemContext,
				group: '5_edit',
				order: 2,
	...
	async run(accessor: ServicesAccessor, item: ITimelineCommandArgument): Promise<void> {
		...
		if (entry) {
			...
			const { confirmed } = await dialogService.confirm({
				type: 'warning',
				message: localize('confirmDeleteMessage', "是否要从 {1} 中删除 {0} 的本地历史记录条目?", entry.workingCopy.name, toLocalHistoryEntryDateLabel(entry.timestamp)),
				detail: localize('confirmDeleteDetail', "此操作不可逆!"),
				primaryButton: localize({ key: 'deleteButtonLabel', comment: ['&& denotes a mnemonic'] }, "删除(&&D)"),
			});
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('localHistory.deleteAll', '全部删除'),
			f1: true,
			category: LOCAL_HISTORY_CATEGORY,
			precondition: CTX_LOCAL_HISTORY_ENABLED
		});
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const { confirmed } = await dialogService.confirm({
			type: 'warning',
			message: localize('confirmDeleteAllMessage', "是否要删除本地历史记录中所有文件的所有条目?"),
			detail: localize('confirmDeleteAllDetail', "此操作不可逆!"),
			primaryButton: localize({ key: 'deleteAllButtonLabel', comment: ['&& denotes a mnemonic'] }, "全部删除(&&D)"),
		});
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('localHistory.create', '创建条目'),
			f1: true,
			category: LOCAL_HISTORY_CATEGORY,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const disposables = new DisposableStore();
		const inputBox = disposables.add(quickInputService.createInputBox());
		inputBox.title = localize('createLocalHistoryEntryTitle', "创建本地历史记录条目");
		inputBox.ignoreFocusOut = true;
		inputBox.placeholder = localize('createLocalHistoryPlaceholder', "输入 '{0}' 的本地历史记录条目的新名称", labelService.getUriBasenameLabel(resource));
		inputBox.show();
