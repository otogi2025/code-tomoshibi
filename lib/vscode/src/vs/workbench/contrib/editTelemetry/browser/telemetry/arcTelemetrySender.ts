...
export class EditTelemetryReportInlineEditArcSender extends Disposable {
	constructor(
		...
		this._register(runOnChange(docWithAnnotatedEdits.value, (_val, _prev, changes) => {
			...
			const reporter = this._store.add(this._instantiationService.createInstance(ArcTelemetryReporter, [0, 30, 120, 300, 600, 900].map(s => s * 1000), _prev, docWithJustReason, edit, res => {
...
export class EditTelemetryReportEditArcForChatOrInlineChatSender extends Disposable {
	constructor(
		...
		this._register(runOnChange(docWithAnnotatedEdits.value, (_val, _prev, changes) => {
			...
			const docWithJustReason = createDocWithJustReason(docWithAnnotatedEdits, this._store);
			const reporter = this._store.add(this._instantiationService.createInstance(ArcTelemetryReporter, [0, 60, 300].map(s => s * 1000), _prev, docWithJustReason, edit, res => {
