...
import { mapObservableArrayCached, derived, IObservable, observableSignal } from '../../../../../base/common/observable.js';
...
export class EditSourceTrackingImpl extends Disposable {
	...
	constructor(
		...
		this._states = mapObservableArrayCached(this, this._annotatedDocuments.documents, (doc, store) => {
			return [doc.document, store.add(this._instantiationService.createInstance(TrackedDocumentInfo, doc, this._statsEnabled, this._agentHostEditMarkerService))] as const;
		});
...
class TrackedDocumentInfo extends Disposable {
	...
	constructor(
		...
		this._store.add(this._instantiationService.createInstance(EditTelemetryReportInlineEditArcSender, _doc.documentWithAnnotations));
		this._store.add(this._instantiationService.createInstance(EditTelemetryReportEditArcForChatOrInlineChatSender, _doc.documentWithAnnotations));
