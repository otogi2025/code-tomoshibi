	...
	constructor(
		...
		this._register(autorun((reader) => {
			...
			reader.store.add(this._statusbarService.addEntry({
				...
				ariaLabel: localize('inlineSuggestionsStatusBar', "内联建议状态栏"),
				text: '',
				tooltip: {
...
export function createAiStatsHover(options: IAiStatsHoverOptions) {
	...
	return n.div({
		...
		n.div({
			...
			[
				n.div({ style: { flex: 1 } }, [localize('aiStatsStatusBarHeader', "AI 使用情况统计信息")]),
					...
					? actionBar([
						{
							action: {
								...
								tooltip: localize('aiStats.statusBar.configure', "配置")
							},
		...
		n.div({ style: { display: 'flex' } }, [
			n.div({ style: { flex: 1, paddingRight: '4px' } }, [
				localize('text1', "AI 插入与手动键入平均值: {0}", aiRatePercent.get()),
			]),
		]),
		n.div({ style: { flex: 1, paddingRight: '4px' } }, [
			localize('text2', "今天接受的内联建议: {0}", options.data.acceptedInlineSuggestionsToday.get()),
		]),
			...
			n.div({
				...
				n.div({ style: { flex: 1 } }, [
					chartViewMode.map(mode =>
						mode === 'days'
							? localize('chartHeaderDays', "AI 速率(按天)")
							: localize('chartHeaderSessions', "AI 速率(按会话)")
					)
				]),
				n.div({
					...
					createToggleButton('days', localize('viewByDays', "天"), Codicon.calendar),
					createToggleButton('sessions', localize('viewBySessions', "会话"), Codicon.listFlat),
				])
			]),
