		...
		if (this.uri === undefined) {
			...
			if (this.pendingRequests.size !== 0) {
				...
				if (this.timelineService.getSources().filter(({ id }) => !this.excludedSources.has(id)).length === 0) {
					this.message = localize('timeline.noTimelineSourcesEnabled', "已筛选掉所有时间线源。");
				} else {
					if (this.configurationService.getValue('workbench.localHistory.enabled') && !this.excludedSources.has('timeline.localHistory')) {
						this.message = localize('timeline.noLocalHistoryYet', "除非文件已被排除或太大，否则本地历史记录将跟踪你保存的最新更改。");
					} else if (this.excludedSources.size > 0) {
						this.message = localize('timeline.noTimelineInfoFromEnabledSources', "未提供筛选的时间线信息。");
					} else {
						this.message = localize('timeline.noTimelineInfo', "未提供时间表信息。");
					}
				}
			}
		} else {
...
const timelineRefresh = registerIcon('timeline-refresh', Codicon.refresh, localize('timelineRefresh', '“刷新时间线”操作的图标。'));
const timelinePin = registerIcon('timeline-pin', Codicon.pin, localize('timelinePin', '“固定时间线”操作的图标。'));
const timelineUnpin = registerIcon('timeline-unpin', Codicon.pinned, localize('timelineUnpin', '“取消固定时间线”操作的图标。'));
