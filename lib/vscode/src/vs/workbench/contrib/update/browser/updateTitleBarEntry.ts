...
registerAction2(class UpdateIndicatorTitleBarAction extends Action2 {
	constructor() {
		super({
			...
			title: localize('updateIndicatorTitleBarAction', '更新'),
			f1: false,
			menu: [{
				id: MenuId.TitleBarAdjacentCenter,
				order: 0,
				when: ContextKeyExpr.and(UPDATE_TITLE_BAR_CONTEXT, InEditorZenModeContext.negate(), UPDATE_TITLE_BAR_CHAT_IN_PROGRESS_CONTEXT.negate()),
			}]
		});
	}
...
export class UpdateTitleBarEntry extends BaseActionViewItem {
	...
	private onStateChange(state: State) {
		...
		switch (state.type) {
			case StateType.Disabled:
				label.textContent = localize('updateIndicator.update', "更新");
				this.content.classList.add('update-disabled');
				break;

			case StateType.CheckingForUpdates:
				label.textContent = localize('updateIndicator.checking', "正在检查...");
				this.renderProgressState(this.content);
				break;

			case StateType.Overwriting:
				label.textContent = localize('updateIndicator.overwriting', "正在更新...");
				this.renderProgressState(this.content);
				break;
			...
			case StateType.Downloading:
				label.textContent = localize('updateIndicator.downloading', "正在下载...");
				this.renderProgressState(this.content, computeProgressPercent(state.downloadedBytes, state.totalBytes));
				break;

			case StateType.Updating:
				label.textContent = localize('updateIndicator.installing', "正在安装...");
				this.renderProgressState(this.content, computeProgressPercent(state.currentProgress, state.maxProgress));
				break;

			case StateType.Restarting:
				label.textContent = localize('updateIndicator.restarting', "正在重启...");
				this.renderProgressState(this.content);
				break;

			case StateType.Cancelling:
				label.textContent = localize('updateIndicator.cancelling', "正在取消...");
				this.renderProgressState(this.content);
				break;
