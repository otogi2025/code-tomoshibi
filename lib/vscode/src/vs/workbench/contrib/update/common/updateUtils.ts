...
export function formatTimeRemaining(seconds: number): string {
	...
	if (hours >= 1) {
		...
		if (formattedHours === '1') {
			return localize('update.timeRemainingHour', "{0} 小时", formattedHours);
		} else {
			return localize('update.timeRemainingHours', "{0} 小时", formattedHours);
		}
	}

	const minutes = Math.floor(seconds / 60);
	if (minutes >= 1) {
		return localize('update.timeRemainingMinutes', "{0} 分钟", minutes);
	}

	return localize('update.timeRemainingSeconds', "{0} 秒", seconds);
}
...
export function formatBytes(bytes: number): string {
	if (bytes < 1024) {
		return localize('update.bytes', "{0} 字节", bytes);
	}
