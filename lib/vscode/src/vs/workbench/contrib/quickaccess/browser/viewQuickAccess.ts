...
export class ViewQuickAccessProvider extends PickerQuickAccessProvider<IViewQuickPickItem> {
	...
	constructor(
		...
		super(ViewQuickAccessProvider.PREFIX, {
			noResultsPick: {
				label: localize('noViewResults', "没有匹配的视图"),
				containerLabel: ''
			}
		});
	}
	...
	private doGetViewPickItems(): Array<IViewQuickPickItem> {
		...
		// Viewlets / Panels
		addPaneComposites(ViewContainerLocation.Sidebar, localize('views', "侧边栏"));
		addPaneComposites(ViewContainerLocation.Panel, localize('panels', "面板"));
		addPaneComposites(ViewContainerLocation.AuxiliaryBar, localize('secondary side bar', "辅助侧边栏"));
		...
		this.terminalGroupService.groups.forEach((group, groupIndex) => {
			group.terminalInstances.forEach((terminal, terminalIndex) => {
				...
				viewEntries.push({
					label,
					containerLabel: localize('terminals', "终端"),
					accept: async () => {
		...
		for (const channel of channels) {
			viewEntries.push({
				label: channel.label,
				containerLabel: localize('channels', "输出"),
...
export class QuickAccessViewPickerAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('quickOpenView', 'Quick Open 视图'),
			category: Categories.View,
