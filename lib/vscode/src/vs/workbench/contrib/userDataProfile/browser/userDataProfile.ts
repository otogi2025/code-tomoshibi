...
export class UserDataProfilesWorkbenchContribution extends Disposable implements IWorkbenchContribution {
		...
		this._register(dndRegistry.registerDropHandler(new class UserDataProfileDropHandler implements IResourceDropHandler {
			async handleDrop(resource: URI, accessor: ServicesAccessor): Promise<boolean> {
				...
				if (uriIdentityService.extUri.extname(resource) === `.${PROFILE_EXTENSION}`) {
					...
					if (!template) {
						notificationService.warn(localize('invalid profile', "删除的配置文件无效。"));
	...
	private registerOpenProfileSubMenu(): void {
		MenuRegistry.appendMenuItem(MenuId.MenubarFileMenu, {
			title: localize('New Profile Window', "使用配置文件新建窗口"),
		...
		return registerAction2(class NewWindowWithProfileAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('newWindowWithProfile', "使用配置文件新建窗口..."),
			...
			async run(accessor: ServicesAccessor) {
				...
				const pick = await quickInputService.pick(
					...
					{
						title: localize('new window with profile', "使用配置文件新建窗口"),
		...
		return registerAction2(class SwitchProfileAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('switchProfile', 'Switch 配置文件...'),
					category: PROFILES_CATEGORY,
					f1: true,
				});
			}
	...
	private registerManageProfilesAction(): IDisposable {
		...
		disposables.add(registerAction2(class ManageProfilesAction extends Action2 {
			constructor() {
				super({
					...
					title: {
						...localize2('manage profiles', "配置文件"),
						mnemonicTitle: localize({ key: 'miOpenProfiles', comment: ['&& denotes a mnemonic'] }, "配置文件(&&P)"),
					},
					menu: [
						{
		...
		disposables.add(MenuRegistry.appendMenuItem(MenuId.CommandPalette, {
			command: {
				...
				title: localize2('open profiles', "打开配置文件(UI)"),
			},
		}));
				...
				if (profiles.length) {
					const picks = await quickInputService.pick(
						profiles.map(profile => ({
							label: profile.name,
							description: profile.id === userDataProfileService.currentProfile.id ? localize('current', "当前") : undefined,
							profile
						})),
						{
