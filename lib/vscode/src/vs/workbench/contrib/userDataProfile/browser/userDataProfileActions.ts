...
class CreateTransientProfileAction extends Action2 {
	...
	static readonly TITLE = localize2('create temporary profile', "具有临时配置文件的新窗口");
	constructor() {
		super({
...
registerAction2(class CleanupProfilesAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('cleanup profile', "清理配置文件"),
...
registerAction2(class ResetWorkspacesAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('reset workspaces', "重置工作区配置文件关联"),
			category: Categories.Developer,
			f1: true,
		});
	}
