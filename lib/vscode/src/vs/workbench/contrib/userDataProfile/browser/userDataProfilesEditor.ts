...
const editIcon = registerIcon('profiles-editor-edit-folder', Codicon.edit, localize('editIcon', '配置文件编辑器中的“编辑文件夹”图标。'));
const removeIcon = registerIcon('profiles-editor-remove-folder', Codicon.close, localize('removeIcon', '配置文件编辑器中的“删除文件夹”图标。'));

export const profilesSashBorder = registerColor('profiles.sashBorder', PANEL_BORDER, localize('profilesSashBorder', "配置文件编辑器拆分视图窗体边框的颜色。"));
...
export class UserDataProfilesEditor extends EditorPane implements IUserDataProfilesEditor {
			...
			actions: {
				getActions: () => {
					...
					if (this.templates.length) {
						actions.push(new SubmenuAction('from.template', localize('from template', "来自模板"), this.getCreateFromTemplateActions()));
	...
	private getTreeContextMenuActions(): IAction[] {
		...
		if (templateActions.length) {
			actions.push(new SubmenuAction('from.template', localize('new from template', "根据模板新建配置文件"), templateActions));
		}
		actions.push(new Separator());
		actions.push(toAction({
			id: 'importProfile',
	...
	private async getProfileUriFromFileSystem(): Promise<URI | null> {
		const profileLocation = await this.fileDialogService.showOpenDialog({
			...
			title: localize('import profile dialog', "选择配置文件模版文件"),
		});
		if (!profileLocation) {
			return null;
		}
...
class ProfileWidget extends Disposable {
	...
	constructor(
		...
		this.builtInLabel = append(title, $('.profile-built-in-label', undefined, localize('builtIn', "内置")));
...
class ProfileNameRenderer extends ProfilePropertyRenderer {
	...
	renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
			...
			this.contextViewService,
			{
				...
				validationOptions: {
					validation: (value) => {
						if (!value) {
							return {
								content: localize('name required', "配置文件名称是必需的，并且必须为非空值。"),
						...
						if (initialName !== value && this.userDataProfilesService.profiles.some(p => !p.isInternal && p.name === value)) {
							return {
								content: localize('profileExists', "具有名称 {0} 的配置文件已经存在。", value),
								type: MessageType.WARNING
							};
						}
						return null;
		...
		const renderName = (profileElement: ProfileTreeElement) => {
			...
			if (isSystemProfile) {
				nameInput.setTooltip(localize('defaultProfileName', "无法更改内置配置文件的名称"));
			} else {
...
class ProfileIconRenderer extends ProfilePropertyRenderer {
	...
	renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
		...
		append(iconValueContainer, $('.profile-description-element', undefined, localize('icon-description', "要显示在活动栏中的配置文件图标")));
...
class UseAsDefaultProfileRenderer extends ProfilePropertyRenderer {
	...
	renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
		...
		append(useAsDefaultProfileContainer, $('.profile-label-element', undefined, localize('use for new windows', "用于新 Windows")));
		const useAsDefaultProfileValueContainer = append(useAsDefaultProfileContainer, $('.profile-use-as-default-container'));
		const useAsDefaultProfileTitle = localize('enable for new windows', "将此配置文件用作新窗口的默认设置");
...
class CopyFromProfileRenderer extends ProfilePropertyRenderer {
	...
	renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
		...
		const copyFromSelectBox = disposables.add(this.instantiationService.createInstance(SelectBox,
			...
			{
				useCustomDrawn: true,
				ariaLabel: localize('copy profile from', "从以下位置复制配置文件"),
			}
		));
...
class ContentsProfileRenderer extends ProfilePropertyRenderer {
	...
	renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
		...
		const optionsLabel = $('.options-header', undefined, $('span', undefined, localize('options', "源")));
		...
		const updateDescription = (element: ProfileTreeElement) => {
			...
			if (element.root instanceof UserDataProfileElement && element.root.profile.isDefault) {
				...
				markdown.appendMarkdown(localize('contents source description', "为此配置文件配置内容源\r\n"));
				if (element.root instanceof NewProfileElement) {
					...
					if (optionName) {
						markdown
							.appendMarkdown(localize('copy info', "- *{0}:* 复制 {1} 配置文件中的内容\r\n", optionName, copyFromName));
					}
					markdown
						.appendMarkdown(localize('default info', "- *默认:* 使用默认配置文件中的内容\r\n"))
	...
	renderTemplate(parent: HTMLElement): IProfilePropertyRendererTemplate {
		...
		const updateTable = () => {
			if (profileElement?.root instanceof UserDataProfileElement && profileElement.root.workspaces?.length) {
				...
				profileWorkspacesDescriptionElement.textContent = localize('no_folder_description', "没有文件夹或工作区在使用此配置文件");
		...
		const renderRadioItems = () => {
			...
			if (root.copyFrom && name) {
				templateData.radio.setItems([
					{
						text: name,
						tooltip: name ? localize('copy from profile description', "复制 {1} 配置文件中的 {0}", resourceTypeTitle, name) : localize('copy description', "复制"),
					},
					...options
				]);
