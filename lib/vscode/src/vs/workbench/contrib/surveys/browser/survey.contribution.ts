...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(
		SurveyEditorPane,
		SurveyEditorPane.ID,
		localize('surveyEditorPaneTitle', "调查")
	),
...
class OpenSurveyAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('openSurvey', "打开调查"),
...
class SurveyAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor) {
		const editorService = accessor.get(IEditorService);
		const helpText = [
			localize('survey.help.overview', "你正在填写调查表。使用 Tab 键可在问题和选项之间移动。"),
			localize('survey.help.select', "在问题中，可使用方向键在选项之间移动，并按空格键或 Enter 进行选择。"),
			localize('survey.help.submit', "按 Tab 跳转到“提交”按钮，然后在回答完必填问题后按 Enter。附加问题可选答。"),
		].join('\n');
