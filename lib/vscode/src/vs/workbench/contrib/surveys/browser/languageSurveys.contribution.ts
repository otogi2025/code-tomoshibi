...
class LanguageSurvey extends Disposable {
	...
	constructor(
		...
		notificationService.prompt(
			Severity.Info,
			localize('helpUs', "帮助我们改善对 {0} 的支持", languageService.getLanguageName(data.languageId) ?? data.languageId),
			[{
				label: localize('takeShortSurvey', "参与小调查"),
				run: () => {
