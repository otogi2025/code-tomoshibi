...
export class AnythingQuickAccessProvider extends PickerQuickAccessProvider<IAnythingQuickPickItem> {
	...
	private static readonly NO_RESULTS_PICK: IAnythingQuickPickItem = {
		label: localize('noAnythingResults', "没有匹配的结果")
	};
	...
	private doGetPicks(
		...
		if (this.pickState.isQuickNavigating) {
			if (picks.length > 0) {
				picks.push({ type: 'separator', label: localize('recentlyOpenedSeparator', "最近打开") } satisfies IQuickPickSeparator);
			}
		...
		return {
			...
			additionalPicks: (async (): Promise<Picks<IAnythingQuickPickItem>> => {
				...
				return additionalPicks.length > 0 ? [
					{ type: 'separator', label: this.configuration.includeSymbols ? localize('fileAndSymbolResultsSeparator', "文件和符号结果") : localize('fileResultsSeparator', "文件结果") },
					...additionalPicks
				] : [];
			})(),
	...
	private createAnythingPick(resourceOrEditor: URI | EditorInput | IResourceEditorInput, configuration: { openSideBySideDirection: 'right' | 'down' | undefined }): IAnythingQuickPickItem {
		...
		const buttonsValue = new Lazy(() => {
			...
			buttons.push({
				...
				tooltip: openSideBySideDirection === 'right' ?
					localize({ key: 'openToSide', comment: ['Open this file in a split editor on the left/right side'] }, "打开转到侧边") :
					localize({ key: 'openToBottom', comment: ['Open this file in a split editor on the bottom'] }, "打开转到底部")
			});
			...
			if (isEditorHistoryEntry) {
				buttons.push({
					...
					tooltip: localize('closeEditor', "从最近打开中删除"),
		...
		return {
			resource,
			label,
			ariaLabel: isDirty ? localize('filePickAriaLabelDirty', "{0} 个未保存的更改", labelAndDescription) : labelAndDescription,
			description,
