			...
			const link = instantiationService.createInstance(Link, div, node, {
				opener: async href => {
					...
					if (parsed.scheme === Schemas.command && message.trusted) {
						...
						if (parsed.scheme === Schemas.command && !message.trusted) {
							notificationService.error(nls.localize('unable to open trust', "无法从不受信任的源打开命令链接: {0}", href));
						} else {
							notificationService.error(nls.localize('unable to open', "无法打开未知链接: {0}", href));
						}
					}
				}
			});
