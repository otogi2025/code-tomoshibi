...
registerAction2(class RestrictSearchToFolderAction extends Action2 {
	constructor() {
		super({
			...
			title: nls.localize2('restrictResultsToFolder', "将搜索限制为文件夹"),
			category,
			keybinding: {
...
registerAction2(class ExpandSelectedTreeCommandAction extends Action2 {
	constructor(
		...
		super({
			...
			title: nls.localize('search.expandRecursively', "以递归方式展开"),
			category,
			menu: [{
				id: MenuId.SearchContext,
...
registerAction2(class IncludeFileTypeInSearchAction extends Action2 {
	constructor() {
		super({
			...
			title: nls.localize2('includeFileTypeInSearch', "在搜索中包括文件类型"),
			category,
			menu: [
				{
					id: MenuId.SearchContext,
	...
	constructor(
		...
		super({
			...
			metadata: {
				...
				args: [
					{
						name: nls.localize('findInFiles.args', "搜索的一组选项"),
						schema: {
							type: 'object',
							properties: {
								query: { 'type': 'string' },
