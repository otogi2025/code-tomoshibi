...
export class SearchWidget extends Widget {
	...
	/** Fires on every keystroke while the "find in terminal" toggle is on. */
	private readonly _onTerminalSearchInputChanged = this._register(new Emitter<void>());
	readonly onTerminalSearchInputChanged: Event<void> = this._onTerminalSearchInputChanged.event;

	/** Fires on Enter (payload `false`) / Shift+Enter (payload `true`) while the toggle is on. */
	...
	private _updateSearchInputAriaLabel(includeHint: boolean): void {
		...
		let searchLabel = nls.localize('label.Search', '搜索: 键入搜索词，然后按 Enter 进行搜索');
		...
		if (includeHint && !this._accessibilityHelpHintAnnounced && this.configurationService.getValue('accessibility.verbosity.find') && this.accessibilityService.isScreenReaderOptimized()) {
			...
			if (keybinding) {
				searchLabel += ', ' + nls.localize('accessibilityHelpHintInLabel', "按 {0} 可获取辅助功能帮助。", keybinding);
				...
				this._labelResetTimeout = disposableTimeout(() => {
					if (this.searchInput) {
						this.searchInput.inputBox.setAriaLabel(nls.localize('label.Search', '搜索: 键入搜索词，然后按 Enter 进行搜索'));
					}
				}, 1000);
			}
		}
	...
	private renderToggleReplaceButton(parent: HTMLElement): void {
		const opts: IButtonOptions = {
			...
			title: nls.localize('search.replace.toggle.button.title', "切换替换"),
	...
	private renderSearchInput(parent: HTMLElement, options: ISearchWidgetOptions): void {
		...
		const inputOptions: IFindInputOptions = {
			label: nls.localize('label.Search', '搜索: 键入搜索词，然后按 Enter 进行搜索'),
			validation: (value: string) => this.validateSearchInput(value),
			placeholder: nls.localize('search.placeHolder', "搜索"),
		...
		this.terminalModeToggle = this._register(new Toggle({
			...
			icon: searchTerminalIcon,
			hoverLifecycleOptions,
			...defaultToggleStyles
		}));
		this.terminalModeToggle.domNode.classList.add('tomoshibi-terminal-toggle');
		this._register(this.terminalModeToggle.onChange(() => this._onDidToggleTerminalMode.fire(this.terminalModeToggle.checked)));

		if (options.showTerminalToggle) {
			searchInputContainer.classList.add('tomoshibi-terminal-toggle-host');
			dom.append(searchInputContainer, this.terminalModeToggle.domNode);
		}
	}

	/** Whether the search box currently targets the active terminal instead of the workspace files. */
	...
	private renderReplaceInput(parent: HTMLElement, options: ISearchWidgetOptions): void {
		...
		this.replaceInput = this._register(new ContextScopedReplaceInput(replaceBox, this.contextViewService, {
			label: nls.localize('label.Replace', '替换: 键入待替换词，然后按 Enter 进行预览'),
			placeholder: nls.localize('search.replace.placeHolder', "替换"),
	...
	private onSearchInputChanged(): void {
		...
		if (this.isTerminalMode()) {
			// In terminal mode the box drives the terminal search addon, never a file search.
			this._onTerminalSearchInputChanged.fire();
			return;
		}
	...
	private onSearchInputKeyDown(keyboardEvent: IKeyboardEvent) {
		if (this.isTerminalMode()) {
			// Enter / Shift+Enter drive the terminal search. Handled here (and not as a keybinding)
			// so that the event never reaches the workbench dispatcher and double-triggers a file search.
			if (keyboardEvent.equals(KeyCode.Enter)) {
				this._onTerminalFindNavigate.fire(false);
				keyboardEvent.preventDefault();
				keyboardEvent.stopPropagation();
				return;
			}
			if (keyboardEvent.equals(KeyMod.Shift | KeyCode.Enter)) {
				this._onTerminalFindNavigate.fire(true);
				keyboardEvent.preventDefault();
				keyboardEvent.stopPropagation();
				return;
			}
