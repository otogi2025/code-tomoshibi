...
const authenticationDefinitionSchema: IJSONSchema = {
	...
	properties: {
		id: {
			type: 'string',
			description: localize('authentication.id', '身份验证提供程序的 ID。')
		},
		label: {
			type: 'string',
			description: localize('authentication.label', '身份验证提供程序的易读名称。'),
		},
		authorizationServerGlobs: {
			type: 'array',
			items: {
				type: 'string',
				description: localize('authentication.authorizationServerGlobs', '与此提供程序支持的授权服务器匹配的 glob 列表。'),
			},
			description: localize('authentication.authorizationServerGlobsDescription', '与此提供程序支持的授权服务器匹配的 glob 列表。')
		}
	}
};
...
const authenticationExtPoint = ExtensionsRegistry.registerExtensionPoint<AuthenticationProviderInformation[]>({
	...
	jsonSchema: {
		description: localize({ key: 'authenticationExtensionPoint', comment: [`'Contributes' means adds here`] }, '添加身份验证'),
		type: 'array',
...
export class AuthenticationService extends Disposable implements IAuthenticationService {
			...
			added.forEach(point => {
				for (const provider of point.value) {
					if (isFalsyOrWhitespace(provider.id)) {
						point.collector.error(localize('authentication.missingId', '提供身份验证必须指定一个 ID。'));
						continue;
					}

					if (isFalsyOrWhitespace(provider.label)) {
						point.collector.error(localize('authentication.missingLabel', '提供身份验证必须指定一个标签。'));
						continue;
					}
					...
					if (!this.declaredProviders.some(p => p.id === provider.id)) {
						...
						point.collector.error(localize('authentication.idConflict', "已注册此身份验证 ID“{0}”", provider.id));
					}
				}
			});
	...
	registerDeclaredAuthenticationProvider(provider: AuthenticationProviderInformation): void {
		if (isFalsyOrWhitespace(provider.id)) {
			throw new Error(localize('authentication.missingId', '提供身份验证必须指定一个 ID。'));
		}
		if (isFalsyOrWhitespace(provider.label)) {
			throw new Error(localize('authentication.missingLabel', '提供身份验证必须指定一个标签。'));
		}
		if (this.declaredProviders.some(p => p.id === provider.id)) {
			throw new Error(localize('authentication.idConflict', "已注册此身份验证 ID“{0}”", provider.id));
		}
