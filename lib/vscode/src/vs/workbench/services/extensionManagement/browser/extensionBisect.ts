...
class ExtensionBisectUi {
	...
	private _showBisectPrompt(): void {
		...
		const message = this._extensionBisectService.disabledCount === 1
			? localize('bisect.singular', "Extension Bisect 处于活动状态，且已禁用 1 个扩展。请从这些选项中进行选择，以检查是否仍然可以重现问题并继续操作。")
			: localize('bisect.plural', "扩展二等分处于活动状态，已禁用 {0} 扩展。请从这些选项中进行选择，检查是否仍可重现问题并继续操作。", this._extensionBisectService.disabledCount);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('title.start', '开始扩展二等分'),
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const res = await dialogService.confirm({
			message: localize('msg.start', "扩展二等分"),
			detail: localize('detail.start', "扩展二等分将通过二分查找的方式确认引起问题的扩展。在此过程中，窗口将会不断重新加载(约{0}次)，每次都必须确认是否出现问题", 2 + Math.log2(extensions.length) | 0),
			primaryButton: localize({ key: 'msg2', comment: ['&& denotes a mnemonic'] }, "开始扩展二等分(&&S)")
		});
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('title.isBad', '继续扩展二等分'),
			category: Categories.Help,
			f1: true,
	...
	async run(accessor: ServicesAccessor, seeingBad: boolean | undefined): Promise<void> {
		...
		if (done.bad) {
			...
			await dialogService.info(
				localize('done.msg', "扩展二等分"),
				localize('done.detail2', "扩展二等分已完成，但未标识任何扩展。这可能是 {0} 的问题。", productService.nameShort)
			);
	...
	private async _checkForBad(dialogService: IDialogService, bisectService: IExtensionBisectService): Promise<boolean | undefined | null> {
		const { result } = await dialogService.prompt<boolean | undefined | null>({
			type: Severity.Info,
			message: localize('msg.next', "扩展二等分"),
			detail: localize('bisect', "扩展二等分处于活动状态，已禁用 {0} 扩展。请从这些选项中进行选择，检查是否仍可重现问题并继续操作。", bisectService.disabledCount),
			buttons: [
				{
					label: localize({ key: 'next.good', comment: ['&& denotes a mnemonic'] }, "我无法重现(&&N)"),
					run: () => false // good now
				},
				{
					label: localize({ key: 'next.bad', comment: ['&& denotes a mnemonic'] }, "我可以重现(&&R)"),
					run: () => true // bad
				},
				{
					label: localize({ key: 'next.stop', comment: ['&& denotes a mnemonic'] }, "停止二等分(&&S)"),
					run: () => undefined // stop
				}
			],
			cancelButton: {
				label: localize({ key: 'next.cancel', comment: ['&& denotes a mnemonic'] }, "取消二等分(&&C)"),
				run: () => null // cancel
			}
		});
		return result;
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('title.stop', '停止扩展二等分'),
			category: Categories.Help,
			f1: true,
			precondition: ExtensionBisectUi.ctxIsBisectActive
		});
