...
import { ExtensionType, IExtension, isLanguagePackExtension, isResolverExtension } from '../../../../platform/extensions/common/extensions.js';
...
export class ExtensionEnablementService extends Disposable implements IWorkbenchExtensionEnablementService {
	...
	constructor(
		...
		if (this.allUserExtensionsDisabled) {
			this.lifecycleService.when(LifecyclePhase.Eventually).then(() => {
				this.notificationService.prompt(Severity.Info, localize('extensionsDisabled', "已暂时禁用所有已安装的扩展。"), [{
					label: localize('Reload', "重新加载并启用扩展"),
	...
	private throwErrorIfCannotChangeEnablement(extension: IExtension, donotCheckDependencies?: boolean): void {
		if (isLanguagePackExtension(extension.manifest)) {
			throw new Error(localize('cannot disable language pack extension', "无法更改 {0} 扩展的启用，因为它提供语言包。", extension.manifest.displayName || extension.identifier.id));
		}

		if (this._isEnabledInEnv(extension)) {
			throw new Error(localize('cannot change enablement environment', "无法更改 {0} 扩展的启用，因为它已在环境中启用", extension.manifest.displayName || extension.identifier.id));
		}
	...
	private throwErrorIfEnablementStateCannotBeChanged(extension: IExtension, enablementStateOfExtension: EnablementState, donotCheckDependencies?: boolean): void {
		switch (enablementStateOfExtension) {
			case EnablementState.DisabledByEnvironment:
				throw new Error(localize('cannot change disablement environment', "无法更改 {0} 扩展的启用，因为它在环境中被禁用", extension.manifest.displayName || extension.identifier.id));
			case EnablementState.DisabledByMalicious:
				throw new Error(localize('cannot change enablement malicious', "无法更改 {0} 扩展的启用，因为它是恶意的", extension.manifest.displayName || extension.identifier.id));
			case EnablementState.DisabledByVirtualWorkspace:
				throw new Error(localize('cannot change enablement virtual workspace', "无法更改 {0} 扩展的启用，因为它不支持虚拟工作区", extension.manifest.displayName || extension.identifier.id));
			case EnablementState.DisabledByExtensionKind:
				throw new Error(localize('cannot change enablement extension kind', "由于扩展类型，{0} 扩展的启用无法更改", extension.manifest.displayName || extension.identifier.id));
			case EnablementState.DisabledByAllowlist:
				throw new Error(localize('cannot change disallowed extension enablement', "无法更改 {0} 扩展的启用，因为它是不允许的", extension.manifest.displayName || extension.identifier.id));
			case EnablementState.DisabledByInvalidExtension:
				throw new Error(localize('cannot change invalid extension enablement', "{0} 扩展的启用无法更改，因为它无效", extension.manifest.displayName || extension.identifier.id));
			case EnablementState.DisabledByExtensionDependency:
				...
				for (const dependency of getExtensionDependencies(this.extensionsManager.extensions, extension)) {
					...
					throw new Error(localize('cannot change enablement dependency', "无法启用“{0}”扩展，因为它依赖于无法启用的“{1}”扩展", extension.manifest.displayName || extension.identifier.id, dependency.manifest.displayName || dependency.identifier.id));
				}
		}
	}
	...
	private throwErrorIfCannotChangeWorkspaceEnablement(extension: IExtension): void {
		if (!this.hasWorkspace) {
			throw new Error(localize('noWorkspace', "没有工作区。"));
		}
