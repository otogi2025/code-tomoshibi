...
export class ExtensionManagementService extends CommontExtensionManagementService implements IWorkbenchExtensionManagementService {
	...
	private getDependentsErrorMessage(extension: ILocalExtension, dependents: ILocalExtension[]): string {
		if (dependents.length === 1) {
			return localize('singleDependentError', "无法卸载扩展程序“{0}”。扩展程序“{1}”依赖于此。",
				extension.manifest.displayName || extension.manifest.name, dependents[0].manifest.displayName || dependents[0].manifest.name);
		}
		if (dependents.length === 2) {
			return localize('twoDependentsError', "无法卸载扩展程序“{0}”。扩展程序“{1}”、“{2}”依赖于此。",
				extension.manifest.displayName || extension.manifest.name, dependents[0].manifest.displayName || dependents[0].manifest.name, dependents[1].manifest.displayName || dependents[1].manifest.name);
		}
		return localize('multipleDependentsError', "无法卸载扩展程序“{0}”。扩展程序“{1}”、“{2}”以及其他扩展程序都依赖于此。",
			extension.manifest.displayName || extension.manifest.name, dependents[0].manifest.displayName || dependents[0].manifest.name, dependents[1].manifest.displayName || dependents[1].manifest.name);
	...
	private async canInstallGalleryExtension(gallery: IGalleryExtension): Promise<true | IMarkdownString> {
		...
		return new MarkdownString().appendText(localize('cannot be installed', "无法安装“{0}”扩展，因为它在此安装程序中不可用。", gallery.displayName || gallery.name));
	}
	...
	private async canInstallResourceExtension(extension: IResourceExtension): Promise<true | IMarkdownString> {
		...
		return new MarkdownString().appendText(localize('cannot be installed', "无法安装“{0}”扩展，因为它在此安装程序中不可用。", extension.manifest.displayName ?? extension.identifier.id));
	}
	...
	async installGalleryExtensions(extensions: InstallExtensionInfo[]): Promise<InstallExtensionResult[]> {
		...
		const manifests = await Promise.all(extensions.map(async ({ extension }) => {
			...
			if (!manifest) {
				throw new Error(localize('Manifest is not found', "安装扩展 {0} 失败: 找不到清单文件。", extension.displayName || extension.name));
			}
			return manifest;
		}));
	...
	async installFromGallery(gallery: IGalleryExtension, installOptions?: InstallOptions, servers?: IExtensionManagementServer[]): Promise<ILocalExtension> {
		const manifest = await this.extensionGalleryService.getManifest(gallery, CancellationToken.None);
		if (!manifest) {
			throw new Error(localize('Manifest is not found', "安装扩展 {0} 失败: 找不到清单文件。", gallery.displayName || gallery.name));
		}
	...
	async getInstallableServers(gallery: IGalleryExtension): Promise<IExtensionManagementServer[]> {
		const manifest = await this.extensionGalleryService.getManifest(gallery, CancellationToken.None);
		if (!manifest) {
			return Promise.reject(localize('Manifest is not found', "安装扩展 {0} 失败: 找不到清单文件。", gallery.displayName || gallery.name));
		}
	...
	private validServers(gallery: IGalleryExtension, manifest: IExtensionManifest, servers: IExtensionManagementServer[]): IExtensionManagementServer[] {
		...
		for (const server of servers) {
			if (!installableServers.includes(server)) {
				const error = new Error(localize('cannot be installed in server', "无法安装“{0}”扩展，因为它在此安装程序“{1}”中不可用。", gallery.displayName || gallery.name, server.label));
	...
	private async getExtensionManagementServersToInstall(gallery: IGalleryExtension, manifest: IExtensionManifest): Promise<IExtensionManagementServer[]> {
		...
		if (!servers.length) {
			const error = new Error(localize('cannot be installed', "无法安装“{0}”扩展，因为它在此安装程序中不可用。", gallery.displayName || gallery.name));
			error.name = ExtensionManagementErrorCode.Unsupported;
			throw error;
		}
		...
		if (this.isExtensionsSyncEnabled()) {
			const { result } = await this.dialogService.prompt<boolean>({
				...
				detail: extensions.length === 1
					? localize('install single extension', "是否要跨设备安装并同步 \"{0}\" 扩展?", extensions[0].displayName)
					: localize('install multiple extensions', "是否要跨设备安装并同步扩展?"),
				buttons: [
					{
	...
	private async checkForTrustedPublishers(extensions: { extension: IGalleryExtension; manifest: IExtensionManifest; checkForPackAndDependencies: boolean }[]): Promise<void> {
		...
		const installButton: IPromptButton<void> = {
			label: allPublishers.length > 1 ? localize({ key: 'trust publishers and install', comment: ['&& denotes a mnemonic'] }, "信任发布者并安装") : localize({ key: 'trust and install', comment: ['&& denotes a mnemonic'] }, "信任发布者和安装"),
			run: () => {
		...
		const learnMoreButton: IPromptButton<void> = {
			label: localize({ key: 'learnMore', comment: ['&& denotes a mnemonic'] }, "了解详细信息(&&L)"),
			run: () => {
		...
		const title = allPublishers.length === 1
			? localize('checkTrustedPublisherTitle', "是否信任发布者 \"{0}\"？", allPublishers[0].publisherDisplayName)
			: allPublishers.length === 2
				? localize('checkTwoTrustedPublishersTitle', "是否信任发布者 \"{0}\" 和 \"{1}\"？", allPublishers[0].publisherDisplayName, allPublishers[1].publisherDisplayName)
				: localize('checkAllTrustedPublishersTitle', "是否信任发布者 \"{0}\" 和 {1} 其他人？", allPublishers[0].publisherDisplayName, allPublishers.length - 1);
		...
		if (untrustedExtensions.length === 1) {
			...
			if (otherUntrustedPublishers.length) {
				...
				customMessage.appendMarkdown(localize('message1', "扩展 {0} 由 {1} 发布。这是你从此发布者安装的第一个扩展。", `[${extension.displayName}](${extension.detailsLink})`, getPublisherLink(extension)));
			}
		} else {
			customMessage.appendMarkdown(localize('multiInstallMessage', "这是你第一次安装来自发布者 {0} 和 {1} 的扩展。", getPublisherLink(allPublishers[0]), getPublisherLink(allPublishers[allPublishers.length - 1])));
		}
		...
		if (verifiedPublishers.length || unverfiiedPublishers.length === 1) {
			for (const publisher of verifiedPublishers) {
				...
				const publisherVerifiedMessage = localize('verifiedPublisherWithName', "{0} 已验证 {1} 的所有权。", getPublisherLink(publisher), `[$(link-external) ${URI.parse(publisher.publisherDomain!.link).authority}](${publisher.publisherDomain!.link})`);
			...
			if (unverfiiedPublishers.length) {
				...
				if (unverfiiedPublishers.length === 1) {
					customMessage.appendMarkdown(`$(${Codicon.unverified.id})&nbsp;${localize('unverifiedPublisherWithName', "{0} 是 [**未验证]({1})。", getPublisherLink(unverfiiedPublishers[0]), unverifiedLink)}`);
				} else {
					customMessage.appendMarkdown(`$(${Codicon.unverified.id})&nbsp;${localize('unverifiedPublishers', "{0} 和 {1} 已[**未**验证]({2})。", unverfiiedPublishers.slice(0, unverfiiedPublishers.length - 1).map(p => getPublisherLink(p)).join(', '), getPublisherLink(unverfiiedPublishers[unverfiiedPublishers.length - 1]), unverifiedLink)}`);
				}
			}
		} else {
			customMessage.appendText('\n');
			customMessage.appendMarkdown(`$(${Codicon.unverified.id})&nbsp;${localize('allUnverifed', "所有发布者均已[**未**验证]({0})。", unverifiedLink)}`);
		}

		customMessage.appendText('\n');
		if (allPublishers.length > 1) {
			customMessage.appendMarkdown(localize('message4', "{0} 无法控制第三方扩展的行为，包括它们如何管理你的个人数据。仅在信任发布者时继续。", this.productService.nameLong));
		} else {
			customMessage.appendMarkdown(localize('message2', "{0} 无法控制第三方扩展的行为，包括它们如何管理你的个人数据。仅在信任发布者时继续。", this.productService.nameLong));
		}
	...
	private async checkForWorkspaceTrust(manifest: IExtensionManifest, requireTrust: boolean): Promise<void> {
		if (requireTrust || this.extensionManifestPropertiesService.getExtensionUntrustedWorkspaceSupportType(manifest) === false) {
			...
			buttons.push({ label: localize('extensionInstallWorkspaceTrustButton', "信任工作区 & 安装"), type: 'ContinueWithTrust' });
			if (!requireTrust) {
				buttons.push({ label: localize('extensionInstallWorkspaceTrustContinueButton', "安装"), type: 'ContinueWithoutTrust' });
			}
			buttons.push({ label: localize('extensionInstallWorkspaceTrustManageButton', "了解详细信息"), type: 'Manage' });
	...
	private async checkInstallingExtensionOnWeb(extension: IGalleryExtension, manifest: IExtensionManifest): Promise<void> {
		...
		const productName = localize('VS Code for Web', "{0} Web 版", this.productService.nameLong);
		...
		const limitedSupportMessage = localize('limited support', "“{0}”在 {1} 中具有有限的功能。", extension.displayName || extension.identifier.id, productName);
		let message: string;
	...
	async toLocalWorkspaceExtension(extension: IScannedExtension): Promise<ILocalExtension> {
		...
		if (extension.manifest.main) {
			if (!(await this.fileService.exists(this.uriIdentityService.extUri.joinPath(extension.location, extension.manifest.main)))) {
				isValid = false;
				validations.push([Severity.Error, localize('main.notFound', "无法激活，因为找不到 {0}", extension.manifest.main)]);
			}
		}
		return {
			identifier: extension.identifier,
