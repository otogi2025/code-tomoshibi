		...
		if (featureState.disabled === undefined) {
			...
			if (feature.access.requireUserConsent) {
				...
				const confirmationResult = await this.dialogService.confirm({
					title: localize('accessExtensionFeature', "访问“{0}”功能", feature.label),
					message: localize('accessExtensionFeatureMessage', "“{0}”扩展希望访问“{1}”功能。", extensionDescription?.displayName ?? extension._lower, feature.label),
					detail: justification ?? feature.description,
					custom: true,
					primaryButton: localize('allow', "允许"),
					cancelButton: localize('disallow', "不允许"),
				});
