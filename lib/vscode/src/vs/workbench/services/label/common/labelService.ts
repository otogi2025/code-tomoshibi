...
const resourceLabelFormattersExtPoint = ExtensionsRegistry.registerExtensionPoint<ResourceLabelFormatter[]>({
	...
	jsonSchema: {
		description: localize('vscode.extension.contributes.resourceLabelFormatters', '提供资源标签格式化规则。'),
		type: 'array',
		items: {
			type: 'object',
			...
			properties: {
				scheme: {
					type: 'string',
					description: localize('vscode.extension.contributes.resourceLabelFormatters.scheme', '要在其上匹配格式化程序的 URI 方案，例如“文件”。支持简单的 glob 模式。'),
				},
				authority: {
					type: 'string',
					description: localize('vscode.extension.contributes.resourceLabelFormatters.authority', '要在其上匹配格式化程序的 URI 权限。支持简单的 glob 模式。'),
				},
				formatting: {
					description: localize('vscode.extension.contributes.resourceLabelFormatters.formatting', "用于格式化 uri 资源标签的规则。"),
					type: 'object',
					properties: {
						label: {
							type: 'string',
							description: localize('vscode.extension.contributes.resourceLabelFormatters.label', "要显示的标签规则。例如，myLabel:/${path}。支持将 ${path}、${scheme}、${authority} 和 ${authoritySuffix} 用作变量。")
						},
						separator: {
							type: 'string',
							description: localize('vscode.extension.contributes.resourceLabelFormatters.separator', "要在 URI 标签显示中所用的分隔符，例如 / 或 ''。")
						},
						stripPathStartingSeparator: {
							type: 'boolean',
							description: localize('vscode.extension.contributes.resourceLabelFormatters.stripPathStartingSeparator', "控制 \"${path}\" 替换项是否应删除起始分隔符字符。")
						},
						tildify: {
							type: 'boolean',
							description: localize('vscode.extension.contributes.resourceLabelFormatters.tildify', "控制是否应在可能的情况下按斜体显示 URI 标签的开头。")
						},
						workspaceSuffix: {
							type: 'string',
							description: localize('vscode.extension.contributes.resourceLabelFormatters.formatting.workspaceSuffix', "附加到工作区标签的后缀。")
						}
					}
				}
			}
...
export class LabelService extends Disposable implements ILabelService {
	...
	private doGetWorkspaceLabel(workspaceUri: URI, options?: { verbose: Verbosity }): string {
		...
		if (isUntitledWorkspace(workspaceUri, this.environmentService)) {
			return localize('untitledWorkspace', "无标题 (工作区)");
		}

		// Workspace: Temporary
		if (isTemporaryWorkspace(workspaceUri)) {
			return localize('temporaryWorkspace', "工作区");
		}
		...
		switch (options?.verbose) {
			case Verbosity.SHORT:
				...
				break;
			case Verbosity.LONG:
				label = localize('workspaceNameVerbose', "{0} (工作区)", this.getUriLabel(joinPath(dirname(workspaceUri), filename)));
				break;
			case Verbosity.MEDIUM:
			default:
				label = localize('workspaceName', "{0} (工作区)", filename);
				break;
		}
