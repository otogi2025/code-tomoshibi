...
export class WebLocaleService implements ILocaleService {
	...
	async setLocale(languagePackItem: ILanguagePackItem, _skipDialog = false): Promise<void> {
		...
		const restartDialog = await this.dialogService.confirm({
			type: 'info',
			message: localize('relaunchDisplayLanguageMessage', "要更改显示语言，{0} 需要重新加载", this.productService.nameLong),
			detail: localize('relaunchDisplayLanguageDetail', "按「重新加载」刷新页面，并把显示语言设为 {0}。", languagePackItem.label),
			primaryButton: localize({ key: 'reload', comment: ['&& denotes a mnemonic character'] }, "重新加载"),
		});
	...
	async clearLocalePreference(): Promise<void> {
		...
		const restartDialog = await this.dialogService.confirm({
			type: 'info',
			message: localize('clearDisplayLanguageMessage', "要更改显示语言，{0} 需要重新加载", this.productService.nameLong),
			detail: localize('clearDisplayLanguageDetail', "按「重新加载」刷新页面，并使用浏览器的语言。"),
			primaryButton: localize({ key: 'reload', comment: ['&& denotes a mnemonic character'] }, "重新加载"),
		});
