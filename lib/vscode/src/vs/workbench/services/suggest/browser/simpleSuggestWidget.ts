...
export const SimpleSuggestContext = {
	HasFocusedSuggestion: new RawContextKey<boolean>('simpleSuggestWidgetHasFocusedSuggestion', false, localize('simpleSuggestWidgetHasFocusedSuggestion', "是否以任何简单建议为中心")),
	HasNavigated: new RawContextKey<boolean>('simpleSuggestWidgetHasNavigated', false, localize('simpleSuggestWidgetHasNavigated', "是否已向下导航简单建议小组件")),
	FirstSuggestionFocused: new RawContextKey<boolean>('simpleSuggestWidgetFirstSuggestionFocused', false, localize('simpleSuggestWidgetFirstSuggestionFocused', "第一个简单建议是否为重点")),
	ExplicitlyInvoked: new RawContextKey<boolean>('simpleSuggestWidgetExplicitlyInvoked', false, localize('simpleSuggestWidgetExplicitlyInvoked', "是否显式调用了简单建议小组件")),
};
...
export class SimpleSuggestWidget<TModel extends SimpleCompletionModel<TItem>, TItem extends SimpleCompletionItem> extends Disposable {
	...
	private static LOADING_MESSAGE: string = localize('suggestWidget.loading', "正在加载...");
	private static NO_SUGGESTIONS_MESSAGE: string = localize('suggestWidget.noSuggestions', "无建议。");
	...
	constructor(
		...
		this._list = this._register(new List<TItem>('SuggestWidget', this._listElement, {
			...
			accessibilityProvider: {
				...
				getWidgetAriaLabel: () => localize('suggest', "建议"),
				...
				getAriaLabel: (item: SimpleCompletionItem) => {
					...
					return localize('ariaCurrenttSuggestionReadDetails', "{0}，文档: {1}", label, docs);
				},
			}
		}));
