...
const SOURCE_SYSTEM = localize('default', "系统");
const SOURCE_EXTENSION = localize('extension', "扩展");
const SOURCE_USER = localize('user', "用户");
...
class KeybindingItemMatches {
	...
	private wordMatchesAltModifier(word: string): boolean {
		...
		if (strings.equalsIgnoreCase(localize('option', "选项"), word)) {
			return true;
		}
		return false;
	}
	...
	private wordMatchesMetaModifier(word: string): boolean {
		...
		if (strings.equalsIgnoreCase(localize('meta', "元数据"), word)) {
			return true;
		}
		return false;
	}
