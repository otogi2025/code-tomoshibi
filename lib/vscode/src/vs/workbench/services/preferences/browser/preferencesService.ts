...
export class PreferencesService extends Disposable implements IPreferencesService {
	...
	openWorkspaceSettings(options: IOpenSettingsOptions = {}): Promise<IEditorPane | undefined> {
		if (!this.workspaceSettingsResource) {
			this.notificationService.info(nls.localize('openFolderFirst', "首先打开一个文件夹或工作区，以创建工作区或文件夹设置。"));
	...
	async openGlobalKeybindingSettings(textual: boolean, options?: IOpenKeybindingsEditorOptions): Promise<void> {
		...
		if (textual) {
			const emptyContents = '// ' + nls.localize('emptyKeybindingsHeader', "将键绑定放在此文件中以覆盖默认值") + '\n[\n]';
			...
			if (openDefaultKeybindings) {
				...
				await Promise.all([
					this.editorService.openEditor({ resource: this.defaultKeybindingsResource, options: { pinned: true, preserveFocus: true, revealIfOpened: true, override: DEFAULT_EDITOR_ASSOCIATION.id }, label: nls.localize('defaultKeybindings', "默认的键绑定"), description: '' }, sourceGroupId),
	...
	openDefaultKeybindingsFile(): Promise<IEditorPane | undefined> {
		return this.editorService.openEditor({ resource: this.defaultKeybindingsResource, label: nls.localize('defaultKeybindings', "默认的键绑定") });
	}
		...
		try {
			...
			if ((<FileOperationError>error).fileOperationResult === FileOperationResult.FILE_NOT_FOUND) {
				try {
					...
					throw new Error(nls.localize('fail.createSettings', "无法创建“{0}”({1})。", this.labelService.getUriLabel(resource, { relative: true }), getErrorMessage(error2)));
				}
			} else {
				throw error;
			}
