...
export class ViewsService extends Disposable implements IViewsService {
	...
	getFocusedViewName(): string {
		const textEditorFocused = this.editorService.activeTextEditorControl?.hasTextFocus() ? localize('editor', "文本编辑器") : undefined;
					...
					super({
						...
						get title(): ICommandActionTitle {
							...
							if (viewContainerLocation === ViewContainerLocation.Sidebar) {
								return { value: localize('show view', "显示 {0}", localizedTitle), original: `Show ${originalTitle}` };
							} else {
								return { value: localize('toggle view', "切换 {0}", localizedTitle), original: `Toggle ${originalTitle}` };
							}
						},
		...
		disposables.add(registerAction2(class OpenViewAction extends Action2 {
			constructor() {
				super({
					...
					metadata: {
						description: localize('open view', "打开视图 {0}", viewDescriptor.name.value),
						args: [
							{
								name: 'options',
								schema: {
									...
									properties: {
										'preserveFocus': {
											type: 'boolean',
											default: false,
											description: localize('preserveFocus', "打开视图时是否保留现有焦点。")
										}
									},
								}
							}
	...
	private registerFocusViewAction(viewDescriptor: IViewDescriptor, category?: string | ILocalizedString): IDisposable {
		return registerAction2(class FocusViewAction extends Action2 {
			constructor() {
				const title = localize2({ key: 'focus view', comment: ['{0} indicates the name of the view to be focused.'] }, "焦点在 {0} 视图上", viewDescriptor.name.value);
				super({
		...
		return registerAction2(class ResetViewLocationAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('resetViewLocation', "重置位置"),
					menu: [{
