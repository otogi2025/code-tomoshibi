...
export class EditorResolverService extends Disposable implements IEditorResolverService {
	...
	private async doHandleConflictingDefaults(resource: URI, editorName: string, untypedInput: IUntypedEditorInput, currentEditor: EditorInput, group: IEditorGroup) {
		...
		const handle = this.notificationService.prompt(Severity.Warning,
			localize('editorResolver.conflictingDefaults', '此资源可使用多个默认编辑器。'),
			[{
				label: localize('editorResolver.configureDefault', '配置默认设置'),
				run: async () => {
			...
			{
				label: localize('editorResolver.keepDefault', '保留“{0}”', editorName),
	...
	private mapEditorsToQuickPickEntry(resource: URI, showDefaultPicker: boolean | undefined, associationType: EditorAssociationType) {
		...
		const currentlyActiveLabel = localize('promptOpenWith.currentlyActive', "活动");
		const currentDefaultLabel = localize('promptOpenWith.currentDefault', "默认");
		const currentDefaultAndActiveLabel = localize('promptOpenWith.currentDefaultAndActive', "活动和默认");
		...
		if (!showDefaultPicker && extname(resource) !== '') {
			...
			const configureDefaultEntry = {
				...
				label: localize('promptOpenWith.configureDefault', "为 \"{0}\" 配置默认编辑器...", `*${extname(resource)}`),
			};
			...
			if (associationType === EditorAssociationType.DiffEditor) {
				const configureDefaultDiffEntry = {
					...
					label: localize('promptOpenWith.configureDefaultDiff', "为“{0}”配置默认编辑器(仅差异)...", `*${extname(resource)}`),
				};
	...
	private async doPickEditor(editor: IUntypedEditorInput, showDefaultPicker?: boolean, updateAssociationType?: EditorAssociationType): Promise<IEditorOptions | undefined> {
		...
		const placeHolderMessage = showDefaultPicker ?
			(updateSettingType === EditorAssociationType.DiffEditor ?
				localize('promptOpenWith.updateDefaultDiffPlaceHolder', "为“{0}”选择新的默认编辑器(仅差异)", `*${extname(resource)}`) :
				localize('promptOpenWith.updateDefaultPlaceHolder', "为 \"{0}\" 选择新的默认编辑器", `*${extname(resource)}`)) :
			localize('promptOpenWith.placeHolder', "为“{0}”选择编辑器", basename(resource));
