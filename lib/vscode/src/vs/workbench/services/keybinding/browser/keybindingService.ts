...
function isValidContributedKeyBinding(keyBinding: ContributedKeyBinding, rejects: string[]): boolean {
	if (!keyBinding) {
		rejects.push(nls.localize('nonempty', "应为非空值。"));
		return false;
	}
	if (typeof keyBinding.command !== 'string') {
		rejects.push(nls.localize('requirestring', "属性“{0}”是必需的，其类型必须是 \"string\"", 'command'));
		return false;
	}
	if (keyBinding.key && typeof keyBinding.key !== 'string') {
		rejects.push(nls.localize('optstring', "属性“{0}”可以省略，否则其类型必须是 \"string\"", 'key'));
		return false;
	}
	if (keyBinding.when && typeof keyBinding.when !== 'string') {
		rejects.push(nls.localize('optstring', "属性“{0}”可以省略，否则其类型必须是 \"string\"", 'when'));
		return false;
	}
	if (keyBinding.mac && typeof keyBinding.mac !== 'string') {
		rejects.push(nls.localize('optstring', "属性“{0}”可以省略，否则其类型必须是 \"string\"", 'mac'));
		return false;
	}
	if (keyBinding.linux && typeof keyBinding.linux !== 'string') {
		rejects.push(nls.localize('optstring', "属性“{0}”可以省略，否则其类型必须是 \"string\"", 'linux'));
		return false;
	}
	if (keyBinding.win && typeof keyBinding.win !== 'string') {
		rejects.push(nls.localize('optstring', "属性“{0}”可以省略，否则其类型必须是 \"string\"", 'win'));
		return false;
	}
	return true;
}
...
const keybindingType = {
	...
	properties: {
		command: {
			description: nls.localize('vscode.extension.contributes.keybindings.command', '要在触发键绑定时运行的命令的标识符。'),
			type: 'string'
		},
		args: {
			description: nls.localize('vscode.extension.contributes.keybindings.args', "要传递给命令以执行的参数。")
		},
		key: {
			description: nls.localize('vscode.extension.contributes.keybindings.key', '键或键序列(用加号连接的键和后面再接空格的键序列都算组合键，如 Ctrl+O 和 Ctrl+L L)。'),
			type: 'string'
		},
		mac: {
			description: nls.localize('vscode.extension.contributes.keybindings.mac', 'Mac 特定的键或键序列。'),
			type: 'string'
		},
		linux: {
			description: nls.localize('vscode.extension.contributes.keybindings.linux', 'Linux 特定的键或键序列。'),
			type: 'string'
		},
		win: {
			description: nls.localize('vscode.extension.contributes.keybindings.win', 'Windows 特定的键或键序列。'),
			type: 'string'
		},
		when: {
			description: nls.localize('vscode.extension.contributes.keybindings.when', '键处于活动状态时的条件。'),
			type: 'string'
		},
	}
...
const keybindingsExtPoint = ExtensionsRegistry.registerExtensionPoint<ContributedKeyBinding | ContributedKeyBinding[]>({
	...
	jsonSchema: {
		description: nls.localize('vscode.extension.contributes.keybindings', "用于键绑定。"),
		oneOf: [
			keybindingType,
			{
				type: 'array',
...
export class WorkbenchKeybindingService extends AbstractKeybindingService {
	...
	private static _getAllCommandsAsComment(boundCommands: Map<string, boolean>): string {
		...
		return '// ' + nls.localize('unboundCommands', "以下是其他可用命令:") + '\n// - ' + pretty;
	}
...
class KeybindingsJsonSchema {
	...
	private readonly schema: IJSONSchema = {
		...
		type: 'array',
		title: nls.localize('keybindings.json.title', "按键绑定配置"),
		...
		definitions: {
			...
			'commandNames': {
				...
				'description': nls.localize('keybindings.json.command', "要执行的命令的名称"),
			},
			'commandType': {
				'anyOf': [ // repetition of this clause here and below is intentional: one is for nice diagnostics & one is for code completion
					...
					},
					{
						...
						'enumDescriptions': this.commandsEnumDescriptions,
						'description': nls.localize('keybindings.json.removalCommand', "要为其删除键盘快捷方式的命令的名称"),
					},
					{
						'type': 'string'
					},
		...
		items: {
			...
			'properties': {
				'key': {
					'type': 'string',
					'description': nls.localize('keybindings.json.key', "键或键序列(用空格分隔)"),
				},
				'command': {
					'anyOf': [
						{
							...
							'then': {
								...
								'errorMessage': nls.localize('keybindings.commandsIsArray', "类型错误。应为 \"{0}\"。字段 \"command\" 不支持运行多个命令。使用命令 \"runCommands\" 以向它传递多个要运行的命令。", 'string')
							},
							'else': {
				...
				'when': {
					'type': 'string',
					'description': nls.localize('keybindings.json.when', "键处于活动状态时的条件。")
				},
				'args': {
					'description': nls.localize('keybindings.json.args', "要传递给命令以执行的参数。")
				},
				'systemWide': {
					'type': 'boolean',
					'default': false,
					'markdownDescription': nls.localize('keybindings.json.systemWide', "当为 `true` 时，将此键绑定注册为系统范围(操作系统全局)快捷方式，即使应用程序未处于焦点状态时也会触发。仅桌面。仅支持单键组合(无和弦)，并且全局触发器会忽略任何 `when` 子句。")
				}
			},
