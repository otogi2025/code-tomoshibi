...
export class BrowserKeyboardMapperFactoryBase extends Disposable {
	...
	setActiveKeyMapping(keymap: IKeyboardMapping | null) {
		...
		if (matchedKeyboardLayout) {
			...
			// 	Severity.Info,
			// 	nls.localize('missing.keyboardlayout', '找不到匹配的键盘布局'),
			// 	[{
			// 		label: nls.localize('keyboardLayoutMissing.configure', "配置"),
			// 		run: () => this._commandService.executeCommand('workbench.action.openKeyboardLayoutPicker')
			// 	}, {
			// 		label: nls.localize('neverAgain', "不再显示"),
			// 		isSecondary: true,
...
const keyboardConfiguration: IConfigurationNode = {
	...
	'properties': {
		'keyboard.layout': {
			'type': 'string',
			'default': 'autodetect',
			'description': nls.localize('keyboard.layout.config', "控制网页版使用的键盘布局。")
		}
	}
};
