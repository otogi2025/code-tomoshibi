...
export abstract class AbstractFileDialogService implements IFileDialogService {
	...
	private async doShowSaveConfirm(fileNamesOrResources: (string | URI)[]): Promise<ConfirmResult> {
		...
		let message: string;
		let detail = nls.localize('saveChangesDetail', "如果不保存，你的更改将丢失。");
		if (fileNamesOrResources.length === 1) {
			message = nls.localize('saveChangesMessage', "是否要保存对 {0} 的更改?", typeof fileNamesOrResources[0] === 'string' ? fileNamesOrResources[0] : resources.basename(fileNamesOrResources[0]));
		} else {
			message = nls.localize('saveChangesMessages', "是否要保存对下列 {0} 个文件的更改?", fileNamesOrResources.length);
			...
			buttons: [
				{
					label: fileNamesOrResources.length > 1 ?
						nls.localize({ key: 'saveAll', comment: ['&& denotes a mnemonic'] }, "全部保存(&&S)") :
						nls.localize({ key: 'save', comment: ['&& denotes a mnemonic'] }, "保存(&&S)"),
					run: () => ConfirmResult.SAVE
				},
				{
					label: nls.localize({ key: 'dontSave', comment: ['&& denotes a mnemonic'] }, "不保存(&&N)"),
	...
	protected async pickFileFolderAndOpenSimplified(schema: string, options: IPickAndOpenOptions, preferNewWindow: boolean): Promise<void> {
		const title = nls.localize('openFileOrFolder.title', '打开文件或文件夹');
	...
	protected async pickFileAndOpenSimplified(schema: string, options: IPickAndOpenOptions, preferNewWindow: boolean): Promise<void> {
		const title = nls.localize('openFile.title', '打开文件');
		const availableFileSystems = this.addFileSchemaIfNeeded(schema);
	...
	protected async pickFolderAndOpenSimplified(schema: string, options: IPickAndOpenOptions): Promise<void> {
		const title = nls.localize('openFolder.title', '打开文件夹');
	...
	protected async pickWorkspaceAndOpenSimplified(schema: string, options: IPickAndOpenOptions): Promise<void> {
		const title = nls.localize('openWorkspace.title', '从文件打开工作区');
		const filters: FileFilter[] = [{ name: nls.localize('filterName.workspace', '工作区'), extensions: [WORKSPACE_EXTENSION] }];
		const availableFileSystems = this.addFileSchemaIfNeeded(schema, true);
	...
	protected async pickFileToSaveSimplified(schema: string, options: ISaveDialogOptions): Promise<URI | undefined> {
		...
		options.title = nls.localize('saveFileAs.title', '另存为');
	...
	protected getPickFileToSaveDialogOptions(defaultUri: URI, availableFileSystems?: string[]): ISaveDialogOptions {
		const options: ISaveDialogOptions = {
			defaultUri,
			title: nls.localize('saveAsTitle', "另存为"),
		...
		options.filters = coalesce([
			{ name: nls.localize('allFiles', "所有文件"), extensions: ['*'] },
			matchingFilter,
			...registeredLanguageFilters,
			{ name: nls.localize('noExt', "无扩展"), extensions: [''] }
		]);
