...
export class FileDialogService extends AbstractFileDialogService implements IFileDialogService {
	...
	async pickFileFolderAndOpen(options: IPickAndOpenOptions): Promise<void> {
		...
		throw new Error(localize('pickFolderAndOpen', "无法打开文件夹，请改为把文件夹添加到工作区。"));
	}
	...
	async pickWorkspaceAndOpen(options: IPickAndOpenOptions): Promise<void> {
		...
		throw new Error(localize('pickWorkspaceAndOpen', "无法打开工作区文件，请改为把文件夹添加到工作区。"));
	}
	...
	private async showUnsupportedBrowserWarning(context: 'save' | 'open'): Promise<undefined> {
		...
		const buttons: IPromptButton<void>[] = [
			{
				label: localize({ key: 'learnMore', comment: ['&& denotes a mnemonic'] }, "了解更多"),
				run: async () => { await this.openerService.open('https://aka.ms/VSCodeWebLocalFileSystemAccess'); }
			}
		];
		if (context === 'open') {
			buttons.push({
				label: localize({ key: 'openFiles', comment: ['&& denotes a mnemonic'] }, "打开文件…"),
				run: async () => {
		...
		await this.dialogService.prompt({
			...
			message: localize('unsupportedBrowserMessage', "不支持打开本地文件夹"),
			detail: localize('unsupportedBrowserDetail', "你的浏览器不支持打开本地文件夹。\n可以打开单个文件，或打开远程仓库。"),
			buttons
		});
