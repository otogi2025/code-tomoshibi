...
export namespace OpenLocalFileCommand {
	...
	export const LABEL = nls.localize('openLocalFile', "打开本地文件...");
...
export namespace SaveLocalFileCommand {
	...
	export const LABEL = nls.localize('saveLocalFile', "保存本地文件...");
	export function handler(): ICommandHandler {
		return accessor => {
...
export namespace OpenLocalFolderCommand {
	...
	export const LABEL = nls.localize('openLocalFolder', "打开本地文件夹...");
	export function handler(): ICommandHandler {
		return accessor => {
			const dialogService = accessor.get(IFileDialogService);
...
export namespace OpenLocalFileFolderCommand {
	...
	export const LABEL = nls.localize('openLocalFileFolder', "打开本地...");
	export function handler(): ICommandHandler {
		return accessor => {
			const dialogService = accessor.get(IFileDialogService);
...
export class SimpleFileDialog extends Disposable implements ISimpleFileDialog {
	...
	private getOptions(options: ISaveDialogOptions | IOpenDialogOptions, isSave: boolean = false): IOpenDialogOptions | undefined {
		...
		if ((this.scheme !== Schemas.file) && !this.fileService.hasProvider(defaultUri)) {
			this.notificationService.info(nls.localize('remoteFileDialog.notConnectedToRemote', '{0} 的文件系统提供程序不可用。', defaultUri.toString()));
			return undefined;
		}
	...
	private async pickResource(isSave: boolean = false): Promise<URI[] | URI | undefined> {
		...
		return new Promise<URI[] | URI | undefined>((resolve) => {
			...
			this.filePickBox.placeholder = nls.localize('remoteFileDialog.placeholder', "文件夹路径");
			...
			if ((isSave && this.environmentService.isEnabledFileDownloads) || (!isSave && this.environmentService.isEnabledFileUploads)) {
				if ((this.scheme !== Schemas.file) && this.options && this.options.availableFileSystems && (this.options.availableFileSystems.length > 1) && (this.options.availableFileSystems.indexOf(Schemas.file) > -1)) {
					...
					this.filePickBox.customLabel = nls.localize('remoteFileDialog.local', '显示本地');
	...
	private setButtons() {
		this.filePickBox.buttons = [{
			...
			tooltip: this._showDotFiles ? nls.localize('remoteFileDialog.hideDotFiles', "隐藏点文件") : nls.localize('remoteFileDialog.showDotFiles', "显示点文件"),
			alwaysVisible: true
		}];
	}
		...
		if ((value.length > 0) && ((value[0] === '~') || this.canTildaEscapeHatch(value))) {
			...
			if ((!newFolderIsOldFolder && (this.endsWithSlash(value) || newFolderIsParent || newFolderIsUnrelated)) || reset) {
				...
				if (stat?.isDirectory && (resources.basename(valueUri) !== '.') && this.endsWithSlash(value)) {
					...
					this.filePickBox.validationMessage = nls.localize('remoteFileDialog.badPath', '路径不存在。使用 ~ 转到主目录。');
	...
	private yesNoPrompt(uri: URI, message: string): Promise<boolean> {
		...
		prompt.customLabel = nls.localize('remoteFileDialog.cancel', '取消');
	...
	private async validate(uri: URI | undefined): Promise<boolean> {
		if (uri === undefined) {
			this.filePickBox.validationMessage = nls.localize('remoteFileDialog.invalidPath', '请输入有效路径。');
		...
		if (this.requiresTrailing) { // save
			if (stat?.isDirectory) {
				// Can't do this
				this.filePickBox.validationMessage = nls.localize('remoteFileDialog.validateFolder', '该文件夹已存在。请使用新的文件名。');
				return false;
			} else if (stat) {
				// Replacing a file.
				// Show a yes/no prompt
				const message = nls.localize('remoteFileDialog.validateExisting', '{0} 已存在。是否确实要覆盖?', resources.basename(uri));
				return this.yesNoPrompt(uri, message);
			} else if (!(isValidBasename(resources.basename(uri), this.isWindows))) {
				// Filename not allowed
				this.filePickBox.validationMessage = nls.localize('remoteFileDialog.validateBadFilename', '请输入有效的文件名。');
				return false;
			} else if (!statDirname) {
				// Folder to save in doesn't exist
				const message = nls.localize('remoteFileDialog.validateCreateDirectory', '文件夹 {0} 不存在。是否要创建它?', resources.basename(resources.dirname(uri)));
				return this.yesNoPrompt(uri, message);
			} else if (!statDirname.isDirectory) {
				this.filePickBox.validationMessage = nls.localize('remoteFileDialog.validateNonexistentDir', '请输入已存在的路径。');
				return false;
			} else if (statDirname.readonly) {
				this.filePickBox.validationMessage = nls.localize('remoteFileDialog.validateReadonlyFolder', '此文件夹不能用作保存目标。请选择其他文件夹');
				return false;
			}
		} else { // open
			if (!stat) {
				...
				if (this.allowFolderSelection && !this.allowFileSelection
					...
					const message = nls.localize('remoteFileDialog.validateCreateDirectoryOpen', '文件夹 {0} 不存在。是否要创建它?', resources.basename(uri));
					...
					try {
						...
						this.filePickBox.validationMessage = nls.localize('remoteFileDialog.createFolderFailed', '无法创建文件夹: {0}', e.message);
						return false;
					}
				}
				...
				this.filePickBox.validationMessage = nls.localize('remoteFileDialog.windowsDriveLetter', '路径开头请使用驱动器号。');
				return false;
			} else if (stat.isDirectory && !this.allowFolderSelection) {
				// Folder selected when folder selection not permitted
				this.filePickBox.validationMessage = nls.localize('remoteFileDialog.validateFileOnly', '请选择文件。');
				return false;
			} else if (!stat.isDirectory && !this.allowFileSelection) {
				// File selected when file selection not permitted
				this.filePickBox.validationMessage = nls.localize('remoteFileDialog.validateFolderOnly', '请选择一个文件夹。');
				return false;
			}
		}
		return true;
