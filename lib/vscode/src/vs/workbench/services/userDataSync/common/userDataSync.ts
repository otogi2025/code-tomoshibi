...
export function getSyncAreaLabel(source: SyncResource): string {
	switch (source) {
		...
		case SyncResource.Prompts: return localize('prompts', "提示和指令");
		...
		case SyncResource.Profiles: return localize('profiles', "个人资料");
...
export const SYNC_VIEW_ICON = registerIcon('settings-sync-view-icon', Codicon.sync, localize('syncViewIcon', '查看“设置同步”视图的图标。'));
