...
export abstract class AbstractTextFileService extends Disposable implements ITextFileService {
	...
	private static readonly TEXTFILE_SAVE_CREATE_SOURCE = SaveSourceRegistry.registerSource('textFileCreate.source', localize('textFileCreate.source', "文件已创建"));
	private static readonly TEXTFILE_SAVE_REPLACE_SOURCE = SaveSourceRegistry.registerSource('textFileOverwrite.source', localize('textFileOverwrite.source', "文件已替换"));
	...
	private provideDecorations(): void {
		...
		const provider = this._register(new class extends Disposable implements IDecorationsProvider {
			...
			readonly label = localize('textFileModelDecorations', "文本文件模型装饰");
			...
			provideDecorations(uri: URI): IDecorationData | undefined {
				...
				if (isReadonly && isOrphaned) {
					return {
						...
						strikethrough: true,
						tooltip: localize('readonlyAndDeleted', "已删除，只读"),
					};
				}
	...
	private async doRead(resource: URI, options?: IReadTextFileOptions & { preferUnbuffered?: boolean }): Promise<[IFileStreamContent, IDecodeStreamResult]> {
		...
		try {
			...
			if ((<DecodeStreamError>error).decodeStreamErrorKind === DecodeStreamErrorKind.STREAM_IS_BINARY) {
				throw new TextFileOperationError(localize('fileBinaryError', "文件似乎是二进制文件，不能作为文本打开"), TextFileOperationResult.FILE_IS_BINARY, options);
			}
	...
	private async confirmOverwrite(resource: URI): Promise<boolean> {
		const { confirmed } = await this.dialogService.confirm({
			type: 'warning',
			message: localize('confirmOverwrite', "“{0}”已存在。是否替换它?", basename(resource)),
			detail: localize('overwriteIrreversible', "名为\"{0}\"的文件或文件夹已存在于\"{1}\"文件夹中。替换它将覆盖其当前内容。", basename(resource), basename(dirname(resource))),
	...
	private async confirmMakeWriteable(resource: URI): Promise<boolean> {
		const { confirmed } = await this.dialogService.confirm({
			type: 'warning',
			message: localize('confirmMakeWriteable', "“{0}”已标记为只读。是否仍要保存?", basename(resource)),
			detail: localize('confirmMakeWriteableDetail', "可通过设置将路径配置为只读。"),
			primaryButton: localize({ key: 'makeWriteableButtonLabel', comment: ['&& denotes a mnemonic'] }, "仍然保存(&&S)")
		});
