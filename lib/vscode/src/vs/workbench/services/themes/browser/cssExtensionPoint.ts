...
const cssExtensionPoint = ExtensionsRegistry.registerExtensionPoint<ICSSExtensionPoint[]>({
	extensionPoint: 'css',
	jsonSchema: {
		description: nls.localize('contributes.css', "提供要在工作台加载的 CSS 文件。"),
		type: 'array',
		items: {
			type: 'object',
			properties: {
				path: {
					description: nls.localize('contributes.css.path', "CSS 文件的路径。该路径是相对于扩展文件夹的。"),
					type: 'string'
				}
			},
			required: ['path']
		...
		cssExtensionPoint.setHandler((extensions, delta) => {
			...
			for (const extension of delta.added) {
				...
				if (!extensionValue || !Array.isArray(extensionValue)) {
					collector.error(nls.localize('invalid.css.configuration', "\"contributes.css\" 必须是数组。"));
					continue;
				}
	...
	private activateExtensionCSS(extension: IExtensionPointUser<ICSSExtensionPoint[]>): void {
		...
		for (const cssContribution of extensionValue) {
			if (!cssContribution.path || typeof cssContribution.path !== 'string') {
				collector.error(nls.localize('invalid.css.path', "\"contributes.css.path\" 必须是字符串。"));
				continue;
			}
			...
			if (!resources.isEqualOrParent(cssLocation, extensionLocation)) {
				collector.warn(nls.localize('invalid.css.path.location', "预期 \"contributes.css.path\" ({0})将包含在扩展的文件夹({1})中。", cssLocation.path, extensionLocation.path));
				continue;
			}
