...
export class WorkbenchThemeService extends Disposable implements IWorkbenchThemeService {
	...
	private async showNewDefaultThemeNotification(previousSettingsId: string | undefined): Promise<void> {
		...
		const keepTheme = await new Promise(resolve => {
			this.notificationService.prompt(
				Severity.Info,
				nls.localize({ key: 'themeUpdatedNotification', comment: ['{0} is the name of the new default theme'] }, "VS Code 具有新的默认主题:“{0}”。", this.getColorTheme().label),
				[
					toAction({
						id: 'themeUpdated.tryItOut',
						label: nls.localize('tryNewTheme', "保留它"),
						run: () => resolve(true)
					}),
					toAction({
						id: 'themeUpdated.noThanks',
						label: nls.localize('noThanks', "不，谢谢"),
						run: () => resolve(false)
					})
				],
				{
	...
	private async internalSetColorTheme(themeIdOrTheme: string | undefined | IWorkbenchColorTheme, settingsTarget: ThemeSettingTarget): Promise<IWorkbenchColorTheme | null> {
		...
		try {
			...
			throw new Error(nls.localize('error.cannotloadtheme', "无法加载 {0}: {1}", themeData.location?.toString(), error.message));
		}
