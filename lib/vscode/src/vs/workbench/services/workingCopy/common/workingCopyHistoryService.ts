...
export class WorkingCopyHistoryModel {
	...
	private static readonly FILE_SAVED_SOURCE = SaveSourceRegistry.registerSource('default.source', localize('default.source', "已保存文件"));
...
export abstract class WorkingCopyHistoryService extends Disposable implements IWorkingCopyHistoryService {
	...
	private static readonly FILE_MOVED_SOURCE = SaveSourceRegistry.registerSource('moved.source', localize('moved.source', "已移动文件"));
	private static readonly FILE_RENAMED_SOURCE = SaveSourceRegistry.registerSource('renamed.source', localize('renamed.source', "已重命名文件"));
...
export class NativeWorkingCopyHistoryService extends WorkingCopyHistoryService {
	...
	private onWillShutdown(e: WillShutdownEvent): void {
		...
		e.join(this.storeAll(e.token), { id: 'join.workingCopyHistory', label: localize('join.workingCopyHistory', "正在保存本地历史记录") });
	}
