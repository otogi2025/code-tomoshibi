...
export class StoredFileWorkingCopy<M extends IStoredFileWorkingCopyModel> extends ResourceWorkingCopy implements IStoredFileWorkingCopy<M> {
	...
	private async doSave(options: IStoredFileWorkingCopySaveAsOptions): Promise<void> {
		...
		return this.progressService.withProgress({
			title: localize('saveParticipants', "正在保存“{0}”", this.name),
	...
	private doSaveSequential(versionId: number, options: IStoredFileWorkingCopySaveAsOptions, progress: IProgress<IProgressStep>, saveCancellation: CancellationTokenSource): Promise<void> {
		return this.saveSequentializer.run(versionId, (async () => {
			...
			progress.report({ message: localize('saveTextFile', "正在写入文件...") });
	...
	private doHandleSaveError(error: Error, options: IStoredFileWorkingCopySaveAsOptions): void {
		...
		if (fileOperationError.fileOperationResult === FileOperationResult.FILE_MODIFIED_SINCE) {
			message = localize('staleSaveError', "未能保存 \"{0}\": 文件的内容较新。是否要用所做的更改覆盖该文件?", this.name);

			primaryActions.push(toAction({ id: 'fileWorkingCopy.overwrite', label: localize('overwrite', "覆盖"), run: () => this.save({ ...options, ignoreModifiedSince: true, reason: SaveReason.EXPLICIT }) }));
			primaryActions.push(toAction({ id: 'fileWorkingCopy.revert', label: localize('revert', "还原"), run: () => this.revert() }));
		}
		...
		// Any other save error
			...
			if (canSaveElevated && (isPermissionDenied || triedToUnlock)) {
				primaryActions.push(toAction({
					...
					label: triedToUnlock ?
						isWindows ? localize('overwriteElevated', "以管理员身份覆盖...") : localize('overwriteElevatedSudo', "以 Sudo 覆盖...") :
						isWindows ? localize('saveElevated', "以管理员身份重试...") : localize('saveElevatedSudo', "以 Sudo 重试。"),
					run: () => {
			...
			// Unlock
				...
				primaryActions.push(toAction({ id: 'fileWorkingCopy.unlock', label: localize('overwrite', "覆盖"), run: () => this.save({ ...options, writeUnlock: true, reason: SaveReason.EXPLICIT }) }));
			}

			// Retry
			else {
				primaryActions.push(toAction({ id: 'fileWorkingCopy.retry', label: localize('retry', "重试"), run: () => this.save({ ...options, reason: SaveReason.EXPLICIT }) }));
			}

			// Save As
			primaryActions.push(toAction({
				id: 'fileWorkingCopy.saveAs',
				label: localize('saveAs', "另存为..."),
				run: async () => {
			...
			if (isWriteLocked) {
				if (triedToUnlock && canSaveElevated) {
					message = isWindows ?
						localize('readonlySaveErrorAdmin', "未能保存 \"{0}\": 文件是只读的。以管理员身份选择 \"以管理员身份覆盖\" 重试。", this.name) :
						localize('readonlySaveErrorSudo', "保存\"{0}\"失败: 文件为只读。选择“覆盖为Sudo”以用超级用户身份重试。", this.name);
				} else {
					message = localize('readonlySaveError', "未能保存 \"{0}\": 文件是只读的。可选择 \"覆盖\" 以尝试使其可写。", this.name);
				}
			} else if (canSaveElevated && isPermissionDenied) {
				message = isWindows ?
					localize('permissionDeniedSaveError', "无法保存“{0}”: 权限不足。选择“以管理员身份覆盖”可作为管理员重试。", this.name) :
					localize('permissionDeniedSaveErrorSudo', "保存 \"{0}\"失败: 权限不足。选择 \"以超级用户身份重试\" 以超级用户身份重试。", this.name);
			} else {
				message = localize({ key: 'genericSaveError', comment: ['{0} is the resource that failed to save and {1} the error message'] }, "未能保存“{0}”: {1}", this.name, toErrorMessage(error, false));
			}
		}
