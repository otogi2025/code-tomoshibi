...
export abstract class AbstractExtensionService extends Disposable implements IExtensionService {
		...
		this._register(this._lifecycleService.onWillShutdown(event => {
			if (this._remoteAgentService.getConnection()) {
				event.join(async () => {
					...
					label: nls.localize('disconnectRemote', "断开远程代理连接"),
				...
				event.join(this._doStopExtensionHosts(), {
					...
					label: nls.localize('stopExtensionHosts', "正在停止扩展主机"),
				});
			}
		}));
	}
	...
	private async _deltaExtensions(lock: ExtensionDescriptionRegistryLock, _toAdd: IExtension[], _toRemove: string[] | IExtension[]): Promise<void> {
		...
		if (result.removedDueToLooping.length > 0) {
			this._notificationService.notify({
				...
				message: nls.localize('looping', "以下扩展因包含依赖循环已被禁用: {0}", result.removedDueToLooping.map(e => `'${e.identifier.value}'`).join(', '))
			});
		}
	...
	private async _handleExtensionTests(): Promise<void> {
		...
		if (!extensionHostManager) {
			const msg = nls.localize('extensionTestError', "找不到可在 {0} 启动测试运行程序的扩展主机。", this._environmentService.extensionTestsLocationURI.toString());
			console.error(msg);
			...
			veto(value, reason) {
				...
				if (typeof value === 'boolean') {
					...
					value.then(value => {
						...
						vetoReasons.add(nls.localize('extensionStopVetoError', "{0} (错误: {1})", reason, toErrorMessage(error)));
					});
				}
			}
		});
		...
		if (!veto) {
			...
			if (!auto) {
				...
				const { confirmed } = await this._dialogService.confirm({
					...
					message: nls.localize('extensionStopVetoMessage', "请确认重启扩展。"),
					detail: vetoReasonsArray.length === 1 ?
						vetoReasonsArray[0] :
						vetoReasonsArray.join('\n -'),
					primaryButton: nls.localize('proceedAnyways', "仍然重启")
				});
	...
	private async _onRemoteExtensionHostCrashed(extensionHost: IExtensionHostManager, reconnectionToken: string): Promise<void> {
		try {
			...
			if (this._remoteCrashTracker.shouldAutomaticallyRestart()) {
				...
				this._notificationService.status(nls.localize('extensionService.autoRestart', "远程扩展主机意外终止。正在重启..."), { hideAfter: 5000 });
				this._startExtensionHostsIfNecessary(false, Array.from(this._allRequestedActivateEvents.keys()));
			} else {
				this._notificationService.prompt(Severity.Error, nls.localize('extensionService.crash', "扩展远程主机在过去 5 分钟内意外终止了 3 次。"),
					[{
						label: nls.localize('restart', "重启远程扩展主机"),
						run: () => {
							this._startExtensionHostsIfNecessary(false, Array.from(this._allRequestedActivateEvents.keys()));
						}
					}]
...
Registry.as<IExtensionFeaturesRegistry>(ExtensionFeaturesExtensions.ExtensionFeaturesRegistry).registerExtensionFeature({
	id: 'activationEvents',
	label: nls.localize('activation', "激活事件"),
	access: {
		canToggle: false
	},
