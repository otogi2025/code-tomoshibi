	...
	async handleURL(uri: URI, options?: IOpenURLOptions): Promise<boolean> {
		...
		if (!trusted) {
			...
			const result = await this.dialogService.confirm({
				message: localize('confirmUrl', "是否允许“{0}”扩展打开此 URI?", extensionDisplayName),
				checkbox: {
					label: localize('rememberConfirmUrl', "不再向我询问此扩展"),
				},
				primaryButton: localize({ key: 'open', comment: ['&& denotes a mnemonic'] }, "打开(&&O)"),
				custom: {
					markdownDetails: [{
...
class ManageAuthorizedExtensionURIsAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('manage', '管理授权扩展 URI...'),
			category: localize2('extensions', '扩展'),
			menu: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (items.length === 0) {
			await quickInputService.pick([{ label: localize('no', '当前没有已授权的扩展 URI。') }]);
			return;
		}
