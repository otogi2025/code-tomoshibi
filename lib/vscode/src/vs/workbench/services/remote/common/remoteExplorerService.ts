...
const getStartedWalkthrough: IJSONSchema = {
	...
	properties: {
		id: {
			description: nls.localize('getStartedWalkthrough.id', '要打开的“入门”演练的 ID。'),
			type: 'string'
		},
	}
};
...
const remoteHelpExtPoint = ExtensionsRegistry.registerExtensionPoint<HelpInformation>({
	...
	jsonSchema: {
		description: nls.localize('RemoteHelpInformationExtPoint', '为远程提供帮助信息'),
		type: 'object',
		properties: {
			'getStarted': {
				description: nls.localize('RemoteHelpInformationExtPoint.getStarted', "项目入门页的 URL 或返回 URL 的命令，或项目扩展提供的演练 ID"),
				oneOf: [
					{ type: 'string' },
			...
			'documentation': {
				description: nls.localize('RemoteHelpInformationExtPoint.documentation', "项目文档页面的 URL 或返回此 URL 的命令"),
				type: 'string'
			},
			'feedback': {
				description: nls.localize('RemoteHelpInformationExtPoint.feedback', "项目反馈报告器的 URL 或返回 URL 的命令"),
				type: 'string',
				markdownDeprecationMessage: nls.localize('RemoteHelpInformationExtPoint.feedback.deprecated', "请改用 {0}", '`reportIssue`')
			},
			'reportIssue': {
				description: nls.localize('RemoteHelpInformationExtPoint.reportIssue', "向项目的问题报告器返回 URL 的 URL 或命令"),
				type: 'string'
			},
			'issues': {
				description: nls.localize('RemoteHelpInformationExtPoint.issues', "项目问题列表的 URL 或返回 URL 的命令"),
				type: 'string'
			}
		}
	}
