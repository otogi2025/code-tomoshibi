...
export const forwardedPortsFeaturesEnabled = new RawContextKey<boolean>('forwardedPortsViewEnabled', false, nls.localize('tunnel.forwardedPortsViewEnabled', "“端口”视图是否已启用。"));
export const forwardedPortsViewEnabled = new RawContextKey<boolean>('forwardedPortsViewOnlyEnabled', false, nls.localize('tunnel.forwardedPortsViewEnabled', "“端口”视图是否已启用。"));
...
export const UserTunnelSource = {
	...
	description: nls.localize('tunnel.source.user', "用户转发")
};
export const AutoTunnelSource = {
	source: TunnelSource.Auto,
	description: nls.localize('tunnel.source.auto', "自动转发")
};
...
export class TunnelModel extends Disposable {
	...
	private async showPortMismatchModalIfNeeded(tunnel: RemoteTunnel, expectedLocal: number, attributes: Attributes | undefined) {
		...
		const mismatchString = nls.localize('remote.localPortMismatch.single', "无法使用本地端口 {0} 转发到远程端口 {1}。\r\n\r\n当已存在使用本地端口 {0} 的其他进程时，通常会发生这种情况。\r\n\r\n已改为使用端口号 {2}。",
			...
			for (const tunnel of tunnels) {
				...
				this.detected.set(makeAddress(tunnel.remoteAddress.host, tunnel.remoteAddress.port), {
					...
					source: {
						...
						description: nls.localize('tunnel.staticallyForwarded', "静态转发")
					}
				});
