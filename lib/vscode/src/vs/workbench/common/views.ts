...
export const VIEWS_LOG_NAME = localize('views log', "视图");
export const defaultViewIcon = registerIcon('default-view-icon', Codicon.window, localize('defaultViewIcon', '默认视图图标。'));
	...
	private addViews(viewDescriptors: IViewDescriptor[], viewContainer: ViewContainer): void {
		...
		for (const viewDescriptor of viewDescriptors) {
			if (this.getView(viewDescriptor.id) !== null) {
				throw new Error(localize('duplicateId', "已注册 ID 为“{0}”的视图", viewDescriptor.id));
			}
...
export class NoTreeViewError extends Error {
	...
	constructor(treeViewId: string) {
		super(localize('treeView.notRegistered', '未注册 ID 为 "{0}" 的树状视图。', treeViewId));
	}
