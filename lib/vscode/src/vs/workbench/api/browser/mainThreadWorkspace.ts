...
export class MainThreadWorkspace extends Disposable implements MainThreadWorkspaceShape {
	...
	private getStatusMessage(extensionName: string, addCount: number, removeCount: number): string {
		...
		if (wantsToAdd && !wantsToDelete) {
			if (addCount === 1) {
				message = localize('folderStatusMessageAddSingleFolder', "扩展“{0}”添加了 1 个文件夹到工作区", extensionName);
			} else {
				message = localize('folderStatusMessageAddMultipleFolders', "扩展“{0}”添加了 {1} 个文件夹到工作区", extensionName, addCount);
			}
		}
		...
		// Delete Folders
			...
			if (removeCount === 1) {
				message = localize('folderStatusMessageRemoveSingleFolder', "扩展“{0}”从工作区删除了 1 个文件夹", extensionName);
			} else {
				message = localize('folderStatusMessageRemoveMultipleFolders', "扩展“{0}”从工作区删除了 {1} 个文件夹", extensionName, removeCount);
			}
		}

		// Change Folders
		else {
			message = localize('folderStatusChangeFolder', "扩展“{0}”更改了工作区中的文件夹", extensionName);
		}
