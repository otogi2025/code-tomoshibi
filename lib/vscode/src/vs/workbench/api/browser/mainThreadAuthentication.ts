...
// contrib/mcp 删除后 inline 过来的三个常量（原 mcpTypes.ts:642 / mcpConfiguration.ts:66,72）。
// 这里是保留侧的认证路径，行为必须保持不变，所以搬定义而不是删逻辑。
...
export class MainThreadAuthentication extends Disposable implements MainThreadAuthenticationShape {
	...
	$showContinueNotification(message: string): Promise<boolean> {
		const yes = nls.localize('yes', "是");
		const no = nls.localize('no', "否");
	...
	private async loginPrompt(provider: IAuthenticationProvider, extensionName: string, recreatingSession: boolean, options?: AuthenticationInteractiveOptions): Promise<boolean> {
		...
		if (customMessage) {
			...
			message = recreatingSession
				? nls.localize('confirmRelogin', "扩展“{0}”希望你使用 {1} 重新登录。", extensionName, provider.label)
				: nls.localize('confirmLogin', "扩展\"{0}\"希望使用{1}登录。", extensionName, provider.label);
		}

		const buttons: IPromptButton<boolean | undefined>[] = [
			{
				label: nls.localize({ key: 'allow', comment: ['&& denotes a mnemonic'] }, "允许(&&A)"),
				run() {
					return true;
				},
			}
		...
		if (options?.learnMore) {
			buttons.push({
				label: nls.localize('learnMore', "了解详细信息"),
				run: async () => {
	...
	private async continueWithIncorrectAccountPrompt(chosenAccountLabel: string, requestedAccountLabel: string): Promise<boolean> {
		const result = await this.dialogService.prompt({
			message: nls.localize('incorrectAccount', "检测到不正确的帐户"),
			detail: nls.localize('incorrectAccountDetail', "所选帐户 {0} 与请求的帐户 {1} 不匹配。", chosenAccountLabel, requestedAccountLabel),
			type: Severity.Warning,
			cancelButton: true,
			buttons: [
				{
					label: nls.localize('keep', '保留 {0}', chosenAccountLabel),
					run: () => chosenAccountLabel
				},
				{
					label: nls.localize('loginWith', '使用 {0} 登录', requestedAccountLabel),
	...
	async $showDeviceCodeModal(userCode: string, verificationUri: string): Promise<boolean> {
		const { result } = await this.dialogService.prompt({
			type: Severity.Info,
			message: nls.localize('deviceCodeTitle', "设备代码身份验证"),
			detail: nls.localize('deviceCodeDetail', "代码: {0}\r\n\r\n要完成身份验证，请导航到 {1} 并输入上面的代码。", userCode, verificationUri),
			buttons: [
				{
					label: nls.localize('copyAndContinue', "复制并继续"),
					run: () => true
				}
			],
			cancelButton: true
		...
		if (result) {
			...
			try {
				...
				this.notificationService.error(nls.localize('failedToOpenUri', "无法打开 {0}", verificationUri));
			}
		}
		return false;
	}
	...
	async $promptForClientRegistration(authorizationServerUrl: string): Promise<{ clientId: string; clientSecret?: string } | undefined> {
		...
		const result = await this.dialogService.prompt({
			type: Severity.Info,
			message: nls.localize('dcrNotSupported', "不支持动态客户端注册"),
			detail: nls.localize('dcrNotSupportedDetail', "授权服务器 \"{0}\" 不支持自动客户端注册。是否要通过手动提供客户端注册(客户端 ID)来继续?\r\n\r\n注意: 注册 OAuth 应用程序时，请确保包括以下重定向 URI:\r\n{1}", authorizationServerUrl, redirectUrls),
			buttons: [
				{
					label: nls.localize('dcrCopyUrlsAndProceed', "复制 URI 并继续"),
					run: async () => {
						try {
							await this.clipboardService.writeText(redirectUrls);
						} catch (error) {
							this.notificationService.error(nls.localize('dcrFailedToCopy', "未能将重定向 URI 复制到剪贴板。"));
						}
						return true;
					}
				},
		...
		const sharedTitle = nls.localize('addClientRegistrationDetails', "添加客户端注册详细信息");

		const clientId = await this.quickInputService.input({
			title: sharedTitle,
			prompt: nls.localize('clientIdPrompt', "输入已向以下重定向 URI 注册的现有客户端 ID: http://127.0.0.1:33418、https://vscode.dev/redirect"),
			placeHolder: nls.localize('clientIdPlaceholder', "OAuth 客户端 ID (azye39d...)"),
			ignoreFocusLost: true,
			validateInput: async (value: string) => {
				if (!value || value.trim().length === 0) {
					return nls.localize('clientIdRequired', "客户端 ID 是必填项");
				}
				return undefined;
			}
		});
		...
		const clientSecret = await this.quickInputService.input({
			title: sharedTitle,
			prompt: nls.localize('clientSecretPrompt', "(可选)输入与客户端 ID \"{0}\" 关联的现有客户端密码，或将此字段留空", clientId),
			placeHolder: nls.localize('clientSecretPlaceholder', "OAuth 客户端密码(wer32o50f...)或将其留空"),
			password: true,
			ignoreFocusLost: true
		});
	...
	async $promptForResourceClientSecret(resourceClientId: string, resource: string): Promise<string | undefined> {
		...
		const value = await this.quickInputService.input({
			title: nls.localize('xaaResourceSecretTitle', "资源客户端密码是必需的"),
			prompt: nls.localize(
				'xaaResourceSecretPrompt',
				"\"{0}\" 处的资源使用按资源的客户端标识符 \"{1}\"。输入匹配的客户端密码(如果没有则留空)。该值会保存在操作系统机密存储中；稍后可通过 mcp.json 中的“设置客户端密码”代码信息指示器进行管理。",
				resource,
				resourceClientId,
			),
			placeHolder: nls.localize('xaaResourceSecretPlaceholder', "资源客户端密码"),
			password: true,
			ignoreFocusLost: true,
		});
		if (value === undefined) {
