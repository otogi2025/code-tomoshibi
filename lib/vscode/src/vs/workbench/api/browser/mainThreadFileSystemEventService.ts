				...
				if (showPreview === undefined) {
					...
					if (data.extensionNames.length === 1) {
						if (operation === FileOperation.CREATE) {
							message = localize('ask.1.create', "扩展 \"{0}\" 希望通过创建此文件来进行重构更改", data.extensionNames[0]);
						} else if (operation === FileOperation.COPY) {
							message = localize('ask.1.copy', "扩展 \"{0}\" 希望通过复制此文件来进行重构更改", data.extensionNames[0]);
						} else if (operation === FileOperation.MOVE) {
							message = localize('ask.1.move', "扩展 \"{0}\" 希望通过移动此文件来进行重构更改", data.extensionNames[0]);
						} else /* if (operation === FileOperation.DELETE) */ {
							message = localize('ask.1.delete', "扩展 \"{0}\" 希望通过删除此文件来进行重构更改", data.extensionNames[0]);
						}
					} else {
						if (operation === FileOperation.CREATE) {
							message = localize({ key: 'ask.N.create', comment: ['{0} is a number, e.g "3 extensions want..."'] }, "{0} 扩展希望通过创建此文件来进行重构更改", data.extensionNames.length);
						} else if (operation === FileOperation.COPY) {
							message = localize({ key: 'ask.N.copy', comment: ['{0} is a number, e.g "3 extensions want..."'] }, "{0} 扩展希望通过复制此文件来进行重构更改", data.extensionNames.length);
						} else if (operation === FileOperation.MOVE) {
							message = localize({ key: 'ask.N.move', comment: ['{0} is a number, e.g "3 extensions want..."'] }, "{0} 扩展希望通过移动此文件来进行重构更改", data.extensionNames.length);
						} else /* if (operation === FileOperation.DELETE) */ {
							message = localize({ key: 'ask.N.delete', comment: ['{0} is a number, e.g "3 extensions want..."'] }, "{0} 扩展希望通过删除此文件来进行重构更改", data.extensionNames.length);
						}
					}
					...
					if (needsConfirmation) {
						...
						const { confirmed } = await dialogService.confirm({
							type: Severity.Info,
							message,
							primaryButton: localize('preview', "显示预览(&&P)"),
							cancelButton: localize('cancel', "跳过更改")
						});
						showPreview = true;
						if (!confirmed) {
							// no changes wanted
						...
						const { result, checkboxChecked } = await dialogService.prompt<Choice>({
							...
							buttons: [
								{
									label: localize({ key: 'ok', comment: ['&& denotes a mnemonic'] }, "确定(&&O)"),
									run: () => Choice.OK
								},
								{
									label: localize({ key: 'preview', comment: ['&& denotes a mnemonic'] }, "显示预览(&&P)"),
									run: () => Choice.Preview
								}
							],
							cancelButton: {
								label: localize('cancel', "跳过更改"),
								run: () => Choice.Cancel
							},
							checkbox: { label: localize('again', "不再询问") }
						});
			...
			private _progressLabel(operation: FileOperation): string {
				switch (operation) {
					case FileOperation.CREATE:
						return localize('msg-create', "正在运行\"文件创建\"参与者...");
					case FileOperation.MOVE:
						return localize('msg-rename', "正在运行\"文件重命名\"参与者...");
					case FileOperation.COPY:
						return localize('msg-copy', "正在运行“文件复制”参与者…");
					case FileOperation.DELETE:
						return localize('msg-delete', "正在运行\"文件删除\"参与者...");
					case FileOperation.WRITE:
						return localize('msg-write', "正在运行“文件写入”参与者...");
				}
			}
		};
...
registerAction2(class ResetMemento extends Action2 {
	constructor() {
		super({
			...
			title: {
				value: localize('label', "“文件操作需要预览”的重置选项"),
