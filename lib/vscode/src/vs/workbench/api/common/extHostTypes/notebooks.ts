...
/**
 * API-compatibility shells for the notebook data types of the `vscode` API.
 *
 * Code-Tomoshibi ships no notebook editor, so `vscode.workspace.notebookDocuments`,
 * `vscode.notebooks` and friends are gone. These plain data classes stay because published
 * extensions construct them unconditionally at module load (the Claude Code extension
 * evaluates `NotebookCellOutputItem.error(new Error('')).mime` at its top level and fails
 * to activate when the constructor is missing). They carry no behaviour beyond what the
 * upstream classes had and are never fed into a notebook.
 */
...
/** The `success`/`timing` summary an extension may attach to a code cell. */
export interface INotebookCellExecutionSummary {
...
export class NotebookRange {
	static isNotebookRange(thing: unknown): thing is NotebookRange {
...
export class NotebookCellData {
	...
	static isNotebookCellDataArray(value: unknown): value is NotebookCellData[] {
		return Array.isArray(value) && (<unknown[]>value).every(elem => NotebookCellData.isNotebookCellData(elem));
	}

	static isNotebookCellData(value: unknown): value is NotebookCellData {
	...
	executionSummary?: INotebookCellExecutionSummary;

	constructor(kind: NotebookCellKind, value: string, languageId: string, mime?: string, outputs?: NotebookCellOutput[], metadata?: Record<string, unknown>, executionSummary?: INotebookCellExecutionSummary) {
		this.kind = kind;
		this.value = value;
...
export class NotebookCellOutputItem {
	...
	static isNotebookCellOutputItem(obj: unknown): obj is NotebookCellOutputItem {
		...
		return typeof (<NotebookCellOutputItem>obj).mime === 'string'
			&& (<NotebookCellOutputItem>obj).data instanceof Uint8Array;
	}
...
export class NotebookCellOutput {
	...
	static isNotebookCellOutput(candidate: unknown): candidate is NotebookCellOutput {
