			...
			this.editor.changeDecorations((accessor) => {
				...
				this.decorationId = accessor.addDecoration(
					Selection.fromPositions(position, position),
					{
						...
						hoverMessage: new MarkdownString().appendText(localize('selectionAnchor', "选择定位点")),
						className: 'selection-anchor'
					}
				);
			});
			this.selectionAnchorSetContextKey.set(!!this.decorationId);
			alert(localize('anchorSet', "定位点设置为 {0}:{1}", position.lineNumber, position.column));
		}
	}
...
class SetSelectionAnchor extends EditorAction {
	constructor() {
		super({
			...
			label: localize2('setSelectionAnchor', "设置选择定位点"),
...
class GoToSelectionAnchor extends EditorAction {
	constructor() {
		super({
			...
			label: localize2('goToSelectionAnchor', "转到选择定位点"),
...
class SelectFromAnchorToCursor extends EditorAction {
	constructor() {
		super({
			...
			label: localize2('selectFromAnchorToCursor', "选择从定位点到光标"),
			precondition: SelectionAnchorSet,
			kbOpts: {
				kbExpr: EditorContextKeys.editorTextFocus,
...
class CancelSelectionAnchor extends EditorAction {
	constructor() {
		super({
			...
			label: localize2('cancelSelectionAnchor', "取消选择定位点"),
			precondition: SelectionAnchorSet,
			kbOpts: {
				kbExpr: EditorContextKeys.editorTextFocus,
