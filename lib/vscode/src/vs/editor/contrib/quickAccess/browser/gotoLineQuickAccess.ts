...
export abstract class AbstractGotoLineQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
	...
	protected provideWithoutTextEditor(picker: IQuickPick<IGotoLineQuickPickItem, { useSeparators: true }>): IDisposable {
		const label = localize('gotoLine.noEditor', "先打开文本编辑器然后跳转到行或偏移量。");
	...
	protected provideWithTextEditor(context: IQuickAccessTextEditorContext, picker: IQuickPick<IGotoLineQuickPickItem, { useSeparators: true }>, token: CancellationToken): IDisposable {
		...
		picker.ariaLabel = localize('gotoLine.ariaLabel', "转到行。键入行号，可选择后跟冒号和列号。");
		...
		const offsetButton: IQuickInputButton = {
			...
			tooltip: localize('gotoLineToggleButton', "切换基于零的偏移"),
		...
		const updatePickerAndEditor = () => {
			...
			picker.items = [{
				...
				ariaLabel: lineNumber
					? localize('gotoLine.itemAriaLabel', "转到第 {0} 行，第 {1} 列。按 Enter 键导航。", lineNumber, column || 1)
					: label,
			}];
	...
	protected parsePosition(editor: IEditor, value: string): Partial<IPosition> & { inOffsetMode?: boolean; label: string } {
		...
		if (!model) {
			return {
				label: localize('gotoLine.noEditor', "先打开文本编辑器然后跳转到行或偏移量。")
			};
		}
		...
		if (value.startsWith(':')) {
			...
			if (isNaN(offset)) {
				...
				return {
					...
					label: this.useZeroBasedOffset ?
						localize('gotoLine.offsetPromptZero', "键入要转到的字符位置(从 0 到 {0})。", maxOffset - 1) :
						localize('gotoLine.offsetPrompt', "键入要转到的字符位置(从 1 到 {0})。", maxOffset)
				};
			} else {
				...
				return {
					...pos,
					inOffsetMode: true,
					label: localize('gotoLine.goToPosition', "按 \"Enter\" 键转到第 {0} 行，第 {1} 列。", pos.lineNumber, visibleColumn)
				};
			}
		} else {
			...
			if (parts.length < 1 || isNaN(lineNumber)) {
				return {
					label: localize('gotoLine.linePrompt', "键入要转到的行号(从 1 到 {0})。", maxLine)
				};
			}
			...
			if (parts.length < 2 || isNaN(column)) {
				return {
					...
					label: parts.length < 2 ?
						localize('gotoLine.lineColumnPrompt', "按 \"Enter\" 键转到第 {0} 行，或输入冒号 : 以添加列号。", lineNumber) :
						localize('gotoLine.columnPrompt', "按 \"Enter\" 键跳转到第 {0} 行，或输入列号(从 1 到 {1})。", lineNumber, maxColumn)
				};
			}
			...
			return {
				lineNumber,
				column: realColumn,
				label: localize('gotoLine.goToPosition', "按 \"Enter\" 键转到第 {0} 行，第 {1} 列。", lineNumber, column)
			};
		}
	}
}
