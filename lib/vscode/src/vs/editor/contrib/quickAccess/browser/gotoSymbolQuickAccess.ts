...
export abstract class AbstractGotoSymbolQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
	...
	protected provideWithoutTextEditor(picker: IQuickPick<IGotoSymbolQuickPickItem, { useSeparators: true }>): IDisposable {
		this.provideLabelPick(picker, localize('cannotRunGotoSymbolWithoutEditor', "要转到符号，首先打开具有符号信息的文本编辑器。"));
	...
	private doProvideWithoutEditorSymbols(context: IQuickAccessTextEditorContext, model: ITextModel, picker: IQuickPick<IGotoSymbolQuickPickItem, { useSeparators: true }>, token: CancellationToken): IDisposable {
		...
		this.provideLabelPick(picker, localize('cannotRunGotoSymbolWithoutSymbolProvider', "活动文本编辑器不提供符号信息。"));
			...
			try {
				...
				if (items.length > 0) {
					...
					if (query.original.length > 0) {
						this.provideLabelPick(picker, localize('noMatchingSymbolResults', "没有匹配的编辑器符号"));
					} else {
						this.provideLabelPick(picker, localize('noSymbolResults', "没有编辑器符号"));
					}
				}
			} finally {
				if (!token.isCancellationRequested) {
	...
	protected async doGetSymbolPicks(symbolsPromise: Promise<DocumentSymbol[]>, query: IPreparedQuery, options: { extraContainerLabel?: string } | undefined, token: CancellationToken, model: ITextModel): Promise<Array<IGotoSymbolQuickPickItem | IQuickPickSeparator>> {
		...
		if (openSideBySideDirection) {
			buttons = [{
				...
				tooltip: openSideBySideDirection === 'right' ? localize('openToSide', "在侧边打开") : localize('openToBottom', "在底部打开")
			}];
		}
		...
		if (filterBySymbolKind) {
			...
			symbolPicks = [
				{ label: localize('symbols', "符号({0})", filteredSymbolPicks.length), type: 'separator' },
...
const FALLBACK_NLS_SYMBOL_KIND = localize('property', "属性({0})");
const NLS_SYMBOL_KIND_CACHE: { [type: number]: string } = {
	[SymbolKind.Method]: localize('method', "方法({0})"),
	[SymbolKind.Function]: localize('function', "函数({0})"),
	[SymbolKind.Constructor]: localize('_constructor', "构造函数 ({0})"),
	[SymbolKind.Variable]: localize('variable', "变量({0})"),
	[SymbolKind.Class]: localize('class', "类({0})"),
	[SymbolKind.Struct]: localize('struct', "结构({0})"),
	[SymbolKind.Event]: localize('event', "事件({0})"),
	[SymbolKind.Operator]: localize('operator', "运算符({0})"),
	[SymbolKind.Interface]: localize('interface', "接口({0})"),
	[SymbolKind.Namespace]: localize('namespace', "命名空间({0})"),
	[SymbolKind.Package]: localize('package', "包({0})"),
	[SymbolKind.TypeParameter]: localize('typeParameter', "类型参数({0})"),
	[SymbolKind.Module]: localize('modules', "模块({0})"),
	[SymbolKind.Property]: localize('property', "属性({0})"),
	[SymbolKind.Enum]: localize('enum', "枚举({0})"),
	[SymbolKind.EnumMember]: localize('enumMember', "枚举成员({0})"),
	[SymbolKind.String]: localize('string', "字符串({0})"),
	[SymbolKind.File]: localize('file', "文件({0})"),
	[SymbolKind.Array]: localize('array', "数组({0})"),
	[SymbolKind.Number]: localize('number', "数字({0})"),
	[SymbolKind.Boolean]: localize('boolean', "布尔值({0})"),
	[SymbolKind.Object]: localize('object', "对象({0})"),
	[SymbolKind.Key]: localize('key', "键({0})"),
	[SymbolKind.Field]: localize('field', "字段({0})"),
	[SymbolKind.Constant]: localize('constant', "常量({0})")
};
