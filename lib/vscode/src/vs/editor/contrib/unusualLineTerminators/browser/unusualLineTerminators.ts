	...
	private async _checkForUnusualLineTerminators(): Promise<void> {
		...
		try {
			...
			result = await this._dialogService.confirm({
				title: nls.localize('unusualLineTerminators.title', "异常行终止符"),
				message: nls.localize('unusualLineTerminators.message', "检测到异常行终止符"),
				detail: nls.localize('unusualLineTerminators.detail', "文件“{0}”包含一个或多个异常的行终止符，例如行分隔符(LS)或段落分隔符(PS)。\r\n\r\n建议从文件中删除它们。可通过“editor.unusualLineTerminators”进行配置。", basename(model.uri)),
				primaryButton: nls.localize({ key: 'unusualLineTerminators.fix', comment: ['&& denotes a mnemonic'] }, "删除异常行终止符(&&R)"),
				cancelButton: nls.localize('unusualLineTerminators.ignore', "忽略")
			});
		} finally {
