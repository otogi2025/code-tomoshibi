	...
	override computeAsync(anchor: HoverAnchor, _lineDecorations: IModelDecoration[], source: HoverStartSource, token: CancellationToken): AsyncIterableProducer<MarkdownHover> {
		...
		return new AsyncIterableProducer<MarkdownHover>(async executor => {
			...
			if (isNonEmptyArray(part.item.hint.textEdits)) {
				executor.emitOne(new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(localize('hint.dbl', "双击以插入"))], false, 10001));
			}
			...
			if (part.part.location || part.part.command) {
				...
				const kb = useMetaKey
					? platform.isMacintosh
						? localize('links.navigate.kb.meta.mac', "cmd + 点击")
						: localize('links.navigate.kb.meta', "ctrl + 点击")
					: platform.isMacintosh
						? localize('links.navigate.kb.alt.mac', "option + 点击")
						: localize('links.navigate.kb.alt', "alt + 点击");

				if (part.part.location && part.part.command) {
					linkHint = new MarkdownString().appendText(localize('hint.defAndCommand', '转到定义 ({0})，点击右键以查看详细信息', kb));
				} else if (part.part.location) {
					linkHint = new MarkdownString().appendText(localize('hint.def', '转到定义（{0}）', kb));
				} else if (part.part.command) {
					linkHint = new MarkdownString(`[${localize('hint.cmd', "执行命令")}](${asCommandLink(part.part.command)} "${part.part.command.title}") (${kb})`, { isTrusted: true });
				}
				if (linkHint) {
