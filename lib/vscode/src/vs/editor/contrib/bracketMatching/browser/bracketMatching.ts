...
const overviewRulerBracketMatchForeground = registerColor('editorOverviewRuler.bracketMatchForeground', '#A0A0A0', nls.localize('overviewRulerBracketMatchForeground', '概览标尺上表示匹配括号的标记颜色。'));

class JumpToBracketAction extends EditorAction {
	constructor() {
		super({
			id: 'editor.action.jumpToBracket',
			label: nls.localize2('smartSelect.jumpBracket', "转到括号"),
...
class SelectToBracketAction extends EditorAction {
	constructor() {
		super({
			...
			label: nls.localize2('smartSelect.selectToBracket', "选择括号所有内容"),
			precondition: undefined,
			metadata: {
				description: nls.localize2('smartSelect.selectToBracketDescription', "选择其中的文本，包括括号或大括号"),
				args: [{
					name: 'args',
					schema: {
						type: 'object',
...
class RemoveBracketsAction extends EditorAction {
	constructor() {
		super({
			...
			label: nls.localize2('smartSelect.removeBrackets', "删除括号"),
			precondition: undefined,
			kbOpts: {
				kbExpr: EditorContextKeys.editorTextFocus,
...
MenuRegistry.appendMenuItem(MenuId.MenubarGoMenu, {
	...
	command: {
		id: 'editor.action.jumpToBracket',
		title: nls.localize({ key: 'miGoToBracket', comment: ['&& denotes a mnemonic'] }, "转到括号(&&B)")
	},
	order: 2
});
