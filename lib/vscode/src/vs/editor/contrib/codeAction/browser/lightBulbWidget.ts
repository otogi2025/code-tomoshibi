...
const GUTTER_LIGHTBULB_ICON = registerIcon('gutter-lightbulb', Codicon.lightBulb, nls.localize('gutterLightbulbWidget', '当编辑器中没有空间时从装订线生成代码操作菜单的图标。'));
const GUTTER_LIGHTBULB_AUTO_FIX_ICON = registerIcon('gutter-lightbulb-auto-fix', Codicon.lightbulbAutofix, nls.localize('gutterLightbulbAutoFixWidget', '当编辑器中没有空间，且快速修复可用时从装订线生成代码操作菜单的图标。'));
const GUTTER_LIGHTBULB_AIFIX_ICON = registerIcon('gutter-lightbulb-sparkle', Codicon.lightbulbSparkle, nls.localize('gutterLightbulbAIFixWidget', '当编辑器中没有空间，且 AI 修复可用时从装订线生成代码操作菜单的图标。'));
const GUTTER_LIGHTBULB_AIFIX_AUTO_FIX_ICON = registerIcon('gutter-lightbulb-aifix-auto-fix', Codicon.lightbulbSparkleAutofix, nls.localize('gutterLightbulbAIFixAutoFixWidget', '当编辑器中没有空间、AI 修复和快速修复可用时从装订线生成代码操作菜单的图标。'));
const GUTTER_SPARKLE_FILLED_ICON = registerIcon('gutter-lightbulb-sparkle-filled', Codicon.sparkleFilled, nls.localize('gutterLightbulbSparkleFilledWidget', '当编辑器中没有空间、AI 修复和快速修复可用时从装订线生成代码操作菜单的图标。'));
...
export function computeLightBulbInfo(actions: CodeActionSet, trigger: CodeActionTrigger, preferredKbLabel: string | undefined, quickFixKbLabel: string | undefined, forGutter: boolean = false): LightBulbInfo | undefined {
	...
	let title: string;
	if (autoRun) {
		title = nls.localize('codeActionAutoRun', "运行：{0}", actions.validActions[0].action.title);
	} else if (actions.hasAutoFix && preferredKbLabel) {
		title = nls.localize('preferredcodeActionWithKb', "显示代码操作。首选可用的快速修复({0})", preferredKbLabel);
	} else if (!actions.hasAutoFix && quickFixKbLabel) {
		title = nls.localize('codeActionWithKb', "显示代码操作({0})", quickFixKbLabel);
	} else {
		title = nls.localize('codeAction', "显示代码操作");
	}
