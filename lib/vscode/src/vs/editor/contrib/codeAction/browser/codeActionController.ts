				...
				if (action && action.action) {
					...
					if (ranges && ranges.length > 0) {
						...
						if (diagnostic.startLineNumber && diagnostic.startColumn) {
							...
							aria.status(localize('editingNewSelection', "上下文: {0} 位于行 {1} 和列 {2}。", selectionText, diagnostic.startLineNumber, diagnostic.startColumn));
						}
					}
				} else {
					currentDecorations.clear();
	...
	private _getActionBarActions(actions: CodeActionSet, at: IAnchor | IPosition, options: IActionShowOptions): IAction[] {
		...
		if (options.includeDisabledActions && actions.validActions.length > 0 && actions.allActions.length !== actions.validActions.length) {
			resultActions.push(this._showDisabled ? {
				id: 'hideMoreActions',
				label: localize('hideMoreActions', '隐藏已禁用项'),
				enabled: true,
				tooltip: '',
				class: undefined,
				run: () => {
				...
				id: 'showMoreActions',
				label: localize('showMoreActions', '显示已禁用项'),
				enabled: true,
				tooltip: '',
				class: undefined,
				run: () => {
