...
export class OneReference {
	...
	get ariaMessage(): string {
		...
		if (!preview) {
			...
			return localize(
				{ key: 'aria.oneReference.preview', comment: ['Placeholders are: 0: filename, 1:line number, 2: column number, 3: preview snippet of source code'] }, "在列 {3} 行 {2} 的 {1} 中的 {0}",
...
export class FileReferences implements IDisposable {
	...
	get ariaMessage(): string {
		...
		if (len === 1) {
			return localize('aria.fileReferences.1', "{0} 中有 1 个符号，完整路径: {1}", basename(this.uri), this.uri.fsPath);
		} else {
			return localize('aria.fileReferences.N', "{1} 中有 {0} 个符号，完整路径: {2}", len, basename(this.uri), this.uri.fsPath);
		}
	}
...
export class ReferencesModel implements IDisposable {
	...
	get ariaMessage(): string {
		if (this.isEmpty) {
			return localize('aria.result.0', "未找到结果");
		} else if (this.references.length === 1) {
			return localize('aria.result.1', "在 {0} 中找到 1 个符号", this.references[0].uri.fsPath);
		} else if (this.groups.length === 1) {
			return localize('aria.result.n1', "在 {1} 中找到 {0} 个符号", this.references.length, this.groups[0].uri.fsPath);
		} else {
			return localize('aria.result.nm', "在 {1} 个文件中找到 {0} 个符号", this.references.length, this.groups.length);
		}
	}
