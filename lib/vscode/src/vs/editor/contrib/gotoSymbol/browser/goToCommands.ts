...
MenuRegistry.appendMenuItem(MenuId.EditorContext, {
	...
	title: nls.localize('peek.submenu', "快速查看"),
	group: 'navigation',
	order: 100
...
export class DefinitionAction extends SymbolNavigationAction {
	...
	protected _getNoResultFoundMessage(info: IWordAtPosition | null): string {
		return info && info.word
			? nls.localize('noResultWord', "未找到“{0}”的任何定义", info.word)
			: nls.localize('generic.noResults', "找不到定义");
	}
...
registerAction2(class OpenDefinitionToSideAction extends DefinitionAction {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.goToDeclToSide.label', "打开侧边的定义"),
...
registerAction2(class PeekDefinitionAction extends DefinitionAction {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.previewDecl.label', "速览定义"),
			precondition: ContextKeyExpr.and(
				EditorContextKeys.hasDefinitionProvider,
...
registerAction2(class PeekTypeDefinitionAction extends TypeDefinitionAction {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.peekTypeDefinition.label', "快速查看类型定义"),
			precondition: ContextKeyExpr.and(
...
class GenericGoToLocationAction extends SymbolNavigationAction {
	...
	constructor(
		...
		super(config, {
			...
			title: nls.localize2('label.generic', "转到任何符号"),
			precondition: ContextKeyExpr.and(
				PeekContext.notInPeekEditor,
				EditorContextKeys.isInEmbeddedEditor.toNegated()
			),
	...
	protected _getNoResultFoundMessage(info: IWordAtPosition | null): string {
		return info && nls.localize('generic.noResult', "无“{0}”的结果", info.word) || '';
	}
...
CommandsRegistry.registerCommand({
	...
	handler: (accessor: ServicesAccessor, resource: any, position: any) => {
		...
		return codeEditorService.openCodeEditor({ resource }, codeEditorService.getFocusedCodeEditor()).then(control => {
			...
			const references = createCancelablePromise(token => getReferencesAtPosition(languageFeaturesService.referenceProvider, control.getModel(), corePosition.Position.lift(position), false, false, token).then(references => new ReferencesModel(references, nls.localize('ref.title', '引用'))));
