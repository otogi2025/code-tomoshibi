...
export const ctxHasSymbols = new RawContextKey('hasSymbols', false, localize('hasSymbols', "是否存在只能通过键盘导航的符号位置。"));
...
class SymbolNavigationService implements ISymbolNavigationService {
	...
	private _showMessage(): void {
		...
		const message = kb
			? localize('location.kb', "{1} 的符号 {0}，下一个使用 {2}", this._currentIdx + 1, this._currentModel!.references.length, kb.getLabel())
			: localize('location', "{1} 的符号 {0}", this._currentIdx + 1, this._currentModel!.references.length);
