...
export class ReferenceWidget extends peekView.PeekViewWidget {
	...
	protected _fillBody(containerElement: HTMLElement): void {
		...
		this._previewNotAvailableMessage = this._instantiationService.createInstance(TextModel, nls.localize('missingPreviewMessage', "无可用预览"), PLAINTEXT_LANGUAGE_ID, TextModel.DEFAULT_CREATION_OPTIONS, null);
	...
	private _onNewModel(): Promise<unknown> {
		...
		if (this._model.isEmpty) {
			this.setTitle('');
			this._messageContainer.innerText = nls.localize('noResults', "无结果");
	...
	private async _revealReference(reference: OneReference, revealParent: boolean): Promise<void> {
		...
		if (reference.uri.scheme !== Schemas.inMemory) {
			...
			this.setTitle(nls.localize('peekView.alternateTitle', "引用"));
		}
