...
class DebugEditorGpuRendererAction extends EditorAction {
	...
	constructor() {
		super({
			...
			label: localize2('gpuDebug.label', "开发人员: 调试编辑器 GPU 呈现器"),
	...
	async run(accessor: ServicesAccessor, editor: ICodeEditor): Promise<void> {
		...
		const choice = await quickInputService.pick([
			{
				label: localize('logTextureAtlasStats.label', "记录纹理图集统计信息"),
				id: 'logTextureAtlasStats',
			},
			{
				label: localize('saveTextureAtlas.label', "保存纹理图集"),
				id: 'saveTextureAtlas',
			},
			{
				label: localize('drawGlyph.label', "绘制字形"),
				id: 'drawGlyph',
			},
		], { canPickMany: false });
		if (!choice) {
