...
export class ContextMenuController implements IEditorContribution {
	...
	private _showScrollbarContextMenu(anchor: IMouseEvent): void {
		...
		const actions: IAction[] = [];
		actions.push(createAction({
			label: nls.localize('context.minimap.minimap', "缩略图"),
		...
		actions.push(createAction({
			label: nls.localize('context.minimap.renderCharacters', "呈现字符"),
		...
		actions.push(createEnumAction<'proportional' | 'fill' | 'fit'>(
			nls.localize('context.minimap.size', "垂直大小"),
			minimapOptions.enabled,
			'editor.minimap.size',
			minimapOptions.size,
			[{
				label: nls.localize('context.minimap.size.proportional', "成比例"),
				value: 'proportional'
			}, {
				label: nls.localize('context.minimap.size.fill', "填充"),
				value: 'fill'
			}, {
				label: nls.localize('context.minimap.size.fit', "适应"),
				value: 'fit'
			}]
		));
		actions.push(createEnumAction<'always' | 'mouseover'>(
			nls.localize('context.minimap.slider', "滑块"),
			minimapOptions.enabled,
			'editor.minimap.showSlider',
			minimapOptions.showSlider,
			[{
				label: nls.localize('context.minimap.slider.mouseover', "鼠标悬停"),
				value: 'mouseover'
			}, {
				label: nls.localize('context.minimap.slider.always', "始终"),
				value: 'always'
			}]
		));
		actions.push(createEnumAction<'right' | 'left'>(
			nls.localize('context.minimap.side', "侧边"),
			minimapOptions.enabled,
			'editor.minimap.side',
			minimapOptions.side,
			[{
				label: nls.localize('context.minimap.side.right', "右侧"),
				value: 'right'
			}, {
				label: nls.localize('context.minimap.side.left', "左侧"),
				value: 'left'
			}]
		));
...
class ShowContextMenu extends EditorAction {
	...
	constructor() {
		super({
			...
			label: nls.localize2('action.showContextMenu.label', "显示编辑器上下文菜单"),
