...
export class SnippetController2 implements IEditorContribution {
	...
	static readonly InSnippetMode = new RawContextKey('inSnippetMode', false, localize('inSnippetMode', "编辑器目前是否在代码片段模式下"));
	static readonly HasNextTabstop = new RawContextKey('hasNextTabstop', false, localize('hasNextTabstop', "在代码片段模式下时是否存在下一制表位"));
	static readonly HasPrevTabstop = new RawContextKey('hasPrevTabstop', false, localize('hasPrevTabstop', "在代码片段模式下时是否存在上一制表位"));
	static readonly InSnippetChoice = new RawContextKey('inSnippetChoice', false, localize('inSnippetChoice', "当前代码片段制表位是否为选项"));
				...
				provideCompletionItems: (model: ITextModel, position: Position) => {
					...
					for (let i = 0; i < activeChoice.choice.options.length; i++) {
						...
						suggestions.push({
							...
							command: { id: 'jumpToNextSnippetPlaceholder', title: localize('next', '转到下一个占位符...') }
						});
					}
					return { suggestions };
				}
