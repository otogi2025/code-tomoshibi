...
export const CONTEXT_RENAME_INPUT_VISIBLE = new RawContextKey<boolean>('renameInputVisible', false, nls.localize('renameInputVisible', "重命名输入小组件是否可见"));
export const CONTEXT_RENAME_INPUT_FOCUSED = new RawContextKey<boolean>('renameInputFocused', false, nls.localize('renameInputFocused', "是否聚焦重命名输入小组件"));
...
export class RenameWidget implements IRenameWidget, IContentWidget, IDisposable {
	...
	beforeRender(): IDimension | null {
		...
		this._label!.innerText = nls.localize({ key: 'label', comment: ['placeholders are keybindings, e.g "F2 to Rename, Shift+F2 to Preview"'] }, "按 {0} 进行重命名，按 {1} 进行预览", this._keybindingService.lookupKeybinding(accept)?.getLabel(), this._keybindingService.lookupKeybinding(preview)?.getLabel());
...
class RenameCandidateListView {
	...
	public setCandidates(candidates: NewSymbolName[]): void {
		...
		aria.status(nls.localize('renameSuggestionsReceivedAria', "已收到 {0} 重命名建议", candidates.length));
	}
...
class InputWithButton implements IDisposable {
	...
	get domNode() {
		if (!this._domNode) {
			...
			this._inputNode.setAttribute('aria-label', nls.localize('renameAriaLabel', "重命名输入。键入新名称并按 \"Enter\" 提交。"));
			...
			this._buttonGenHoverText = nls.localize('generateRenameSuggestionsButton', "生成新的名称建议");
