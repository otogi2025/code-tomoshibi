...
class RenameSkeleton {
	...
	private async _provideRenameEdits(newName: string, i: number, rejects: string[], token: CancellationToken): Promise<WorkspaceEdit & Rejection> {
		...
		if (!result) {
			return this._provideRenameEdits(newName, i + 1, rejects.concat(nls.localize('no result', "无结果。")), token);
	...
	async run(): Promise<void> {
		...
		try {
			...
			if (e instanceof CancellationError) {
				...
				if (typeof e === 'string' || isMarkdownString(e)) {
					MessageController.get(this.editor)?.showMessage(e || nls.localize('resolveRenameLocationFailed', "解析重命名位置时发生未知错误"), position);
				}
			}
			return undefined;
		...
		const renameOperation = raceCancellation(skeleton.provideRenameEdits(inputFieldResult.newName, cts2.token), cts2.token).then(async renameResult => {
			...
			this._bulkEditService.apply(renameResult, {
				...
				label: nls.localize('label', "正在将“{0}”重命名为“{1}”", loc?.text, inputFieldResult.newName),
				code: 'undoredo.rename',
				quotableLabel: nls.localize('quotableLabel', "将 {0} 重命名为 {1}", loc?.text, inputFieldResult.newName),
				respectAutoSaveConfig: true,
				reason: EditSources.rename(loc?.text, inputFieldResult.newName),
			}).then(result => {
				trace('edits applied');
				if (result.ariaSummary) {
					alert(nls.localize('aria', "成功将“{0}”重命名为“{1}”。摘要: {2}", loc.text, inputFieldResult.newName, result.ariaSummary));
				}
			}).catch(err => {
				trace(`error when applying edits ${JSON.stringify(err, null, '\t')}`);
				this._notificationService.error(nls.localize('rename.failedApply', "重命名无法应用修改"));
			...
			this._notificationService.error(nls.localize('rename.failed', "重命名无法计算修改"));
			this._logService.error(err);
...
registerAction2(class FocusNextRenameSuggestion extends Action2 {
	constructor() {
		super({
			...
			title: {
				...nls.localize2('focusNextRenameSuggestion', "聚焦下一个重命名建议"),
			},
			precondition: CONTEXT_RENAME_INPUT_VISIBLE,
			keybinding: [
				{
...
registerAction2(class FocusPreviousRenameSuggestion extends Action2 {
	constructor() {
		super({
			...
			title: {
				...nls.localize2('focusPreviousRenameSuggestion', "聚焦上一个重命名建议"),
			},
			precondition: CONTEXT_RENAME_INPUT_VISIBLE,
			keybinding: [
				{
...
Registry.as<IConfigurationRegistry>(Extensions.Configuration).registerConfiguration({
	...
	properties: {
		'editor.rename.enablePreview': {
			...
			description: nls.localize('enablePreview', "启用/禁用重命名之前预览更改的功能"),
			default: true,
			type: 'boolean'
		}
	}
