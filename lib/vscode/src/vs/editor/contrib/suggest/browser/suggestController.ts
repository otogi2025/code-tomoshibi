...
export class SuggestController implements IEditorContribution {
	...
	private _alertCompletionItem(item: CompletionItem): void {
		if (isNonEmptyArray(item.completion.additionalTextEdits)) {
			const msg = nls.localize('aria.alert.snippet', "选择“{0}”后进行了其他 {1} 次编辑", item.textLabel, item.completion.additionalTextEdits.length);
			alert(msg);
		}
	}
...
registerEditorCommand(new SuggestCommand({
	...
	menuOpts: [{
		...
		title: nls.localize('accept.insert', "插入"),
		group: 'left',
		order: 1,
		...
		menuId: suggestWidgetStatusbarMenu,
		title: nls.localize('accept.replace', "替换"),
		group: 'left',
		order: 1,
...
registerEditorCommand(new SuggestCommand({
	...
	menuOpts: {
		...
		title: nls.localize('focus.suggestion', "选择")
	}
}));
...
registerEditorCommand(new SuggestCommand({
	...
	menuOpts: [{
		...
		title: nls.localize('detail.more', "显示更少")
	}, {
		menuId: suggestWidgetStatusbarMenu,
		group: 'right',
		order: 1,
		when: ContextKeyExpr.and(SuggestContext.DetailsVisible.toNegated(), SuggestContext.CanResolve),
		title: nls.localize('detail.less', "显示更多")
	}]
}));
...
registerEditorAction(class extends EditorAction {
	...
	constructor() {
		super({
			...
			label: nls.localize2('suggest.reset.label', "重置建议小组件大小"),
