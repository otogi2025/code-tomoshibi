	...
	public replaceAll(): boolean {
		if (this._model) {
			if (this._editor.getModel()?.isTooLargeForHeapOperation()) {
				this._notificationService.warn(nls.localize('too.large.for.replaceall', "文件太大，无法执行全部替换操作。"));
				return false;
			}
...
export class StartFindWithArgsAction extends EditorAction {
	...
	constructor() {
		super({
			...
			label: nls.localize2('startFindWithArgsAction', "使用参数查找"),
...
export class StartFindWithSelectionAction extends EditorAction {
	...
	constructor() {
		super({
			...
			label: nls.localize2('startFindWithSelectionAction', "使用所选内容查找"),
			precondition: undefined,
			kbOpts: {
				kbExpr: null,
				primary: 0,
	...
	if (!runMatch()) {
		...
		if (!runMatch()) {
			...
			if (wasFindWidgetVisible && state.matchesCount === 0 && state.searchString) {
				alertFn(nls.localize('ariaSearchNoResult', "为“{1}”找到 {0}", NLS_NO_RESULTS, state.searchString));
			}
		}
	}
}
...
export const NextMatchFindAction = registerMultiEditorAction(new MultiEditorAction({
	...
	label: nls.localize2('findNextMatchAction', "查找下一个"),
	precondition: undefined,
	kbOpts: [{
...
export const PreviousMatchFindAction = registerMultiEditorAction(new MultiEditorAction({
	...
	label: nls.localize2('findPreviousMatchAction', "查找上一个"),
	precondition: undefined,
	kbOpts: [{
		kbExpr: EditorContextKeys.focus,
...
export class MoveToMatchFindAction extends EditorAction {
	...
	constructor() {
		super({
			...
			label: nls.localize2('findMatchAction.goToMatch', "转到“匹配”..."),
	...
	public run(accessor: ServicesAccessor, editor: ICodeEditor): void | Promise<void> {
		...
		if (matchesCount < 1) {
			...
			notificationService.notify({
				...
				message: nls.localize('findMatchAction.noResults', "没有匹配项，换个词试试")
			});
			return;
		}
		...
		inputBox.placeholder = nls.localize('findMatchAction.inputPlaceHolder', "键入数字以转到特定匹配项(介于 1 和 {0} 之间)", matchesCount);
		...
		const updatePickerAndEditor = (value: string) => {
			...
			if (typeof index === 'number') {
				...
				inputBox.validationMessage = nls.localize('findMatchAction.inputValidationMessage', "请键入介于 1 和 {0} 之间的数字", controller.getState().matchesCount);
...
export class NextSelectionMatchFindAction extends SelectionMatchFindAction {
	...
	constructor() {
		super({
			...
			label: nls.localize2('nextSelectionMatchFindAction', "查找下一个选择"),
			precondition: undefined,
			kbOpts: {
				kbExpr: EditorContextKeys.focus,
...
export class PreviousSelectionMatchFindAction extends SelectionMatchFindAction {
	...
	constructor() {
		super({
			...
			label: nls.localize2('previousSelectionMatchFindAction', "查找上一个选择"),
			precondition: undefined,
			kbOpts: {
				kbExpr: EditorContextKeys.focus,
