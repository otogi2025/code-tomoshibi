...
const findCollapsedIcon = registerIcon('find-collapsed', Codicon.chevronRight, nls.localize('findCollapsedIcon', '用于指示编辑器查找小组件已折叠的图标。'));
const findExpandedIcon = registerIcon('find-expanded', Codicon.chevronDown, nls.localize('findExpandedIcon', '用于指示编辑器查找小组件已展开的图标。'));

export const findSelectionIcon = registerIcon('find-selection', Codicon.selection, nls.localize('findSelectionIcon', '编辑器查找小组件中的“在选定内容中查找”图标。'));
export const findReplaceIcon = registerIcon('find-replace', Codicon.replace, nls.localize('findReplaceIcon', '编辑器查找小组件中的“替换”图标。'));
export const findReplaceAllIcon = registerIcon('find-replace-all', Codicon.replaceAll, nls.localize('findReplaceAllIcon', '编辑器查找小组件中的“全部替换”图标。'));
export const findPreviousMatchIcon = registerIcon('find-previous-match', Codicon.arrowUp, nls.localize('findPreviousMatchIcon', '编辑器查找小组件中的“查找上一个”图标。'));
export const findNextMatchIcon = registerIcon('find-next-match', Codicon.arrowDown, nls.localize('findNextMatchIcon', '编辑器查找小组件中的“查找下一个”图标。'));
...
const NLS_FIND_DIALOG_LABEL = nls.localize('label.findDialog', "查找/替换");
const NLS_FIND_INPUT_LABEL = nls.localize('label.find', "查找");
const NLS_FIND_INPUT_PLACEHOLDER = nls.localize('placeholder.find', "查找");
const NLS_PREVIOUS_MATCH_BTN_LABEL = nls.localize('label.previousMatchButton', "上一个匹配项");
const NLS_NEXT_MATCH_BTN_LABEL = nls.localize('label.nextMatchButton', "下一个匹配项");
const NLS_TOGGLE_SELECTION_FIND_TITLE = nls.localize('label.toggleSelectionFind', "在选定内容中查找");
const NLS_CLOSE_BTN_LABEL = nls.localize('label.closeButton', "关闭");
const NLS_REPLACE_INPUT_LABEL = nls.localize('label.replace', "替换");
const NLS_REPLACE_INPUT_PLACEHOLDER = nls.localize('placeholder.replace', "替换");
const NLS_REPLACE_BTN_LABEL = nls.localize('label.replaceButton', "替换");
const NLS_REPLACE_ALL_BTN_LABEL = nls.localize('label.replaceAllButton', "全部替换");
const NLS_TOGGLE_REPLACE_MODE_BTN_LABEL = nls.localize('label.toggleReplaceButton', "切换替换");
const NLS_MATCHES_COUNT_LIMIT_TITLE = nls.localize('title.matchesCountLimit', "仅高亮了前 {0} 个结果，但所有查找操作均针对全文。", MATCHES_LIMIT);
export const NLS_MATCHES_LOCATION = nls.localize('label.matchesLocation', "第 {0} 项，共 {1} 项");
export const NLS_NO_RESULTS = nls.localize('label.noResults', "无结果");
...
export class FindWidget extends Widget implements IOverlayWidget, IVerticalSashLayoutProvider {
	...
	private _getAriaLabel(label: string, currentMatch: Range | null, searchString: string): string {
		...
		if (label === NLS_NO_RESULTS) {
			result = searchString === ''
				? nls.localize('ariaSearchNoResultEmpty', "找到 {0}", label)
				: nls.localize('ariaSearchNoResult', "为“{1}”找到 {0}", label, searchString);
		} else if (currentMatch) {
			const ariaLabel = nls.localize('ariaSearchNoResultWithLineNum', "在 {2} 处找到“{1}”的 {0}", label, searchString, currentMatch.startLineNumber + ':' + currentMatch.startColumn);
			...
			result = nls.localize('ariaSearchNoResultWithLineNumNoCurrentMatch', "为“{1}”找到 {0}", label, searchString);
		}
	...
	private _updateFindInputAriaLabel(): void {
		...
		if (!this._accessibilityHelpHintAnnounced && this._configurationService.getValue('accessibility.verbosity.find') && this._accessibilityService.isScreenReaderOptimized()) {
			...
			if (accessibilityHelpKeybinding) {
				const hint = nls.localize('accessibilityHelpHintInLabel', "按 {0} 可获取辅助功能帮助", accessibilityHelpKeybinding);
