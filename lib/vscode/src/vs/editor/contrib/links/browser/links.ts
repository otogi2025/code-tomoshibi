	...
	public openLinkOccurrence(occurrence: LinkOccurrence, openToSide: boolean, fromUserGesture = false): void {
		...
		link.resolve(CancellationToken.None).then(uri => {
			...
			if (messageOrError === 'invalid') {
				this.notificationService.warn(nls.localize('invalid.url', '此链接格式不正确，无法打开: {0}', link.url!.toString()));
			} else if (messageOrError === 'missing') {
				this.notificationService.warn(nls.localize('missing.url', '此链接目标已丢失，无法打开。'));
			} else {
				onUnexpectedError(err);
			}
		});
...
function getHoverMessage(link: Link, useMetaKey: boolean): MarkdownString {
	...
	const label = link.tooltip
		? link.tooltip
		: executeCmd
			? nls.localize('links.navigate.executeCmd', '执行命令')
			: nls.localize('links.navigate.follow', '打开链接');

	const kb = useMetaKey
		? platform.isMacintosh
			? nls.localize('links.navigate.kb.meta.mac', "cmd + 单击")
			: nls.localize('links.navigate.kb.meta', "ctrl + 单击")
		: platform.isMacintosh
			? nls.localize('links.navigate.kb.alt.mac', "option + 单击")
			: nls.localize('links.navigate.kb.alt', "alt + 单击");
	...
	if (link.url) {
		...
		if (/^command:/i.test(link.url.toString())) {
			...
			if (match) {
				...
				nativeLabel = nls.localize('tooltip.explanation', "执行命令 {0}", commandId);
			}
		}
...
class OpenLinkAction extends EditorAction {
	...
	constructor() {
		super({
			...
			label: nls.localize2('label', "打开链接"),
