...
export class DeleteInsideWord extends EditorAction {
	...
	constructor() {
		super({
			id: 'deleteInsideWord',
			precondition: EditorContextKeys.writable,
			label: nls.localize2('deleteInsideWord', "删除 Word"),
			metadata: {
				description: nls.localize2('deleteInsideWord.description', "删除光标处的字词"),
				args: [{
					name: 'args',
					schema: {
						type: 'object',
						properties: {
							'onlyWord': {
								type: 'boolean',
								default: false,
								description: nls.localize('deleteInsideWord.args.onlyWord', "仅删除字词并保留周围的空格")
							}
						}
					}
				}]
