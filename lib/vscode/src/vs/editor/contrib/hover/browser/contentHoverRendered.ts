...
class RenderedContentHoverParts extends Disposable {
	...
	public getAccessibleHoverContentAtIndex(index: number): string {
		...
		if (renderedPart.type === 'statusBar') {
			const statusBarDescription = [localize('hoverAccessibilityStatusBar', "这是悬停状态栏。")];
			for (const action of renderedPart.actions) {
				const keybinding = action.actionKeybindingLabel;
				if (keybinding) {
					statusBarDescription.push(localize('hoverAccessibilityStatusBarActionWithKeybinding', "它有一个标签为 {0} 和键绑定为 {1} 的操作。", action.actionLabel, keybinding));
				} else {
					statusBarDescription.push(localize('hoverAccessibilityStatusBarActionWithoutKeybinding', "它有一个标签为 {0} 的操作。", action.actionLabel));
				}
			}
