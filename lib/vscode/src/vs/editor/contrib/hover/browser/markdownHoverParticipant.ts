...
const $ = dom.$;
const increaseHoverVerbosityIcon = registerIcon('hover-increase-verbosity', Codicon.addSmall, nls.localize('increaseHoverVerbosity', '用于提高悬停详细程度的图标。'));
const decreaseHoverVerbosityIcon = registerIcon('hover-decrease-verbosity', Codicon.removeSmall, nls.localize('decreaseHoverVerbosity', '用于降低悬停详细程度的图标。'));
...
export class MarkdownHoverParticipant implements IEditorHoverParticipant<MarkdownHover> {
	...
	public createLoadingMessage(anchor: HoverAnchor): MarkdownHover | null {
		return new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(nls.localize('modesContentHover.loading', "正在加载..."))], false, 2000);
	}
...
export function labelForHoverVerbosityAction(keybindingService: IKeybindingService, action: HoverVerbosityAction): string {
	switch (action) {
		case HoverVerbosityAction.Increase:
			return keybindingService.appendKeybinding(nls.localize('increaseVerbosity', "提高悬停详细程度"), INCREASE_HOVER_VERBOSITY_ACTION_ID);
		case HoverVerbosityAction.Decrease:
			return keybindingService.appendKeybinding(nls.localize('decreaseVerbosity', "降低悬停详细程度"), DECREASE_HOVER_VERBOSITY_ACTION_ID);
	}
}
