...
export class MarkerHoverParticipant implements IEditorHoverParticipant<MarkerHover> {
	...
	private _renderMarkerStatusbar(context: IEditorHoverRenderContext, markerHover: MarkerHover): IDisposable {
		...
		if (!this._editor.getOption(EditorOption.readOnly)) {
				...
				if (IMarkerData.makeKey(this.recentMarkerCodeActionsInfo.marker) === IMarkerData.makeKey(markerHover.marker)) {
					if (!this.recentMarkerCodeActionsInfo.hasCodeActions) {
						if (menuActions.length === 0) {
							quickfixPlaceholderElement.textContent = nls.localize('noQuickFixes', "没有可用的快速修复");
						}
					}
				} else {
			...
			const updatePlaceholderDisposable = this.recentMarkerCodeActionsInfo && !this.recentMarkerCodeActionsInfo.hasCodeActions ? Disposable.None : disposableTimeout(() => quickfixPlaceholderElement.textContent = nls.localize('checkingForQuickFixes', "正在检查快速修复..."), 200, disposables);
