...
const InlineCompletionsSnoozing = new RawContextKey<boolean>('inlineCompletions.snoozed', false, localize('inlineCompletions.snoozed', "当前是否推迟内联补全"));
...
export class SnoozeInlineCompletion extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('action.inlineSuggest.snooze', "推迟内联建议"),
	...
	private async getDurationFromUser(quickInputService: IQuickInputService, storageService: IStorageService): Promise<number | undefined> {
		...
		const predefinedItems: (IQuickPickItem & { value: number })[] = [
			{ label: localize('snooze.1minute', "1 分钟"), id: '1', value: 60_000 },
			{ label: localize('snooze.5minutes', "5 分钟"), id: '5', value: 300_000 },
			{ label: localize('snooze.10minutes', "10 分钟"), id: '10', value: 600_000 },
			{ label: localize('snooze.15minutes', "15 分钟"), id: '15', value: 900_000 },
			{ label: localize('snooze.30minutes', "30 分钟"), id: '30', value: 1_800_000 },
			{ label: localize('snooze.60minutes', "60 分钟"), id: '60', value: 3_600_000 },
		];
		...
		if (lastSelectedDuration > 0 && !predefinedItems.some(item => item.value === lastSelectedDuration)) {
			...
			const customItem: IQuickPickItem & { value: number } = {
				label: localize('snooze.lastCustom', "{0} 分钟(上次使用时间)", minutes),
				id: 'last-custom',
				value: lastSelectedDuration,
				description: localize('snooze.lastUsed', "上次使用持续时间自定义"),
			};
		...
		items.push({ label: localize('snooze.custom', "自定义..."), id: 'custom', value: -1 });

		const picked = await quickInputService.pick(items, {
			placeHolder: localize('snooze.placeholder', "选择内联建议的推迟时长"),
	...
	private async getCustomDurationFromUser(quickInputService: IQuickInputService, storageService: IStorageService): Promise<number | undefined> {
		const customMinutes = await quickInputService.input({
			placeHolder: localize('snooze.customPlaceholder', "持续时间(分钟) (例如 90)"),
			prompt: localize('snooze.customPrompt', "输入推迟持续时间(分钟)"),
			validateInput: async (value) => {
				const n = Number(value);
				if (isNaN(n) || n <= 0 || !Number.isFinite(n)) {
					return localize('snooze.invalidInput', "请输入一个正数");
				}
				return undefined;
			}
		});
...
export class CancelSnoozeInlineCompletion extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('action.inlineSuggest.cancelSnooze', "取消推迟内联建议"),
