...
const accessibleDiffViewerInsertIcon = registerIcon('diff-review-insert', Codicon.add, localize('accessibleDiffViewerInsertIcon', '可访问差异查看器中“插入”的图标。'));
const accessibleDiffViewerRemoveIcon = registerIcon('diff-review-remove', Codicon.remove, localize('accessibleDiffViewerRemoveIcon', '可访问差异查看器中“删除”的图标。'));
const accessibleDiffViewerCloseIcon = registerIcon('diff-review-close', Codicon.close, localize('accessibleDiffViewerCloseIcon', '可访问差异查看器中“关闭”的图标。'));
...
class View extends Disposable {
	...
	private _render(store: DisposableStore): void {
		...
		container.setAttribute('aria-label', localize('ariaLabel', '可访问的差异查看器。使用向上和向下箭头导航。'));
		...
		for (const viewItem of group?.lines || []) {
			...
			if (viewItem.type === LineType.Header) {
				...
				const getAriaLines = (lines: number) =>
					lines === 0 ? localize('no_lines_changed', "未更改行")
						: lines === 1 ? localize('one_line_changed', "更改了 1 行")
							: localize('more_lines_changed', "更改了 {0} 行", lines);
	...
	private _createRow(
		...
		if (lineContent.length === 0) {
			lineContent = localize('blankLine', "空白");
		}
		...
		switch (item.type) {
			case LineType.Unchanged:
				if (item.originalLineNumber === item.modifiedLineNumber) {
					ariaLabel = localize({ key: 'unchangedLine', comment: ['The placeholders are contents of the line and should not be translated.'] }, "{0} 未更改的行 {1}", lineContent, item.originalLineNumber);
				} else {
					ariaLabel = localize('equalLine', "{0}原始行{1}修改的行{2}", lineContent, item.originalLineNumber, item.modifiedLineNumber);
				}
				break;
			case LineType.Added:
				ariaLabel = localize('insertLine', "+ {0}修改的行{1}", lineContent, item.modifiedLineNumber);
				break;
			case LineType.Deleted:
				ariaLabel = localize('deleteLine', "- {0}原始行{1}", lineContent, item.originalLineNumber);
				break;
		}
