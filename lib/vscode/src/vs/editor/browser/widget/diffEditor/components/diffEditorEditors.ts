...
export class DiffEditorEditors extends Disposable {
	...
	private _updateAriaLabel(ariaLabel: string | undefined): string | undefined {
		...
		const ariaNavigationTip = localize('diff-aria-navigation-tip', ' 使用 {0} 打开辅助功能帮助。', this._keybindingService.lookupKeybinding('editor.action.accessibilityHelp')?.getAriaLabel());
