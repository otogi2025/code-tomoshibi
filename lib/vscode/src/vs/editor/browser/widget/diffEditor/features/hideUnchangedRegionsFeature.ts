	...
	constructor(
		...
		const unchangedLinesDecorationShow: IModelDecorationOptions = {
			...
			glyphMarginHoverMessage: new MarkdownString(undefined, { isTrusted: true, supportThemeIcons: true })
				.appendMarkdown(localize('foldUnchanged', '折叠未更改的区域')),
	...
	constructor(
		...
		this._register(autorun(reader => {
			...
			if (!this._hide) {
				...
				const linesHiddenText = localize('hiddenLines', '{0} 个隐藏的行', lineCount);
...
class CollapsedCodeOverlayWidget extends ViewZoneOverlayWidget {
	private readonly _nodes = h('div.diff-hidden-lines', [
		h('div.top@top', { title: localize('diff.hiddenLines.top', '单击或拖动可在上面显示更多内容') }),
		h('div.center@content', { style: { display: 'flex' } }, [
			h('div@first', { style: { display: 'flex', justifyContent: 'center', alignItems: 'center', flexShrink: '0' } },
				[$('a', { title: localize('showUnchangedRegion', '显示未更改的区域'), role: 'button', onclick: () => { this._unchangedRegion.showAll(undefined); } },
					...renderLabelWithIcons('$(unfold)'))]
			),
			h('div@others', { style: { display: 'flex', justifyContent: 'center', alignItems: 'center' } }),
		]),
		h('div.bottom@bottom', { title: localize('diff.bottom', '单击或拖动可在下方显示更多内容'), role: 'button' }),
	]);
	...
	constructor(
		...
		this._register(autorun(reader => {
			...
			if (!this._hide) {
				...
				const linesHiddenText = localize('hiddenLines', '{0} 个隐藏的行', lineCount);
				const span = $('span', { title: localize('diff.hiddenLines.expandAll', '双击展开') }, linesHiddenText);
