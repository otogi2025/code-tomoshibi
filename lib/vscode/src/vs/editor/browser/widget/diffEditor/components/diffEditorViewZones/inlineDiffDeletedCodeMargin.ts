				...
				getActions: () => {
					...
					actions.push(new Action(
						...
						isDeletion
							? (_diff.original.length > 1
								? localize('diff.clipboard.copyDeletedLinesContent.label', "复制已删除的行")
								: localize('diff.clipboard.copyDeletedLinesContent.single.label', "复制已删除的行"))
							: (_diff.original.length > 1
								? localize('diff.clipboard.copyChangedLinesContent.label', "复制更改的行")
								: localize('diff.clipboard.copyChangedLinesContent.single.label', "复制更改的行")),
						undefined,
						true,
						async () => {
					...
					if (_diff.original.length > 1) {
						actions.push(new Action(
							...
							isDeletion
								? localize('diff.clipboard.copyDeletedLineContent.label', "复制已删除的行({0})",
									_diff.original.startLineNumber + currentLineNumberOffset)
								: localize('diff.clipboard.copyChangedLineContent.label', "复制更改的行({0})",
									_diff.original.startLineNumber + currentLineNumberOffset),
							undefined,
							true,
							async () => {
					...
					if (!readOnly) {
						actions.push(new Action(
							...
							localize('diff.inline.revertChange.label', "还原此更改"),
							undefined,
							true,
							async () => {
