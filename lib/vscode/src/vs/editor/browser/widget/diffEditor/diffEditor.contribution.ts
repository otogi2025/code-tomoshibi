...
MenuRegistry.appendMenuItem(MenuId.EditorTitle, {
	command: {
		...
		title: localize('useInlineViewWhenSpaceIsLimited', "空间受限时使用内联视图"),
...
MenuRegistry.appendMenuItem(MenuId.EditorTitle, {
	command: {
		...
		title: localize('showMoves', "显示移动的代码块"),
		icon: Codicon.move,
...
for (const ctx of [
	...
	MenuRegistry.appendMenuItem(MenuId.DiffEditorSelectionToolbar, {
		command: {
			id: new RevertHunkOrSelection().desc.id,
			title: localize('revertSelection', "还原所选内容"),
			icon: ctx.icon,
		},
		when: ContextKeyExpr.and(EditorContextKeys.diffEditorModifiedWritable, ctx.key),
		order: 5,
...
MenuRegistry.appendMenuItem(MenuId.EditorTitle, {
	command: {
		...
		title: localize('Open Accessible Diff Viewer', "打开可访问差异查看器"),
		precondition: ContextKeyExpr.has('isInDiffEditor'),
	},
	order: 10,
	group: '2_diff',
