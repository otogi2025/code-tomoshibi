...
export class ToggleCollapseUnchangedRegions extends Action2 {
	constructor() {
		super({
			...
			title: localize2('toggleCollapseUnchangedRegions', '切换折叠未更改的区域'),
			icon: Codicon.map,
...
export class ToggleShowMovedCodeBlocks extends Action2 {
	constructor() {
		super({
			...
			title: localize2('toggleShowMovedCodeBlocks', '切换显示移动的代码块'),
			precondition: ContextKeyExpr.has('isInDiffEditor'),
		});
	}
...
export class ToggleUseInlineViewWhenSpaceIsLimited extends Action2 {
	constructor() {
		super({
			...
			title: localize2('toggleUseInlineViewWhenSpaceIsLimited', '在空间受限时切换使用内联视图'),
			precondition: ContextKeyExpr.has('isInDiffEditor'),
		});
	}
...
const diffEditorCategory: ILocalizedString = localize2('diffEditor', "差异编辑器");

export class SwitchSide extends EditorAction2 {
	constructor() {
		super({
			id: 'diffEditor.switchSide',
			title: localize2('switchSide', '切换侧面'),
...
export class ExitCompareMove extends EditorAction2 {
	constructor() {
		super({
			...
			title: localize2('exitCompareMove', '退出比较移动'),
			icon: Codicon.close,
...
export class CollapseAllUnchangedRegions extends EditorAction2 {
	constructor() {
		super({
			...
			title: localize2('collapseAllUnchangedRegions', '折叠所有未更改的区域'),
			icon: Codicon.fold,
			precondition: ContextKeyExpr.has('isInDiffEditor'),
			f1: true,
			category: diffEditorCategory,
...
export class ShowAllUnchangedRegions extends EditorAction2 {
	constructor() {
		super({
			...
			title: localize2('showAllUnchangedRegions', '显示所有未更改的区域'),
			icon: Codicon.unfold,
			precondition: ContextKeyExpr.has('isInDiffEditor'),
			f1: true,
			category: diffEditorCategory,
...
const accessibleDiffViewerCategory: ILocalizedString = localize2('accessibleDiffViewer', "可访问的差异查看器");
...
export class AccessibleDiffViewerNext extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('editor.action.accessibleDiffViewer.next', '转至下一个差异'),
...
export class AccessibleDiffViewerPrev extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('editor.action.accessibleDiffViewer.prev', '转至上一个差异'),
			category: accessibleDiffViewerCategory,
			precondition: ContextKeyExpr.has('isInDiffEditor'),
			keybinding: {
