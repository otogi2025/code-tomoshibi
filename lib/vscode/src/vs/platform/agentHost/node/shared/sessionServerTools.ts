...
function getSessionToolDisplay(toolName: string, _args: unknown, result?: IServerToolDisplayResult): IServerToolDisplay | undefined {
	switch (toolName) {
		case SessionServerToolName.ListSessions: {
			...
			return {
				displayName: localize('toolName.listSessions', "列出会话"),
		...
		case SessionServerToolName.CreateSession:
			return {
				displayName: localize('toolName.createSession', "创建会话"),
				invocationMessage: localize('toolInvoke.createSession', "正在创建会话"),
				pastTenseMessage: localize('toolComplete.createSession', "已创建会话"),
			};
		case SessionServerToolName.CreateChat:
			return {
				displayName: localize('toolName.createChat', "创建聊天"),
		...
		case SessionServerToolName.SendMessage:
			return {
				displayName: localize('toolName.sendMessage', "发送消息"),
		...
		case SessionServerToolName.GetSessionContext:
			return {
				displayName: localize('toolName.getSessionContext', "获取会话上下文"),
		...
		case SessionServerToolName.GetCurrentSession:
			return {
				displayName: localize('toolName.getCurrentSession', "获取当前会话"),
		...
		case SessionServerToolName.DeleteSession:
			return {
				displayName: localize('toolName.deleteSession', "删除会话"),
				invocationMessage: localize('toolInvoke.deleteSession', "正在删除会话"),
				pastTenseMessage: localize('toolComplete.deleteSession', "已删除的会话"),
			};
		default:
			return undefined;
	}
