...
function getPlanActionDescription(actionId: string): { label: string; description: string } | undefined {
	switch (actionId) {
		case 'autopilot':
			return {
				label: localize('agentHost.planReview.autopilot.label', "使用 Autopilot 实现"),
		...
		case 'autopilot_fleet':
			return {
				label: localize('agentHost.planReview.autopilotFleet.label', "使用 Autopilot Fleet 实现"),
		...
		case 'interactive':
			return {
				label: localize('agentHost.planReview.interactive.label', "实现计划"),
				description: localize('agentHost.planReview.interactive.description', "实施计划，针对每个操作征求意见并获得批准。"),
			};
		case 'exit_only':
			return {
				label: localize('agentHost.planReview.exitOnly.label', "仅批准计划"),
				description: localize('agentHost.planReview.exitOnly.description', "批准计划但不执行。我会自行实施。"),
			};
		default:
			return undefined;
	}
...
export class CopilotAgentSession extends Disposable {
	...
	private async _send(prompt: string, attachments: readonly MessageAttachment[] | undefined, mode: CopilotSdkMode | undefined): Promise<void> {
		...
		if (slashCommand?.command === 'compact') {
			try {
				...
				this.emitInitialMarkdown(localize('copilotAgent.compactionCompleted', "已完成压缩"));
			} catch (err) {
	...
	private async _requestUnsandboxedCommandConfirmation(request: IUnsandboxedCommandConfirmationRequest): Promise<boolean> {
		...
		const confirmationTitle = blockedDomains
			? localize('agentHost.unsandboxedCommandConfirmation.title.blockedDomains', "是否在沙盒外运行命令以访问 {0}?", blockedDomains)
			: localize('agentHost.unsandboxedCommandConfirmation.title.generic', "在沙盒外部运行命令?");
		const invocationMessage = request.reason
			? localize('agentHost.unsandboxedCommandConfirmation.reason', "退出沙盒的原因: {0}", request.reason)
			: blockedDomains
				? localize('agentHost.unsandboxedCommandConfirmation.blockedDomains', "此命令需要访问被阻止的网络域: {0}。", blockedDomains)
				: localize('agentHost.unsandboxedCommandConfirmation.generic', "此命令需要在沙盒外部运行。");
	...
	private async _handleExitPlanModeRequest(data: ExitPlanModeRequest, _invocation: { sessionId: string }): Promise<IExitPlanModeResponse> {
		...
		const inputRequest: ChatInputRequestWithPlanReview = {
			id: requestId,
			planReview: {
				title: localize('agentHost.planReview.title', "审阅计划"),
				content: data.summary || localize('agentHost.planReview.fallbackSummary', "计划现已就绪，可供审阅。"),
				actions,
			...
			questions: [{
				...
				id: questionId,
				title: localize('agentHost.planReview.title', "审阅计划"),
				message: localize('agentHost.planReview.questionMessage', "你希望如何继续?"),
				required: true,
				options,
