...
export class CopilotAgent extends Disposable implements IAgent {
	...
	getDescriptor(): IAgentDescriptor {
		return {
			provider: 'copilotcli',
			displayName: 'Copilot',
			description: localize('copilotAgent.description', "在本地智能体主机进程中运行的 Copilot SDK 智能体"),
			capabilities: {
	...
	private _createThinkingLevelConfigSchemaProperty(reasoningEfforts: readonly string[] | undefined, defaultReasoningEffort: string | undefined, modelId: string | undefined): ConfigPropertySchema | undefined {
		...
		return {
			type: 'string',
			title: localize('copilot.modelThinkingLevel.title', "思考层级"),
			description: localize('copilot.modelThinkingLevel.description', "控制模型使用的推理强度。"),
	...
	private _createContextSizeConfigSchemaProperty(billing: ICAPIModelBilling | undefined): ConfigPropertySchema | undefined {
		...
		if (this._isPreferLongContextEnabled() && !hasLongContextSurcharge(billing)) {
			return {
				type: 'number',
				title: localize('copilot.modelContextSize.title', "上下文大小"),
				description: localize('copilot.modelContextSize.description', "选择此模型的上下文窗口大小。"),
				default: longContextMax,
				enum: [longContextMax],
				enumLabels: [formatTokenCount(longContextMax)],
				enumDescriptions: [
					localize('copilot.modelContextSize.longerSessions', "会话更长"),
				],
			};
		}
		...
		return {
			...
			enumDescriptions: [
				localize('copilot.modelContextSize.default', "默认"),
				localize('copilot.modelContextSize.longerSessions', "会话更长"),
			],
		};
	}
	...
	public async resolveConfiguredCustomization(customization: PluginCustomization): Promise<IResolvedCustomization> {
		...
		if (!parsed) {
			return {
				customization: {
					...customization,
					load: { kind: CustomizationLoadStatus.Error, message: localize('copilotAgent.pluginParseError', "解析插件时出错。") },
				},
			};
		}
