...
export class AgentHostCommitOperationHandler implements IChangesetOperationHandler {
	...
	private async _invoke(params: InvokeChangesetOperationParams, token: CancellationToken, signal: AbortSignal): Promise<InvokeChangesetOperationResult> {
		...
		if (!hasUncommitted) {
			return { message: { markdown: localize('agentHost.changeset.commit.noChanges', "没有可提交的未提交更改。") } };
		}
		this._throwIfCancelled(token);
		...
		if (!authToken) {
			throw new ProtocolError(
				AHP_AUTH_REQUIRED,
				localize('agentHost.changeset.commit.authRequired', "登录 GitHub Copilot 以生成提交消息。"),
				[copilotResource],
			);
		}
		...
		if (!diffs || diffs.length === 0) {
			throw new ProtocolError(JsonRpcErrorCodes.InternalError, localize('agentHost.changeset.commit.diffFailed', "无法计算未提交的更改，因而无法生成提交消息。"));
		}
		this._throwIfCancelled(token);
		...
		try {
			...
			if (this._isAuthFailure(err)) {
				throw new ProtocolError(
					AHP_AUTH_REQUIRED,
					localize('agentHost.changeset.commit.authExpired', "生成提交消息需要身份验证。请登录 GitHub Copilot，然后重试。"),
					[copilotResource],
				);
			}
			throw err;
		...
		if (!message) {
			throw new ProtocolError(JsonRpcErrorCodes.InternalError, localize('agentHost.changeset.commit.emptyMessage', "生成的提交消息为空。"));
		}
		this._throwIfCancelled(token);
		...
		return { message: { markdown: localize('agentHost.changeset.commit.committed', "已提交更改，消息为: `{0}`", message.split('\n')[0]) } };
	}
	...
	private _throwIfCancelled(token: CancellationToken): void {
		if (token.isCancellationRequested) {
			throw new ProtocolError(JsonRpcErrorCodes.InternalError, localize('agentHost.changeset.commit.cancelled', "已取消提交操作。"));
		}
	}
}
