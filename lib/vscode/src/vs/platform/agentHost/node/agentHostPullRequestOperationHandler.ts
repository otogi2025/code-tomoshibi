...
export class AgentHostPullRequestOperationHandler implements IChangesetOperationHandler {
	...
	private async _invoke(params: InvokeChangesetOperationParams, token: CancellationToken, signal: AbortSignal): Promise<InvokeChangesetOperationResult> {
		...
		if (!authToken) {
			throw new ProtocolError(
				AHP_AUTH_REQUIRED,
				localize('agentHost.changeset.pr.authRequired', "使用仓库访问权限登录 GitHub，以创建拉取请求。"),
				[repoResource],
			);
		}
		...
		if (branchChanges === undefined) {
			throw new ProtocolError(JsonRpcErrorCodes.InternalError, localize('agentHost.changeset.pr.computeChangesFailed', "无法计算分支更改，因而无法创建拉取请求。"));
		}
		if (branchChanges !== undefined && branchChanges.length === 0) {
			throw new ProtocolError(JsonRpcErrorCodes.InternalError, localize('agentHost.changeset.pr.noChanges', "没有可为其创建拉取请求的分支更改。"));
		}
		this._throwIfCancelled(token);
	...
	private async _finalize(
		...
		if (pr.nodeId) {
			...
			autoMergeError = localize('agentHost.changeset.pr.autoMerge.noNodeId', "GitHub 未返回拉取请求标识符。");
			autoMergeOutcome = 'failed';
	...
	private _buildMessage(pr: CreatedPullRequest, isExisting: boolean, autoMergeOutcome: 'none' | 'enabled' | 'failed', autoMergeError: string | undefined): string {
		...
		switch (this._autoMergeMethod) {
			case 'SQUASH':
				mergeMethodLabel = localize('agentHost.changeset.pr.autoMerge.squash', "压缩");
				break;
			case 'REBASE':
				mergeMethodLabel = localize('agentHost.changeset.pr.autoMerge.rebase', "变基");
				break;
			default:
				mergeMethodLabel = localize('agentHost.changeset.pr.autoMerge.merge', "合并");
				break;
		}
		...
		if (isExisting) {
			switch (autoMergeOutcome) {
				case 'enabled':
					return localize('agentHost.changeset.pr.existing.autoMerge', "拉取请求 [#{0}]({1})已存在；已启用自动合并({2})。", pr.number, pr.url, mergeMethodLabel);
				case 'failed':
					return localize('agentHost.changeset.pr.existing.autoMergeFailed', "拉取请求[#{0}]({1})已存在，但无法启用自动合并: {2}", pr.number, pr.url, autoMergeError ?? '');
				default:
					return localize('agentHost.changeset.pr.existing', "拉取请求[#{0}]({1})已存在。", pr.number, pr.url);
			}
		}

		switch (autoMergeOutcome) {
			case 'enabled':
				return localize('agentHost.changeset.pr.created.autoMerge', "已创建拉取请求 [#{0}]({1})，且已启用自动合并({2})。", pr.number, pr.url, mergeMethodLabel);
			case 'failed':
				return localize('agentHost.changeset.pr.created.autoMergeFailed', "已创建拉取请求 [#{0}]({1})，但无法启用自动合并: {2}", pr.number, pr.url, autoMergeError ?? '');
			default:
				return this._draft
					? localize('agentHost.changeset.pr.createdDraft', "创建了草稿拉取请求[#{0}]({1})。", pr.number, pr.url)
					: localize('agentHost.changeset.pr.created', "已创建拉取请求[#{0}]({1})。", pr.number, pr.url);
		}
	}

	private _throwIfCancelled(token: CancellationToken): void {
		if (token.isCancellationRequested) {
			throw new ProtocolError(JsonRpcErrorCodes.InternalError, localize('agentHost.changeset.pr.cancelled', "拉取请求操作已取消。"));
		}
	}
	...
	private _formatCommitMessage(branchName: string): string {
		return localize('agentHost.changeset.pr.commitMessage', "针对 {0} 的智能体主机更改", branchName);
	}

	private _formatBody(branchName: string, baseBranchName: string): string {
		return localize('agentHost.changeset.pr.body', "从 `{0}` 创建，目标为 `{1}`。", branchName, baseBranchName);
	}
