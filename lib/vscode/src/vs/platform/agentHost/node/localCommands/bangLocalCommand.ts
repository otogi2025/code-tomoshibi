...
export class BangLocalCommand extends Disposable implements ILocalChatCommand {
	...
	private async _run(turnChannel: ProtocolURI, turnId: string, command: string): Promise<void> {
		...
		const displayName = localize('agentHostBang.terminal', "终端");
		...
		try {
			...
			ctx.dispatch(turnChannel, {
				...
				result: {
					success: false,
					pastTenseMessage: localize('agentHostBang.failed', "运行命令失败"),
	...
	private _summarizeResult(result: IShellCommandResult): { success: boolean; pastTenseMessage: string } {
		switch (result.status) {
			case 'completed': {
				...
				return exitCode === 0
					? { success: true, pastTenseMessage: localize('agentHostBang.ran', "已运行命令") }
					: { success: false, pastTenseMessage: localize('agentHostBang.exited', "命令已退出，代码为 {0}", exitCode) };
			}
			case 'timeout':
				return { success: false, pastTenseMessage: localize('agentHostBang.timedOut', "命令超时") };
			case 'shellExited':
				return { success: false, pastTenseMessage: localize('agentHostBang.shellExited', "Shell 意外退出") };
			case 'background':
				return { success: true, pastTenseMessage: localize('agentHostBang.background', "命令正在后台运行") };
			case 'altBuffer':
				return { success: true, pastTenseMessage: localize('agentHostBang.interactive', "命令已打开交互式终端") };
		}
	}
}
