...
export class WSLRemoteAgentHostMainService extends Disposable implements IWSLRemoteAgentHostMainService {
	...
	async connect(config: IWSLAgentHostConfig): Promise<IWSLConnectResult> {
		...
		reportProgress(localize('wslProgressDetectingPlatform', "正在检测 {0} 中的平台...", distro));
		const { os: targetOs, arch: targetArch } = await this._resolvePlatform(distro);

		reportProgress(localize('wslProgressPreparingCLI', "正在 {0} 中准备 CLI...", distro));
		...
		reportProgress(localize('wslProgressConnecting', "正在连接到 {0} 中的智能体主机...", distro));
		let ws: WebSocket;
		try {
	...
	private async _resolvePlatform(distro: string): Promise<{ os: string; arch: string }> {
		...
		if (!resolved) {
			throw new Error(localize('wslUnsupportedPlatform', "不支持的 WSL 发行版平台: {0}", result.stdout.trim()));
		}
		return resolved;
	}
