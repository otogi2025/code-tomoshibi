...
const codexSessionConfigSchema = createSchema({
	[CodexSessionConfigKey.PermissionsPreset]: schemaProperty<CodexPermissionsPreset>({
		type: 'string',
		title: localize('codex.sessionConfig.permissionsPreset', "审批"),
		description: localize('codex.sessionConfig.permissionsPresetDescription', "Codex 在请求审批之前可自行完成多少工作。"),
		enum: [...CODEX_PERMISSIONS_PRESETS],
		enumLabels: [
			localize('codex.sessionConfig.permissionsPreset.default', "默认权限"),
			localize('codex.sessionConfig.permissionsPreset.autoReview', "自动审阅"),
			localize('codex.sessionConfig.permissionsPreset.fullAccess', "完全访问权限"),
		],
		enumDescriptions: [
			localize('codex.sessionConfig.permissionsPreset.defaultDescription', "Codex 可读取和编辑工作区中的文件，并运行常规本地命令。它会在使用 Internet 或超出工作区之前询问。"),
			localize('codex.sessionConfig.permissionsPreset.autoReviewDescription', "与“默认”具有相同的工作区访问权限，但审批请求会通过自动审阅者进行路由，不会提示你。"),
			localize('codex.sessionConfig.permissionsPreset.fullAccessDescription', "Codex 可编辑工作区外部的文件并使用 Internet，无需询问。仅在需要完全计算机访问权限时使用。"),
		],
	...
	[CodexSessionConfigKey.ApprovalPolicy]: schemaProperty<CodexApprovalPolicy>({
		type: 'string',
		title: localize('codex.sessionConfig.approvalPolicy', "审批"),
		description: localize('codex.sessionConfig.approvalPolicyDescription', "Codex 如何请求批准工具调用。"),
		enum: ['never', 'on-request', 'on-failure', 'untrusted'],
		enumLabels: [
			localize('codex.sessionConfig.approvalPolicy.never', "无升级"),
			localize('codex.sessionConfig.approvalPolicy.onRequest', "需要时询问"),
			localize('codex.sessionConfig.approvalPolicy.onFailure', "Ask on Failure"),
			localize('codex.sessionConfig.approvalPolicy.untrusted', "更频繁地询问"),
		],
		enumDescriptions: [
			localize('codex.sessionConfig.approvalPolicy.neverDescription', "永不请求提升权限；无法在沙盒中运行的命令将被拒绝。"),
			localize('codex.sessionConfig.approvalPolicy.onRequestDescription', "仅当 Codex 确定某个命令需要提升权限时才询问。"),
			...
			localize('codex.sessionConfig.approvalPolicy.untrustedDescription', "在更多命令类别前先询问，以便更仔细地查看操作。"),
		],
		default: 'on-request',
		sessionMutable: true,
	}),
	[CodexSessionConfigKey.SandboxMode]: schemaProperty<SandboxMode>({
		type: 'string',
		title: localize('codex.sessionConfig.sandboxMode', "沙盒"),
		description: localize('codex.sessionConfig.sandboxModeDescription', "应用于工具调用的文件系统和网络限制。"),
		enum: ['read-only', 'workspace-write', 'danger-full-access'],
		enumLabels: [
			localize('codex.sessionConfig.sandboxMode.readOnly', "只读"),
			localize('codex.sessionConfig.sandboxMode.workspaceWrite', "工作区写入"),
			localize('codex.sessionConfig.sandboxMode.dangerFullAccess', "完全访问权限(危险)"),
		],
		enumDescriptions: [
			localize('codex.sessionConfig.sandboxMode.readOnlyDescription', "工具调用可以读取工作区，但不能修改文件。"),
			localize('codex.sessionConfig.sandboxMode.workspaceWriteDescription', "工具调用可以在工作区内读取和写入；网络单独控制。"),
			localize('codex.sessionConfig.sandboxMode.dangerFullAccessDescription', "工具调用具有不受限制的磁盘和网络访问权限。"),
		],
	...
	[CodexSessionConfigKey.WebSearchMode]: schemaProperty<WebSearchMode>({
		type: 'string',
		title: localize('codex.sessionConfig.webSearchMode', "Web 搜索"),
		description: localize('codex.sessionConfig.webSearchModeDescription', "模型的 Web 搜索工具可用性。"),
		enum: ['disabled', 'cached', 'live'],
		enumLabels: [
			localize('codex.sessionConfig.webSearchMode.disabled', "已禁用"),
			localize('codex.sessionConfig.webSearchMode.cached', "仅缓存"),
			localize('codex.sessionConfig.webSearchMode.live', "实时"),
		],
		default: 'disabled',
	...
	[CodexSessionConfigKey.ModelReasoningEffort]: schemaProperty<ReasoningEffort>({
		type: 'string',
		title: localize('codex.sessionConfig.modelReasoningEffort', "推理工作量"),
		description: localize('codex.sessionConfig.modelReasoningEffortDescription', "控制 Codex 使用的推理强度。"),
	...
	[CodexSessionConfigKey.Personality]: schemaProperty<Personality>({
		type: 'string',
		title: localize('codex.sessionConfig.personality', "个人设置"),
		description: localize('codex.sessionConfig.personalityDescription', "Codex 通信时使用的语气。"),
		enum: ['none', 'friendly', 'pragmatic'],
		enumLabels: [
			localize('codex.sessionConfig.personality.none', "默认"),
			localize('codex.sessionConfig.personality.friendly', "友好"),
			localize('codex.sessionConfig.personality.pragmatic', "Pragmatic"),
		],
		enumDescriptions: [
			localize('codex.sessionConfig.personality.noneDescription', "使用 Codex 的内置默认语气。"),
			localize('codex.sessionConfig.personality.friendlyDescription', "更温馨、更具对话感。"),
			localize('codex.sessionConfig.personality.pragmaticDescription', "语气简练、直奔主题，聚焦于操作。"),
		],
		default: 'none',
		sessionMutable: true,
	}),
	[CodexSessionConfigKey.ReasoningSummary]: schemaProperty<ReasoningSummary>({
		type: 'string',
		title: localize('codex.sessionConfig.reasoningSummary', "推理摘要"),
		description: localize('codex.sessionConfig.reasoningSummaryDescription', "Codex 如何在响应流中汇总其推理。"),
		enum: ['auto', 'concise', 'detailed', 'none'],
		enumLabels: [
			localize('codex.sessionConfig.reasoningSummary.auto', "自动"),
			localize('codex.sessionConfig.reasoningSummary.concise', "简洁"),
			localize('codex.sessionConfig.reasoningSummary.detailed', "详细"),
			localize('codex.sessionConfig.reasoningSummary.none', "无"),
		],
		default: 'auto',
		sessionMutable: true,
	}),
	[CodexSessionConfigKey.AdditionalDirectories]: schemaProperty<string[]>({
		type: 'array',
		title: localize('codex.sessionConfig.additionalDirectories', "其他可写目录"),
		description: localize('codex.sessionConfig.additionalDirectoriesDescription', "除工作区外，沙盒还可写入的绝对路径。仅在沙盒为工作区写入时适用。"),
		items: { type: 'string', title: localize('codex.sessionConfig.additionalDirectories.item', "目录") },
		enumDynamic: true,
		default: [],
		sessionMutable: true,
	}),
	[CodexSessionConfigKey.NetworkAccessEnabled]: schemaProperty<boolean>({
		type: 'boolean',
		title: localize('codex.sessionConfig.networkAccessEnabled', "网络"),
		description: localize('codex.sessionConfig.networkAccessEnabledDescription', "允许沙盒工具调用发出出站网络请求。仅在沙盒为工作区写入时适用。"),
		default: false,
		sessionMutable: true,
	}),
		...
		return {
			...
			properties: {
				[CODEX_THINKING_LEVEL_KEY]: {
					type: 'string',
					title: localize('codex.modelThinkingLevel.title', "思考层级"),
					description: localize('codex.modelThinkingLevel.description', "控制 Codex 使用的推理强度。"),
					default: 'medium',
					enum: [...CODEX_REASONING_EFFORTS],
					enumLabels: CODEX_REASONING_EFFORTS.map(getReasoningEffortLabel),
					enumDescriptions: CODEX_REASONING_EFFORTS.map(effort => getReasoningEffortDescription(effort) ?? ''),
