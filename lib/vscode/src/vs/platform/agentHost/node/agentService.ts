...
export class AgentService extends Disposable implements IAgentService {
	...
	async createSession(config?: IAgentCreateSessionConfig): Promise<URI> {
		...
		if (config?.fork) {
			...
			const forkedTitlePrefix = localize('agentHost.forkedTitlePrefix', "已派生:");
		...
		if (options?.fork) {
			...
			if (forkIndex < 0) {
				...
				forkedTitle = forkedSourceTitle
					...
					: localize('agentHost.forkedChatFallback', "分叉聊天");
	...
	private _buildImportedTitle(turns: readonly Turn[]): string {
		...
		if (!firstText) {
			return localize('agentHost.importedSessionFallback', "新会话");
		}
		const MAX = 60;
	...
	emitDownloadProgress(packageId: string, displayName: string, receivedBytes: number, totalBytes: number | undefined, terminal: boolean): void {
		...
		const message = localize('agentHost.download.agentSdkTitle', "正在下载 {0} 智能体", displayName);
