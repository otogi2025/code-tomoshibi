...
export function buildExitPlanModeConfirmationState(input: Record<string, unknown>, toolUseID: string): ToolCallPendingConfirmationState {
	...
	return {
		...
		confirmationTitle: localize('claude.exitPlanMode.title', "准备好编写代码了吗?"),
		options: [
			{ id: 'approve', label: localize('claude.exitPlanMode.approve', "批准"), kind: ConfirmationOptionKind.Approve },
			{ id: 'deny', label: localize('claude.exitPlanMode.deny', "拒绝"), kind: ConfirmationOptionKind.Deny },
		],
	};
}
