...
export function getClaudeToolDisplayName(toolName: string): string {
	...
	switch (toolName) {
		case 'Bash': return localize('claude.tool.bash', "运行 shell 命令");
		case 'BashOutput': return localize('claude.tool.bashOutput', "读取 shell 输出");
		case 'KillBash': return localize('claude.tool.killBash', "终止 shell 命令");
		case 'Read': return localize('claude.tool.read', "读取文件");
		case 'Glob': return localize('claude.tool.glob', "查找文件");
		case 'Grep': return localize('claude.tool.grep', "搜索文件");
		case 'LS': return localize('claude.tool.ls', "列出目录");
		case 'NotebookRead': return localize('claude.tool.notebookRead', "读取笔记本");
		case 'Write': return localize('claude.tool.write', "写入文件");
		case 'Edit': return localize('claude.tool.edit', "编辑文件");
		case 'MultiEdit': return localize('claude.tool.multiEdit', "编辑文件");
		case 'NotebookEdit': return localize('claude.tool.notebookEdit', "编辑笔记本");
		case 'TodoWrite': return localize('claude.tool.todoWrite', "更新待办事项列表");
		case 'WebFetch': return localize('claude.tool.webFetch', "提取 URL");
		case 'Task':
		case 'Agent': return localize('claude.tool.task', "运行子智能体任务");
		case 'ExitPlanMode': return localize('claude.tool.exitPlanMode', "准备好编写代码了吗?");
		case 'AskUserQuestion': return localize('claude.tool.askUserQuestion', "向用户提问");
	...
	if (toolName.startsWith(MCP_TOOL_PREFIX)) {
		return localize('claude.tool.mcp', "运行 MCP 工具 {0}", toolName.slice(MCP_TOOL_PREFIX.length));
	}
	return toolName;
}
...
export function getClaudeConfirmationTitle(toolName: string): string {
	switch (getClaudePermissionKind(toolName)) {
		case 'shell':
			return localize('claude.permission.shell.title', "是否在终端中运行?");
		case 'write':
			return localize('claude.permission.write.title', "是否编辑文件?");
		case 'read':
			return localize('claude.permission.read.title', "是否读取文件?");
		case 'url':
			return localize('claude.permission.url.title', "是否提取 URL?");
		case 'skill':
		...
		case 'mcp': {
			const serverName = toolName.startsWith(MCP_TOOL_PREFIX)
				...
				: undefined;
			return serverName
				? localize('claude.permission.mcp.title', "是否允许来自 {0} 的工具?", serverName)
				: localize('claude.permission.default.title', "是否允许工具调用?");
		}
		case 'custom-tool':
		default:
			return localize('claude.permission.default.title', "是否允许工具调用?");
	}
}
...
export function getClaudeInvocationMessage(
	...
	switch (toolName) {
		case 'Bash': {
			...
			if (firstLine) {
				return md(localize('claude.toolInvoke.bashCmd', "正在运行 {0}", appendEscapedMarkdownInlineCode(truncate(firstLine, 80))));
			}
			return localize('claude.toolInvoke.bash', "正在运行 shell 命令");
		}
		case 'BashOutput':
			return localize('claude.toolInvoke.bashOutput', "正在读取 shell 输出");
		case 'KillBash':
		...
		case 'WebFetch': {
			...
			if (url) {
				return md(localize('claude.toolInvoke.webFetch', "正在提取 {0}", `[${escapeMarkdownLinkLabel(truncate(url, 80))}](${url})`));
			}
			return localize('claude.toolInvoke.webFetchGeneric', "正在提取 URL");
		}
		case 'Task':
		case 'Agent': {
...
export function getClaudePastTenseMessage(
	...
	if (!success) {
		return localize('claude.toolComplete.failed', "“{0}”失败", displayName);
	}
	...
	switch (toolName) {
		case 'Bash': {
			const firstLine = firstShellLine(input);
			if (firstLine) {
				return md(localize('claude.toolComplete.bashCmd', "已运行 {0}", appendEscapedMarkdownInlineCode(truncate(firstLine, 80))));
			}
			return localize('claude.toolComplete.bash', "已运行 Shell 命令");
		}
		case 'BashOutput':
			return localize('claude.toolComplete.bashOutput', "读取 shell 输出");
		case 'KillBash':
		...
		case 'WebFetch': {
			const url = readStringField(input, 'url');
			if (url) {
				return md(localize('claude.toolComplete.webFetch', "已提取 {0}", `[${escapeMarkdownLinkLabel(truncate(url, 80))}](${url})`));
			}
			return localize('claude.toolComplete.webFetchGeneric', "已提取 URL");
		}
		case 'Task':
		case 'Agent':
			return localize('claude.toolComplete.task', "已运行子智能体");
		case 'Skill': {
			const skill = readStringField(input, 'skill');
			if (skill) {
