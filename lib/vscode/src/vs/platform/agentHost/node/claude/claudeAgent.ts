...
export class ClaudeAgent extends Disposable implements IAgent {
	...
	getDescriptor(): IAgentDescriptor {
		return {
			...
			description: localize('claudeAgent.description', "由 Anthropic Claude Agent SDK 支持的 Claude 智能体"),
			capabilities: {
	...
	resolveSessionConfig(_params: IAgentResolveSessionConfigParams): Promise<ResolveSessionConfigResult> {
		...
		const sessionSchema = createSchema({
			[ClaudeSessionConfigKey.PermissionMode]: schemaProperty<ClaudePermissionMode>({
				type: 'string',
				title: localize('claude.sessionConfig.permissionMode', "审批"),
				description: localize('claude.sessionConfig.permissionModeDescription', "Claude 如何处理工具审批。"),
				enum: ['default', 'acceptEdits', 'plan', 'auto', 'bypassPermissions'],
				enumLabels: [
					localize('claude.sessionConfig.permissionMode.default', "编辑前询问"),
					localize('claude.sessionConfig.permissionMode.acceptEdits', "自动编辑"),
					localize('claude.sessionConfig.permissionMode.plan', "计划模式"),
					localize('claude.sessionConfig.permissionMode.auto', "自动模式"),
					localize('claude.sessionConfig.permissionMode.bypassPermissions', "绕过权限"),
				],
				enumDescriptions: [
					localize('claude.sessionConfig.permissionMode.defaultDescription', "Claude 会在编辑文件前先询问。"),
					localize('claude.sessionConfig.permissionMode.acceptEditsDescription', "Claude 会直接编辑文件而不询问，并在使用其他工具前询问。"),
					localize('claude.sessionConfig.permissionMode.planDescription', "Claude 会在进行更改前创建计划。"),
					localize('claude.sessionConfig.permissionMode.autoDescription', "Claude 会针对每个工具操作决定是否询问。"),
					localize('claude.sessionConfig.permissionMode.bypassPermissionsDescription', "Claude 会直接运行所有工具而不询问。"),
				],
				default: 'default',
				sessionMutable: true,
			}),
