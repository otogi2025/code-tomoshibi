		...
		definitions: {
			style: {
				...
				properties: {
					...
					fontStyle: {
						type: 'string',
						description: nls.localize('schema.token.fontStyle', '设置规则的所有字形: "italic" (斜体)、"bold" (粗体)、"underline" (下划线)、"strikethrough" (删除线) 或是上述的组合。所有未列出的字形都将取消设置。空字符串将取消设置的所有字形。'),
						pattern: fontStylePattern,
						patternErrorMessage: nls.localize('schema.fontStyle.error', '字形必须为 "italic" (斜体)、"bold" (粗体)、"underline" (下划线)、"strikethrough" (删除线) 或是上述的组合。空字符串将取消设置的所有字形。'),
						defaultSnippets: [
					...
					bold: {
						type: 'boolean',
						description: nls.localize('schema.token.bold', '将字形设置为粗体或取消粗体设置。请注意，如果存在 "fontStyle"，则会替代此设置。'),
					},
					italic: {
						type: 'boolean',
						description: nls.localize('schema.token.italic', '将字形设置为倾斜或取消倾斜设置。请注意，如果存在 "fontStyle"，则会替代此设置。'),
					},
					underline: {
						type: 'boolean',
						description: nls.localize('schema.token.underline', '将字形设置为下划线或取消下划线设置。请注意，如果存在 "fontStyle"，则会替代此设置。'),
					},
					strikethrough: {
						type: 'boolean',
						description: nls.localize('schema.token.strikethrough', '将字形设置为下划线或取消下划线设置。请注意，如果存在 "fontStyle"，则会替代此设置。'),
					}
...
function createDefaultTokenClassificationRegistry(): TokenClassificationRegistry {
	...
	registerTokenType('comment', nls.localize('comment', "注释的样式。"), [['comment']]);
	registerTokenType('string', nls.localize('string', "字符串的样式。"), [['string']]);
	registerTokenType('keyword', nls.localize('keyword', "关键字的样式。"), [['keyword.control']]);
	registerTokenType('number', nls.localize('number', "数字样式。"), [['constant.numeric']]);
	registerTokenType('regexp', nls.localize('regexp', "表达式的样式。"), [['constant.regexp']]);
	registerTokenType('operator', nls.localize('operator', "运算符的样式。"), [['keyword.operator']]);

	registerTokenType('namespace', nls.localize('namespace', "命名空间的样式。"), [['entity.name.namespace']]);

	registerTokenType('type', nls.localize('type', "类型的样式。"), [['entity.name.type'], ['support.type']]);
	registerTokenType('struct', nls.localize('struct', "结构样式。"), [['entity.name.type.struct']]);
	registerTokenType('class', nls.localize('class', "类样式。"), [['entity.name.type.class'], ['support.class']]);
	registerTokenType('interface', nls.localize('interface', "接口样式。"), [['entity.name.type.interface']]);
	registerTokenType('enum', nls.localize('enum', "枚举的样式。"), [['entity.name.type.enum']]);
	registerTokenType('typeParameter', nls.localize('typeParameter', "类型参数的样式。"), [['entity.name.type.parameter']]);

	registerTokenType('function', nls.localize('function', "函数样式"), [['entity.name.function'], ['support.function']]);
	registerTokenType('member', nls.localize('member', "成员函数的样式"), [], 'method', 'Deprecated use `method` instead');
	registerTokenType('method', nls.localize('method', "成员(成员函数)的样式"), [['entity.name.function.member'], ['support.function']]);
	registerTokenType('macro', nls.localize('macro', "宏样式。"), [['entity.name.function.preprocessor']]);

	registerTokenType('variable', nls.localize('variable', "变量的样式。"), [['variable.other.readwrite'], ['entity.name.variable']]);
	registerTokenType('parameter', nls.localize('parameter', "参数样式。"), [['variable.parameter']]);
	registerTokenType('property', nls.localize('property', "属性的样式。"), [['variable.other.property']]);
	registerTokenType('enumMember', nls.localize('enumMember', "枚举成员的样式。"), [['variable.other.enummember']]);
	registerTokenType('event', nls.localize('event', "事件的样式。"), [['variable.other.event']]);
	registerTokenType('decorator', nls.localize('decorator', "修饰器和注释的样式。"), [['entity.name.decorator'], ['entity.name.function']]);

	registerTokenType('label', nls.localize('labels', "文本样式"), undefined);

	// default token modifiers

	registry.registerTokenModifier('declaration', nls.localize('declaration', "所有符号声明的样式。"), undefined);
	registry.registerTokenModifier('documentation', nls.localize('documentation', "用于文档中引用的样式。"), undefined);
	registry.registerTokenModifier('static', nls.localize('static', "用于静态符号的样式。"), undefined);
	registry.registerTokenModifier('abstract', nls.localize('abstract', "用于抽象符号的样式。"), undefined);
	registry.registerTokenModifier('deprecated', nls.localize('deprecated', "用于已弃用的符号的样式。"), undefined);
	registry.registerTokenModifier('modification', nls.localize('modification', "用于写入访问的样式。"), undefined);
	registry.registerTokenModifier('async', nls.localize('async', "用于异步的符号的样式。"), undefined);
	registry.registerTokenModifier('readonly', nls.localize('readonly', "用于只读符号的样式。"), undefined);
