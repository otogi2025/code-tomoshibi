...
export const fontIdErrorMessage = localize('schema.fontId.formatError', '字体 ID 只能包含字母、数字、下划线和短划线。');
	...
	private iconSchema: IJSONSchema & { properties: IJSONSchemaMap } = {
		definitions: {
			icons: {
				type: 'object',
				properties: {
					fontId: { type: 'string', description: localize('iconDefinition.fontId', '要使用的字体的 ID。如果未设置，则使用最先定义的字体。'), pattern: fontIdRegex.source, patternErrorMessage: fontIdErrorMessage },
					fontCharacter: { type: 'string', description: localize('iconDefinition.fontCharacter', '与图标定义关联的字体字符。') }
				},
...
export const widgetClose = registerIcon('widget-close', Codicon.close, localize('widgetClose', '小组件中“关闭”操作的图标。'));

export const gotoPreviousLocation = registerIcon('goto-previous-location', Codicon.arrowUp, localize('previousChangeIcon', '“转到上一个编辑器位置”图标。'));
export const gotoNextLocation = registerIcon('goto-next-location', Codicon.arrowDown, localize('nextChangeIcon', '“转到下一个编辑器位置”图标。'));
