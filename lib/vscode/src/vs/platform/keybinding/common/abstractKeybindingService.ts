	...
	private _expectAnotherChord(firstChord: string, keypressLabel: string | null): void {
		...
		switch (this._currentChords.length) {
			...
			case 1:
				...
				this._currentChordStatusMessage = this._notificationService.status(nls.localize('first.chord', "({0})已按下。正在等待按下第二个键...", keypressLabel));
				break;
			default: {
				const fullKeypressLabel = this._currentChords.map(({ label }) => label).join(', ');
				this._currentChordStatusMessage = this._notificationService.status(nls.localize('next.chord', "已按下({0})。正在等待第二个键...", fullKeypressLabel));
			}
		}
		...
		switch (resolveResult.kind) {
			...
			case ResultKind.NoMatchingKb: {
				...
				if (this.inChordMode) {
					...
					this._notificationService.status(nls.localize('missing.chord', "组合键({0}，{1})不是命令。", currentChordsLabel, keypressLabel), { hideAfter: 10 * 1000 /* 10s */ });
					this._leaveChordMode();
