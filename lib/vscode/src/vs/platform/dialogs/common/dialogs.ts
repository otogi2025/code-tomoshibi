		...
		switch (kind) {
			case DialogKind.Confirmation: {
				...
				if (confirmationDialog.primaryButton) {
					...
					buttons.push(localize({ key: 'yesButton', comment: ['&& denotes a mnemonic'] }, "是(&&Y)"));
				}

				if (confirmationDialog.cancelButton) {
					buttons.push(confirmationDialog.cancelButton);
				} else {
					buttons.push(localize('cancelButton', "取消"));
				}
			...
			case DialogKind.Prompt: {
				...
				if (buttons.length === 0) {
					buttons.push(localize({ key: 'okButton', comment: ['&& denotes a mnemonic'] }, "确定(&&O)"));
				}
...
export function getFileNamesMessage(fileNamesOrResources: readonly (string | URI)[]): string {
	...
	if (fileNamesOrResources.length > MAX_CONFIRM_FILES) {
		if (fileNamesOrResources.length - MAX_CONFIRM_FILES === 1) {
			message.push(localize('moreFile', "...1 个其他文件未显示"));
		} else {
			message.push(localize('moreFiles', "...{0} 个其他文件未显示", fileNamesOrResources.length - MAX_CONFIRM_FILES));
		}
	}
