		...
		if (externalRemoval.length > 0) {
			messages.push(
				nls.localize(
					...
					"以下文件已关闭并且已在磁盘上修改: {0}。", externalRemoval.join(', ')
				)
			);
		}
		if (noParallelUniverses.length > 0) {
			messages.push(
				nls.localize(
					...
					"以下文件已以不兼容的方式修改: {0}。", noParallelUniverses.join(', ')
				));
		}
...
export class UndoRedoService implements IUndoRedoService {
	...
	private _checkWorkspaceUndo(strResource: string, element: WorkspaceStackElement, editStackSnapshot: EditStackSnapshot, checkInvalidatedResources: boolean): WorkspaceVerificationError | null {
		if (element.removedResources) {
			return this._tryToSplitAndUndo(
				...
				nls.localize(
					...
					"无法在所有文件中撤消“{0}”。{1}", element.label, element.removedResources.createMessage()
				)
			);
		}
		if (checkInvalidatedResources && element.invalidatedResources) {
			return this._tryToSplitAndUndo(
				...
				nls.localize(
					{ key: 'cannotWorkspaceUndo', comment: ['{0} is a label for an operation. {1} is another message.'] },
					"无法在所有文件中撤消“{0}”。{1}", element.label, element.invalidatedResources.createMessage()
				)
			);
		}
		...
		if (cannotUndoDueToResources.length > 0) {
			return this._tryToSplitAndUndo(
				...
				nls.localize(
					...
					"无法撤消所有文件的“{0}”，因为已更改 {1}", element.label, cannotUndoDueToResources.join(', ')
				)
			);
		}
		...
		if (cannotLockDueToResources.length > 0) {
			return this._tryToSplitAndUndo(
				...
				nls.localize(
					...
					"无法跨所有文件撤销“{0}”，因为 {1} 上已有一项撤消或重做操作正在运行", element.label, cannotLockDueToResources.join(', ')
				)
			);
		}
		...
		if (!editStackSnapshot.isValid()) {
			return this._tryToSplitAndUndo(
				...
				nls.localize(
					...
					"无法跨所有文件撤销“{0}”，因为同时发生了一项撤消或重做操作", element.label
				)
			);
		}
	...
	private async _confirmAndExecuteWorkspaceUndo(strResource: string, element: WorkspaceStackElement, editStackSnapshot: EditStackSnapshot, undoConfirmed: boolean): Promise<void> {
		...
		if (element.canSplit() && !this._isPartOfUndoGroup(element)) {
			...
			const { result } = await this._dialogService.prompt<UndoChoice>({
				type: Severity.Info,
				message: nls.localize('confirmWorkspace', "是否要在所有文件中撤消“{0}”?", element.label),
				buttons: [
					{
						label: nls.localize({ key: 'ok', comment: ['{0} denotes a number that is > 1, && denotes a mnemonic'] }, "在 {0} 个文件中撤消(&&U)", editStackSnapshot.editStacks.length),
						run: () => UndoChoice.All
					},
					{
						label: nls.localize({ key: 'nok', comment: ['&& denotes a mnemonic'] }, "撤消此文件(&&F)"),
						run: () => UndoChoice.This
					}
				],
				cancelButton: {
	...
	private _resourceUndo(editStack: ResourceEditStack, element: ResourceStackElement, undoConfirmed: boolean): Promise<void> | void {
		...
		if (editStack.locked) {
			const message = nls.localize(
				...
				"无法撤销“{0}”，因为已有一项撤消或重做操作正在运行。", element.label
			);
			this._notificationService.warn(message);
			return;
		}
	...
	private async _confirmAndContinueUndo(strResource: string, sourceId: number, element: StackElement): Promise<void> {
		const result = await this._dialogService.confirm({
			message: nls.localize('confirmDifferentSource', "是否要撤消“{0}”?", element.label),
			primaryButton: nls.localize({ key: 'confirmDifferentSource.yes', comment: ['&& denotes a mnemonic'] }, "是(&&Y)"),
			cancelButton: nls.localize('confirmDifferentSource.no', "否")
		});
		...
		if (element.removedResources) {
			return this._tryToSplitAndRedo(
				...
				nls.localize(
					...
					"无法在所有文件中重做“{0}”。{1}", element.label, element.removedResources.createMessage()
				)
			);
		}
		if (checkInvalidatedResources && element.invalidatedResources) {
			return this._tryToSplitAndRedo(
				...
				nls.localize(
					{ key: 'cannotWorkspaceRedo', comment: ['{0} is a label for an operation. {1} is another message.'] },
					"无法在所有文件中重做“{0}”。{1}", element.label, element.invalidatedResources.createMessage()
				)
			);
		}
		...
		if (cannotRedoDueToResources.length > 0) {
			return this._tryToSplitAndRedo(
				...
				nls.localize(
					...
					"无法对所有文件重做“{0}”，因为已更改 {1}", element.label, cannotRedoDueToResources.join(', ')
				)
			);
		}
		...
		if (cannotLockDueToResources.length > 0) {
			return this._tryToSplitAndRedo(
				...
				nls.localize(
					...
					"无法跨所有文件重做“{0}”，因为 {1} 上已有一项撤消或重做操作正在运行", element.label, cannotLockDueToResources.join(', ')
				)
			);
		}
		...
		if (!editStackSnapshot.isValid()) {
			return this._tryToSplitAndRedo(
				...
				nls.localize(
					...
					"无法跨所有文件重做“{0}”，因为同时发生了一项撤消或重做操作", element.label
				)
			);
		}
	...
	private _resourceRedo(editStack: ResourceEditStack, element: ResourceStackElement): Promise<void> | void {
		...
		if (editStack.locked) {
			const message = nls.localize(
				...
				"无法重做“{0}”，因为已有一项撤消或重做操作正在运行。", element.label
			);
			this._notificationService.warn(message);
			return;
		}
