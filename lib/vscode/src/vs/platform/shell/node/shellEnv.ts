		...
		if (!unixShellEnvPromise) {
			unixShellEnvPromise = Promises.withAsyncBody<NodeJS.ProcessEnv>(async (resolve, reject) => {
				...
				const timeout = setTimeout(() => {
					cts.dispose(true);
					reject(new Error(localize('resolveShellEnvTimeout', "无法在合理的时间内解析 shell 环境。请检查 shell 配置，然后重启。")));
				}, timeoutValue);
				...
				try {
					...
					if (!isCancellationError(error) && !cts.token.isCancellationRequested) {
						reject(new Error(localize('resolveShellEnvError', "无法解析 shell 环境: {0}", toErrorMessage(error))));
					} else {
						resolve({});
					}
				} finally {
...
async function doResolveUnixShellEnv(logService: ILogService, token: CancellationToken): Promise<typeof process.env> {
	...
	return new Promise<typeof process.env>((resolve, reject) => {
		...
		child.on('close', (code, signal) => {
			...
			if (code || signal) {
				return reject(new Error(localize('resolveShellEnvExitError', "来自生成的 shell 的意外退出代码(代码 {0}、信号 {1})", code, signal)));
			}
