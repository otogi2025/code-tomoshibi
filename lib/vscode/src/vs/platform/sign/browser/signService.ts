...
export class SignService extends AbstractSignService implements ISignService {
	...
	protected override getValidator(): Promise<IVsdaValidator> {
		// VSDA is a private Microsoft dependency and is intentionally absent from
		// open-source builds. Do not ask the AMD loader for files that cannot exist:
		// besides two wasted requests, the failed module load can hold up the remote
		// handshake before the workbench has created its root element. The server has
		// the matching no-VSDA fallback and still validates the connection token.
		if (!this.productService.serverLicense?.length) {
			return Promise.reject(new Error('VSDA is not available in this open-source build'));
		}
