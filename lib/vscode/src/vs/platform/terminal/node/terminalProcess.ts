...
export class TerminalProcess extends Disposable implements ITerminalChildProcess {
	...
	async start(): Promise<ITerminalLaunchError | ITerminalLaunchResult | undefined> {
		...
		try {
			...
			if (errorMessage?.includes('Cannot launch conpty')) {
				return { message: localize('conptyLaunchFailed', "启动期间发生本机异常(无法启动 conpty)。已移除 Winpty，有关更多详细信息，请参阅 {0}。还可以尝试启用 `{1}` 设置。", 'https://code.visualstudio.com/updates/v1_109#_removal-of-winpty-support', 'terminal.integrated.windowsUseConptyDll') };
			}
	...
	private async _validateCwd(): Promise<undefined | ITerminalLaunchError> {
		try {
			...
			if (!result.isDirectory()) {
				return { message: localize('launchFail.cwdNotDirectory', "启动目录(cwd)“{0}”不是一个目录", this._initialCwd.toString()) };
			}
		} catch (err) {
			if (err?.code === 'ENOENT') {
				return { message: localize('launchFail.cwdDoesNotExist', "启动目录(cwd)“{0}”不存在", this._initialCwd.toString()) };
			}
		}
	...
	private async _validateExecutable(): Promise<undefined | ITerminalLaunchError> {
		...
		if (!executable) {
			return { message: localize('launchFail.executableDoesNotExist', "shell 可执行文件“{0}”的路径不存在", slc.executable) };
		}

		try {
			const result = await fs.promises.stat(executable);
			if (!result.isFile() && !result.isSymbolicLink()) {
				return { message: localize('launchFail.executableIsNotFileOrSymlink', "shell 可执行文件 \"{0}\" 的路径非文件或符号链接", slc.executable) };
			}
