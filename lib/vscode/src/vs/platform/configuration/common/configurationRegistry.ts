...
class ConfigurationRegistry extends Disposable implements IConfigurationRegistry {
	...
	constructor() {
		...
		this.defaultLanguageConfigurationOverridesNode = {
			id: 'defaultOverrides',
			title: nls.localize('defaultLanguageConfigurationOverrides.title', "默认语言配置替代"),
			properties: {}
		};
	...
	private updateDefaultOverrideProperty(key: string, newDefaultOverride: IConfigurationDefaultOverrideValue, source: ConfigurationDefaultSource | undefined): void {
		const property: IRegisteredConfigurationPropertySchema = {
			...
			description: nls.localize('defaultLanguageConfiguration.description', "配置要为 {0} 替代的设置。", getLanguageTagSettingPlainKey(key)),
	...
	private updateOverridePropertyPatternKey(): void {
		for (const overrideIdentifier of this.overrideIdentifiers.values()) {
			...
			const resourceLanguagePropertiesSchema: IJSONSchema = {
				type: 'object',
				description: nls.localize('overrideSettings.defaultDescription', "针对某种语言，配置替代编辑器设置。"),
				errorMessage: nls.localize('overrideSettings.errorMessage', "此设置不支持按语言配置。"),
				$ref: resourceLanguageSettingsSchemaId,
			};
...
export function validateProperty(property: string, schema: IRegisteredConfigurationPropertySchema, extensionId?: string): string | null {
	if (!property.trim()) {
		return nls.localize('config.property.empty', "无法注册空属性");
	}
	if (OVERRIDE_PROPERTY_REGEX.test(property)) {
		return nls.localize('config.property.languageDefault', "无法注册“{0}”。其符合描述特定语言编辑器设置的表达式 \"\\\\[.*\\\\]$\"。请使用 \"configurationDefaults\"。", property);
	}
	if (configurationRegistry.getConfigurationProperties()[property] !== undefined && (!extensionId || !EXTENSION_UNIFICATION_EXTENSION_IDS.has(extensionId.toLowerCase()))) {
		return nls.localize('config.property.duplicate', "无法注册“{0}”。此属性已注册。", property);
	}
	if (schema.policy && schema.policyReference) {
		return nls.localize('config.policy.bothPolicyAndReference', "无法注册“{0}”。设置不得同时声明 \"policy\" 和 \"policyReference\"。", property);
	}
	if (schema.policy?.name && configurationRegistry.getPolicyConfigurations().get(schema.policy?.name) !== undefined) {
		return nls.localize('config.policy.duplicate', "无法注册“{0}”。关联的策略 {1} 已向 {2} 注册。若要将另一个设置附加到同一策略，请使用 \"policyReference\"。", property, schema.policy?.name, configurationRegistry.getPolicyConfigurations().get(schema.policy?.name));
	}
	return null;
}
