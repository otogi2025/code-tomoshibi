...
export const InQuickInputContextKey = new RawContextKey<boolean>(inQuickInputContextKeyValue, false, localize('inQuickInput', "键盘焦点是否在快速输入控件内"));
...
export const QuickInputAlignmentContextKey = new RawContextKey<'top' | 'center' | undefined>(quickInputAlignmentContextKeyValue, 'top', localize('quickInputAlignment', "快速输入项的对齐方式"));

export const quickInputTypeContextKeyValue = 'quickInputType';
export const QuickInputTypeContextKey = new RawContextKey<QuickInputType>(quickInputTypeContextKeyValue, undefined, localize('quickInputType', "当前可见快速输入的类型"));

export const endOfQuickInputBoxContextKeyValue = 'cursorAtEndOfQuickInputBox';
export const EndOfQuickInputBoxContextKey = new RawContextKey<boolean>(endOfQuickInputBoxContextKeyValue, false, localize('cursorAtEndOfQuickInputBox', "快速输入中的光标是否位于输入框的末尾"));
...
export const backButton = {
	...
	tooltip: localize('quickInput.back', "上一步"),
	handle: -1 // TODO
};
...
export abstract class QuickInput extends Disposable implements IQuickInput {
	protected static readonly noPromptMessage = localize('inputModeEntry', "按 \"Enter\" 以确认或按 \"Esc\" 以取消");
...
export class QuickPick<T extends IQuickPickItem, O extends { useSeparators: boolean } = { useSeparators: false }> extends QuickInput implements IQuickPick<T, O> {
	...
	private static readonly DEFAULT_ARIA_LABEL = localize('quickInputBox.ariaLabel', "在此输入可缩小结果范围。");
	...
	get okLabel() {
		return this._okLabel ?? localize('ok', "确定");
	}
...
export class InputBox extends QuickInput implements IInputBox {
	...
	set prompt(prompt: string | undefined) {
		this._prompt = prompt;
		this.noValidationMessage = prompt
			? localize('inputModeEntryDescription', "{0} (按 \"Enter\" 以确认或按 \"Esc\" 以取消)", prompt)
