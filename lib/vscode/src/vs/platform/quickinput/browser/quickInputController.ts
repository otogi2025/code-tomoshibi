...
export class QuickInputController extends Disposable {
	...
	private getUI(showInActiveContainer?: boolean): QuickInputUI {
		...
		const checkAll = this._register(new TriStateCheckbox(localize('quickInput.checkAll', "切换所有复选框"), false, { ...defaultCheckboxStyles, size: 15 }));
		...
		const visibleCount = this._register(new CountBadge(visibleCountContainer, { countFormat: localize({ key: 'quickInput.visibleCount', comment: ['This tells the user how many items are shown in a list of items to select from. The items can be anything. Currently not visible, but read by screen readers.'] }, "{0} 个结果") }, this.styles.countBadge));

		const countContainer = dom.append(filterContainer, $('.quick-input-count'));
		countContainer.setAttribute('aria-live', 'polite');
		const count = this._register(new CountBadge(countContainer, { countFormat: localize({ key: 'quickInput.countSelected', comment: ['This tells the user how many items are selected in a list of items to select from. The items can be anything.'] }, "已选 {0} 项") }, this.styles.countBadge));
		...
		customButton.label = localize('custom', "自定义");
	...
	private show(controller: IQuickInput) {
		...
		backButton.tooltip = backKeybindingLabel ? localize('quickInput.backWithKeybinding', "后退 ({0})", backKeybindingLabel) : localize('quickInput.back', "上一步");
