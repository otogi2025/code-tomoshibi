...
export class FileService extends Disposable implements IFileService {
	...
	protected async withProvider(resource: URI): Promise<IFileSystemProvider> {
		...
		if (!isAbsolutePath(resource)) {
			throw new FileOperationError(localize('invalidPath', "无法解析具有相对文件路径\"{0}\"的文件系统提供程序", this.resourceForError(resource)), FileOperationResult.FILE_INVALID_PATH);
		}
		...
		if (!provider) {
			...
			error.message = localize('noProviderFound', "ENOPRO: 未找到资源 \"{0}\" 的文件系统提供程序", resource.toString());
	...
	async resolve(resource: URI, options?: IResolveFileOptions): Promise<IFileStat> {
		try {
			...
			if (toFileSystemProviderErrorCode(error) === FileSystemProviderErrorCode.FileNotFound) {
				throw new FileOperationError(localize('fileNotFoundError', "无法解析不存在的文件 '{0}'", this.resourceForError(resource)), FileOperationResult.FILE_NOT_FOUND);
			}
	...
	private async doValidateCreateFile(resource: URI, options?: ICreateFileOptions): Promise<void> {
		...
		if (!options?.overwrite && await this.exists(resource)) {
			throw new FileOperationError(localize('fileExists', "如果未设置覆盖标记，则无法创建文件“{0}”，因为它已存在", this.resourceForError(resource)), FileOperationResult.FILE_MODIFIED_SINCE, options);
		}
	}
	...
	async writeFile(resource: URI, bufferOrReadableOrStream: VSBuffer | VSBufferReadable | VSBufferReadableStream, options?: IWriteFileOptions): Promise<IFileStatWithMetadata> {
		...
		try {
			...
			throw new FileOperationError(localize('err.write', "无法写入文件\"{0}\"({1})", this.resourceForError(resource), ensureFileSystemProviderError(error).toString()), toFileOperationResult(error), writeFileOptions);
		}
	...
	private async validateWriteFile(provider: IFileSystemProviderWithFileReadWriteCapability | IFileSystemProviderWithOpenReadWriteCloseCapability, resource: URI, bufferOrReadableOrStream: VSBuffer | VSBufferReadable | VSBufferReadableStream, options?: IWriteFileOptions): Promise<{ stat: IStat | undefined; buffer: VSBuffer | VSBufferReadable | VSBufferReadableStream | VSBufferReadableBufferedStream | undefined }> {
		...
		if (unlock && !(provider.capabilities & FileSystemProviderCapabilities.FileWriteUnlock)) {
			throw new Error(localize('writeFailedUnlockUnsupported', "无法解锁文件“{0}”，因为提供程序不支持它。", this.resourceForError(resource)));
		}

		// Validate append support
		if (options?.append && !hasFileAppendCapability(provider)) {
			throw new FileOperationError(localize('err.noAppend', "方案“{0}”的文件系统提供程序不支持追加", this.resourceForError(resource)), FileOperationResult.FILE_PERMISSION_DENIED);
		}
		...
		if (atomic) {
			if (!(provider.capabilities & FileSystemProviderCapabilities.FileAtomicWrite)) {
				throw new Error(localize('writeFailedAtomicUnsupported1', "无法以原子方式写入文件 \"{0}\"，因为提供程序不支持它。", this.resourceForError(resource)));
			}

			if (!(provider.capabilities & FileSystemProviderCapabilities.FileReadWrite)) {
				throw new Error(localize('writeFailedAtomicUnsupported2', "无法以原子方式写入文件 '{0}'，因为提供程序不支持未缓冲的写入。", this.resourceForError(resource)));
			}

			if (unlock) {
				throw new Error(localize('writeFailedAtomicUnlock', "无法解锁文件 \"{0}\"，因为已启用原子写入。", this.resourceForError(resource)));
			}
		}
		...
		if ((stat.type & FileType.Directory) !== 0) {
			throw new FileOperationError(localize('fileIsDirectoryWriteError', "无法写入实际上是一个目录的文件\"{0}\"", this.resourceForError(resource)), FileOperationResult.FILE_IS_DIRECTORY, options);
		}
		...
		if (
			...
			throw new FileOperationError(localize('fileModifiedError', "自以下时间已修改的文件:"), FileOperationResult.FILE_MODIFIED_SINCE, options);
		}
	...
	private restoreReadError(error: Error, resource: URI, options?: IReadFileStreamOptions): FileOperationError {
		const message = localize('err.read', "无法读取文件'{0}' ({1})", this.resourceForError(resource), ensureFileSystemProviderError(error).toString());
	...
	private async validateReadFile(resource: URI, options?: IReadFileStreamOptions): Promise<IFileStatWithMetadata> {
		...
		if (stat.isDirectory) {
			throw new FileOperationError(localize('fileIsDirectoryReadError', "无法读取实际上是一个目录的文件\"{0}\"", this.resourceForError(resource)), FileOperationResult.FILE_IS_DIRECTORY, options);
		}

		// Throw if file not modified since (unless disabled)
		if (typeof options?.etag === 'string' && options.etag !== ETAG_DISABLED && options.etag === stat.etag) {
			throw new NotModifiedSinceFileOperationError(localize('fileNotModifiedError', "自以下时间未修改的文件:"), stat, options);
		}
	...
	private validateReadFileLimits(resource: URI, size: number, options?: IReadFileStreamOptions): void {
		if (typeof options?.limits?.size === 'number' && size > options.limits.size) {
			throw new TooLargeFileOperationError(localize('fileTooLargeError', "无法读取文件“{0}”，该文件太大，无法打开", this.resourceForError(resource)), FileOperationResult.FILE_TOO_LARGE, size, options);
		}
	}
	...
	private async doValidateMoveCopy(sourceProvider: IFileSystemProvider, source: URI, targetProvider: IFileSystemProvider, target: URI, mode: 'move' | 'copy', overwrite?: boolean): Promise<{ exists: boolean; isSameResourceWithDifferentPathCase: boolean }> {
		...
		if (sourceProvider === targetProvider) {
			...
			if (isSameResourceWithDifferentPathCase && mode === 'copy') {
				throw new Error(localize('unableToMoveCopyError1', "当源\"{0}\"与目标\"{1}\"在不区分大小写的文件系统上具有不同路径大小写时，无法复制", this.resourceForError(source), this.resourceForError(target)));
			}

			if (!isSameResourceWithDifferentPathCase && providerExtUri.isEqualOrParent(target, source)) {
				throw new Error(localize('unableToMoveCopyError2', "当源\"{0}\"是目标\"{1}\"的父级时，无法移动/复制。", this.resourceForError(source), this.resourceForError(target)));
			}
		}
		...
		if (exists && !isSameResourceWithDifferentPathCase) {
			...
			if (!overwrite) {
				throw new FileOperationError(localize('unableToMoveCopyError3', "无法移动/复制\"{0}\"，因为目标\"{1}\"已存在于目标位置。", this.resourceForError(source), this.resourceForError(target)), FileOperationResult.FILE_MOVE_CONFLICT);
			}
			...
			if (sourceProvider === targetProvider) {
				...
				if (providerExtUri.isEqualOrParent(source, target)) {
					throw new Error(localize('unableToMoveCopyError4', "无法将\"{0}\"移动/复制到\"{1}\"中，因为文件将替换包含该文件的文件夹。", this.resourceForError(source), this.resourceForError(target)));
				}
			}
		}
	...
	private async mkdirp(provider: IFileSystemProvider, directory: URI): Promise<void> {
		...
		while (!providerExtUri.isEqual(directory, providerExtUri.dirname(directory))) {
			try {
				...
				if ((stat.type & FileType.Directory) === 0) {
					throw new Error(localize('mkdirExistsError', "无法创建已存在但不是目录的文件夹\"{0}\"", this.resourceForError(directory)));
				}
	...
	private async doValidateDelete(resource: URI, options?: Partial<IFileDeleteOptions>): Promise<IFileSystemProvider> {
		...
		if (useTrash && !(provider.capabilities & FileSystemProviderCapabilities.Trash)) {
			throw new Error(localize('deleteFailedTrashUnsupported', "无法通过回收站删除文件\"{0}\"，因为提供程序不支持它。", this.resourceForError(resource)));
		}

		// Validate atomic support
		const atomic = options?.atomic;
		if (atomic && !(provider.capabilities & FileSystemProviderCapabilities.FileAtomicDelete)) {
			throw new Error(localize('deleteFailedAtomicUnsupported', "无法自动删除文件\"{0}\"，因为提供程序不支持它。", this.resourceForError(resource)));
		}

		if (useTrash && atomic) {
			throw new Error(localize('deleteFailedTrashAndAtomicUnsupported', "无法以原子方式删除文件 \"{0}\"，因为已启用使用回收站。", this.resourceForError(resource)));
		}
		...
		if (stat) {
			...
			throw new FileOperationError(localize('deleteFailedNotFound', "无法删除不存在的文件 '{0}'", this.resourceForError(resource)), FileOperationResult.FILE_NOT_FOUND);
		}
		...
		if (!recursive) {
			...
			if (stat.isDirectory && Array.isArray(stat.children) && stat.children.length > 0) {
				throw new Error(localize('deleteFailedNonEmptyFolder', "无法删除非空文件夹“{0}”。", this.resourceForError(resource)));
			}
		}
	...
	protected throwIfFileSystemIsReadonly<T extends IFileSystemProvider>(provider: T, resource: URI): T {
		if (provider.capabilities & FileSystemProviderCapabilities.Readonly) {
			throw new FileOperationError(localize('err.readonly', "无法修改只读文件“{0}”", this.resourceForError(resource)), FileOperationResult.FILE_PERMISSION_DENIED);
		}
