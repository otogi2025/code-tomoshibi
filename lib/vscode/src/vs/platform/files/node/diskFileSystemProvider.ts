...
export class DiskFileSystemProvider extends AbstractDiskFileSystemProvider implements
	...
	private async doWriteFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions, disableWriteLock?: boolean): Promise<void> {
		...
		try {
			...
			if (!opts.create || !opts.overwrite) {
				...
				if (fileExists) {
					if (!opts.overwrite) {
						throw createFileSystemProviderError(localize('fileExists', "文件已存在"), FileSystemProviderErrorCode.FileExists);
					}
				} else {
					if (!opts.create) {
						throw createFileSystemProviderError(localize('fileNotExists', "文件不存在"), FileSystemProviderErrorCode.FileNotFound);
					}
				}
			}
	...
	async rename(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<void> {
		...
		try {
			...
			if (error.code === 'EINVAL' || error.code === 'EBUSY' || error.code === 'ENAMETOOLONG') {
				error = new Error(localize('moveError', "无法将 \"{0}\" 移动到 \"{1}\" ({2}) 中。", basename(fromFilePath), basename(dirname(toFilePath)), error.toString()));
			}
	...
	async copy(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<void> {
		...
		try {
			...
			// to something the user can better understand
			if (error.code === 'EINVAL' || error.code === 'EBUSY' || error.code === 'ENAMETOOLONG') {
				error = new Error(localize('copyError', "无法将 \"{0}\" 复制到 \"{1}\" ({2}) 中。", basename(fromFilePath), basename(dirname(toFilePath)), error.toString()));
			}
	...
	private async validateMoveCopy(from: URI, to: URI, mode: 'move' | 'copy', overwrite?: boolean): Promise<void> {
		...
		if (isSameResourceWithDifferentPathCase) {
			...
			if (mode === 'copy') {
				throw createFileSystemProviderError(localize('fileCopyErrorPathCase', "文件不能复制到仅大小写不同的相同路径"), FileSystemProviderErrorCode.FileExists);
			}
		...
		if (!fromStat) {
			throw createFileSystemProviderError(localize('fileMoveCopyErrorNotFound', "要移动/复制的文件不存在"), FileSystemProviderErrorCode.FileNotFound);
		}
		...
		if (!overwrite) {
			throw createFileSystemProviderError(localize('fileMoveCopyErrorExists', "目标上的文件已存在，因此不会移动/复制到其中，除非指定了覆盖"), FileSystemProviderErrorCode.FileExists);
		}
