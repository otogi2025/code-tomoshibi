...
const errorEmptyString = localize('contextkey.parser.error.emptyString', "上下文键表达式为空");
const hintEmptyString = localize('contextkey.parser.error.emptyString.hint', "忘记写入表达式了吗? 还可以放置 \"false\" 或 \"true\" 以始终分别评估为 false 或 true。");
const errorNoInAfterNot = localize('contextkey.parser.error.noInAfterNot', "\"not\" 后面的 \"in\"。");
const errorClosingParenthesis = localize('contextkey.parser.error.closingParenthesis', "右括号 \")\"");
const errorUnexpectedToken = localize('contextkey.parser.error.unexpectedToken', "意外的令牌");
const hintUnexpectedToken = localize('contextkey.parser.error.unexpectedToken.hint', "忘记在令牌之前放置 && 或 || 了吗?");
const errorUnexpectedEOF = localize('contextkey.parser.error.unexpectedEOF', "意外的表达式结尾");
const hintUnexpectedEOF = localize('contextkey.parser.error.unexpectedEOF.hint', "忘记放置上下文键了吗?");
...
export class Parser {
	...
	private _errExpectedButGot(expected: string, got: Token, additionalInfo?: string) {
		const message = localize('contextkey.parser.error.expectedButGot', "应为: {0}\r\n收到的: \"{1}\"。", expected, Scanner.getLexeme(got));
		...
		if (parser.lexingErrors.length > 0) {
			return parser.lexingErrors.map((se: LexingError) => ({
				errorMessage: se.additionalInfo ?
					localize('contextkey.scanner.errorForLinterWithHint', "意外的令牌。提示: {0}", se.additionalInfo) :
					localize('contextkey.scanner.errorForLinter', "意外的令牌。"),
				offset: se.offset,
