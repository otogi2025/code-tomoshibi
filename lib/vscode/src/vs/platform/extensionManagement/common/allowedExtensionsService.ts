...
export class AllowedExtensionsService extends Disposable implements IAllowedExtensionsService {
	...
	isAllowed(extension: IGalleryExtension | IExtension | { id: string; publisherDisplayName: string | undefined; version?: string; prerelease?: boolean; targetPlatform?: TargetPlatform }): true | IMarkdownString {
		...
		const extensionReason = new MarkdownString(nls.localize('specific extension not allowed', "它不在[允许列表]({0})中", settingsCommandLink));
		if (!isUndefined(extensionValue)) {
			...
			if (extensionValue === 'stable' && prerelease) {
				return new MarkdownString(nls.localize('extension prerelease not allowed', "此扩展的预发布版本不在[允许列表]({0})中", settingsCommandLink));
			}
			if (version !== '*' && Array.isArray(extensionValue) && !extensionValue.some(v => {
				...
				return new MarkdownString(nls.localize('specific version of extension not allowed', "此扩展的版本 {0} 不在[允许列表]({1})中", version, settingsCommandLink));
			}
			return true;
		}
		...
		if (!isUndefined(publisherValue)) {
			if (isBoolean(publisherValue)) {
				return publisherValue ? true : new MarkdownString(nls.localize('publisher not allowed', "来自此发布者的扩展不在[允许列表]({1})中", publisherKey, settingsCommandLink));
			}
			if (publisherValue === 'stable' && prerelease) {
				return new MarkdownString(nls.localize('prerelease versions from this publisher not allowed', "来自此发布者的预发布版本不在[允许列表]({1})中", publisherKey, settingsCommandLink));
			}
			return true;
		}
