	...
	async install(vsix: URI, options: InstallOptions = {}): Promise<ILocalExtension> {
		...
		try {
			...
			if (manifest.engines && manifest.engines.vscode && !isEngineValid(manifest.engines.vscode, this.productService.version, this.productService.date)) {
				throw new Error(nls.localize('incompatible', "无法安装扩展“{0}”，因为它与 VS Code“{1}”不兼容。", extensionId, this.productService.version));
			}
			...
			if (allowedToInstall !== true) {
				throw new Error(nls.localize('notAllowed', "无法安装此扩展，因为 {0}", allowedToInstall.value));
			}
	...
	private async downloadAndExtractGalleryExtension(extensionKey: ExtensionKey, gallery: IGalleryExtension, operation: InstallOperation, options: InstallExtensionTaskOptions, token: CancellationToken): Promise<ExtractExtensionResult> {
		...
		try {
			...
			if (!new ExtensionKey(gallery.identifier, gallery.version).equals(new ExtensionKey({ id: getGalleryExtensionId(manifest.publisher, manifest.name) }, manifest.version))) {
				throw new ExtensionManagementError(nls.localize('invalidManifest', "无法安装“{0}”扩展，因为清单与市场不匹配", gallery.identifier.id), ExtensionManagementErrorCode.Invalid);
			}
	...
	private async downloadExtension(extension: IGalleryExtension, operation: InstallOperation, verifySignature: boolean, clientTargetPlatform?: TargetPlatform): Promise<{ readonly location: URI; readonly verificationStatus: ExtensionSignatureVerificationCode | undefined }> {
		...
		if (
			...
			if (!verificationStatus) {
				throw new ExtensionManagementError(nls.localize('signature verification not executed', "签名验证未执行。"), ExtensionManagementErrorCode.SignatureVerificationInternal);
			}
			...
			switch (verificationStatus) {
				...
				case ExtensionSignatureVerificationCode.NotSigned:
					throw new ExtensionManagementError(nls.localize('signature verification failed', "签名验证失败，出现“{0}”错误。", verificationStatus), ExtensionManagementErrorCode.SignatureVerificationFailed);
			}

			throw new ExtensionManagementError(nls.localize('signature verification failed', "签名验证失败，出现“{0}”错误。", verificationStatus), ExtensionManagementErrorCode.SignatureVerificationInternal);
		}
...
export class ExtensionsScanner extends Disposable {
	...
	async extractUserExtension(extensionKey: ExtensionKey, zipPath: string, removeIfExists: boolean, token: CancellationToken): Promise<ILocalExtension> {
		...
		if (await this.fileService.exists(extensionLocation)) {
			...
			try {
				...
				throw new ExtensionManagementError(nls.localize('errorDeleting', "安装扩展“{1}”时无法删除现有文件夹“{0}”。请手动删除此文件夹，然后重试", extensionLocation.fsPath, extensionKey.id), ExtensionManagementErrorCode.Delete);
			}
		}
	...
	async scanLocalExtension(location: URI, type: ExtensionType, profileLocation?: URI): Promise<ILocalExtension> {
		try {
			...
			throw new ExtensionManagementError(nls.localize('cannot read', "无法从 {0} 读取扩展", location.path), ExtensionManagementErrorCode.ScanningExtension);
		} catch (error) {
	...
	protected async doRun(token: CancellationToken): Promise<ILocalExtension> {
		...
		if (existingSystemExtension) {
			if (!existingSystemExtension.forceAutoUpdate) {
				throw new ExtensionManagementError(nls.localize('builtinAutoUpdate', "扩展“{0}”是内置扩展，不允许在当前产品质量“{1}”下更新。", existingSystemExtension.identifier.id, this.productService.quality), ExtensionManagementErrorCode.Incompatible);
			}
			if (semver.gt(existingSystemExtension.manifest.version, this.manifest.version)) {
				throw new ExtensionManagementError(nls.localize('builtinVersion', "扩展“{0}”是版本为“{1}”的内置扩展，无法降级到版本“{2}”。", existingSystemExtension.identifier.id, existingSystemExtension.manifest.version, this.manifest.version), ExtensionManagementErrorCode.Incompatible);
			}
		}
			...
			if (existingExtension) {
				if (this.extensionKey.equals(new ExtensionKey(existingExtension.identifier, existingExtension.manifest.version))) {
					try {
						...
						throw new Error(nls.localize('restartCode', "请在重新安装{0}之前重新启动 VS Code。", this.manifest.displayName || this.manifest.name));
					}
				}
			}
