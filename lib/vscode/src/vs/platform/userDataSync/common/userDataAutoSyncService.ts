	...
	private async createAndRunSyncTask(disableCache: boolean, token: CancellationToken): Promise<void> {
		...
		if (this.manifest === null && await this.userDataSyncService.hasPreviouslySynced()) {
			if (this.hasSyncServiceChanged()) {
				if (await this.hasDefaultServiceChanged()) {
					throw new UserDataAutoSyncError(localize('default service changed', "默认服务已更改，因此无法同步"), UserDataSyncErrorCode.DefaultServiceChanged);
				} else {
					throw new UserDataAutoSyncError(localize('service changed', "同步服务已更改，因此无法同步"), UserDataSyncErrorCode.ServiceChanged);
				}
			} else {
				// Sync was turned off in the cloud
				throw new UserDataAutoSyncError(localize('turned off', "无法同步，因为同步在云中已关闭"), UserDataSyncErrorCode.TurnedOff);
			}
		}
		...
		if (sessionId && this.manifest && sessionId !== this.manifest.session) {
			if (this.hasSyncServiceChanged()) {
				...
				throw new UserDataAutoSyncError(localize('session expired', "无法同步，因为当前会话已过期"), UserDataSyncErrorCode.SessionExpired);
			}
		}
		...
		if (currentMachine?.disabled) {
			...
			throw new UserDataAutoSyncError(localize('turned off machine', "无法同步，因为已从另一台计算机上关闭了此计算机上的同步。"), UserDataSyncErrorCode.TurnedOff);
		}
