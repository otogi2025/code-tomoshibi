	...
	renderElement(element: IActionListItem<T>, _index: number, data: IActionMenuTemplateData): void {
		...
		if (element.hover !== undefined) {
			...
			if (this._supportsPreview && element.canPreview) {
				data.container.title = localize({ key: 'label-preview', comment: ['placeholders are keybindings, e.g "F2 to Apply, Shift+F2 to Preview"'] }, "按 {0} 以应用，按 {1} 以预览", actionTitle, previewTitle);
			} else {
				data.container.title = localize({ key: 'label', comment: ['placeholder is a keybinding, e.g "F2 to Apply"'] }, "按 {0} 以应用", actionTitle);
			}
		} else {
			data.container.title = '';
		}
		...
		if (element.onRemove) {
			toolbarActions.push(toAction({
				...
				label: localize('actionList.remove', "移除"),
	...
	constructor(
					...
					if (element.kind === ActionListItemKind.Action) {
						...
						if (element.inlineToggle) {
							label = label + ', ' + (element.inlineToggle.checked
								? localize('actionList.inlineToggle.on', "{0}，开", element.inlineToggle.label)
								: localize('actionList.inlineToggle.off', "{0}，关", element.inlineToggle.label));
						}
						if (element.disabled) {
							label = localize({ key: 'customQuickFixWidget.labels', comment: [`Action widget labels for accessibility.`] }, "{0}，禁用原因: {1}", label, element.disabled);
						}
						if (element.submenuActions?.length) {
							label = localize('actionList.submenuHint', "{0}，按向右键访问选项", label);
						}
						return label;
					}
					return null;
				},
				getWidgetAriaLabel: () => localize({ key: 'customQuickFixWidget', comment: [`An action widget option`] }, "操作小组件"),
				getRole: (e) => {
					switch (e.kind) {
		...
		if (this._options?.showFilter || this._options?.secondaryHeading) {
			...
			if (this._options?.showFilter) {
				...
				this._filterInput.placeholder = this._options?.filterPlaceholder ?? localize('actionList.filter.placeholder', "搜索...");
				this._filterInput.setAttribute('aria-label', localize('actionList.filter.ariaLabel', "筛选项目"));
		...
		if (this._options?.headerText) {
			...
			if (this._options.headerDismiss) {
				...
				dismissButton.setAttribute('aria-label', localize('actionList.header.dismiss', "关闭"));
				const dismiss = () => {
					onDismiss();
