...
class YamlParser {
	...
	private parseBlockMapping(baseIndent: number, inlineFirstEntry = false): YamlMapNode {
		...
		while (this.currentToken().type !== TokenType.EOF) {
			...
			if (indent !== baseIndent) {
				if (indent > baseIndent) {
					this.emitError(
						localize('unexpectedIndentation', '意外缩进(预期为 {0}，实际为 {1})', baseIndent, indent),
			...
			if (!this.options.allowDuplicateKeys && seenKeys.has(entry.key.value)) {
				this.emitError(
					localize('duplicateKey', '重复的键: "{0}"', entry.key.value),
	...
	private parseMappingEntry(baseIndent: number): { key: YamlScalarNode; value: YamlNode } | undefined {
		...
		if (colon.type !== TokenType.Colon) {
			this.emitError(localize('expectedColon', '应为 ":"'), colon.startOffset, colon.endOffset, 'expected-colon');
			return undefined;
		}
	...
	private parseMappingValue(baseIndent: number, colonToken: Token): YamlNode {
		...
		if (afterNewline.type === TokenType.EOF) {
			...
			this.emitError(localize('missingValue', '缺少值'), colonToken.startOffset, colonToken.endOffset, 'missing-value');
	...
	private parseSequenceItemValue(baseIndent: number, dashToken: Token): YamlNode {
		...
		if (this.currentToken().type === TokenType.EOF) {
			this.emitError(localize('missingSeqItemValue', '缺少序列项的值'), dashToken.startOffset, dashToken.endOffset, 'missing-value');
	...
	private parseFlowMap(): YamlMapNode {
		...
		while (this.currentToken().type !== TokenType.FlowMapEnd && this.currentToken().type !== TokenType.EOF) {
			...
			if (this.currentToken().type === TokenType.Scalar) {
				...
				this.emitError(localize('expectedMappingKey', '预期的映射键'), this.currentToken().startOffset, this.currentToken().endOffset, 'expected-key');
				break;
			}
		...
		if (endToken.type === TokenType.FlowMapEnd) {
			...
			this.emitError(localize('expectedFlowMapEnd', '应为 "}"'), endToken.startOffset, endToken.endOffset, 'expected-flow-map-end');
		}
	...
	private parseFlowSeq(): YamlSequenceNode {
		...
		while (this.currentToken().type !== TokenType.FlowSeqEnd && this.currentToken().type !== TokenType.EOF) {
			...
			if (this.currentToken().type === TokenType.FlowMapStart) {
				...
				this.emitError(localize('unexpectedTokenInFlowSeq', '流序列中出现意外的标记'), this.currentToken().startOffset, this.currentToken().endOffset, 'unexpected-token');
				this.advance();
				continue;
			}
		...
		if (endToken.type === TokenType.FlowSeqEnd) {
			...
			this.emitError(localize('expectedFlowSeqEnd', '应为 "]"'), endToken.startOffset, endToken.endOffset, 'expected-flow-seq-end');
		}
