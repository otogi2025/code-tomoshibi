...
function extractEntry(stream: Readable, fileName: string, mode: number, targetPath: string, options: IOptions, token: CancellationToken): Promise<void> {
	...
	if (!targetDirName.startsWith(targetPath)) {
		return Promise.reject(new Error(nls.localize('invalid file', "提取 {0} 时出错。文件无效。", fileName)));
	}
	...
	return new Promise<void>((c, e) => {
		...
		zipfile.once('close', () => last.then(() => {
			if (token.isCancellationRequested || zipfile.entryCount === extractedEntriesCount) {
				...
				e(new ExtractError('Incomplete', new Error(nls.localize('incompleteExtract', "解压不完整。找到了 {0} / {1} 个项目", extractedEntriesCount, zipfile.entryCount))));
			}
		}, e));
		zipfile.readEntry();
...
function read(zipPath: string, filePath: string): Promise<Readable> {
	return openZip(zipPath).then(zipfile => {
		return new Promise<Readable>((c, e) => {
			...
			zipfile.once('close', () => e(new Error(nls.localize('notFound', "在 Zip 中找不到 {0}。", filePath))));
		});
	});
}
