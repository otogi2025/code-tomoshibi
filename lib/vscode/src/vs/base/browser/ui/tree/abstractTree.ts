...
class FindWidget<T, TFilterData> extends Disposable {
	...
	constructor(
		...
		this.findInput = this._register(new FindInput(this.elements.findInput, contextViewProvider, {
			label: localize('type to search', "要搜索的类型"),
			placeholder,
		...
		const closeAction = this._register(new Action('close', localize('close', "关闭"), 'codicon codicon-close', true, () => this.dispose()));
...
export abstract class AbstractFindController<T, TFilterData> implements IDisposable {
	...
	constructor(
		...
		this._placeholder = options.placeholder ?? localize('type to search', "要搜索的类型");
	}
	...
	protected renderMessage(showNotFound: boolean, warningMessage?: string): void {
		if (showNotFound) {
			if (this.tree.options.showNotFoundMessage ?? true) {
				this.widget?.showMessage({ type: MessageType.WARNING, content: warningMessage ?? localize('not found', "没有找到结果") });
			} else {
	...
	protected alertResults(results: number): void {
		if (!results) {
			alert(localize('replFindNoResults', "无结果"));
		} else {
			alert(localize('foundResults', "{0} 个结果", results));
		}
	}
...
export class FindController<T, TFilterData> extends AbstractFindController<T, TFilterData> {
	...
	set mode(mode: TreeFindMode) {
		...
		this.placeholder = isFilterMode ? localize('type to filter', "要筛选的类型") : localize('type to search', "要搜索的类型");
		...
		if (isMacintosh) {
			this.modelDisposables.add(model.onDidChangeCollapseState(e => {
				...
				if (node.collapsible && !deep && this.isDOMFocused()) {
					alert(node.collapsed ? localize('treeNodeCollapsed', "已折叠") : localize('treeNodeExpanded', "已展开"));
				}
			}));
		}
	}
