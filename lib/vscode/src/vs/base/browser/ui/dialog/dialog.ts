...
export class Dialog extends Disposable {
	...
	private getIconAriaLabel(): string {
		let typeLabel = localize('dialogInfoMessage', '信息');
		switch (this.options.type) {
			case 'error':
				typeLabel = localize('dialogErrorMessage', '错误');
				break;
			case 'warning':
				typeLabel = localize('dialogWarningMessage', '警告');
				break;
			case 'pending':
				typeLabel = localize('dialogPendingMessage', '正在进行');
				break;
			case 'none':
			case 'info':
			case 'question':
	...
	async show(): Promise<IDialogResult> {
		...
		return new Promise<IDialogResult>(resolve => {
			...
			if (!this.options.disableCloseAction && !this.options.disableCloseButton) {
				...
				const action = this._register(new Action('dialog.close', localize('dialogClose', "关闭对话框"), ThemeIcon.asClassName(Codicon.dialogClose), true, async () => {
					resolve({
						button: this.options.cancelId || 0,
						checkboxChecked: this.checkbox ? this.checkbox.checked : undefined
					});
