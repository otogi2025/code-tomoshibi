...
export class InputBox extends Widget {
	...
	private _showMessage(): void {
		...
		if (this.message.type === MessageType.ERROR) {
			alertText = nls.localize('alertErrorMessage', "错误: {0}", this.message.content);
		} else if (this.message.type === MessageType.WARNING) {
			alertText = nls.localize('alertWarningMessage', "警告: {0}", this.message.content);
		} else {
			alertText = nls.localize('alertInfoMessage', "信息: {0}", this.message.content);
		}
...
export class HistoryInputBox extends InputBox implements IHistoryNavigationWidget {
	...
	public showNextValue(): void {
		...
		this.value = next ?? '';
		aria.status(this.value ? this.value : nls.localize('clearedInput', "清除的输入"));
	}
