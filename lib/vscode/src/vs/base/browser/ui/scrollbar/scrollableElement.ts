...
/**
 * The default vertical scrollbar size (px) used when a scrollable element does not
 * pass an explicit size. Sized for a fingertip: this product is driven by touch, so
 * a mouse-sized 10px slider cannot be grabbed reliably.
 */
export const DEFAULT_SCROLLBAR_SIZE = 20;

/**
 * The default horizontal scrollbar size (px). Horizontal scrollbars steal height from
 * the content below them, so they stay slightly slimmer than the vertical ones.
 */
export const DEFAULT_HORIZONTAL_SCROLLBAR_SIZE = 16;

export interface IDefaultScrollbarSizes {
	readonly horizontal: number;
	readonly vertical: number;
}

let globalDefaultScrollbarSizes: IDefaultScrollbarSizes = { horizontal: DEFAULT_HORIZONTAL_SCROLLBAR_SIZE, vertical: DEFAULT_SCROLLBAR_SIZE };
const _onDidChangeDefaultScrollbarSizeEmitter = new Emitter<IDefaultScrollbarSizes>();
export const onDidChangeDefaultScrollbarSize: Event<IDefaultScrollbarSizes> = _onDidChangeDefaultScrollbarSizeEmitter.event;
 ...
 *
 * Passing `DEFAULT_SCROLLBAR_SIZE` restores the per-axis defaults; any other value
 * is applied to both axes.
 */
export function setGlobalDefaultScrollbarSize(size: number): void {
	const sizes: IDefaultScrollbarSizes = size === DEFAULT_SCROLLBAR_SIZE
		? { horizontal: DEFAULT_HORIZONTAL_SCROLLBAR_SIZE, vertical: DEFAULT_SCROLLBAR_SIZE }
		: { horizontal: size, vertical: size };
	if (sizes.horizontal !== globalDefaultScrollbarSizes.horizontal || sizes.vertical !== globalDefaultScrollbarSizes.vertical) {
		globalDefaultScrollbarSizes = sizes;
		_onDidChangeDefaultScrollbarSizeEmitter.fire(sizes);
	}
}
...
export abstract class AbstractScrollableElement extends Widget {
	...
	protected constructor(element: HTMLElement, options: ScrollableElementCreationOptions, scrollable: Scrollable) {
		...
		if (!hSizeExplicit || !vSizeExplicit) {
			this._register(onDidChangeDefaultScrollbarSize(newSizes => {
				this.updateOptions({
					...(!hSizeExplicit ? { horizontalScrollbarSize: newSizes.horizontal } : {}),
					...(!vSizeExplicit ? { verticalScrollbarSize: newSizes.vertical } : {}),
				});
			}));
		}
	}
...
function resolveOptions(opts: ScrollableElementCreationOptions): ScrollableElementResolvedOptions {
	const result: ScrollableElementResolvedOptions = {
		...
		horizontalScrollbarSize: (typeof opts.horizontalScrollbarSize !== 'undefined' ? opts.horizontalScrollbarSize : globalDefaultScrollbarSizes.horizontal),
		horizontalSliderSize: (typeof opts.horizontalSliderSize !== 'undefined' ? opts.horizontalSliderSize : 0),
		horizontalHasArrows: (typeof opts.horizontalHasArrows !== 'undefined' ? opts.horizontalHasArrows : false),

		vertical: (typeof opts.vertical !== 'undefined' ? opts.vertical : ScrollbarVisibility.Auto),
		verticalScrollbarSize: (typeof opts.verticalScrollbarSize !== 'undefined' ? opts.verticalScrollbarSize : globalDefaultScrollbarSizes.vertical),
