 ...
 * Sized for a fingertip rather than a mouse pointer, since this product is driven by touch.
 */
const MINIMUM_SLIDER_SIZE = 44;
...
export class ScrollbarState {
	...
	private static _computeValues(oppositeScrollbarSize: number, arrowSize: number, visibleSize: number, scrollSize: number, scrollPosition: number) {
		...
		// The minimum is clamped to the representable size so a very short scrollbar cannot end up with a slider longer than its own track.
		const minimumSliderSize = Math.min(MINIMUM_SLIDER_SIZE, computedRepresentableSize);
		const computedSliderSize = Math.round(Math.max(minimumSliderSize, Math.floor(visibleSize * computedRepresentableSize / scrollSize)));
