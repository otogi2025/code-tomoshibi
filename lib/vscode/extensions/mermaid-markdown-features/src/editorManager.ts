...
class MermaidPreview extends Disposable {
	...
	public static create(
		...
		const webviewPanel = vscode.window.createWebviewPanel(
			...
			title ?? vscode.l10n.t('Mermaid 图表'),
			viewColumn,
			{
	...
	private _getHtml(): string {
		...
		const togglePanModeLabel = vscode.l10n.t('切换平移模式');
		const zoomOutLabel = vscode.l10n.t('缩小');
		const zoomInLabel = vscode.l10n.t('放大');
		const resetPanZoomLabel = vscode.l10n.t('重置平移和缩放');
