				...
				case DiagnosticCode.link_noSuchHeaderInFile: {
					...
					if (hrefText) {
						const fix = new vscode.CodeAction(
							vscode.l10n.t('从链接验证中排除 "{0}"。', hrefText),
							vscode.CodeActionKind.QuickFix);
...
function registerMarkdownStatusItem(selector: vscode.DocumentSelector, commandManager: CommandManager): vscode.Disposable {
	...
	const update = () => {
		...
		if (enabled) {
			statusItem.text = vscode.l10n.t('已启用 Markdown 链接验证');
			statusItem.command = {
				command: commandId,
				arguments: [false],
				title: vscode.l10n.t('禁用'),
				tooltip: vscode.l10n.t('禁用 Markdown 链接验证'),
			};
		} else {
			statusItem.text = vscode.l10n.t('已禁用 Markdown 链接验证');
			statusItem.command = {
				command: commandId,
				arguments: [true],
				title: vscode.l10n.t('启用'),
				tooltip: vscode.l10n.t('启用 Markdown 链接验证'),
			};
		}
	};
	update();
