...
class UpdateLinksOnFileRenameHandler extends Disposable {
			...
			if (this.#pendingRenames.size) {
				this.#delayer.trigger(() => {
					vscode.window.withProgress({
						...
						title: vscode.l10n.t("正在检查要更新的 Markdown 链接")
					}, () => this.#flushRenames());
				});
			}
		}));
	...
	async #promptUser(newResources: readonly vscode.Uri[]): Promise<boolean> {
		...
		const alwaysItem: vscode.MessageItem = {
			title: vscode.l10n.t("始终"),
		};

		const neverItem: vscode.MessageItem = {
			title: vscode.l10n.t("从不"),
		};

		const choice = await vscode.window.showInformationMessage(
			newResources.length === 1
				? vscode.l10n.t("是否更新“{0}”的 Markdown 链接?", Utils.basename(newResources[0]))
				: this.#getConfirmMessage(vscode.l10n.t("是否更新以下 {0} 个文件的 Markdown 链接?", newResources.length), newResources), {
			modal: true,
	...
	#getConfirmMessage(start: string, resourcesToConfirm: readonly vscode.Uri[]): string {
		...
		if (resourcesToConfirm.length > MAX_CONFIRM_FILES) {
			if (resourcesToConfirm.length - MAX_CONFIRM_FILES === 1) {
				paths.push(vscode.l10n.t("...1 个其他文件未显示"));
			} else {
				paths.push(vscode.l10n.t("...{0} 个其他文件未显示", resourcesToConfirm.length - MAX_CONFIRM_FILES));
			}
		}
